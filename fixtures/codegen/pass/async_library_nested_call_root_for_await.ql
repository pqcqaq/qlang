...
async fn helper() -> Int {
    var total = 0
    for await value in array_env(1).payload.values {
        total = total + value
    }
    for await value in tuple_env(3).payload.values {
        total = total + value
    }
    for await value in task_tuple_env(5).payload.values {
        total = total + value
    }
    for await value in deep_task_env(7).outer.payload.tasks {
        total = total + value
    }
    return total
}
