...
#[test]
fn allows_passing_arithmetic_const_alias_sourced_composed_dynamic_task_handle_to_helper_and_consuming_sibling_array_element()
 {
    let diagnostics = diagnostic_messages(
        r#"
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected arithmetic const alias-sourced composed dynamic helper consume to preserve sibling array element availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_passing_guarded_arithmetic_const_alias_sourced_composed_dynamic_task_handle_to_helper_and_consuming_sibling_array_element()
 {
    let diagnostics = diagnostic_messages(
        r#"
use INDEX as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic const alias-sourced composed dynamic helper consume to preserve sibling array element availability, got {diagnostics:?}"
    );
}
...
#[test]
fn allows_spawning_arithmetic_static_alias_sourced_composed_dynamic_queued_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected arithmetic static alias-sourced composed dynamic queued spawn after forwarded alias to preserve sibling array element availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_spawning_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued spawn after forwarded alias to preserve sibling array element availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_spawning_guarded_arithmetic_static_alias_sourced_composed_dynamic_bundle_alias_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic bundle-alias-forwarded spawn after forwarded alias to preserve sibling array element availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_spawning_guarded_arithmetic_static_alias_sourced_composed_dynamic_bundle_alias_inline_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
async fn main() -> Wrap {
    ...
    let bundle_tasks = bundle_root
    let second = await env.tail
    let running = spawn forward(bundle_tasks[0])
    return await running
}
"#,
    );
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic bundle-alias-inline-forwarded spawn after forwarded alias to preserve sibling array element availability, got {diagnostics:?}"
    );
}
...
#[test]
fn allows_spawning_guarded_arithmetic_static_alias_sourced_composed_dynamic_bundle_chain_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic bundle-chain-forwarded spawn after forwarded alias to preserve sibling array element availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_spawning_guarded_arithmetic_static_alias_sourced_composed_dynamic_bundle_chain_inline_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic bundle-chain-inline-forwarded spawn after forwarded alias to preserve sibling array element availability, got {diagnostics:?}"
    );
}
...
#[test]
fn allows_spawning_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_root_alias_inline_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
async fn main() -> Wrap {
    ...
    let queued = queued_tasks[0]
    let second = await env.tail
    let running = spawn forward(queued)
    return await running
}
"#,
    );
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-root-alias-inline-forwarded spawn after forwarded alias to preserve sibling array element availability, got {diagnostics:?}"
    );
}
...
#[test]
fn allows_spawning_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_root_chain_inline_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-root-chain-inline-forwarded spawn after forwarded alias to preserve sibling array element availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_spawning_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_root_chain_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-root-chain-forwarded spawn after forwarded alias to preserve sibling array element availability, got {diagnostics:?}"
    );
}
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-root-forwarded spawn after forwarded alias to preserve sibling array element availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_spawning_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_local_alias_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-local-alias spawn after forwarded alias to preserve sibling array element availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_spawning_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_local_chain_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-local-chain spawn after forwarded alias to preserve sibling array element availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_spawning_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_local_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-local-forwarded spawn after forwarded alias to preserve sibling array element availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_spawning_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_local_inline_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-local-inline-forwarded spawn after forwarded alias to preserve sibling array element availability, got {diagnostics:?}"
    );
}
...
#[test]
fn allows_awaiting_guarded_arithmetic_static_alias_sourced_composed_dynamic_bundle_alias_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic bundle-alias-forwarded await after forwarded alias to preserve sibling task availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_awaiting_guarded_arithmetic_static_alias_sourced_composed_dynamic_bundle_alias_inline_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic bundle-alias-inline-forwarded await after forwarded alias to preserve sibling task availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_awaiting_guarded_arithmetic_static_alias_sourced_composed_dynamic_bundle_chain_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic bundle-chain-forwarded await after forwarded alias to preserve sibling task availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_awaiting_guarded_arithmetic_static_alias_sourced_composed_dynamic_bundle_chain_inline_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic bundle-chain-inline-forwarded await after forwarded alias to preserve sibling task availability, got {diagnostics:?}"
    );
}
...
#[test]
fn allows_awaiting_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_root_alias_inline_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
async fn main() -> Wrap {
    ...
    let queued_tasks = queue_root
    let queued = queued_tasks[0]
    let second = await forward(queued)
    let extra = await env.bundle.tasks[1]
    let tail = await env.tail
    return second
}
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-root-alias-inline-forwarded await after forwarded alias to preserve sibling task availability, got {diagnostics:?}"
    );
}
...
#[test]
fn allows_awaiting_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_root_chain_inline_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-root-chain-inline-forwarded await after forwarded alias to preserve sibling task availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_awaiting_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_root_chain_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-root-chain-forwarded await after forwarded alias to preserve sibling task availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_awaiting_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_root_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-root-forwarded await after forwarded alias to preserve sibling task availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_awaiting_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_local_alias_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-local-alias await after forwarded alias to preserve sibling task availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_awaiting_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_local_chain_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-local-chain await after forwarded alias to preserve sibling task availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_awaiting_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_local_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-local-forwarded await after forwarded alias to preserve sibling task availability, got {diagnostics:?}"
    );
}

#[test]
fn allows_awaiting_guarded_arithmetic_static_alias_sourced_composed_dynamic_queued_local_inline_forwarded_task_handle_after_forwarded_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected guarded arithmetic static alias-sourced composed dynamic queued-local-inline-forwarded await after forwarded alias to preserve sibling task availability, got {diagnostics:?}"
    );
}
...
"#,
    ...
    assert!(
        diagnostics.contains(&"local `tasks` was used after move".to_string()),
        "expected branch-selected const alias-backed dynamic task-handle index reuse through tasks[0] to be a definite move, got {diagnostics:?}"
    );
}
...
#[test]
fn allows_reinitializing_same_branch_selected_static_alias_backed_projected_dynamic_task_handle_array_index_before_reuse_via_alias()
 {
    let diagnostics = diagnostic_messages(
        r#"
use SLOT as INDEX_ALIAS
...
"#,
    ...
    assert!(
        diagnostics.is_empty(),
        "expected branch-selected static alias-backed projected dynamic task-handle index reinit to restore tasks[0], got {diagnostics:?}"
    );
}
