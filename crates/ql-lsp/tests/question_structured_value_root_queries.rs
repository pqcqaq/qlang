...
impl RootKind {
    ...
    fn dep_member_decl(self) -> &'static str {
        match self {
            Self::Function => "pub fn maybe_load() -> Option[Config]",
            Self::Static => "pub static MAYBE: Option[Config]",
        }
    }
}
...
fn build_source(root: RootKind, structured: StructuredKind, broken: bool) -> String {
    let prefix = if broken { "let next = " } else { "return " };
...
fn assert_root_queries(
    ...
    let root_position = offset_to_position(source, nth_offset(source, alias, 2));
    ...
    if broken {
        ...
        let hover = hover_for_dependency_values(source, &package, root_position)
            .expect("structured question value root hover should exist without semantic analysis");
        ...
        let definition = definition_for_dependency_values(source, &package, root_position).expect(
            "structured question value root definition should exist without semantic analysis",
        );
        ...
        let declaration = declaration_for_dependency_values(source, &package, root_position)
            .expect(
                "structured question value root declaration should exist without semantic analysis",
            );
        ...
        let without_declaration =
            references_for_dependency_values(uri, source, &package, root_position, false).expect(
                "structured question value root references should exist without semantic analysis",
            );
        assert_eq!(
            without_declaration,
            vec![
        ...
        let with_declaration =
            references_for_dependency_values(uri, source, &package, root_position, true)
                .expect(
                    "structured question value root references with declaration should exist without semantic analysis",
                );
        ...
        let hover = hover_for_package_analysis(source, &analysis, &package, root_position)
            .expect("structured question value root hover should exist");
        let HoverContents::Markup(markup) = hover.contents else {
            panic!("hover should use markdown")
        };
        assert!(markup.value.contains("**struct** `Config`"));
        ...
        let definition =
            definition_for_package_analysis(uri, source, &analysis, &package, root_position)
                .expect("structured question value root definition should exist");
        let GotoDefinitionResponse::Scalar(location) = definition else {
            panic!("definition should be one location")
        };
        assert_dependency_location(&location, dep_qi, "pub struct Config {\n    value: Int,\n}");
        ...
        let declaration =
            declaration_for_package_analysis(uri, source, &analysis, &package, root_position)
                .expect("structured question value root declaration should exist");
        let GotoDeclarationResponse::Scalar(location) = declaration else {
            panic!("declaration should be one location")
        };
        assert_dependency_location(&location, dep_qi, "pub struct Config {\n    value: Int,\n}");
        ...
        let without_declaration =
            references_for_package_analysis(uri, source, &analysis, &package, root_position, false)
                .expect("structured question value root references should exist");
        assert_eq!(
            without_declaration,
            vec![
                Location::new(
        ...
        let with_declaration =
            references_for_package_analysis(uri, source, &analysis, &package, root_position, true)
                .expect("structured question value root references with declaration should exist");
        assert_eq!(with_declaration.len(), 4);
        assert_dependency_location(
            &with_declaration[0],
            dep_qi,
...
#[test]
fn root_queries_work_on_if_question_function_value_receivers() {
    run_root_query_case(RootKind::Function, StructuredKind::If, false);
}

#[test]
fn root_queries_work_on_if_question_function_value_receivers_without_semantic_analysis() {
    run_root_query_case(RootKind::Function, StructuredKind::If, true);
}

#[test]
fn root_queries_work_on_match_question_function_value_receivers() {
    run_root_query_case(RootKind::Function, StructuredKind::Match, false);
}

#[test]
fn root_queries_work_on_match_question_function_value_receivers_without_semantic_analysis() {
    run_root_query_case(RootKind::Function, StructuredKind::Match, true);
}

#[test]
fn root_queries_work_on_if_question_static_value_receivers() {
    run_root_query_case(RootKind::Static, StructuredKind::If, false);
}

#[test]
fn root_queries_work_on_if_question_static_value_receivers_without_semantic_analysis() {
    run_root_query_case(RootKind::Static, StructuredKind::If, true);
}

#[test]
fn root_queries_work_on_match_question_static_value_receivers() {
    run_root_query_case(RootKind::Static, StructuredKind::Match, false);
}

#[test]
fn root_queries_work_on_match_question_static_value_receivers_without_semantic_analysis() {
