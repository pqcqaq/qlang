...
impl CaseKind {
    ...
    fn access_suffix(self) -> &'static str {
        match self {
            Self::BlockTupleField => ".leaf.value",
            Self::BlockArrayMethod => ".leaf().value",
        }
    }
...
fn run_type_definition_case(case: CaseKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-for-loop-block-iterable-{}-member-type-definition{}",
        case.label(),
...
"#,
    ...
    if broken {
        ...
        .expect("block iterable member type definition should exist without semantic analysis");
        ...
        let definition =
            type_definition_for_package_analysis(&uri, &source, &analysis, &package, position)
                .expect("block iterable member type definition should exist");
        assert_targets_dependency_type(
            definition,
            &dep_qi,
            "pub struct Leaf {\n    value: Int,\n}",
...
#[test]
fn type_definition_bridge_follows_block_tuple_for_loop_field_member_types() {
    run_type_definition_case(CaseKind::BlockTupleField, false);
}

#[test]
fn type_definition_fallback_follows_block_tuple_for_loop_field_member_types() {
    run_type_definition_case(CaseKind::BlockTupleField, true);
}

#[test]
fn type_definition_bridge_follows_block_array_for_loop_method_member_types() {
    run_type_definition_case(CaseKind::BlockArrayMethod, false);
}

#[test]
fn type_definition_fallback_follows_block_array_for_loop_method_member_types() {
    run_type_definition_case(CaseKind::BlockArrayMethod, true);
}
