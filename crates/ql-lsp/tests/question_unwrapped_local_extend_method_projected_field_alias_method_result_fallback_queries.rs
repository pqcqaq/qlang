...
use ql_lsp::bridge::{
    declaration_for_dependency_struct_fields, definition_for_dependency_struct_fields,
    hover_for_dependency_struct_fields, references_for_dependency_struct_fields, span_to_range,
};
...
#[test]
fn dependency_field_queries_work_on_question_unwrapped_local_extend_method_result_projected_field_alias_method_result_members_without_semantic_analysis(
) {
    let temp = TempDir::new(
        "ql-lsp-question-unwrapped-local-extend-method-projected-field-alias-method-result-query-broken",
    );
...
pub fn read(config: Cfg) -> Int {
    ...
    let alias = current
    let broken: Int = "oops"
    return alias.get().value + alias.get().value
}
"#;
    ...
    .expect(
        "dependency extend-method-result projected-field alias local method-result member hover should exist without semantic analysis",
    );
    ...
    .expect(
        "dependency extend-method-result projected-field alias local method-result member definition should exist without semantic analysis",
    );
    ...
    .expect(
        "dependency extend-method-result projected-field alias local method-result member declaration should exist without semantic analysis",
    );
    ...
    .expect(
        "dependency extend-method-result projected-field alias local method-result member references should exist without semantic analysis",
    );
    ...
    .expect(
        "dependency extend-method-result projected-field alias local method-result member references should exist without declaration",
    );
