...
impl CaseKind {
    ...
    fn access_suffix(self) -> &'static str {
        match self {
            Self::IfTupleField => ".leaf.value",
            Self::MatchArrayMethod => ".leaf().value",
        }
    }
...
fn run_type_definition_case(case: CaseKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-for-loop-structured-receiver-{}-member-type-definition{}",
        case.label(),
...
"#,
    ...
    if broken {
        ...
        .expect(
            "structured iterable member type definition should exist without semantic analysis",
        );
        ...
        let definition =
            type_definition_for_package_analysis(&uri, &source, &analysis, &package, position)
                .expect("structured iterable member type definition should exist");
        assert_targets_dependency_type(
            definition,
            &dep_qi,
            "pub struct Leaf {\n    value: Int,\n}",
...
#[test]
fn type_definition_bridge_follows_for_loop_if_tuple_field_member_types() {
    run_type_definition_case(CaseKind::IfTupleField, false);
}

#[test]
fn type_definition_fallback_follows_for_loop_if_tuple_field_member_types() {
    run_type_definition_case(CaseKind::IfTupleField, true);
}

#[test]
fn type_definition_bridge_follows_for_loop_match_array_method_member_types() {
    run_type_definition_case(CaseKind::MatchArrayMethod, false);
}

#[test]
fn type_definition_fallback_follows_for_loop_match_array_method_member_types() {
    run_type_definition_case(CaseKind::MatchArrayMethod, true);
}
