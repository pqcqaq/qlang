...
#[test]
fn root_query_fallback_surfaces_dependency_question_unwrapped_local_extend_method_projected_field_alias_roots(
) {
    let temp = TempDir::new(
        "ql-lsp-question-unwrapped-local-extend-method-projected-field-alias-value-root-fallback",
    );
...
"#;
    ...
    let position = offset_to_position(source, alias_usage);

    let hover = hover_for_dependency_values(source, &package, position).expect(
        "dependency extend-method-result projected-field alias value root hover should exist without semantic analysis",
    );
    ...
    let definition = definition_for_dependency_values(source, &package, position).expect(
        "dependency extend-method-result projected-field alias value root definition should exist without semantic analysis",
    );
    ...
    let declaration = declaration_for_dependency_values(source, &package, position).expect(
        "dependency extend-method-result projected-field alias value root declaration should exist without semantic analysis",
    );
    ...
    let without_declaration =
        references_for_dependency_values(&uri, source, &package, position, false).expect(
            "dependency extend-method-result projected-field alias value root references should exist without semantic analysis",
        );
    assert_eq!(
        without_declaration,
        vec![
    ...
    let with_declaration = references_for_dependency_values(&uri, source, &package, position, true)
        .expect(
            "dependency extend-method-result projected-field alias value root references with declaration should exist without semantic analysis",
        );
