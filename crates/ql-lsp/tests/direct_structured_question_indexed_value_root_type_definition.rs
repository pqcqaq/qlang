...
fn run_type_definition_case(structured: StructuredKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-structured-question-indexed-{}-value-root-type-definition{}",
        structured.label(),
...
"#,
    ...
    if broken {
        ...
        let definition = type_definition_for_dependency_values(&source, &package, position)
            .expect("direct structured question indexed value root type definition should exist");
        ...
        let definition =
            type_definition_for_package_analysis(&uri, &source, &analysis, &package, position)
                .expect(
                    "direct structured question indexed value root type definition should exist",
                );
        assert_targets_dependency_type(
            definition,
            &dep_qi,
...
#[test]
fn type_definition_bridge_follows_if_direct_structured_question_indexed_value_roots() {
    run_type_definition_case(StructuredKind::If, false);
}

#[test]
fn type_definition_bridge_follows_match_direct_structured_question_indexed_value_roots() {
    run_type_definition_case(StructuredKind::Match, false);
}

#[test]
fn type_definition_fallback_follows_if_direct_structured_question_indexed_value_roots() {
    run_type_definition_case(StructuredKind::If, true);
}

#[test]
fn type_definition_fallback_follows_match_direct_structured_question_indexed_value_roots() {
