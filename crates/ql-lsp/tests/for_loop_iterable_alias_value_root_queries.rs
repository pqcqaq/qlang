...
impl BindingKind {
    fn label(self) -> &'static str {
        match self {
            Self::TupleAlias => "tuple-alias",
            Self::ArrayAlias => "array-alias",
        }
    }

    fn dep_struct_name(self) -> &'static str {
        match self {
            Self::TupleAlias => "Config",
            Self::ArrayAlias => "Child",
        }
    }

    fn dep_struct_snippet(self) -> &'static str {
        match self {
            Self::TupleAlias => "pub struct Config {\n    value: Int,\n}",
            Self::ArrayAlias => "pub struct Child {\n    value: Int,\n}",
        }
    }
...
pub fn read(config: Cfg) -> Int {{
    let items = [config.child(), config.child()]
    for current in items {{
        return current.value + current.value
    }}
{tail}}}
...
fn run_root_query_case(binding: BindingKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-for-loop-iterable-alias-{}-value-root-query{}",
        binding.label(),
...
"#,
    ...
    if broken {
        ...
        .expect("dependency iterable alias value root hover should exist");
        ...
        .expect("dependency iterable alias value root definition should exist");
        let GotoDefinitionResponse::Scalar(location) = definition else {
            panic!("definition should be one location")
        };
        assert_dependency_location(&location, &dep_qi, binding.dep_struct_snippet());
        ...
        .expect("dependency iterable alias value root declaration should exist");
        ...
        .expect("dependency iterable alias value root references should exist");
        assert_eq!(
            without_declaration,
            vec![
                Location::new(
        ...
        .expect("dependency iterable alias value root references with declaration should exist");
        assert_eq!(with_declaration.len(), 4);
        assert_dependency_location(&with_declaration[0], &dep_qi, binding.dep_struct_snippet());
        assert_eq!(
            with_declaration[1..],
            [
                Location::new(
...
#[test]
fn root_query_bridge_surfaces_dependency_for_loop_named_tuple_alias_value_roots() {
    run_root_query_case(BindingKind::TupleAlias, false);
}

#[test]
fn root_query_bridge_surfaces_dependency_for_loop_named_array_alias_value_roots() {
    run_root_query_case(BindingKind::ArrayAlias, false);
}

#[test]
fn root_query_fallback_surfaces_dependency_for_loop_named_tuple_alias_value_roots() {
    run_root_query_case(BindingKind::TupleAlias, true);
}

#[test]
fn root_query_fallback_surfaces_dependency_for_loop_named_array_alias_value_roots() {
    run_root_query_case(BindingKind::ArrayAlias, true);
}
