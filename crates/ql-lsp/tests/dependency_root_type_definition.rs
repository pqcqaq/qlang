...
#[test]
fn type_definition_bridge_follows_structured_question_wrapped_dependency_static_iterable_roots() {
    let temp = TempDir::new("ql-lsp-structured-question-static-iterable-root-type-definition");
    let app_root = temp.path().join("workspace").join("app");
    let app_path = temp
        .path()
        .join("workspace")
...
"#;
    ...
    .expect("structured dependency question iterable static root type definition should exist");
    assert_targets_dependency_struct(
        definition,
        &dep_qi,
        "pub struct Child {\n    value: Int,\n}",
