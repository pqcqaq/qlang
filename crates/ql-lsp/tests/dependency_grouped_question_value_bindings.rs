...
#[test]
fn package_bridge_completes_dependency_fields_for_grouped_question_function_result_receivers() {
    let temp = TempDir::new("ql-lsp-grouped-question-function-value-field-completion");
...
#[test]
fn package_bridge_completes_dependency_methods_for_grouped_question_function_result_receivers() {
    let temp = TempDir::new("ql-lsp-grouped-question-function-value-method-completion");
    let app_root = temp.path().join("workspace").join("app");
    let source = r#"
package demo.app
...
"#,
    ...
    let Some(CompletionResponse::Array(items)) = completion_for_dependency_member_fields(
        ...
        panic!("grouped question static value field completion should exist");
    };
    assert_eq!(items.len(), 1);
    assert_eq!(items[0].label, "value");
    assert_eq!(items[0].kind, Some(CompletionItemKind::FIELD));
...
"#,
    ...
    let Some(CompletionResponse::Array(items)) = completion_for_dependency_methods(
        ...
        panic!("grouped question static value method completion should exist");
    };
    assert_eq!(items.len(), 1);
    assert_eq!(items[0].label, "get");
    assert_eq!(items[0].kind, Some(CompletionItemKind::METHOD));
