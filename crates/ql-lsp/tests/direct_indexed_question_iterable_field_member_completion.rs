...
fn run_completion_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-indexed-question-iterable-field-member-completion{}",
...
"#,
    ...
    let Some(CompletionResponse::Array(items)) = completion else {
        panic!(
            "direct indexed question iterable field member completion should exist{}",
            if broken {
...
#[test]
fn completion_works_on_direct_indexed_question_iterable_field_members() {
    run_completion_case(false);
}

#[test]
fn completion_fallback_works_on_direct_indexed_question_iterable_field_members() {
