...
"#,
        wrapped = structured.wrap("config.child()"),
...
fn run_type_definition_case(member: MemberKind, structured: StructuredKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-structured-receiver-{}-{}-member-type-definition{}",
        structured.label(),
        member.label(),
...
"#,
    ...
    if broken {
        ...
        .expect(
            "structured receiver member type definition should exist without semantic analysis",
        );
        ...
        let definition =
            type_definition_for_package_analysis(&uri, &source, &analysis, &package, position)
                .expect("structured receiver member type definition should exist");
        assert_targets_dependency_type(
            definition,
            &dep_qi,
            "pub struct Leaf {\n    value: Int,\n}",
...
#[test]
fn type_definition_bridge_follows_if_structured_receiver_field_member_types() {
    run_type_definition_case(MemberKind::Field, StructuredKind::If, false);
}

#[test]
fn type_definition_fallback_follows_if_structured_receiver_field_member_types() {
    run_type_definition_case(MemberKind::Field, StructuredKind::If, true);
}

#[test]
fn type_definition_bridge_follows_match_structured_receiver_field_member_types() {
    run_type_definition_case(MemberKind::Field, StructuredKind::Match, false);
}

#[test]
fn type_definition_fallback_follows_match_structured_receiver_field_member_types() {
    run_type_definition_case(MemberKind::Field, StructuredKind::Match, true);
}

#[test]
fn type_definition_bridge_follows_if_structured_receiver_method_member_types() {
    run_type_definition_case(MemberKind::Method, StructuredKind::If, false);
}

#[test]
fn type_definition_fallback_follows_if_structured_receiver_method_member_types() {
    run_type_definition_case(MemberKind::Method, StructuredKind::If, true);
}

#[test]
fn type_definition_bridge_follows_match_structured_receiver_method_member_types() {
    run_type_definition_case(MemberKind::Method, StructuredKind::Match, false);
}

#[test]
fn type_definition_fallback_follows_match_structured_receiver_method_member_types() {
