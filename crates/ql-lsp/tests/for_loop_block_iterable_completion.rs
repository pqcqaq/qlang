...
impl CaseKind {
    ...
    fn completion_suffix(self) -> &'static str {
        match self {
            Self::BlockTupleField => ".va",
            Self::BlockArrayMethod => ".ge",
        }
    }
    ...
    fn expected_kind(self) -> CompletionItemKind {
        match self {
            Self::BlockTupleField => CompletionItemKind::FIELD,
            Self::BlockArrayMethod => CompletionItemKind::FUNCTION,
        }
    }
...
fn run_completion_case(case: CaseKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-for-loop-block-iterable-{}-completion{}",
        case.label(),
...
"#,
    ...
    if broken {
        ...
        let Some(CompletionResponse::Array(items)) = (match case {
            ...
            panic!("block iterable receiver completion should exist without semantic analysis");
        };
        ...
        let Some(CompletionResponse::Array(items)) =
            ...
            panic!("block iterable receiver completion should exist");
        };
        assert_eq!(items.len(), 1);
        assert_completion_item(case, items[0].clone());
    }
...
#[test]
fn dependency_field_completion_works_on_block_tuple_for_loop_receivers() {
    run_completion_case(CaseKind::BlockTupleField, false);
}

#[test]
fn dependency_field_completion_works_on_block_tuple_for_loop_receivers_without_semantic_analysis() {
    run_completion_case(CaseKind::BlockTupleField, true);
}

#[test]
fn dependency_method_completion_works_on_block_array_for_loop_receivers() {
    run_completion_case(CaseKind::BlockArrayMethod, false);
}

#[test]
fn dependency_method_completion_works_on_block_array_for_loop_receivers_without_semantic_analysis()
{
    run_completion_case(CaseKind::BlockArrayMethod, true);
}
