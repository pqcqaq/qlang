...
#[test]
fn package_bridge_completes_dependency_fields_for_for_loop_named_tuple_alias_receivers() {
    let temp = TempDir::new("ql-lsp-for-loop-alias-field-completion");
...
"#,
    ...
    let Some(CompletionResponse::Array(items)) = completion_for_package_analysis(
        ...
        panic!("dependency iterable alias field completion should exist");
    };
...
#[test]
fn dependency_method_definition_works_on_for_loop_named_match_alias_without_semantic_analysis() {
    let temp = TempDir::new("ql-lsp-for-loop-alias-method-query-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("dependency iterable alias method definition should exist");
