...
impl CaseKind {
    ...
    const fn destructuring_pattern(self) -> &'static str {
        match self {
...
fn build_source(case: CaseKind, member: MemberKind, broken: bool) -> String {
...
fn assert_member_queries(
    ...
    if broken {
        ...
        .expect("iterable call destructured member hover should exist without semantic analysis");
        ...
        .expect(
            "iterable call destructured member definition should exist without semantic analysis",
        );
        ...
        .expect(
            "iterable call destructured member declaration should exist without semantic analysis",
        );
        ...
        .expect(
            "iterable call destructured member references should exist without semantic analysis",
        );
        ...
        .expect("iterable call destructured member references should exist without declaration");
        ...
        .expect("iterable call destructured member hover should exist");
        let HoverContents::Markup(markup) = hover.contents else {
            panic!("hover should use markdown")
        };
        assert!(markup.value.contains(member.hover_kind()));
        ...
        .expect("iterable call destructured member definition should exist");
        let GotoDefinitionResponse::Scalar(location) = definition else {
            panic!("definition should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("iterable call destructured member declaration should exist");
        let GotoDeclarationResponse::Scalar(location) = declaration else {
            panic!("declaration should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("iterable call destructured member references should exist");
        assert_eq!(with_declaration.len(), 3);
        assert_location_targets_dependency_name(
            &with_declaration[0],
            dep_qi,
...
fn run_member_query_case(case: CaseKind, member: MemberKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-{}-destructured-iterable-call-{}-member-query{}",
        case.test_name(),
        member.label(),
...
"#,
    ...
    let source = build_source(case, member, broken);
...
#[test]
fn tuple_destructured_dependency_iterable_call_field_queries_work() {
    run_member_query_case(CaseKind::Tuple, MemberKind::Field, false);
}

#[test]
fn tuple_destructured_dependency_iterable_call_field_queries_work_without_semantic_analysis() {
    run_member_query_case(CaseKind::Tuple, MemberKind::Field, true);
}

#[test]
fn tuple_destructured_dependency_iterable_call_method_queries_work() {
    run_member_query_case(CaseKind::Tuple, MemberKind::Method, false);
}

#[test]
fn tuple_destructured_dependency_iterable_call_method_queries_work_without_semantic_analysis() {
    run_member_query_case(CaseKind::Tuple, MemberKind::Method, true);
}

#[test]
fn array_destructured_dependency_iterable_call_field_queries_work() {
    run_member_query_case(CaseKind::Array, MemberKind::Field, false);
}

#[test]
fn array_destructured_dependency_iterable_call_field_queries_work_without_semantic_analysis() {
    run_member_query_case(CaseKind::Array, MemberKind::Field, true);
}

#[test]
fn array_destructured_dependency_iterable_call_method_queries_work() {
    run_member_query_case(CaseKind::Array, MemberKind::Method, false);
}

#[test]
fn array_destructured_dependency_iterable_call_method_queries_work_without_semantic_analysis() {
    run_member_query_case(CaseKind::Array, MemberKind::Method, true);
}
