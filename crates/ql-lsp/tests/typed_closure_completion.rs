...
fn run_completion_case(member: MemberKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-typed-closure-{}-completion{}",
        member.label(),
...
"#,
    ...
    if broken {
        ...
        let Some(CompletionResponse::Array(items)) = (match member {
            ...
            panic!("typed closure member completion should exist without semantic analysis");
        };
        ...
        let Some(CompletionResponse::Array(items)) =
            ...
            panic!("typed closure member completion should exist");
        };
        assert_eq!(items.len(), 1);
        assert_completion_item(member, items[0].clone());
    }
...
#[test]
fn dependency_field_completion_works_on_typed_closure_receivers() {
    run_completion_case(MemberKind::Field, false);
}

#[test]
fn dependency_field_completion_works_on_typed_closure_receivers_without_semantic_analysis() {
    run_completion_case(MemberKind::Field, true);
}

#[test]
fn dependency_method_completion_works_on_typed_closure_receivers() {
    run_completion_case(MemberKind::Method, false);
}

#[test]
fn dependency_method_completion_works_on_typed_closure_receivers_without_semantic_analysis() {
