...
#[test]
fn type_definition_fallback_follows_question_unwrapped_local_extend_method_alias_method_result_value_roots(
) {
    let temp = TempDir::new(
        "ql-lsp-question-unwrapped-local-extend-method-alias-method-result-value-root-fallback-type-definition",
    );
...
"#;
    ...
    let definition = type_definition_for_dependency_values(source, &package, position).expect(
        "dependency extend-method-result alias local method-result value root type definition should exist without semantic analysis",
    );
