...
fn run_type_definition_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-indexed-iterable-method-member-type-definition{}",
...
pub fn read(config: Cfg) -> Int {
    let value = config.children()[0].leaf().value
    return "oops"
}
"#
    } else {
...
"#
    ...
    if broken {
        ...
        let definition = type_definition_for_dependency_method_types(source, &package, position)
            .expect("direct indexed iterable method member type definition should exist");
...
#[test]
fn type_definition_bridge_follows_direct_indexed_iterable_method_member_types() {
    run_type_definition_case(false);
}

#[test]
fn type_definition_fallback_follows_direct_indexed_iterable_method_member_types() {
