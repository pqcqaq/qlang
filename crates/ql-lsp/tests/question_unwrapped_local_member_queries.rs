...
fn assert_member_queries(
    ...
    if broken {
        ...
        .expect("question-unwrapped local member hover should exist without semantic analysis");
        ...
        .expect("question-unwrapped local member definition should exist without semantic analysis");
        ...
        .expect("question-unwrapped local member declaration should exist without semantic analysis");
        ...
        .expect("question-unwrapped local member references should exist without semantic analysis");
        ...
        .expect("question-unwrapped local member hover should exist");
        let HoverContents::Markup(markup) = hover.contents else {
            panic!("hover should use markdown")
        };
        assert!(markup.value.contains(member.hover_kind()));
        ...
        .expect("question-unwrapped local member definition should exist");
        let GotoDefinitionResponse::Scalar(definition_location) = definition else {
            panic!("definition should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("question-unwrapped local member declaration should exist");
        let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
            panic!("declaration should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("question-unwrapped local member references should exist");
        assert_eq!(with_declaration.len(), 3);
        assert_location_targets_dependency_name(
            &with_declaration[0],
            dep_qi,
...
fn run_member_query_case(root: RootKind, member: MemberKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-question-unwrapped-local-{}-{}-member-query{}",
        root.label(),
        member.label(),
...
#[test]
fn dependency_field_queries_work_on_question_unwrapped_local_receivers() {
    run_member_query_case(RootKind::Field, MemberKind::Field, false);
}

#[test]
fn dependency_field_queries_work_on_question_unwrapped_local_receivers_without_semantic_analysis() {
    run_member_query_case(RootKind::Field, MemberKind::Field, true);
}

#[test]
fn dependency_method_queries_work_on_question_unwrapped_local_receivers() {
    run_member_query_case(RootKind::Method, MemberKind::Method, false);
}

#[test]
fn dependency_method_queries_work_on_question_unwrapped_local_receivers_without_semantic_analysis() {
    run_member_query_case(RootKind::Method, MemberKind::Method, true);
}
