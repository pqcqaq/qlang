...
#[test]
fn package_bridge_completes_dependency_fields_for_for_loop_dependency_method_array_receivers() {
    let temp = TempDir::new("ql-lsp-for-loop-method-iterable-field-completion");
...
"#,
    ...
    let Some(CompletionResponse::Array(items)) = completion_for_package_analysis(
        ...
        panic!("dependency method iterable field completion should exist");
    };
...
#[test]
fn dependency_method_definition_works_on_for_loop_dependency_method_tuple_receivers_without_semantic_analysis()
 {
    let temp = TempDir::new("ql-lsp-for-loop-method-iterable-query-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("dependency method iterable method definition should exist");
