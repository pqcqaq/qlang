...
#[test]
fn package_bridge_completes_dependency_fields_for_for_loop_question_field_iterables() {
    let temp = TempDir::new("ql-lsp-for-loop-question-field-completion");
...
"#,
    ...
    let Some(CompletionResponse::Array(items)) = completion_for_package_analysis(
        ...
        panic!("dependency question field iterable completion should exist");
    };
...
"#,
    ...
    let Some(CompletionResponse::Array(items)) = completion_for_dependency_methods(
        ...
        panic!("dependency question method iterable completion should exist");
    };
    assert_eq!(items.len(), 1);
...
#[test]
fn dependency_method_definition_works_on_for_loop_question_method_iterables_without_semantic_analysis()
 {
    let temp = TempDir::new("ql-lsp-for-loop-question-method-query-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("dependency question method iterable definition should exist");
