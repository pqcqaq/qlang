...
#[test]
fn dependency_field_completion_works_on_direct_indexed_question_iterable_field_receivers() {
    let temp = TempDir::new("ql-lsp-direct-indexed-question-field-completion");
...
pub fn read(config: Cfg) -> Int {
    return config.maybe_children()?[0].va
}
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!("direct indexed question iterable field completion should exist");
    };
...
#[test]
fn dependency_field_completion_works_on_direct_indexed_question_iterable_field_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-direct-indexed-question-field-completion-broken");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read(config: Cfg) -> Int {
    return config.maybe_children()?[0].va + "oops"
}
"#;
    temp.write("workspace/app/src/lib.ql", source);
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!(
            "direct indexed question iterable field completion should exist without semantic analysis"
        );
    };
...
#[test]
fn dependency_field_member_completion_works_on_direct_indexed_question_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-direct-indexed-question-field-member-completion-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!(
            "direct indexed question iterable field member completion should exist without semantic analysis"
        );
    };
...
#[test]
fn dependency_method_member_completion_works_on_direct_indexed_question_iterable_receivers() {
    let temp = TempDir::new("ql-lsp-direct-indexed-question-method-member-completion");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!("direct indexed question iterable method member completion should exist");
    };
...
#[test]
fn dependency_method_member_completion_works_on_direct_indexed_question_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-direct-indexed-question-method-member-completion-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!(
            "direct indexed question iterable method member completion should exist without semantic analysis"
        );
    };
...
#[test]
fn dependency_method_result_member_completion_works_on_direct_indexed_question_iterable_receivers()
{
    let temp = TempDir::new("ql-lsp-direct-indexed-question-method-result-member-completion");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read(config: Cfg) -> Int {
    let value = config.maybe_children()?[0].leaf().va
    return value
}
"#;
    temp.write("workspace/app/src/lib.ql", source);
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!("direct indexed question iterable method-result member completion should exist");
    };
...
#[test]
fn dependency_method_result_member_completion_works_on_direct_indexed_question_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-direct-indexed-question-method-result-member-completion-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!(
            "direct indexed question iterable method-result member completion should exist without semantic analysis"
        );
    };
...
#[test]
fn dependency_field_queries_work_on_direct_indexed_question_iterable_field_receivers() {
    let temp = TempDir::new("ql-lsp-direct-indexed-question-field-queries");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read(config: Cfg) -> Int {
    let first = config.maybe_children()?[0].value
    let second = config.maybe_children()?[1].value
    return first + second
}
"#;
    ...
    .expect("direct indexed question iterable field hover should exist");
    ...
    .expect("direct indexed question iterable field definition should exist");
    ...
    .expect("direct indexed question iterable field declaration should exist");
    ...
    .expect("direct indexed question iterable field references should exist");
...
#[test]
fn dependency_method_member_queries_work_on_direct_indexed_question_iterable_receivers() {
    let temp = TempDir::new("ql-lsp-direct-indexed-question-method-member-queries");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("direct indexed question iterable method member hover should exist");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    ...
    .expect("direct indexed question iterable method member definition should exist");
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    ...
    .expect("direct indexed question iterable method member declaration should exist");
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    assert_location_targets_dependency_name(
    ...
    .expect("direct indexed question iterable method member references should exist");
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(
        &with_declaration[0],
        &dep_qi,
...
#[test]
fn dependency_method_member_queries_work_on_direct_indexed_question_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-direct-indexed-question-method-member-queries-broken");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read(config: Cfg) -> Int {
    let first = config.maybe_children()?[0].leaf()
    let second = config.maybe_children()?[1].leaf()
    return "oops"
}
"#;
    let app_file = temp.write("workspace/app/src/lib.ql", source);
    ...
    .expect("direct indexed question iterable method member hover should exist without semantic analysis");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    assert!(markup.value.contains("**method** `leaf`"));
    ...
    .expect(
        "direct indexed question iterable method member definition should exist without semantic analysis",
    );
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    ...
    .expect(
        "direct indexed question iterable method member declaration should exist without semantic analysis",
    );
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    ...
    .expect(
        "direct indexed question iterable method member references should exist without semantic analysis",
    );
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(
        &with_declaration[0],
    ...
    .expect(
        "direct indexed question iterable method member references should exist without declaration in fallback",
    );
    assert_eq!(without_declaration.len(), 2);
    assert!(without_declaration
        .iter()
...
#[test]
fn dependency_method_result_member_queries_work_on_direct_indexed_question_iterable_receivers() {
    let temp = TempDir::new("ql-lsp-direct-indexed-question-method-result-member-queries");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("direct indexed question iterable method-result member hover should exist");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    assert!(markup.value.contains("**field** `value`"));
    ...
    .expect("direct indexed question iterable method-result member definition should exist");
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    assert_location_targets_dependency_name(&definition_location, &dep_qi, "value: Int", "value");
    ...
    .expect("direct indexed question iterable method-result member declaration should exist");
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    assert_location_targets_dependency_name(&declaration_location, &dep_qi, "value: Int", "value");
...
#[test]
fn dependency_method_result_member_queries_work_on_direct_indexed_question_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-direct-indexed-question-method-result-member-queries-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("direct indexed question iterable method-result member hover should exist without semantic analysis");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    assert!(markup.value.contains("**field** `value`"));
    ...
    .expect(
        "direct indexed question iterable method-result member definition should exist without semantic analysis",
    );
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    ...
    .expect(
        "direct indexed question iterable method-result member declaration should exist without semantic analysis",
    );
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    ...
    .expect(
        "direct indexed question iterable method-result member references should exist without semantic analysis",
    );
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(
        &with_declaration[0],
    ...
    .expect(
        "direct indexed question iterable method-result member references should exist without declaration in fallback",
    );
    assert_eq!(without_declaration.len(), 2);
    assert!(without_declaration
        .iter()
...
#[test]
fn dependency_field_member_queries_work_on_direct_indexed_question_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-direct-indexed-question-field-member-queries-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("direct indexed question iterable field member hover should exist without semantic analysis");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    assert!(markup.value.contains("**field** `leaf`"));
    ...
    .expect(
        "direct indexed question iterable field member definition should exist without semantic analysis",
    );
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    ...
    .expect(
        "direct indexed question iterable field member declaration should exist without semantic analysis",
    );
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    ...
    .expect(
        "direct indexed question iterable field member references should exist without semantic analysis",
    );
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(&with_declaration[0], &dep_qi, "leaf: Leaf", "leaf");
    ...
    .expect(
        "direct indexed question iterable field member references should exist without declaration in fallback",
    );
    assert_eq!(without_declaration.len(), 2);
    assert!(
        without_declaration
...
"#;
    ...
    .expect(
        "direct indexed question iterable method definition should exist without semantic analysis",
    );
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    ...
    .expect(
        "direct indexed question iterable method declaration should exist without semantic analysis"
    );
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    ...
    .expect(
        "direct indexed question iterable method references should exist without semantic analysis",
    );
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(
        &with_declaration[0],
