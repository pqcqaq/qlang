...
#[test]
fn dependency_field_completion_works_on_direct_indexed_iterable_field_receivers() {
    let temp = TempDir::new("ql-lsp-direct-indexed-field-completion");
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!("direct indexed iterable field completion should exist");
    };
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!("direct indexed iterable method completion should exist without semantic analysis");
    };
...
#[test]
fn dependency_method_result_member_completion_works_on_direct_indexed_iterable_receivers() {
    let temp = TempDir::new("ql-lsp-direct-indexed-method-result-member-completion");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read(config: Cfg) -> Int {
    let value = config.children()[0].leaf().va
    return value
}
"#;
    temp.write("workspace/app/src/lib.ql", source);
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!("direct indexed iterable method-result member completion should exist");
    };
...
#[test]
fn dependency_method_result_member_completion_works_on_direct_indexed_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-direct-indexed-method-result-member-completion-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!(
            "direct indexed iterable method-result member completion should exist without semantic analysis"
        );
    };
...
#[test]
fn dependency_field_member_completion_works_on_direct_indexed_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-direct-indexed-field-member-completion-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!("direct indexed iterable field member completion should exist without semantic analysis");
    };
...
#[test]
fn dependency_field_completion_works_on_direct_indexed_iterable_field_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-direct-indexed-field-completion-broken");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read(config: Cfg) -> Int {
    return config.children[0].va + "oops"
}
"#;
    temp.write("workspace/app/src/lib.ql", source);
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!("direct indexed iterable field completion should exist without semantic analysis");
    };
...
#[test]
fn dependency_field_queries_work_on_direct_indexed_iterable_field_receivers() {
    let temp = TempDir::new("ql-lsp-direct-indexed-field-queries");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("direct indexed iterable field hover should exist");
    ...
    .expect("direct indexed iterable field definition should exist");
    ...
    .expect("direct indexed iterable field declaration should exist");
    ...
    .expect("direct indexed iterable field references should exist");
    ...
    .expect("direct indexed iterable field references should exist without declaration");
...
#[test]
fn dependency_field_member_queries_work_on_direct_indexed_iterable_receivers() {
    let temp = TempDir::new("ql-lsp-direct-indexed-field-member-queries");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read(config: Cfg) -> Int {
    let first = config.children()[0].leaf.value
    let second = config.children()[1].leaf.value
    return first + second
}
"#;
    let app_file = temp.write("workspace/app/src/lib.ql", source);
    ...
    .expect("direct indexed iterable field member hover should exist");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    ...
    .expect("direct indexed iterable field member definition should exist");
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    ...
    .expect("direct indexed iterable field member declaration should exist");
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    ...
    .expect("direct indexed iterable field member references should exist");
    assert_eq!(with_declaration.len(), 3);
    ...
    .expect("direct indexed iterable field member references should exist without declaration");
    assert_eq!(without_declaration.len(), 2);
...
#[test]
fn dependency_field_member_queries_work_on_direct_indexed_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-direct-indexed-field-member-queries-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("direct indexed iterable field member hover should exist without semantic analysis");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    assert!(markup.value.contains("**field** `leaf`"));
    ...
    .expect(
        "direct indexed iterable field member definition should exist without semantic analysis",
    );
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    ...
    .expect(
        "direct indexed iterable field member declaration should exist without semantic analysis",
    );
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    ...
    .expect(
        "direct indexed iterable field member references should exist without semantic analysis",
    );
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(&with_declaration[0], &dep_qi, "leaf: Leaf", "leaf");
    ...
    .expect(
        "direct indexed iterable field member references should exist without declaration in fallback",
    );
    assert_eq!(without_declaration.len(), 2);
    assert!(
        without_declaration
...
#[test]
fn dependency_method_member_queries_work_on_direct_indexed_iterable_receivers() {
    let temp = TempDir::new("ql-lsp-direct-indexed-method-member-queries");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("direct indexed iterable method member hover should exist");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    ...
    .expect("direct indexed iterable method member definition should exist");
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    assert_location_targets_dependency_name(
    ...
    .expect("direct indexed iterable method member declaration should exist");
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    assert_location_targets_dependency_name(
    ...
    .expect("direct indexed iterable method member references should exist");
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(
        &with_declaration[0],
        &dep_qi,
    ...
    .expect("direct indexed iterable method member references should exist without declaration");
    assert_eq!(without_declaration.len(), 2);
    assert!(without_declaration
        .iter()
        .all(|location| location.uri == uri));
...
#[test]
fn dependency_method_result_member_queries_work_on_direct_indexed_iterable_receivers() {
    let temp = TempDir::new("ql-lsp-direct-indexed-method-result-member-queries");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("direct indexed iterable method-result member hover should exist");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    assert!(markup.value.contains("**field** `value`"));
    ...
    .expect("direct indexed iterable method-result member definition should exist");
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    assert_location_targets_dependency_name(&definition_location, &dep_qi, "value: Int", "value");
    ...
    .expect("direct indexed iterable method-result member declaration should exist");
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    assert_location_targets_dependency_name(&declaration_location, &dep_qi, "value: Int", "value");
    ...
    .expect("direct indexed iterable method-result member references should exist");
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(&with_declaration[0], &dep_qi, "value: Int", "value");
    assert!(with_declaration[1..]
        .iter()
    ...
    .expect("direct indexed iterable method-result member references should exist without declaration");
    assert_eq!(without_declaration.len(), 2);
    assert!(without_declaration
        .iter()
        .all(|location| location.uri == uri));
...
#[test]
fn dependency_method_queries_work_on_direct_indexed_iterable_method_receivers() {
    let temp = TempDir::new("ql-lsp-direct-indexed-method-queries");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read(config: Cfg) -> Int {
    let first = config.children()[0].get()
    let second = config.children()[1].get()
    return first + second
}
"#;
    let app_file = temp.write("workspace/app/src/lib.ql", source);
    ...
    .expect("direct indexed iterable method hover should exist");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    ...
    .expect("direct indexed iterable method definition should exist");
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    assert_location_targets_dependency_name(
    ...
    .expect("direct indexed iterable method declaration should exist");
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    assert_location_targets_dependency_name(
    ...
    .expect("direct indexed iterable method references should exist");
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(
        &with_declaration[0],
        &dep_qi,
    ...
    .expect("direct indexed iterable method references should exist without declaration");
    assert_eq!(without_declaration.len(), 2);
    assert!(without_declaration
        .iter()
        .all(|location| location.uri == uri));
...
#[test]
fn dependency_method_member_queries_work_on_direct_indexed_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-direct-indexed-method-member-queries-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("direct indexed iterable method member hover should exist without semantic analysis");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    assert!(markup.value.contains("**method** `leaf`"));
    ...
    .expect("direct indexed iterable method member definition should exist without semantic analysis");
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    assert_location_targets_dependency_name(
    ...
    .expect("direct indexed iterable method member declaration should exist without semantic analysis");
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    assert_location_targets_dependency_name(
    ...
    .expect("direct indexed iterable method member references should exist without semantic analysis");
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(
        &with_declaration[0],
        &dep_qi,
    ...
    .expect(
        "direct indexed iterable method member references should exist without declaration in fallback",
    );
    assert_eq!(without_declaration.len(), 2);
    assert!(without_declaration
        .iter()
...
#[test]
fn dependency_method_result_member_queries_work_on_direct_indexed_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-direct-indexed-method-result-member-queries-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("direct indexed iterable method-result member hover should exist without semantic analysis");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    assert!(markup.value.contains("**field** `value`"));
    ...
    .expect("direct indexed iterable method-result member definition should exist without semantic analysis");
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    assert_location_targets_dependency_name(&definition_location, &dep_qi, "value: Int", "value");
    ...
    .expect("direct indexed iterable method-result member declaration should exist without semantic analysis");
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    assert_location_targets_dependency_name(
    ...
    .expect("direct indexed iterable method-result member references should exist without semantic analysis");
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(&with_declaration[0], &dep_qi, "value: Int", "value");
    ...
    .expect(
        "direct indexed iterable method-result member references should exist without declaration in fallback",
    );
    assert_eq!(without_declaration.len(), 2);
    assert!(without_declaration
        .iter()
...
"#;
    ...
    let hover =
        hover_for_dependency_methods(source, &package, offset_to_position(source, first_offset))
            .expect("direct indexed iterable method hover should exist without semantic analysis");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    assert!(markup.value.contains("**method** `get`"));
    ...
    .expect("direct indexed iterable method definition should exist without semantic analysis");
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    assert_location_targets_dependency_name(
    ...
    .expect("direct indexed iterable method declaration should exist without semantic analysis");
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    assert_location_targets_dependency_name(
    ...
    .expect("direct indexed iterable method references should exist without semantic analysis");
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(
        &with_declaration[0],
        &dep_qi,
    ...
    .expect(
        "direct indexed iterable method references should exist without declaration in fallback",
    );
    assert_eq!(without_declaration.len(), 2);
    assert!(
        without_declaration
