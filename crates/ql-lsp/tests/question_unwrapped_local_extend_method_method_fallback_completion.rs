...
#[test]
fn dependency_method_completion_works_on_question_unwrapped_local_extend_method_result_receivers_without_semantic_analysis(
) {
    let temp =
        TempDir::new("ql-lsp-question-unwrapped-local-extend-method-method-completion-broken");
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!(
            "dependency extend-method-result local method completion should exist without semantic analysis"
        );
    };
