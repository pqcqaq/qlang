...
fn dependency_method_queries_work_on_question_function_value_receivers_without_semantic_analysis() {
    let temp = TempDir::new("ql-lsp-question-function-method-member-query-broken");
...
"#;
    ...
    let expected_first = span_to_range(
        source,
        ql_span::Span::new(first_offset, first_offset + "get".len()),
    );
    let expected_second = span_to_range(
        source,
        ql_span::Span::new(second_offset, second_offset + "get".len()),
    );
