...
{broken_line}    return {receiver}.value + {receiver}.value
}}
"#,
...
fn run_root_query_case(root: RootKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-question-unwrapped-{}-value-root-query{}",
        root.label(),
...
"#,
    ...
    if broken {
        ...
        let hover = hover_for_dependency_values(&source, &package, root_position).expect(
            "question-unwrapped dependency value root hover should exist without semantic analysis",
        );
        ...
        let definition = definition_for_dependency_values(&source, &package, root_position)
            .expect(
                "question-unwrapped dependency value root definition should exist without semantic analysis",
            );
        ...
        let declaration = declaration_for_dependency_values(&source, &package, root_position)
            .expect(
                "question-unwrapped dependency value root declaration should exist without semantic analysis",
            );
        ...
        let without_declaration =
            references_for_dependency_values(&uri, &source, &package, root_position, false)
                .expect(
                    "question-unwrapped dependency value root references should exist without semantic analysis",
                );
        assert_eq!(
            without_declaration,
            vec![
        ...
        let with_declaration =
            references_for_dependency_values(&uri, &source, &package, root_position, true)
                .expect(
                    "question-unwrapped dependency value root references with declaration should exist without semantic analysis",
                );
        ...
        let hover = hover_for_package_analysis(&source, &analysis, &package, root_position)
            .expect("question-unwrapped dependency value root hover should exist");
        let HoverContents::Markup(markup) = hover.contents else {
            panic!("hover should use markdown")
        };
        match root {
        ...
        let definition =
            definition_for_package_analysis(&uri, &source, &analysis, &package, root_position)
                .expect("question-unwrapped dependency value root definition should exist");
        let GotoDefinitionResponse::Scalar(location) = definition else {
            panic!("definition should be one location")
        };
        ...
        let declaration =
            declaration_for_package_analysis(&uri, &source, &analysis, &package, root_position)
                .expect("question-unwrapped dependency value root declaration should exist");
        let GotoDeclarationResponse::Scalar(location) = declaration else {
            panic!("declaration should be one location")
        };
        assert_dependency_location(&location, &dep_qi, "child");
        ...
        .expect("question-unwrapped dependency value root references should exist");
        assert_eq!(
            without_declaration,
            vec![
                Location::new(
        ...
        .expect(
            "question-unwrapped dependency value root references with declaration should exist",
        );
        assert_eq!(with_declaration.len(), 3);
...
{
    run_root_query_case(RootKind::Field, true);
}

#[test]
fn root_query_bridge_surfaces_question_unwrapped_dependency_method_roots() {
    run_root_query_case(RootKind::Method, false);
}
