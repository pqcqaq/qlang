...
pub fn read(flag: Bool) -> Int {{
    let first = ({receiver})[0].leaf()
    let second = ({receiver})[1].leaf()
{tail}}}
"#,
...
fn run_query_case(structured: StructuredKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-structured-question-indexed-{}-method-member-query{}",
        structured.label(),
...
"#,
    ...
    if broken {
        ...
        .expect("direct structured question indexed method member hover should exist");
        ...
        .expect("direct structured question indexed method member definition should exist");
        ...
        .expect("direct structured question indexed method member declaration should exist");
        ...
        .expect("direct structured question indexed method member references should exist");
        ...
        .expect(
            "direct structured question indexed method member references without declaration should exist",
        );
...
#[test]
fn queries_work_on_if_direct_structured_question_indexed_method_members() {
    run_query_case(StructuredKind::If, false);
}

#[test]
fn queries_work_on_match_direct_structured_question_indexed_method_members() {
    run_query_case(StructuredKind::Match, false);
}

#[test]
fn queries_fallback_on_if_direct_structured_question_indexed_method_members() {
    run_query_case(StructuredKind::If, true);
}

#[test]
fn queries_fallback_on_match_direct_structured_question_indexed_method_members() {
