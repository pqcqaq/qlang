...
pub fn read(config: Cfg) -> Int {
    let value = config.children[0].value
    return value
}
"#;
    temp.write("workspace/app/src/lib.ql", source);
    ...
    let definition = type_definition_for_package_analysis(
        ...
        &analysis,
        &package,
        offset_to_position(source, nth_offset(source, "children", 1)),
    )
    .expect("direct indexed iterable field value root type definition should exist");
    assert_targets_dependency_type(
        definition,
        &dep_qi,
        "pub struct Child {\n    value: Int,\n}",
...
#[test]
fn type_definition_fallback_follows_dependency_direct_indexed_imported_iterable_value_roots() {
    let temp = TempDir::new("ql-lsp-direct-indexed-imported-value-root-type-definition-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("direct indexed imported iterable value root type definition should exist");
    assert_targets_dependency_type(
        definition,
        &dep_qi,
        "pub struct Child {\n    value: Int,\n}",
