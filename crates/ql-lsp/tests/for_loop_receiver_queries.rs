...
use ql_lsp::bridge::{
    ...
    references_for_dependency_values, span_to_range,
};
...
#[test]
fn dependency_field_definition_works_on_for_loop_inline_tuple_receiver() {
    let temp = TempDir::new("ql-lsp-for-loop-field-query");
...
#[test]
fn dependency_method_definition_works_on_for_loop_inline_array_receiver_without_semantic_analysis()
{
    let temp = TempDir::new("ql-lsp-for-loop-method-query-broken");
    let app_root = temp.path().join("workspace").join("app");
...
#[test]
fn broken_source_references_fall_back_to_for_loop_dependency_value_root() {
    let temp = TempDir::new("ql-lsp-for-loop-value-query-broken");
    let app_root = temp.path().join("workspace").join("app");
    let app_path = temp
        .path()
        .join("workspace")
...
"#;
    ...
    .expect("dependency value references should exist");
    ...
    assert_eq!(
        references[0]
            .uri
            .to_file_path()
            .expect("dependency URI should convert to file path")
            .canonicalize()
