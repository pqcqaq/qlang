...
pub fn read() -> Int {{
{broken_line}    let first = kids()?[0]{suffix}
    return first
}}
"#,
...
fn run_type_definition_case(member: MemberKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-grouped-question-indexed-iterable-{}-member-type-definition{}",
        member.label(),
...
"#,
    ...
    if broken {
        ...
        .expect("grouped question indexed iterable member type definition should exist");
