...
fn query_offsets(source: &str) -> [usize; 5] {
    [
...
fn run_root_query_case(structured: StructuredKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-structured-question-indexed-{}-value-root-query{}",
        structured.label(),
...
"#,
    ...
    let offsets = query_offsets(&source);
    let root_position = offset_to_position(&source, offsets[1]);
    ...
    if broken {
        ...
        let hover = hover_for_dependency_values(&source, &package, root_position)
            .expect("direct structured question indexed value root hover should exist");
        ...
        let definition = definition_for_dependency_values(&source, &package, root_position)
            .expect("direct structured question indexed value root definition should exist");
        ...
        let declaration = declaration_for_dependency_values(&source, &package, root_position)
            .expect("direct structured question indexed value root declaration should exist");
        ...
        let without_declaration =
            references_for_dependency_values(&uri, &source, &package, root_position, false)
                .expect("direct structured question indexed value root references should exist");
        assert_eq!(
            without_declaration,
            vec![
                Location::new(
                    ...
                    span_to_range(
                        &source,
                        Span::new(offsets[1], offsets[1] + "maybe_children".len()),
                    ),
                ),
                Location::new(
                    uri.clone(),
                    span_to_range(
                        &source,
                        Span::new(offsets[2], offsets[2] + "maybe_children".len()),
                    ),
                ),
                Location::new(
                    uri.clone(),
                    span_to_range(
                        &source,
                        Span::new(offsets[3], offsets[3] + "maybe_children".len()),
                    ),
                ),
                Location::new(
                    uri.clone(),
                    span_to_range(
                        &source,
                        Span::new(offsets[4], offsets[4] + "maybe_children".len()),
                    ),
                ),
            ]
        );
        ...
        let with_declaration =
            references_for_dependency_values(&uri, &source, &package, root_position, true)
                .expect(
                    "direct structured question indexed value root references with declaration should exist",
                );
        ...
        assert_eq!(
            ...
            [
                Location::new(
                    ...
                    span_to_range(
                        &source,
                        Span::new(offsets[0], offsets[0] + "maybe_children".len()),
                    ),
                ),
                Location::new(
                    uri.clone(),
...
#[test]
fn root_queries_work_on_if_direct_structured_question_indexed_value_roots() {
    run_root_query_case(StructuredKind::If, false);
}

#[test]
fn root_queries_work_on_match_direct_structured_question_indexed_value_roots() {
    run_root_query_case(StructuredKind::Match, false);
}

#[test]
fn root_queries_fallback_on_if_direct_structured_question_indexed_value_roots() {
    run_root_query_case(StructuredKind::If, true);
}

#[test]
fn root_queries_fallback_on_match_direct_structured_question_indexed_value_roots() {
