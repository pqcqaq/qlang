...
fn run_query_case(structured: StructuredKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-structured-question-indexed-{}-field-member-query{}",
        structured.label(),
...
"#,
    ...
    if broken {
        ...
        .expect("direct structured question indexed field member hover should exist");
        ...
        .expect("direct structured question indexed field member definition should exist");
        let GotoDefinitionResponse::Scalar(location) = definition else {
            panic!("definition should be one location")
        };
        assert_dependency_location(&location, &dep_qi, "leaf: Leaf", "leaf");
        ...
        .expect("direct structured question indexed field member declaration should exist");
        ...
        .expect("direct structured question indexed field member references should exist");
        assert_eq!(with_declaration.len(), 3);
        assert_dependency_location(&with_declaration[0], &dep_qi, "leaf: Leaf", "leaf");
        ...
        .expect(
            "direct structured question indexed field member references without declaration should exist",
        );
...
#[test]
fn queries_work_on_if_direct_structured_question_indexed_field_members() {
    run_query_case(StructuredKind::If, false);
}

#[test]
fn queries_work_on_match_direct_structured_question_indexed_field_members() {
    run_query_case(StructuredKind::Match, false);
}

#[test]
fn queries_fallback_on_if_direct_structured_question_indexed_field_members() {
    run_query_case(StructuredKind::If, true);
}

#[test]
fn queries_fallback_on_match_direct_structured_question_indexed_field_members() {
