...
#[test]
fn root_query_fallback_surfaces_dependency_question_unwrapped_local_roots() {
    let temp = TempDir::new("ql-lsp-question-unwrapped-local-value-root-fallback-query");
...
"#;
    ...
    let hover = hover_for_dependency_values(source, &package, position)
        .expect("dependency value root hover should exist without semantic analysis");
    ...
    let definition = definition_for_dependency_values(source, &package, position)
        .expect("dependency value root definition should exist without semantic analysis");
    let GotoDefinitionResponse::Scalar(location) = definition else {
        panic!("definition should be one location")
    };
    assert_dependency_location(&location, &dep_qi, "pub struct Child {\n    value: Int,\n}");

    let declaration = declaration_for_dependency_values(source, &package, position)
        .expect("dependency value root declaration should exist without semantic analysis");
    ...
    let without_declaration =
        references_for_dependency_values(&uri, source, &package, position, false)
            .expect("dependency value root references should exist without semantic analysis");
    assert_eq!(
        without_declaration,
        vec![
            Location::new(
    ...
    let with_declaration = references_for_dependency_values(&uri, source, &package, position, true)
        .expect(
            "dependency value root references with declaration should exist without semantic analysis",
        );
