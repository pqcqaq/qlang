...
#[test]
fn package_bridge_completes_dependency_fields_for_for_loop_if_question_field_iterables() {
    let temp = TempDir::new("ql-lsp-for-loop-question-structured-field-completion");
...
"#,
    ...
    let Some(CompletionResponse::Array(items)) = completion_for_package_analysis(
        ...
        panic!("question structured iterable field completion should exist");
    };
...
#[test]
fn dependency_method_definition_works_on_for_loop_match_question_method_iterables_without_semantic_analysis()
 {
    let temp = TempDir::new("ql-lsp-for-loop-question-structured-method-query-broken");
    let app_root = temp.path().join("workspace").join("app");
