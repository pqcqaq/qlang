...
fn run_completion_case(member: MemberKind, structured: StructuredKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-structured-receiver-{}-{}-completion{}",
        structured.label(),
        member.label(),
...
"#,
    ...
    if broken {
        ...
        let Some(CompletionResponse::Array(items)) = (match member {
            ...
            panic!("structured receiver member completion should exist without semantic analysis");
        };
        ...
        let Some(CompletionResponse::Array(items)) =
            ...
            panic!("structured receiver member completion should exist");
        };
        assert_eq!(items.len(), 1);
        assert_completion_item(member, items[0].clone());
    }
...
#[test]
fn dependency_field_completion_works_on_if_structured_receivers() {
    run_completion_case(MemberKind::Field, StructuredKind::If, false);
}

#[test]
fn dependency_field_completion_works_on_if_structured_receivers_without_semantic_analysis() {
    run_completion_case(MemberKind::Field, StructuredKind::If, true);
}

#[test]
fn dependency_field_completion_works_on_match_structured_receivers() {
    run_completion_case(MemberKind::Field, StructuredKind::Match, false);
}

#[test]
fn dependency_field_completion_works_on_match_structured_receivers_without_semantic_analysis() {
    run_completion_case(MemberKind::Field, StructuredKind::Match, true);
}

#[test]
fn dependency_method_completion_works_on_if_structured_receivers() {
    run_completion_case(MemberKind::Method, StructuredKind::If, false);
}

#[test]
fn dependency_method_completion_works_on_if_structured_receivers_without_semantic_analysis() {
    run_completion_case(MemberKind::Method, StructuredKind::If, true);
}

#[test]
fn dependency_method_completion_works_on_match_structured_receivers() {
    run_completion_case(MemberKind::Method, StructuredKind::Match, false);
}

#[test]
fn dependency_method_completion_works_on_match_structured_receivers_without_semantic_analysis() {
