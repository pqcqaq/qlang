...
{broken_line}    return config.maybe_children()?[0].leaf().value.inner
}}
"#
    )
}
...
fn run_type_definition_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-indexed-question-iterable-method-result-member-type-definition{}",
...
"#,
    ...
    if broken {
        ...
        .expect(
            "direct indexed question iterable method-result member type definition should exist",
        );
...
#[test]
fn type_definition_bridge_follows_direct_indexed_question_iterable_method_result_member_types() {
    run_type_definition_case(false);
}

#[test]
fn type_definition_fallback_follows_direct_indexed_question_iterable_method_result_member_types() {
