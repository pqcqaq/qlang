...
"#
    ...
    fn source(self, broken: bool) -> &'static str {
        match (self, broken) {
            (Self::Field, false) => {
                r#"
package demo.app
...
fn run_root_query_case(kind: RootKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-indexed-{}-value-root-query{}",
        kind.label(),
...
"#,
    ...
    let dep_qi = temp.write("workspace/dep/dep.qi", kind.dependency_qi());
    temp.write(
...
"#,
    ...
    let first_usage = nth_offset(source, "children", 1);
    let second_usage = nth_offset(source, "children", 2);
    ...
    if broken {
        ...
        let hover =
            hover_for_dependency_values(source, &package, offset_to_position(source, first_usage))
                .expect("direct indexed iterable value root hover should exist");
        ...
        .expect("direct indexed iterable value root definition should exist");
        ...
        .expect("direct indexed iterable value root declaration should exist");
        ...
        .expect("direct indexed iterable value root references should exist");
        assert_eq!(
            without_declaration,
            vec![
                Location::new(
                    ...
                    span_to_range(
                        source,
                        Span::new(first_usage, first_usage + "children".len()),
                    ),
                ),
                Location::new(
                    uri.clone(),
                    span_to_range(
                        source,
                        Span::new(second_usage, second_usage + "children".len()),
                    ),
                ),
            ]
        );
