...
{broken_line}    let first = config.maybe_children()?[0]{suffix}
    return first
}}
"#,
...
fn run_type_definition_case(member: MemberKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-indexed-question-iterable-{}-member-type-definition{}",
        member.label(),
...
"#,
    ...
    if broken {
        ...
        .expect("direct indexed question iterable member type definition should exist");
...
#[test]
fn type_definition_bridge_follows_direct_indexed_question_iterable_field_member_types() {
    run_type_definition_case(MemberKind::Field, false);
}

#[test]
fn type_definition_fallback_follows_direct_indexed_question_iterable_field_member_types() {
    run_type_definition_case(MemberKind::Field, true);
}

#[test]
fn type_definition_bridge_follows_direct_indexed_question_iterable_method_member_types() {
    run_type_definition_case(MemberKind::Method, false);
}

#[test]
fn type_definition_fallback_follows_direct_indexed_question_iterable_method_member_types() {
