...
#[test]
fn dependency_method_definition_works_on_for_loop_match_array_receiver_without_semantic_analysis() {
    let temp = TempDir::new("ql-lsp-for-loop-match-method-query-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("dependency structured loop method definition should exist");
