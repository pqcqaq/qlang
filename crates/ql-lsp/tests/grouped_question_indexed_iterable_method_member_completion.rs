...
fn run_completion_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-grouped-question-indexed-iterable-method-member-completion{}",
...
"#,
    ...
    let Some(CompletionResponse::Array(items)) = completion else {
        panic!(
            "grouped question indexed iterable method member completion should exist{}",
            if broken {
...
#[test]
fn completion_works_on_grouped_question_indexed_iterable_method_members() {
    run_completion_case(false);
}

#[test]
fn completion_fallback_works_on_grouped_question_indexed_iterable_method_members() {
