...
#[test]
fn dependency_method_completion_works_on_question_unwrapped_local_extend_method_result_projected_field_alias_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new(
        "ql-lsp-question-unwrapped-local-extend-method-projected-field-alias-method-completion-broken",
    );
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!(
            "dependency extend-method-result projected-field alias method completion should exist without semantic analysis"
        );
    };
