...
fn run_query_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-indexed-question-iterable-field-member-query{}",
...
"#,
    ...
    if broken {
        ...
        .expect(
            "direct indexed question iterable field member hover should exist without semantic analysis",
        );
...
#[test]
fn queries_work_on_direct_indexed_question_iterable_field_members() {
    run_query_case(false);
}

#[test]
fn queries_fallback_on_direct_indexed_question_iterable_field_members() {
    run_query_case(true);
}
