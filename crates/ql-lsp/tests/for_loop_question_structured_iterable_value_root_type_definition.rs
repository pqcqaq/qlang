...
fn run_type_definition_case(binding: BindingKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-for-loop-question-structured-iterable-{}-value-root-type-definition{}",
        binding.label(),
...
"#,
    ...
    if broken {
        ...
        .expect("structured dependency question iterable value root type definition should exist");
...
#[test]
fn type_definition_bridge_follows_dependency_for_loop_if_question_structured_field_bindings() {
    run_type_definition_case(BindingKind::Field, false);
}

#[test]
fn type_definition_bridge_follows_dependency_for_loop_match_question_structured_method_bindings() {
    run_type_definition_case(BindingKind::Method, false);
}

#[test]
fn type_definition_fallback_follows_dependency_for_loop_if_question_structured_field_bindings() {
    run_type_definition_case(BindingKind::Field, true);
}

#[test]
fn type_definition_fallback_follows_dependency_for_loop_match_question_structured_method_bindings()
{
    run_type_definition_case(BindingKind::Method, true);
}
