...
#[test]
fn package_bridge_completes_dependency_fields_for_for_loop_inline_tuple_receivers() {
    let temp = TempDir::new("ql-lsp-for-loop-field-completion");
...
"#,
    ...
    let Some(CompletionResponse::Array(items)) = completion_for_package_analysis(
        ...
        panic!("dependency loop field completion should exist");
    };
...
#[test]
fn package_bridge_completes_dependency_methods_for_for_loop_inline_array_receivers_without_semantic_analysis()
 {
    let temp = TempDir::new("ql-lsp-for-loop-method-broken-completion");
    let app_root = temp.path().join("workspace").join("app");
    let source = r#"
package demo.app
...
"#,
    ...
    let Some(CompletionResponse::Array(items)) = completion_for_dependency_methods(
        ...
        panic!("dependency loop method completion should exist without semantic analysis");
    };
    assert_eq!(items.len(), 1);
