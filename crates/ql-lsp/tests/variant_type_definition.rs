...
use ql_lsp::bridge::{
    span_to_range, type_definition_for_dependency_variants, type_definition_for_package_analysis,
};
...
fn assert_targets_dependency_enum(
    ...
    assert_eq!(
        ...
        span_to_range(
            &artifact,
            ql_span::Span::new(enum_def, enum_def + snippet.len())
        )
    );
}

#[test]
fn type_definition_bridge_follows_dependency_variant_enum_types() {
    let temp = TempDir::new("ql-lsp-variant-type-definition");
...
pub fn main(flag: Bool) -> Int {
    let command = if flag { Cmd.Retry } else { Cmd.Abort }
    match command {
        Cmd.Retry => 1,
        Cmd.Abort => 0,
    }
...
"#;
    ...
    .expect("dependency variant type definition should exist");
...
#[test]
fn type_definition_bridge_follows_dependency_variant_enum_types_without_semantic_analysis() {
    let temp = TempDir::new("ql-lsp-variant-type-definition-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("dependency variant type definition should exist without semantic analysis");
    assert_targets_dependency_enum(
        definition,
        &dep_qi,
        "pub enum Command {\n    Retry,\n    Abort,\n}",
