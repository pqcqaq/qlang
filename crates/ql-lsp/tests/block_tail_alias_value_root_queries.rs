...
fn run_root_query_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-block-tail-alias-value-root-query{}",
    ...
    if broken {
        ...
        let hover =
            hover_for_dependency_values(source, &package, offset_to_position(source, alias_usage))
                .expect("dependency block-tail alias value root hover should exist");
        ...
        .expect("dependency block-tail alias value root references should exist");
        assert_eq!(
            without_declaration,
            vec![
                Location::new(
        ...
        .expect("dependency block-tail alias value root references with declaration should exist");
...
#[test]
fn root_query_bridge_surfaces_dependency_block_tail_alias_value_roots() {
    run_root_query_case(false);
}

#[test]
fn root_query_fallback_surfaces_dependency_block_tail_alias_value_roots() {
