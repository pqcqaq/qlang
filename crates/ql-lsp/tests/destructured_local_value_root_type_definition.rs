...
fn run_type_definition_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-destructured-local-value-root-type-definition{}",
...
"#,
    ...
    if broken {
        ...
        let definition =
            type_definition_for_dependency_values(source, &package, offset_to_position(source, child_usage))
                .expect("dependency destructured local value root type definition should exist");
...
#[test]
fn type_definition_bridge_follows_dependency_destructured_local_value_roots() {
    run_type_definition_case(false);
}

#[test]
fn type_definition_fallback_follows_dependency_destructured_local_value_roots() {
