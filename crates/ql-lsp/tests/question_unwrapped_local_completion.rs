...
fn run_completion_case(root: RootKind, member: MemberKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-question-unwrapped-local-{}-{}-completion{}",
        root.label(),
        member.label(),
...
"#,
    ...
    temp.write("workspace/dep/dep.qi", root.dependency_qi());
    temp.write(
...
"#,
    ...
    if broken {
        ...
        let Some(CompletionResponse::Array(items)) = (match member {
            ...
            panic!("question-unwrapped local member completion should exist without semantic analysis");
        };
        ...
        let Some(CompletionResponse::Array(items)) =
            ...
            panic!("question-unwrapped local member completion should exist");
        };
        assert_eq!(items.len(), 1);
        assert_completion_item(member, items[0].clone());
    }
...
#[test]
fn dependency_field_completion_works_on_question_unwrapped_local_receivers() {
    run_completion_case(RootKind::Field, MemberKind::Field, false);
}

#[test]
fn dependency_field_completion_works_on_question_unwrapped_local_receivers_without_semantic_analysis(
) {
    run_completion_case(RootKind::Field, MemberKind::Field, true);
}

#[test]
fn dependency_method_completion_works_on_question_unwrapped_local_receivers() {
    run_completion_case(RootKind::Method, MemberKind::Method, false);
}

#[test]
fn dependency_method_completion_works_on_question_unwrapped_local_receivers_without_semantic_analysis(
) {
    run_completion_case(RootKind::Method, MemberKind::Method, true);
}
