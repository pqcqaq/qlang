...
fn run_completion_case(member: MemberKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-for-loop-iterable-alias-{}-completion{}",
        member.label(),
...
"#,
    ...
    if broken {
        ...
        let Some(CompletionResponse::Array(items)) = (match member {
            ...
            panic!("iterable alias member completion should exist without semantic analysis");
        };
        ...
        let Some(CompletionResponse::Array(items)) =
            ...
            panic!("iterable alias member completion should exist");
        };
        assert_eq!(items.len(), 1);
        assert_member_completion_item(member, &items[0]);
    }
...
#[test]
fn dependency_field_completion_works_on_for_loop_iterable_alias_receivers() {
...
#[test]
fn dependency_method_completion_works_on_for_loop_iterable_alias_receivers() {
