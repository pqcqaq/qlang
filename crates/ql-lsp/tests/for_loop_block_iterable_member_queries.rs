...
impl CaseKind {
    fn label(self) -> &'static str {
        match self {
            Self::BlockTupleField => "block-tuple-field",
            Self::BlockArrayMethod => "block-array-method",
        }
    }

    fn member_token(self) -> &'static str {
        match self {
            Self::BlockTupleField => "value",
            Self::BlockArrayMethod => "get",
        }
    }
    ...
    fn hover_kind(self) -> &'static str {
        match self {
            Self::BlockTupleField => "**field** `value`",
            Self::BlockArrayMethod => "**method** `get`",
        }
    }

    fn hover_detail(self) -> &'static str {
        match self {
            Self::BlockTupleField => "field value: Int",
            Self::BlockArrayMethod => "fn get(self) -> Int",
        }
    }

    fn declaration_anchor(self) -> &'static str {
        match self {
            Self::BlockTupleField => "value: Int",
            Self::BlockArrayMethod => "pub fn get(self) -> Int",
        }
    }
...
fn assert_member_queries(
    ...
    if broken {
        ...
        let hover = match case {
            CaseKind::BlockTupleField => hover_for_dependency_struct_fields(
                source,
                &package,
                offset_to_position(source, first_offset),
            ),
            CaseKind::BlockArrayMethod => hover_for_dependency_methods(
                source,
                &package,
                offset_to_position(source, first_offset),
            ),
        }
        .expect("block iterable member hover should exist without semantic analysis");
        ...
        let definition = match case {
            CaseKind::BlockTupleField => definition_for_dependency_struct_fields(
                source,
                &package,
                offset_to_position(source, first_offset),
            ),
            CaseKind::BlockArrayMethod => definition_for_dependency_methods(
                source,
                &package,
                offset_to_position(source, first_offset),
            ),
        }
        .expect("block iterable member definition should exist without semantic analysis");
        ...
        let declaration = match case {
            CaseKind::BlockTupleField => declaration_for_dependency_struct_fields(
                source,
                &package,
                offset_to_position(source, second_offset),
            ),
            CaseKind::BlockArrayMethod => declaration_for_dependency_methods(
                source,
                &package,
                offset_to_position(source, second_offset),
            ),
        }
        .expect("block iterable member declaration should exist without semantic analysis");
        ...
        let with_declaration = match case {
            CaseKind::BlockTupleField => references_for_dependency_struct_fields(
                uri,
                source,
                &package,
                offset_to_position(source, second_offset),
            ...
            CaseKind::BlockArrayMethod => references_for_dependency_methods(
                uri,
                source,
                &package,
                offset_to_position(source, second_offset),
        ...
        .expect("block iterable member references should exist without semantic analysis");
        ...
        .expect("block iterable member references should exist without declaration");
        ...
        .expect("block iterable member hover should exist");
        let HoverContents::Markup(markup) = hover.contents else {
            panic!("hover should use markdown")
        };
        assert!(markup.value.contains(case.hover_kind()));
        ...
        .expect("block iterable member definition should exist");
        let GotoDefinitionResponse::Scalar(definition_location) = definition else {
            panic!("definition should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("block iterable member declaration should exist");
        let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
            panic!("declaration should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("block iterable member references should exist");
        assert_eq!(with_declaration.len(), 3);
        assert_location_targets_dependency_name(
            &with_declaration[0],
            dep_qi,
...
fn run_member_query_case(case: CaseKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-for-loop-block-iterable-{}-queries{}",
        case.label(),
...
#[test]
fn dependency_field_queries_work_on_block_tuple_for_loop_receivers() {
    run_member_query_case(CaseKind::BlockTupleField, false);
}

#[test]
fn dependency_field_queries_work_on_block_tuple_for_loop_receivers_without_semantic_analysis() {
    run_member_query_case(CaseKind::BlockTupleField, true);
}

#[test]
fn dependency_method_queries_work_on_block_array_for_loop_receivers() {
    run_member_query_case(CaseKind::BlockArrayMethod, false);
}

#[test]
fn dependency_method_queries_work_on_block_array_for_loop_receivers_without_semantic_analysis() {
    run_member_query_case(CaseKind::BlockArrayMethod, true);
}
