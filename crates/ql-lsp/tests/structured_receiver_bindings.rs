...
#[test]
fn dependency_field_definition_works_on_if_result_local_receiver() {
    let temp = TempDir::new("ql-lsp-structured-receiver-if");
...
#[test]
fn dependency_method_definition_works_on_match_result_local_receiver_without_semantic_analysis() {
    let temp = TempDir::new("ql-lsp-structured-receiver-match-broken");
    let app_root = temp.path().join("workspace").join("app");
