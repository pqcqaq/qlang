...
pub fn main() -> Int {{
    {prefix}{receiver}.value + {receiver}.value{suffix}
}}
"#,
...
fn assert_root_queries(
    ...
    if broken {
        ...
        let hover = hover_for_dependency_values(source, &package, root_position)
            .expect("grouped dependency question root hover should exist");
        ...
        let definition = definition_for_dependency_values(source, &package, root_position)
            .expect("grouped dependency question root definition should exist");
        ...
        let declaration = declaration_for_dependency_values(source, &package, root_position)
            .expect("grouped dependency question root declaration should exist");
        ...
        let without_declaration =
            references_for_dependency_values(uri, source, &package, root_position, false)
                .expect("grouped dependency question root references should exist");
        assert_eq!(
            without_declaration,
            vec![
                Location::new(
...
#[test]
fn root_queries_work_on_grouped_question_function_value_roots() {
    run_root_query_case(RootKind::Function, false);
}

#[test]
fn root_queries_work_on_grouped_question_function_value_roots_without_semantic_analysis() {
    run_root_query_case(RootKind::Function, true);
}

#[test]
fn root_queries_work_on_grouped_question_static_value_roots() {
    run_root_query_case(RootKind::Static, false);
}

#[test]
fn root_queries_work_on_grouped_question_static_value_roots_without_semantic_analysis() {
