...
fn run_completion_case(member: MemberKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-for-loop-field-iterable-{}-completion{}",
        member.label(),
...
"#,
    ...
    if broken {
        ...
        let Some(CompletionResponse::Array(items)) = (match member {
            ...
            panic!("field-iterable member completion should exist without semantic analysis");
        };
        ...
        let Some(CompletionResponse::Array(items)) =
            ...
            panic!("field-iterable member completion should exist");
        };
        assert_eq!(items.len(), 1);
        assert_member_completion_item(member, &items[0]);
    }
...
#[test]
fn dependency_field_completion_works_on_for_loop_field_iterable_receivers() {
    run_completion_case(MemberKind::Field, false);
}

#[test]
fn dependency_field_completion_works_on_for_loop_field_iterable_receivers_without_semantic_analysis(
) {
    run_completion_case(MemberKind::Field, true);
}

#[test]
fn dependency_method_completion_works_on_for_loop_field_iterable_receivers() {
    run_completion_case(MemberKind::Method, false);
}

#[test]
fn dependency_method_completion_works_on_for_loop_field_iterable_receivers_without_semantic_analysis(
) {
