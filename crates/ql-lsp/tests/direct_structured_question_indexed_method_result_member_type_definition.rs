...
{broken_line}    return ({receiver})[0].leaf().value.inner
}}
"#,
...
fn run_type_definition_case(structured: StructuredKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-structured-question-indexed-method-result-member-type-definition-{}{}",
        structured.label(),
...
"#,
    ...
    let position = offset_to_position(&source, nth_offset(&source, "value", 1));
    ...
    if broken {
        ...
        let definition =
            type_definition_for_dependency_struct_field_types(&source, &package, position)
                .expect(
                    "direct structured question indexed method-result member type definition should exist without semantic analysis",
                );
        ...
        .expect(
            "direct structured question indexed method-result member type definition should exist",
        );
        assert_targets_dependency_type(
            definition,
            &dep_qi,
...
#[test]
fn type_definition_bridge_follows_if_direct_structured_question_indexed_method_result_member_types()
{
    run_type_definition_case(StructuredKind::If, false);
}

#[test]
fn type_definition_fallback_follows_if_direct_structured_question_indexed_method_result_member_types(
) {
    run_type_definition_case(StructuredKind::If, true);
}

#[test]
fn type_definition_bridge_follows_match_direct_structured_question_indexed_method_result_member_types(
) {
    run_type_definition_case(StructuredKind::Match, false);
}

#[test]
fn type_definition_fallback_follows_match_direct_structured_question_indexed_method_result_member_types(
) {
