...
impl CaseKind {
    fn label(self) -> &'static str {
        match self {
            Self::ConfigChildrenField => "config-children-field",
            Self::SelfPairMethod => "self-pair-method",
        }
    }
    ...
    fn hover_kind(self) -> &'static str {
        match self {
            Self::ConfigChildrenField => "**field** `value`",
            Self::SelfPairMethod => "**method** `get`",
        }
    }
    ...
    fn declaration_anchor(self) -> &'static str {
        match self {
            Self::ConfigChildrenField => "value: Int",
            Self::SelfPairMethod => "pub fn get(self) -> Int",
        }
    }
    ...
    fn build_source(self, broken: bool) -> String {
        ...
        match self {
            Self::ConfigChildrenField => format!(
                r#"
package demo.app
...
fn assert_member_queries(
    ...
    if broken {
        ...
        let hover = match case {
            CaseKind::ConfigChildrenField => hover_for_dependency_struct_fields(
                source,
                &package,
                offset_to_position(source, first_offset),
            ),
            CaseKind::SelfPairMethod => hover_for_dependency_methods(
                source,
                &package,
                offset_to_position(source, first_offset),
            ),
        }
        .expect("local-method iterable member hover should exist without semantic analysis");
        ...
        let definition = match case {
            CaseKind::ConfigChildrenField => definition_for_dependency_struct_fields(
                source,
                &package,
                offset_to_position(source, first_offset),
            ),
            CaseKind::SelfPairMethod => definition_for_dependency_methods(
                source,
                &package,
                offset_to_position(source, first_offset),
            ),
        }
        .expect("local-method iterable member definition should exist without semantic analysis");
        ...
        let declaration = match case {
            CaseKind::ConfigChildrenField => declaration_for_dependency_struct_fields(
                source,
                &package,
                offset_to_position(source, second_offset),
            ),
            CaseKind::SelfPairMethod => declaration_for_dependency_methods(
                source,
                &package,
                offset_to_position(source, second_offset),
            ),
        }
        .expect("local-method iterable member declaration should exist without semantic analysis");
        ...
        let with_declaration = match case {
            CaseKind::ConfigChildrenField => references_for_dependency_struct_fields(
                uri,
                source,
                &package,
                offset_to_position(source, second_offset),
            ...
            CaseKind::SelfPairMethod => references_for_dependency_methods(
                uri,
                source,
                &package,
                offset_to_position(source, second_offset),
        ...
        .expect("local-method iterable member references should exist without semantic analysis");
        ...
        .expect("local-method iterable member hover should exist");
        let HoverContents::Markup(markup) = hover.contents else {
            panic!("hover should use markdown")
        };
        assert!(markup.value.contains(case.hover_kind()));
        ...
        .expect("local-method iterable member definition should exist");
        let GotoDefinitionResponse::Scalar(definition_location) = definition else {
            panic!("definition should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("local-method iterable member declaration should exist");
        let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
            panic!("declaration should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("local-method iterable member references should exist");
        assert_eq!(with_declaration.len(), 3);
        assert_location_targets_dependency_name(
            &with_declaration[0],
            dep_qi,
...
fn run_member_query_case(case: CaseKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-for-loop-local-method-iterable-{}-queries{}",
        case.label(),
...
#[test]
fn dependency_field_queries_work_on_for_loop_local_method_iterable_receivers() {
    run_member_query_case(CaseKind::ConfigChildrenField, false);
}
...
#[test]
fn dependency_method_queries_work_on_for_loop_self_local_method_iterable_receivers() {
    run_member_query_case(CaseKind::SelfPairMethod, false);
}
