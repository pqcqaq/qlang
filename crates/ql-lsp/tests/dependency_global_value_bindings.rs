...
#[test]
fn package_bridge_completes_dependency_fields_for_imported_dependency_const_receivers() {
    let temp = TempDir::new("ql-lsp-dependency-global-field-completion");
...
"#,
    ...
    let Some(CompletionResponse::Array(items)) = completion_for_package_analysis(
        ...
        panic!("dependency const field completion should exist");
    };
...
#[test]
fn dependency_field_definition_works_on_question_wrapped_imported_dependency_static_receivers() {
    let temp = TempDir::new("ql-lsp-dependency-global-question-field");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("dependency static field definition should exist");

    assert_targets_dependency_snippet(definition, &dep_qi, "value");
}

#[test]
fn dependency_method_definition_works_on_for_loop_imported_dependency_const_iterable_receivers_without_semantic_analysis()
 {
    let temp = TempDir::new("ql-lsp-dependency-global-iterable-query-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("dependency iterable const method definition should exist");
