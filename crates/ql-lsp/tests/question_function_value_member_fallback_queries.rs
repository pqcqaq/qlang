...
fn dependency_field_queries_work_on_question_function_value_receivers_without_semantic_analysis() {
    let temp = TempDir::new("ql-lsp-question-function-value-member-query-broken");
...
pub fn read() -> Int {
    let first = maybe_load()?.value
    let second = maybe_load()?.value
    return "oops"
}
"#;
    ...
    let expected_first = span_to_range(
        source,
        ql_span::Span::new(first_offset, first_offset + "value".len()),
    );
    let expected_second = span_to_range(
        source,
        ql_span::Span::new(second_offset, second_offset + "value".len()),
    );
