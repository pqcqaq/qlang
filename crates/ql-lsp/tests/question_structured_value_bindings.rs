...
#[test]
fn dependency_field_definition_works_on_structured_question_function_and_static_value_receivers() {
    let temp = TempDir::new("ql-lsp-question-structured-value-field-binding");
...
pub fn read(flag: Bool) -> Int {{
    if flag {{
        return ({function_root}).value
    }}
    return ({static_root}).value
}}
"#,
        function_root = StructuredKind::If.wrap("maybe_load()?"),
        static_root = StructuredKind::Match.wrap("child?"),
    );
    ...
    .expect("structured question function field definition should exist");
    ...
    .expect("structured question static field definition should exist");
    assert_targets_dependency_snippet(static_definition, &dep_qi, "value");
}

#[test]
fn dependency_method_definition_works_on_structured_question_function_and_static_value_receivers_without_semantic_analysis()
 {
    let temp = TempDir::new("ql-lsp-question-structured-value-method-binding-broken");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read(flag: Bool) -> Int {{
    if flag {{
        let first = ({function_root}).get()
    }} else {{
        let second = ({static_root}).get()
    }}
    return "oops"
}}
"#,
        function_root = StructuredKind::Match.wrap("maybe_load()?"),
        static_root = StructuredKind::If.wrap("child?"),
    );
    temp.write("workspace/app/src/lib.ql", &source);
    ...
    .expect("structured question function method definition should exist");
    ...
    .expect("structured question static method definition should exist");
