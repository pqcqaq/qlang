...
#[test]
fn dependency_field_definition_works_on_destructured_local_receivers() {
    let temp = TempDir::new("ql-lsp-destructured-local-field-binding");
...
#[test]
fn dependency_method_definition_works_on_destructured_local_receivers_without_semantic_analysis() {
    let temp = TempDir::new("ql-lsp-destructured-local-method-binding-broken");
    let app_root = temp.path().join("workspace").join("app");
