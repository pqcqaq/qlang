...
fn assert_root_queries(
    ...
    if broken {
        ...
        let hover = hover_for_dependency_values(source, &package, root_position)
            .expect("dependency question iterable root hover should exist");
        ...
        let definition = definition_for_dependency_values(source, &package, root_position)
            .expect("dependency question iterable root definition should exist");
        ...
        let declaration = declaration_for_dependency_values(source, &package, root_position)
            .expect("dependency question iterable root declaration should exist");
        ...
        let without_declaration =
            references_for_dependency_values(uri, source, &package, root_position, false)
                .expect("dependency question iterable root references should exist");
        assert_eq!(
            without_declaration,
            vec![
                Location::new(
        ...
        let with_declaration =
            references_for_dependency_values(uri, source, &package, root_position, true).expect(
                "dependency question iterable root references with declaration should exist",
            );
...
#[test]
fn root_queries_work_on_question_function_iterable_roots() {
    run_root_query_case(RootKind::Function, false);
}

#[test]
fn root_queries_work_on_question_function_iterable_roots_without_semantic_analysis() {
    run_root_query_case(RootKind::Function, true);
}

#[test]
fn root_queries_work_on_question_static_iterable_roots() {
    run_root_query_case(RootKind::Static, false);
}

#[test]
fn root_queries_work_on_question_static_iterable_roots_without_semantic_analysis() {
