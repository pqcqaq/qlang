...
#[test]
fn dependency_field_definition_works_on_typed_closure_receivers() {
    let temp = TempDir::new("ql-lsp-typed-closure-field");
...
#[test]
fn dependency_method_definition_works_on_typed_closure_receivers_without_semantic_analysis() {
    let temp = TempDir::new("ql-lsp-typed-closure-method-broken");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read(config: Cfg) -> Int {
    let project = (current: Cfg) => current.get()
    let value = project(config)
    return "oops"
}
"#;
    temp.write("workspace/app/src/lib.ql", source);
