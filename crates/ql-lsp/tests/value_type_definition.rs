...
#[test]
fn type_definition_bridge_follows_dependency_struct_types_for_named_local_values() {
    let temp = TempDir::new("ql-lsp-value-type-definition");
...
"#;
    ...
    let definition = type_definition_for_package_analysis(
        ...
        &analysis,
        &package,
        offset_to_position(source, nth_offset(source, "alias", 2)),
    )
    .expect("dependency value type definition should exist");
...
#[test]
fn type_definition_bridge_follows_dependency_struct_types_for_named_local_values_without_semantic_analysis()
 {
    let temp = TempDir::new("ql-lsp-value-type-definition-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("dependency value type definition should exist without semantic analysis");
    assert_targets_dependency_struct(
        definition,
        &dep_qi,
        "pub struct Config {\n    value: Int,\n}",
