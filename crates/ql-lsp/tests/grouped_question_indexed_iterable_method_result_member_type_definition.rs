...
pub fn read() -> Int {{
{broken_line}    return kids()?[0].leaf().value.inner
}}
"#
    )
}
...
fn run_type_definition_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-grouped-question-indexed-iterable-method-result-member-type-definition{}",
...
"#,
    ...
    if broken {
        ...
        .expect(
            "grouped question indexed iterable method-result member type definition should exist",
        );
...
#[test]
fn type_definition_bridge_follows_grouped_question_indexed_iterable_method_result_member_types() {
    run_type_definition_case(false);
}

#[test]
fn type_definition_fallback_follows_grouped_question_indexed_iterable_method_result_member_types() {
