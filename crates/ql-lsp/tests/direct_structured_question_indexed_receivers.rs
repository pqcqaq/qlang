...
#[test]
fn dependency_field_completion_works_on_direct_if_question_indexed_receivers() {
    let temp = TempDir::new("ql-lsp-direct-structured-question-indexed-field-completion");
...
pub fn read(flag: Bool) -> Int {
    return (if flag { maybe_children()? } else { maybe_children()? })[0].va
}
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!("direct structured question indexed field completion should exist");
    };
...
#[test]
fn dependency_method_completion_works_on_direct_if_question_indexed_receivers() {
    let temp = TempDir::new("ql-lsp-direct-structured-question-indexed-method-completion");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read(flag: Bool) -> Int {
    let value = (if flag { maybe_children()? } else { maybe_children()? })[0].ge
    return value
}
"#;
    temp.write("workspace/app/src/lib.ql", source);
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!("direct structured question indexed method completion should exist");
    };
...
#[test]
fn dependency_method_queries_work_on_direct_if_question_indexed_receivers_without_semantic_analysis()
 {
    let temp = TempDir::new("ql-lsp-direct-structured-question-indexed-method-queries-broken");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read(flag: Bool) -> Int {
    let first = (if flag { maybe_children()? } else { maybe_children()? })[0].get()
    let second = (if flag { maybe_children()? } else { maybe_children()? })[1].get()
    return "oops"
}
"#;
    ...
    .expect(
        "direct structured question indexed method hover should exist without semantic analysis",
    );
    ...
    .expect(
        "direct structured question indexed method definition should exist without semantic analysis"
    );
    ...
    .expect(
        "direct structured question indexed method declaration should exist without semantic analysis"
    );
    ...
    .expect("direct structured question indexed method references should exist without semantic analysis");
    ...
    .expect(
        "direct structured question indexed method references should exist without declaration in fallback",
    );
...
#[test]
fn dependency_method_queries_work_on_direct_if_question_indexed_receivers() {
    let temp = TempDir::new("ql-lsp-direct-structured-question-indexed-method-queries");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("direct structured question indexed method hover should exist");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    assert!(markup.value.contains("**method** `get`"));
    ...
    .expect("direct structured question indexed method definition should exist");
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    assert_location_targets_dependency_name(
    ...
    .expect("direct structured question indexed method declaration should exist");
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    assert_location_targets_dependency_name(
    ...
    .expect("direct structured question indexed method references should exist");
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(
        &with_declaration[0],
        &dep_qi,
    ...
    .expect("direct structured question indexed method references should exist without declaration");
    assert_eq!(without_declaration.len(), 2);
    assert!(
        without_declaration
            .iter()
