...
use common::request::{
    TempDir, did_open_via_request, goto_implementation_via_request, initialize_service,
    initialized_service_with_open_documents, nth_offset, nth_offset_in_context, offset_to_position,
};
use ql_lsp::Backend;
use tower_lsp::lsp_types::request::GotoImplementationResponse;
use tower_lsp::lsp_types::{DidOpenTextDocumentParams, Location, TextDocumentItem, Url};
...
struct WorkspaceDependencyTraitMethodFixture {
    _temp: TempDir,
    app_source: String,
    app_uri: Url,
    tools_source: String,
...
fn setup_workspace_dependency_trait_method_fixture(
    prefix: &str,
    app_source: &str,
    tools_source: &str,
    bots_source: Option<&str>,
) -> WorkspaceDependencyTraitMethodFixture {
    let temp = TempDir::new(prefix);
    let app_path = temp.write("workspace/packages/app/src/main.ql", app_source);
    temp.write(
        "workspace/packages/core/src/lib.ql",
...
"#,
    ...
    let bots_path =
        bots_source.map(|bots_source| temp.write("workspace/packages/bots/src/lib.ql", bots_source));
    temp.write(
        "workspace/qlang.toml",
        if bots_source.is_some() {
            r#"
[workspace]
...
#[tokio::test(flavor = "current_thread")]
async fn implementation_request_uses_broken_open_local_dependency_method_call() {
    let temp =
        TempDir::new("ql-lsp-implementation-request-open-local-dependency-method-call-broken-open");
    let app_path = temp.write(
        "workspace/packages/app/src/main.ql",
        r#"
package demo.app
...
"#
    .to_owned();
    assert!(
        ql_analysis::analyze_source(&open_alpha_source).is_err(),
        "open dependency source should stay broken for this regression",
    );
    let source = fs::read_to_string(&app_path).expect("app source should read");
    let app_uri = Url::from_file_path(&app_path).expect("app path should convert to URI");
    let alpha_uri =
    ...
    assert_eq!(
        goto_implementation_via_request(&mut service, app_uri.clone(), pulse_position).await,
        None,
        "disk-only implementation should miss broken-open dependency method call",
    );

    did_open_via_request(&mut service, alpha_uri.clone(), open_alpha_source.clone()).await;
    let implementation = goto_implementation_via_request(&mut service, app_uri, pulse_position)
        .await
        .expect("broken-open dependency method call implementation should use open source");
    let GotoImplementationResponse::Scalar(location) = implementation else {
        panic!("broken-open dependency concrete method call should resolve to one implementation")
    };
    assert_eq!(location.uri, alpha_uri);
    assert_eq!(
        location.range.start,
...
async fn implementation_request_returns_array_for_workspace_dependency_trait_method_call() {
    let fixture = setup_workspace_dependency_trait_method_fixture(
        "ql-lsp-implementation-request-workspace-dependency-trait-method-call",
        r#"
package demo.app
...
"#,
    ...
    let bots_source = fixture
        .bots_source
        .as_ref()
        .expect("bots source should exist for aggregation fixture");
    let bots_uri = fixture
        .bots_uri
        .as_ref()
    ...
    let implementation = goto_implementation_via_request(
        &mut service,
        fixture.app_uri.clone(),
        offset_to_position(&fixture.app_source, nth_offset(&fixture.app_source, "run()", 1)),
    )
    .await
    ...
    assert_eq!(locations.len(), 2);
    for (uri, source) in [
        (&fixture.tools_uri, &fixture.tools_source),
        (bots_uri, bots_source),
    ] {
        assert!(
            locations.iter().any(|location| {
                location.uri == *uri
...
async fn implementation_request_prefers_open_workspace_source_for_dependency_trait_method_call() {
    let fixture = setup_workspace_dependency_trait_method_fixture(
        "ql-lsp-implementation-request-open-workspace-dependency-trait-method-call",
        r#"
package demo.app
...
"#
    ...
    assert_eq!(uri, fixture.tools_uri);
    assert_eq!(
        range.start,
        offset_to_position(
            &open_tools_source,
...
{
    let fixture = setup_workspace_dependency_trait_method_fixture(
        "ql-lsp-implementation-request-broken-open-dependency-trait-method-call",
        r#"
package demo.app
...
"#
    .to_owned();
    assert!(
        ql_analysis::analyze_source(&fixture.app_source).is_err(),
        "current source should stay broken for this regression",
    );
