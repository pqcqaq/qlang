...
#[test]
fn dependency_field_definition_works_on_local_bound_from_dependency_method_result() {
    let temp = TempDir::new("ql-lsp-method-result-field");
...
#[test]
fn dependency_method_definition_works_on_local_bound_from_dependency_method_result_without_semantic_analysis()
 {
    let temp = TempDir::new("ql-lsp-method-result-method-broken");
    let app_root = temp.path().join("workspace").join("app");
