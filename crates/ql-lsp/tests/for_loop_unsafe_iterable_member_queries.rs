...
impl CaseKind {
    ...
    fn hover_kind(self) -> &'static str {
        match self {
            Self::UnsafeTupleField => "**field** `value`",
            Self::UnsafeArrayMethod => "**method** `get`",
        }
    }
    ...
    fn declaration_anchor(self) -> &'static str {
        match self {
            Self::UnsafeTupleField => "value: Int",
            Self::UnsafeArrayMethod => "pub fn get(self) -> Int",
        }
    }
...
fn assert_member_queries(
    ...
    if broken {
        ...
        let hover = match case {
            CaseKind::UnsafeTupleField => hover_for_dependency_struct_fields(
                source,
                &package,
                offset_to_position(source, first_offset),
            ),
            CaseKind::UnsafeArrayMethod => hover_for_dependency_methods(
                source,
                &package,
                offset_to_position(source, first_offset),
            ),
        }
        .expect("unsafe iterable member hover should exist without semantic analysis");
        ...
        let definition = match case {
            CaseKind::UnsafeTupleField => definition_for_dependency_struct_fields(
                source,
                &package,
                offset_to_position(source, first_offset),
            ),
            CaseKind::UnsafeArrayMethod => definition_for_dependency_methods(
                source,
                &package,
                offset_to_position(source, first_offset),
            ),
        }
        .expect("unsafe iterable member definition should exist without semantic analysis");
        ...
        let declaration = match case {
            CaseKind::UnsafeTupleField => declaration_for_dependency_struct_fields(
                source,
                &package,
                offset_to_position(source, second_offset),
            ),
            CaseKind::UnsafeArrayMethod => declaration_for_dependency_methods(
                source,
                &package,
                offset_to_position(source, second_offset),
            ),
        }
        .expect("unsafe iterable member declaration should exist without semantic analysis");
        ...
        let with_declaration = match case {
            CaseKind::UnsafeTupleField => references_for_dependency_struct_fields(
                uri,
                source,
                &package,
                offset_to_position(source, second_offset),
            ...
            CaseKind::UnsafeArrayMethod => references_for_dependency_methods(
                uri,
                source,
                &package,
                offset_to_position(source, second_offset),
        ...
        .expect("unsafe iterable member references should exist without semantic analysis");
        ...
        .expect("unsafe iterable member references should exist without declaration");
        ...
        .expect("unsafe iterable member hover should exist");
        let HoverContents::Markup(markup) = hover.contents else {
            panic!("hover should use markdown")
        };
        assert!(markup.value.contains(case.hover_kind()));
        ...
        .expect("unsafe iterable member definition should exist");
        let GotoDefinitionResponse::Scalar(definition_location) = definition else {
            panic!("definition should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("unsafe iterable member declaration should exist");
        let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
            panic!("declaration should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("unsafe iterable member references should exist");
        assert_eq!(with_declaration.len(), 3);
        assert_location_targets_dependency_name(
            &with_declaration[0],
            dep_qi,
...
fn run_member_query_case(case: CaseKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-for-loop-unsafe-iterable-{}-queries{}",
        case.label(),
...
#[test]
fn dependency_field_queries_work_on_unsafe_tuple_for_loop_receivers() {
    run_member_query_case(CaseKind::UnsafeTupleField, false);
}

#[test]
fn dependency_field_queries_work_on_unsafe_tuple_for_loop_receivers_without_semantic_analysis() {
    run_member_query_case(CaseKind::UnsafeTupleField, true);
}

#[test]
fn dependency_method_queries_work_on_unsafe_array_for_loop_receivers() {
    run_member_query_case(CaseKind::UnsafeArrayMethod, false);
}

#[test]
fn dependency_method_queries_work_on_unsafe_array_for_loop_receivers_without_semantic_analysis() {
    run_member_query_case(CaseKind::UnsafeArrayMethod, true);
}
