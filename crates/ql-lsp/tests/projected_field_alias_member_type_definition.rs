...
fn run_type_definition_case(member: MemberKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-projected-field-alias-{}-member-type-definition{}",
        member.label(),
...
"#,
    ...
    if broken {
        ...
        .expect(
            "projected-field alias member type definition should exist without semantic analysis",
        );
        ...
        let definition =
            type_definition_for_package_analysis(&uri, &source, &analysis, &package, position)
                .expect("projected-field alias member type definition should exist");
        assert_targets_dependency_type(
            definition,
            &dep_qi,
            "pub struct Leaf {\n    value: Int,\n}",
...
#[test]
fn type_definition_bridge_follows_projected_field_alias_field_member_types() {
    run_type_definition_case(MemberKind::Field, false);
}

#[test]
fn type_definition_fallback_follows_projected_field_alias_field_member_types() {
    run_type_definition_case(MemberKind::Field, true);
}

#[test]
fn type_definition_bridge_follows_projected_field_alias_method_member_types() {
    run_type_definition_case(MemberKind::Method, false);
}

#[test]
fn type_definition_fallback_follows_projected_field_alias_method_member_types() {
