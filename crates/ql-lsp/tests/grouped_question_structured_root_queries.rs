...
fn assert_root_queries(
    ...
    if broken {
        ...
        let hover = hover_for_dependency_values(source, &package, root_position)
            .expect("grouped structured dependency question root hover should exist");
        ...
        let definition = definition_for_dependency_values(source, &package, root_position)
            .expect("grouped structured dependency question root definition should exist");
        ...
        let declaration = declaration_for_dependency_values(source, &package, root_position)
            .expect("grouped structured dependency question root declaration should exist");
        ...
        let without_declaration =
            references_for_dependency_values(uri, source, &package, root_position, false)
                .expect("grouped structured dependency question root references should exist");
        assert_eq!(
            without_declaration,
            vec![
                Location::new(
...
fn run_root_query_case(root: RootKind, structured: StructuredKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-grouped-question-{}-{}-root-query{}",
        root.label(),
        structured.label(),
...
#[test]
fn root_queries_work_on_if_grouped_question_function_value_receivers() {
    run_root_query_case(RootKind::Function, StructuredKind::If, false);
}

#[test]
fn root_queries_work_on_if_grouped_question_function_value_receivers_without_semantic_analysis() {
    run_root_query_case(RootKind::Function, StructuredKind::If, true);
}

#[test]
fn root_queries_work_on_match_grouped_question_function_value_receivers() {
    run_root_query_case(RootKind::Function, StructuredKind::Match, false);
}

#[test]
fn root_queries_work_on_match_grouped_question_function_value_receivers_without_semantic_analysis()
{
    run_root_query_case(RootKind::Function, StructuredKind::Match, true);
}

#[test]
fn root_queries_work_on_if_grouped_question_static_value_receivers() {
    run_root_query_case(RootKind::Static, StructuredKind::If, false);
}

#[test]
fn root_queries_work_on_if_grouped_question_static_value_receivers_without_semantic_analysis() {
    run_root_query_case(RootKind::Static, StructuredKind::If, true);
}

#[test]
fn root_queries_work_on_match_grouped_question_static_value_receivers() {
