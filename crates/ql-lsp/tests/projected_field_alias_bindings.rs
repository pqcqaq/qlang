...
#[test]
fn dependency_field_definition_works_on_local_alias_bound_from_dependency_field_projection() {
    let temp = TempDir::new("ql-lsp-projected-field-alias-field");
...
#[test]
fn dependency_method_definition_works_on_local_alias_bound_from_dependency_field_projection_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-projected-field-alias-method-broken");
    let app_root = temp.path().join("workspace").join("app");
