...
#[test]
fn hover_bridge_preserves_deferred_multi_segment_import_alias_type_paths_in_function_signatures() {
    let source = r#"
use Command as Cmd
...
"#;
    ...
    let hover = hover_for_analysis(
        source,
        &analysis,
        span_to_range(source, nth_span(source, "main", 1)).start,
    )
    ...
    assert!(markup.value.contains("**function** `main`"));
    assert!(
        markup
            .value
            .contains("fn main(value: Cmd.Scope.Config) -> Cmd.Scope.Config")
    );
    assert!(!markup.value.contains("fn main(value: Command) -> Command"));
}
...
"#;
    ...
    let hover = hover_for_analysis(source, &analysis, shorthand_position)
        .expect("deeper shorthand token hover should exist");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown content");
    };
    assert!(markup.value.contains("**local** `x`"));
    ...
    let definition = definition_for_analysis(&uri, source, &analysis, shorthand_position)
        .expect("deeper shorthand token definition should exist");
    let GotoDefinitionResponse::Scalar(location) = definition else {
        panic!("expected scalar definition location");
    };
    assert_eq!(location.uri, uri);
...
"#;
    ...
    let prepare = prepare_rename_for_analysis(source, &analysis, shorthand_position)
        .expect("deeper shorthand prepare rename should exist");
...
"#;
    ...
    let prepare = prepare_rename_for_analysis(source, &analysis, shorthand_position)
        .expect("deeper import shorthand prepare rename should exist");
    let PrepareRenameResponse::RangeWithPlaceholder { range, placeholder } = prepare else {
        panic!("expected range plus placeholder");
    };
    assert_eq!(
    ...
    let edit = rename_for_analysis(&uri, source, &analysis, shorthand_position, "feed")
        .expect("rename should validate")
        .expect("rename should produce edits");
...
"#;
    ...
    let sync_for_await_position = span_to_range(source, nth_span(source, "await", 1)).start;
    let sync_spawn_position = span_to_range(source, nth_span(source, "spawn", 1)).start;
    let sync_await_position = span_to_range(source, nth_span(source, "await", 2)).start;
    let async_for_await_position = span_to_range(source, nth_span(source, "await", 3)).start;
    let async_spawn_position = span_to_range(source, nth_span(source, "spawn", 2)).start;
    let async_await_position = span_to_range(source, nth_span(source, "await", 4)).start;

    let sync_for_await = async_context_for_analysis(source, &analysis, sync_for_await_position)
        .expect("sync for-await async context should exist");
    assert_eq!(
    ...
    let async_for_await = async_context_for_analysis(source, &analysis, async_for_await_position)
        .expect("async for-await async context should exist");
    assert_eq!(
...
#[test]
fn async_context_bridge_covers_impl_and_extend_methods() {
    let source = r#"
struct Counter {
    value: Int,
}
...
"#;
    ...
    let sync_spawn = async_context_for_analysis(source, &analysis, sync_spawn_position)
        .expect("sync impl spawn context should exist");
    assert_eq!(
        sync_spawn.range,
        span_to_range(source, nth_span(source, "spawn", 1))
    );
    ...
    let sync_await = async_context_for_analysis(source, &analysis, sync_await_position)
        .expect("sync impl await context should exist");
    assert_eq!(
        sync_await.range,
        span_to_range(source, nth_span(source, "await", 1))
    );
    ...
    let async_spawn = async_context_for_analysis(source, &analysis, async_spawn_position)
        .expect("async impl spawn context should exist");
    assert_eq!(
        async_spawn.range,
        span_to_range(source, nth_span(source, "spawn", 2))
    );
    ...
    let async_await = async_context_for_analysis(source, &analysis, async_await_position)
        .expect("async impl await context should exist");
    assert_eq!(
        async_await.range,
        span_to_range(source, nth_span(source, "await", 2))
    );
    ...
    let sync_for_await = async_context_for_analysis(source, &analysis, sync_for_await_position)
        .expect("sync extend for-await context should exist");
    assert_eq!(
        sync_for_await.range,
        span_to_range(source, nth_span(source, "await", 3))
    );
    ...
    let async_for_await = async_context_for_analysis(source, &analysis, async_for_await_position)
        .expect("async extend for-await context should exist");
    assert_eq!(
        async_for_await.range,
        span_to_range(source, nth_span(source, "await", 4))
    );
