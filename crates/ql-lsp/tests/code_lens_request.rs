...
#[tokio::test(flavor = "current_thread")]
async fn code_lens_request_counts_open_real_stdlib_consumers() {
    let temp = TempDir::new("ql-lsp-code-lens-real-stdlib-open-consumer");
...
"#;
    ...
    let max_int_position = offset_to_position(&core_source, nth_offset(&core_source, "max_int", 1));
    let max_int_lens = reference_lens_at(&lenses, max_int_position).unwrap_or_else(|| {
        panic!("std.core max_int open-doc reference code lens should exist: {lenses:#?}")
    });
    let locations = max_int_lens
        .command
        .as_ref()
    ...
    assert!(
        locations
            .iter()
            .any(|location| location.uri == workspace.app_uri),
        "real stdlib codeLens should include unsaved open app consumer references: {locations:#?}",
    );
}
