...
fn run_query_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-grouped-question-indexed-iterable-method-member-query{}",
...
"#,
    ...
    if broken {
        ...
        .expect(
            "grouped question indexed iterable method member hover should exist without semantic analysis",
        );
        ...
        .expect(
            "grouped question indexed iterable method member references should exist without declaration",
        );
        assert_eq!(without_declaration.len(), 2);
        assert!(without_declaration
            .iter()
...
#[test]
fn queries_work_on_grouped_question_indexed_iterable_method_members() {
    run_query_case(false);
}

#[test]
fn queries_fallback_on_grouped_question_indexed_iterable_method_members() {
    run_query_case(true);
}
