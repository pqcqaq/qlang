...
{broken_line}    return config.children()[0].leaf().value.inner
}}
"#
    )
}
...
fn run_type_definition_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-indexed-iterable-method-result-member-type-definition{}",
...
"#,
    ...
    if broken {
        ...
        .expect("direct indexed iterable method-result member type definition should exist");
        ...
        let definition =
            type_definition_for_package_analysis(&uri, &source, &analysis, &package, position)
                .expect(
                    "direct indexed iterable method-result member type definition should exist",
                );
        assert_targets_dependency_type(
            definition,
            &dep_qi,
...
#[test]
fn type_definition_bridge_follows_direct_indexed_iterable_method_result_member_types() {
    run_type_definition_case(false);
}

#[test]
fn type_definition_fallback_follows_direct_indexed_iterable_method_result_member_types() {
