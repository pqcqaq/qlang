...
#[test]
fn dependency_extend_method_completion_works_on_question_unwrapped_local_method_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-question-unwrapped-local-extend-method-completion-broken");
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!("question-unwrapped local extend-member completion should exist without semantic analysis");
    };
