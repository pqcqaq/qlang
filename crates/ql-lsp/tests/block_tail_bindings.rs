...
#[test]
fn dependency_field_definition_works_on_block_tail_receivers() {
    let temp = TempDir::new("ql-lsp-block-tail-field");
...
#[test]
fn dependency_method_definition_works_on_block_tail_receivers_without_semantic_analysis() {
    let temp = TempDir::new("ql-lsp-block-tail-method-broken");
    let app_root = temp.path().join("workspace").join("app");
