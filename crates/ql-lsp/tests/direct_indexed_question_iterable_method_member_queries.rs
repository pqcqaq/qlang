...
fn run_query_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-indexed-question-iterable-method-member-query{}",
...
"#,
    ...
    if broken {
        ...
        .expect(
            "direct indexed question iterable method member hover should exist without semantic analysis",
        );
        ...
        assert_dependency_location(&location, &dep_qi, "pub fn leaf(self) -> Leaf", "leaf");
        ...
        .expect(
            "direct indexed question iterable method member references should exist without declaration",
        );
        assert_eq!(without_declaration.len(), 2);
        assert!(without_declaration
            .iter()
...
#[test]
fn queries_work_on_direct_indexed_question_iterable_method_members() {
    run_query_case(false);
}

#[test]
fn queries_fallback_on_direct_indexed_question_iterable_method_members() {
    run_query_case(true);
}
