...
#[test]
fn package_bridge_completes_dependency_fields_for_for_loop_if_grouped_question_iterables() {
    let temp = TempDir::new("ql-lsp-for-loop-grouped-question-structured-field-completion");
...
"#,
    ...
    let Some(CompletionResponse::Array(items)) = completion_for_package_analysis(
        ...
        panic!("grouped structured question field iterable completion should exist");
    };
...
#[test]
fn package_bridge_completes_dependency_fields_for_for_loop_if_grouped_question_iterables_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-for-loop-grouped-question-structured-field-broken-completion");
    let app_root = temp.path().join("workspace").join("app");
    let source = r#"
package demo.app
...
#[test]
fn package_bridge_completes_dependency_methods_for_for_loop_match_grouped_question_iterables_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-for-loop-grouped-question-structured-method-broken-completion");
    let app_root = temp.path().join("workspace").join("app");
    let source = r#"
package demo.app
...
"#,
    ...
    let Some(CompletionResponse::Array(items)) = completion_for_dependency_methods(
        ...
        panic!("grouped structured question method iterable completion should exist");
    };
    assert_eq!(items.len(), 1);
...
#[test]
fn dependency_method_definition_works_on_for_loop_match_grouped_question_iterables_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-for-loop-grouped-question-structured-method-query-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("grouped structured question method iterable definition should exist");
