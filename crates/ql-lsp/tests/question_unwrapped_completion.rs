...
fn run_completion_case(case: CaseKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-question-unwrapped-{}-completion{}",
        case.label(),
...
"#,
    ...
    temp.write("workspace/dep/dep.qi", case.dependency_qi());
    temp.write(
...
"#,
    ...
    if broken {
        ...
        let Some(CompletionResponse::Array(items)) = (match case {
            CaseKind::Field => completion_for_dependency_member_fields(&source, &package, position),
            CaseKind::Method => completion_for_dependency_methods(&source, &package, position),
        }) else {
            panic!("question-unwrapped member completion should exist without semantic analysis");
        };
        ...
        let Some(CompletionResponse::Array(items)) =
            ...
            panic!("question-unwrapped member completion should exist");
        };
        assert_eq!(items.len(), 1);
        assert_completion_item(case, items[0].clone());
    }
...
#[test]
fn dependency_field_completion_works_on_question_unwrapped_receivers() {
    run_completion_case(CaseKind::Field, false);
}

#[test]
fn dependency_field_completion_works_on_question_unwrapped_receivers_without_semantic_analysis() {
    run_completion_case(CaseKind::Field, true);
}

#[test]
fn dependency_method_completion_works_on_question_unwrapped_receivers() {
    run_completion_case(CaseKind::Method, false);
}

#[test]
fn dependency_method_completion_works_on_question_unwrapped_receivers_without_semantic_analysis() {
    run_completion_case(CaseKind::Method, true);
}
