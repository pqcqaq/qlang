...
#[test]
fn dependency_field_definition_works_on_question_unwrapped_local_receivers() {
    let temp = TempDir::new("ql-lsp-question-unwrapped-local-field");
...
#[test]
fn dependency_method_definition_works_on_question_unwrapped_local_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-question-unwrapped-local-method-broken");
    let app_root = temp.path().join("workspace").join("app");
