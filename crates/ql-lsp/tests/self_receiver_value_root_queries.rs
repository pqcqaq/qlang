...
"#,
    ...
    let self_usage = nth_offset(source, "self", 2);
    ...
    if broken {
        ...
        let hover = hover_for_dependency_values(source, &package, offset_to_position(source, self_usage))
        ...
        let definition =
            definition_for_dependency_values(source, &package, offset_to_position(source, self_usage))
        ...
        let declaration =
            declaration_for_dependency_values(source, &package, offset_to_position(source, self_usage))
        ...
        let without_declaration = references_for_dependency_values(
            ...
            source,
            &package,
            offset_to_position(source, self_usage),
            false,
        )
...
#[test]
fn root_query_bridge_surfaces_dependency_self_receiver_roots() {
    run_root_query_case(false);
}

#[test]
fn root_query_fallback_surfaces_dependency_self_receiver_roots() {
