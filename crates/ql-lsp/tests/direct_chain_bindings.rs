...
fn assert_targets_dependency_type(
    ...
    let start = artifact
        .find(snippet)
        .expect("type snippet should exist in dependency artifact");
    assert_eq!(
        range,
        span_to_range(&artifact, ql_span::Span::new(start, start + snippet.len()))
    );
...
#[test]
fn dependency_field_definition_works_on_direct_dependency_method_result_receiver() {
    let temp = TempDir::new("ql-lsp-direct-chain-field");
...
#[test]
fn dependency_field_type_definition_works_on_direct_dependency_method_result_receiver_without_semantic_analysis()
 {
    let temp = TempDir::new("ql-lsp-direct-chain-field-type-broken");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read(config: Cfg) -> Int {
    let count = config.child().value.count
    return "oops"
}
"#;
    temp.write("workspace/app/src/lib.ql", source);
    ...
    .expect("dependency field type definition should exist");
...
#[test]
fn dependency_method_type_definition_works_on_direct_dependency_method_result_receiver_without_semantic_analysis()
 {
    let temp = TempDir::new("ql-lsp-direct-chain-method-type-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    let definition = type_definition_for_dependency_method_types(
        source,
        &package,
        offset_to_position(source, nth_offset(source, "next", 1)),
    )
...
#[test]
fn dependency_field_definition_works_on_direct_if_result_receiver() {
    let temp = TempDir::new("ql-lsp-direct-if-field");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read(config: Cfg, flag: Bool) -> Int {
    return (if flag { config.child() } else { config.child() }).value
}
"#;
    temp.write("workspace/app/src/lib.ql", source);
...
#[test]
fn dependency_method_definition_works_on_direct_match_result_receiver_without_semantic_analysis() {
    let temp = TempDir::new("ql-lsp-direct-match-method-broken");
    let app_root = temp.path().join("workspace").join("app");
