...
#[test]
fn dependency_field_definition_works_on_named_local_receivers() {
    let temp = TempDir::new("ql-lsp-named-local-field");
...
#[test]
fn dependency_method_definition_works_on_named_local_receivers_without_semantic_analysis() {
    let temp = TempDir::new("ql-lsp-named-local-method-broken");
    let app_root = temp.path().join("workspace").join("app");
