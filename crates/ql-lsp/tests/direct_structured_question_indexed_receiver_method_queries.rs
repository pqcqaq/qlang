...
fn assert_dependency_location(location: &Location, dep_qi: &Path, anchor: &str, name: &str) {
    let artifact = fs::read_to_string(dep_qi)
        ...
        .replace("\r\n", "\n");
    let anchor_start = artifact.find(anchor).expect("dependency anchor should exist");
...
fn run_query_case(structured: StructuredKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-structured-question-indexed-{}-receiver-method-query{}",
        structured.label(),
...
"#,
    ...
    if broken {
        ...
        .expect("direct structured question indexed receiver method hover should exist");
        ...
        .expect("direct structured question indexed receiver method definition should exist");
        let GotoDefinitionResponse::Scalar(location) = definition else {
            panic!("definition should be one location")
        };
        assert_dependency_location(&location, &dep_qi, "pub fn get(self) -> Int", "get");
        ...
        .expect("direct structured question indexed receiver method declaration should exist");
        ...
        .expect("direct structured question indexed receiver method references should exist");
        ...
        .expect(
            "direct structured question indexed receiver method references without declaration should exist",
        );
...
#[test]
fn queries_work_on_if_direct_structured_question_indexed_receiver_methods() {
    run_query_case(StructuredKind::If, false);
}

#[test]
fn queries_work_on_match_direct_structured_question_indexed_receiver_methods() {
    run_query_case(StructuredKind::Match, false);
}

#[test]
fn queries_fallback_on_if_direct_structured_question_indexed_receiver_methods() {
    run_query_case(StructuredKind::If, true);
}

#[test]
fn queries_fallback_on_match_direct_structured_question_indexed_receiver_methods() {
