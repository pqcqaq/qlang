...
impl CaseKind {
    ...
    const fn value(self) -> &'static str {
        match self {
            Self::Tuple => "(config.child, config.child)",
            Self::Array => "[config.child, config.child]",
        }
    }
}
...
fn run_type_definition_case(case: CaseKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-{}-destructured-member-type-definition{}",
        case.test_name(),
...
"#,
    ...
    let value_field = nth_offset(&source, "value", 1);
    ...
    if broken {
        ...
        let analysis =
            analyze_source(&source).expect("source should still parse for type definition");
        ...
        let definition = type_definition_for_package_analysis(
            ...
            &analysis,
            &package,
            offset_to_position(&source, value_field),
        )
        .expect("dependency destructured member type definition should exist");
...
#[test]
fn type_definition_bridge_follows_tuple_destructured_dependency_member_types() {
    run_type_definition_case(CaseKind::Tuple, false);
}

#[test]
fn type_definition_fallback_follows_tuple_destructured_dependency_member_types() {
    run_type_definition_case(CaseKind::Tuple, true);
}

#[test]
fn type_definition_bridge_follows_array_destructured_dependency_member_types() {
    run_type_definition_case(CaseKind::Array, false);
}

#[test]
fn type_definition_fallback_follows_array_destructured_dependency_member_types() {
    run_type_definition_case(CaseKind::Array, true);
}
