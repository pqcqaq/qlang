...
use ql_lsp::bridge::{span_to_range, type_definition_for_dependency_struct_field_types};
...
#[test]
fn type_definition_fallback_follows_question_unwrapped_local_extend_method_projected_field_alias_method_result_member_types(
) {
    let temp = TempDir::new(
        "ql-lsp-question-unwrapped-local-extend-method-projected-field-alias-method-result-type-definition-broken",
    );
...
"#;
    ...
    .expect(
        "dependency extend-method-result projected-field alias local method-result member type definition should exist without semantic analysis",
    );
    assert_targets_dependency_type(definition, &dep_qi, "pub struct Scalar {\n    inner: Int,\n}");
}
