...
fn run_root_query_case(structured: StructuredKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-structured-question-indexed-{}-method-result-value-root-query{}",
        structured.label(),
...
"#,
    ...
    let root_position = offset_to_position(&source, current_usage);
    ...
    if broken {
        ...
        let hover = hover_for_dependency_values(&source, &package, root_position)
            .expect("direct structured question indexed method-result value root hover should exist");
        ...
        let definition = definition_for_dependency_values(&source, &package, root_position)
            .expect(
                "direct structured question indexed method-result value root definition should exist",
            );
        ...
        let declaration = declaration_for_dependency_values(&source, &package, root_position)
            .expect(
                "direct structured question indexed method-result value root declaration should exist",
            );
        ...
        let without_declaration =
            references_for_dependency_values(&uri, &source, &package, root_position, false)
                .expect(
                    "direct structured question indexed method-result value root references should exist",
                );
        assert_eq!(
            without_declaration,
            vec![
                Location::new(
                    ...
                    span_to_range(
                        &source,
                        Span::new(current_usage, current_usage + "current".len()),
                    ),
                ),
                Location::new(
                    uri.clone(),
        ...
        let with_declaration =
            references_for_dependency_values(&uri, &source, &package, root_position, true)
                .expect(
                    "direct structured question indexed method-result value root references with declaration should exist",
                );
        assert_eq!(with_declaration.len(), 4);
        assert_dependency_location(&with_declaration[0], &dep_qi, "pub struct Leaf {\n    value: Int,\n}");
        assert_eq!(
            &with_declaration[1..],
            &[
                Location::new(
        ...
        .expect("direct structured question indexed method-result value root references should exist");
        assert_eq!(
            without_declaration,
            vec![
                Location::new(
...
#[test]
fn root_queries_work_on_if_direct_structured_question_indexed_method_result_value_roots() {
    run_root_query_case(StructuredKind::If, false);
}

#[test]
fn root_queries_work_on_match_direct_structured_question_indexed_method_result_value_roots() {
    run_root_query_case(StructuredKind::Match, false);
}

#[test]
fn root_queries_fallback_on_if_direct_structured_question_indexed_method_result_value_roots() {
    run_root_query_case(StructuredKind::If, true);
}

#[test]
fn root_queries_fallback_on_match_direct_structured_question_indexed_method_result_value_roots() {
    run_root_query_case(StructuredKind::Match, true);
}
