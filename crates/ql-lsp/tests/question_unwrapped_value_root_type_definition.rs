...
{broken_line}    return {receiver}.value
}}
"#,
...
fn run_type_definition_case(root: RootKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-question-unwrapped-{}-value-root-type-definition{}",
        root.label(),
...
"#,
    ...
    if broken {
        ...
        let definition = type_definition_for_dependency_values(&source, &package, position)
            .expect("question-unwrapped dependency value root type definition should exist");
...
#[test]
fn type_definition_bridge_follows_question_unwrapped_dependency_field_value_roots() {
    run_type_definition_case(RootKind::Field, false);
}

#[test]
fn type_definition_fallback_follows_question_unwrapped_dependency_field_value_roots() {
    run_type_definition_case(RootKind::Field, true);
}

#[test]
fn type_definition_bridge_follows_question_unwrapped_dependency_method_value_roots() {
    run_type_definition_case(RootKind::Method, false);
}

#[test]
fn type_definition_fallback_follows_question_unwrapped_dependency_method_value_roots() {
    run_type_definition_case(RootKind::Method, true);
}
