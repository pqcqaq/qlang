...
#[test]
fn type_definition_fallback_follows_question_unwrapped_local_extend_method_method_result_member_types(
) {
    let temp = TempDir::new(
        "ql-lsp-question-unwrapped-local-extend-method-method-result-type-definition-broken",
    );
...
pub fn read(config: Cfg) -> Int {
    ...
    let broken: Int = "oops"
    return current.get().value.inner
}
"#;
    ...
    .expect(
        "dependency extend-method-result local method-result member type definition should exist without semantic analysis",
    );
