...
impl ReceiverKind {
    ...
    fn receiver_expr(self, member: MemberKind) -> &'static str {
        match (self, member) {
            (Self::FunctionResult, MemberKind::Field) => "load().va",
            (Self::FunctionResult, MemberKind::Method) => "load().ge",
            (Self::StaticValue, MemberKind::Field) => "current.va",
            (Self::StaticValue, MemberKind::Method) => "current.ge",
        }
    }
...
pub fn read() -> Int {{
{broken_line}    return {receiver_expr}
}}
"#,
        imports = receiver.imports(),
        broken_line = broken_line,
        receiver_expr = receiver.receiver_expr(member),
    )
}
...
fn run_completion_case(receiver: ReceiverKind, member: MemberKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-dependency-root-{}-{}-completion{}",
        receiver.label(),
        member.label(),
...
"#,
    ...
    temp.write("workspace/dep/dep.qi", receiver.dep_qi());
    temp.write(
...
"#,
    ...
    let completion_expr = receiver.receiver_expr(member);
    let position = offset_to_position(
        &source,
        nth_offset(&source, completion_expr, 1) + completion_expr.len(),
    );
    ...
    if broken {
        ...
        let Some(CompletionResponse::Array(items)) = (match member {
            ...
            panic!("dependency root member completion should exist without semantic analysis");
        };
        ...
        let Some(CompletionResponse::Array(items)) =
            ...
            panic!("dependency root member completion should exist");
        };
        assert_eq!(items.len(), 1);
        assert_completion_item(member, items[0].clone());
    }
...
#[test]
fn dependency_field_completion_works_on_imported_dependency_function_result_receivers() {
    run_completion_case(ReceiverKind::FunctionResult, MemberKind::Field, false);
}
...
#[test]
fn dependency_method_completion_works_on_imported_dependency_function_result_receivers() {
    run_completion_case(ReceiverKind::FunctionResult, MemberKind::Method, false);
}
...
#[test]
fn dependency_field_completion_works_on_imported_dependency_static_receivers() {
    run_completion_case(ReceiverKind::StaticValue, MemberKind::Field, false);
}
...
#[test]
fn dependency_method_completion_works_on_imported_dependency_static_receivers() {
    run_completion_case(ReceiverKind::StaticValue, MemberKind::Method, false);
}
