...
fn build_source(broken: bool) -> String {
    let tail = if broken {
        "    return \"oops\"\n"
    } else {
        "    return first + second\n"
...
fn run_query_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-indexed-question-iterable-receiver-field-query{}",
...
"#,
    ...
    .expect("direct indexed question-iterable receiver field hover should exist");
    ...
    .expect("direct indexed question-iterable receiver field definition should exist");
    ...
    .expect("direct indexed question-iterable receiver field declaration should exist");
    ...
    .expect("direct indexed question-iterable receiver field references should exist");
    ...
    .expect(
        "direct indexed question-iterable receiver field references without declaration should exist",
    );
...
#[test]
fn queries_work_on_direct_indexed_question_iterable_receiver_fields() {
    run_query_case(false);
}

#[test]
fn queries_fallback_on_direct_indexed_question_iterable_receiver_fields() {
    run_query_case(true);
}
