...
fn assert_member_queries(
    ...
    if broken {
        ...
        .expect("grouped question iterable member hover should exist without semantic analysis");
        ...
        .expect("grouped question iterable member hover should exist");
        let HoverContents::Markup(markup) = hover.contents else {
            panic!("hover should use markdown")
        };
        assert!(markup.value.contains(member.hover_kind()));
        ...
        .expect("grouped question iterable member definition should exist");
        let GotoDefinitionResponse::Scalar(definition_location) = definition else {
            panic!("definition should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("grouped question iterable member declaration should exist");
        let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
            panic!("declaration should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("grouped question iterable member references should exist");
        assert_eq!(with_declaration.len(), 3);
        assert_location_targets_dependency_name(
            &with_declaration[0],
            dep_qi,
...
fn run_member_query_case(member: MemberKind, root: RootKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-for-loop-grouped-question-iterable-{}-{}-queries{}",
        member.label(),
        match root {
