...
fn run_completion_case(root: RootKind, member: MemberKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-grouped-question-{}-{}-completion{}",
        root.label(),
        member.label(),
...
"#,
    ...
    if broken {
        ...
        let Some(CompletionResponse::Array(items)) = (match member {
            ...
            panic!(
                "grouped question value member completion should exist without semantic analysis"
            );
        };
        ...
        let Some(CompletionResponse::Array(items)) =
            ...
            panic!("grouped question value member completion should exist");
        };
        assert_eq!(items.len(), 1);
        assert_completion_item(member, items[0].clone());
    }
...
#[test]
fn dependency_field_completion_works_on_grouped_question_function_value_receivers() {
    run_completion_case(RootKind::Function, MemberKind::Field, false);
}

#[test]
fn dependency_field_completion_works_on_grouped_question_function_value_receivers_without_semantic_analysis()
 {
    run_completion_case(RootKind::Function, MemberKind::Field, true);
}

#[test]
fn dependency_method_completion_works_on_grouped_question_function_value_receivers() {
    run_completion_case(RootKind::Function, MemberKind::Method, false);
}

#[test]
fn dependency_method_completion_works_on_grouped_question_function_value_receivers_without_semantic_analysis()
 {
    run_completion_case(RootKind::Function, MemberKind::Method, true);
}

#[test]
fn dependency_field_completion_works_on_grouped_question_static_value_receivers() {
    run_completion_case(RootKind::Static, MemberKind::Field, false);
}

#[test]
fn dependency_field_completion_works_on_grouped_question_static_value_receivers_without_semantic_analysis()
 {
    run_completion_case(RootKind::Static, MemberKind::Field, true);
}

#[test]
fn dependency_method_completion_works_on_grouped_question_static_value_receivers() {
    run_completion_case(RootKind::Static, MemberKind::Method, false);
}

#[test]
fn dependency_method_completion_works_on_grouped_question_static_value_receivers_without_semantic_analysis()
 {
