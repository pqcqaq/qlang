...
fn run_type_definition_case(case: CaseKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-question-unwrapped-{}-member-type-definition{}",
        case.label(),
...
"#,
    ...
    if broken {
        ...
        .expect("question-unwrapped member type definition should exist without semantic analysis");
        ...
        let definition =
            type_definition_for_package_analysis(&uri, &source, &analysis, &package, position)
                .expect("question-unwrapped member type definition should exist");
        assert_targets_dependency_type(
            definition,
            &dep_qi,
            "pub struct Leaf {\n    value: Int,\n}",
...
#[test]
fn type_definition_bridge_follows_question_unwrapped_field_member_types() {
    run_type_definition_case(CaseKind::Field, false);
}

#[test]
fn type_definition_fallback_follows_question_unwrapped_field_member_types() {
    run_type_definition_case(CaseKind::Field, true);
}

#[test]
fn type_definition_bridge_follows_question_unwrapped_method_member_types() {
    run_type_definition_case(CaseKind::Method, false);
}

#[test]
fn type_definition_fallback_follows_question_unwrapped_method_member_types() {
    run_type_definition_case(CaseKind::Method, true);
}
