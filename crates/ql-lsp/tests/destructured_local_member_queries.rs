...
pub fn read(config: Cfg) -> Int {{
    ...
    let first = child{suffix}
    let second = child{suffix}
    return "oops"
}}
"#,
...
fn assert_member_queries(
    ...
    if broken {
        ...
        .expect("destructured local member hover should exist without semantic analysis");
        ...
        .expect("destructured local member definition should exist without semantic analysis");
        ...
        .expect("destructured local member declaration should exist without semantic analysis");
        ...
        .expect("destructured local member references should exist without semantic analysis");
        ...
        .expect("destructured local member hover should exist");
        let HoverContents::Markup(markup) = hover.contents else {
            panic!("hover should use markdown")
        };
        assert!(markup.value.contains(member.hover_kind()));
        ...
        .expect("destructured local member definition should exist");
        let GotoDefinitionResponse::Scalar(definition_location) = definition else {
            panic!("definition should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("destructured local member declaration should exist");
        let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
            panic!("declaration should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("destructured local member references should exist");
        assert_eq!(with_declaration.len(), 3);
        assert_location_targets_dependency_name(
            &with_declaration[0],
            dep_qi,
...
fn run_member_query_case(member: MemberKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-destructured-local-{}-member-query{}",
        member.label(),
...
#[test]
fn dependency_field_queries_work_on_destructured_local_receivers() {
    run_member_query_case(MemberKind::Field, false);
}

#[test]
fn dependency_field_queries_work_on_destructured_local_receivers_without_semantic_analysis() {
    run_member_query_case(MemberKind::Field, true);
}

#[test]
fn dependency_method_queries_work_on_destructured_local_receivers() {
    run_member_query_case(MemberKind::Method, false);
}

#[test]
fn dependency_method_queries_work_on_destructured_local_receivers_without_semantic_analysis() {
