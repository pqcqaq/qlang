...
fn build_source(member: MemberKind, structured: StructuredKind, broken: bool) -> String {
    ...
    let wrapped = structured.wrap("config.child()");
    format!(
        r#"
package demo.app
...
fn assert_member_queries(
    ...
    if broken {
        ...
        .expect("structured receiver member hover should exist without semantic analysis");
        ...
        .expect("structured receiver member definition should exist without semantic analysis");
        ...
        .expect("structured receiver member declaration should exist without semantic analysis");
        ...
        .expect("structured receiver member references should exist without semantic analysis");
        ...
        .expect("structured receiver member references should exist without declaration");
        ...
        .expect("structured receiver member hover should exist");
        let HoverContents::Markup(markup) = hover.contents else {
            panic!("hover should use markdown")
        };
        assert!(markup.value.contains(member.hover_kind()));
        ...
        .expect("structured receiver member definition should exist");
        let GotoDefinitionResponse::Scalar(definition_location) = definition else {
            panic!("definition should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("structured receiver member declaration should exist");
        let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
            panic!("declaration should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("structured receiver member references should exist");
        assert_eq!(with_declaration.len(), 3);
        assert_location_targets_dependency_name(
            &with_declaration[0],
            dep_qi,
...
fn run_member_query_case(member: MemberKind, structured: StructuredKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-structured-receiver-{}-{}-member-query{}",
        structured.label(),
        member.label(),
...
#[test]
fn dependency_field_queries_work_on_if_structured_receivers() {
    run_member_query_case(MemberKind::Field, StructuredKind::If, false);
}

#[test]
fn dependency_field_queries_work_on_if_structured_receivers_without_semantic_analysis() {
    run_member_query_case(MemberKind::Field, StructuredKind::If, true);
}

#[test]
fn dependency_field_queries_work_on_match_structured_receivers() {
    run_member_query_case(MemberKind::Field, StructuredKind::Match, false);
}

#[test]
fn dependency_field_queries_work_on_match_structured_receivers_without_semantic_analysis() {
    run_member_query_case(MemberKind::Field, StructuredKind::Match, true);
}

#[test]
fn dependency_method_queries_work_on_if_structured_receivers() {
    run_member_query_case(MemberKind::Method, StructuredKind::If, false);
}

#[test]
fn dependency_method_queries_work_on_if_structured_receivers_without_semantic_analysis() {
    run_member_query_case(MemberKind::Method, StructuredKind::If, true);
}

#[test]
fn dependency_method_queries_work_on_match_structured_receivers() {
    run_member_query_case(MemberKind::Method, StructuredKind::Match, false);
}

#[test]
fn dependency_method_queries_work_on_match_structured_receivers_without_semantic_analysis() {
