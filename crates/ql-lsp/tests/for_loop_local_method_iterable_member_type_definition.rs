...
impl CaseKind {
    ...
    fn access_suffix(self) -> &'static str {
        match self {
            Self::ConfigChildrenField => ".leaf.value",
            Self::SelfPairMethod => ".leaf().value",
        }
    }
...
fn run_type_definition_case(case: CaseKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-for-loop-local-method-iterable-{}-member-type-definition{}",
        case.label(),
...
"#,
    ...
    if broken {
        ...
        .expect(
            "local-method iterable member type definition should exist without semantic analysis",
        );
        ...
        let definition =
            type_definition_for_package_analysis(&uri, &source, &analysis, &package, position)
                .expect("local-method iterable member type definition should exist");
        assert_targets_dependency_type(
            definition,
            &dep_qi,
            "pub struct Leaf {\n    value: Int,\n}",
...
#[test]
fn type_definition_bridge_follows_for_loop_local_method_iterable_field_member_types() {
    run_type_definition_case(CaseKind::ConfigChildrenField, false);
}

#[test]
fn type_definition_fallback_follows_for_loop_local_method_iterable_field_member_types() {
    run_type_definition_case(CaseKind::ConfigChildrenField, true);
}

#[test]
fn type_definition_bridge_follows_for_loop_self_local_method_iterable_method_member_types() {
    run_type_definition_case(CaseKind::SelfPairMethod, false);
}

#[test]
fn type_definition_fallback_follows_for_loop_self_local_method_iterable_method_member_types() {
    run_type_definition_case(CaseKind::SelfPairMethod, true);
}
