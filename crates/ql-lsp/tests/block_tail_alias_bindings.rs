...
#[test]
fn dependency_field_definition_works_on_local_alias_bound_from_dependency_block_tail_result() {
    let temp = TempDir::new("ql-lsp-block-tail-alias-field");
...
#[test]
fn dependency_method_definition_works_on_local_alias_bound_from_dependency_block_tail_result_without_semantic_analysis()
 {
    let temp = TempDir::new("ql-lsp-block-tail-alias-method-broken");
    let app_root = temp.path().join("workspace").join("app");
