...
fn run_method_result_root_query_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-indexed-question-iterable-method-result-value-root-query{}",
...
pub fn read(config: Cfg) -> Int {
    let current = config.maybe_children()?[0].leaf()
...
"#
    ...
    if broken {
        ...
        .expect("direct indexed question iterable method-result value root hover should exist");
        ...
        .expect(
            "direct indexed question iterable method-result value root definition should exist",
        );
        ...
        .expect(
            "direct indexed question iterable method-result value root declaration should exist",
        );
        ...
        .expect(
            "direct indexed question iterable method-result value root references should exist",
        );
        assert_eq!(
            without_declaration,
            vec![
        ...
        .expect(
            "direct indexed question iterable method-result value root references with declaration should exist",
        );
...
#[test]
fn root_query_bridge_surfaces_direct_indexed_question_iterable_method_result_value_roots() {
    run_method_result_root_query_case(false);
}

#[test]
fn root_query_fallback_surfaces_direct_indexed_question_iterable_method_result_value_roots() {
