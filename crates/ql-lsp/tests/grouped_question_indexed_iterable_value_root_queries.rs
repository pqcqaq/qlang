...
fn run_root_query_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-grouped-question-indexed-iterable-value-root-query{}",
...
"#
    ...
    let first_usage = nth_offset(source, "kids", 2);
    let second_usage = nth_offset(source, "kids", 3);
    ...
    if broken {
        ...
        let hover =
            hover_for_dependency_values(source, &package, offset_to_position(source, first_usage))
                .expect("grouped question indexed iterable value root hover should exist");
        ...
        .expect("grouped question indexed iterable value root definition should exist");
        ...
        let declaration = declaration_for_dependency_values(
            source,
            &package,
            offset_to_position(source, second_usage),
        )
        .expect("grouped question indexed iterable value root declaration should exist");
        ...
        .expect("grouped question indexed iterable value root references should exist");
        assert_eq!(
            without_declaration,
            vec![
                Location::new(
                    uri.clone(),
                    span_to_range(source, Span::new(first_usage, first_usage + "kids".len())),
                ),
                Location::new(
                    uri.clone(),
                    span_to_range(source, Span::new(second_usage, second_usage + "kids".len())),
                ),
            ]
        );
        ...
        .expect(
            "grouped question indexed iterable value root references with declaration should exist",
        );
...
#[test]
fn root_query_bridge_surfaces_grouped_question_indexed_iterable_value_roots() {
    run_root_query_case(false);
}

#[test]
fn root_query_fallback_surfaces_grouped_question_indexed_iterable_value_roots() {
    run_root_query_case(true);
}

fn run_method_result_root_query_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-grouped-question-indexed-iterable-method-result-value-root-query{}",
        if broken { "-broken" } else { "" }
    ));
    let app_root = temp.path().join("workspace").join("app");
    let app_path = temp
...
"#
    ...
    if broken {
        ...
        .expect("grouped question indexed iterable method-result value root hover should exist");
        let HoverContents::Markup(markup) = hover.contents else {
            panic!("hover should use markdown")
        };
        ...
        .expect(
            "grouped question indexed iterable method-result value root definition should exist",
        );
        let GotoDefinitionResponse::Scalar(location) = definition else {
            panic!("definition should be one location")
        };
        ...
        .expect(
            "grouped question indexed iterable method-result value root declaration should exist",
        );
        let GotoDeclarationResponse::Scalar(location) = declaration else {
            panic!("declaration should be one location")
        };
        ...
        .expect(
            "grouped question indexed iterable method-result value root references should exist",
        );
        assert_eq!(
            without_declaration,
            vec![
        ...
        .expect(
            "grouped question indexed iterable method-result value root references with declaration should exist",
        );
        assert_eq!(with_declaration.len(), 4);
        assert_dependency_location(
            &with_declaration[0],
...
#[test]
fn root_query_bridge_surfaces_grouped_question_indexed_iterable_method_result_value_roots() {
    run_method_result_root_query_case(false);
}

#[test]
fn root_query_fallback_surfaces_grouped_question_indexed_iterable_method_result_value_roots() {
