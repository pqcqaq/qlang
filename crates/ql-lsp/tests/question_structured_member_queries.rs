...
fn build_source(member: MemberKind, structured: StructuredKind, broken: bool) -> String {
    let wrapped = structured.wrap(member.receiver_expr());
...
fn assert_member_queries(
    ...
    if broken {
        ...
        .expect("structured question member definition should exist without semantic analysis");
        ...
        .expect("structured question member declaration should exist without semantic analysis");
        ...
        .expect("structured question member references should exist without semantic analysis");
        ...
        .expect("structured question member definition should exist");
        let GotoDefinitionResponse::Scalar(definition_location) = definition else {
            panic!("definition should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("structured question member declaration should exist");
        let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
            panic!("declaration should be one location")
        };
        assert_location_targets_dependency_name(
        ...
        .expect("structured question member references should exist");
        assert_eq!(with_declaration.len(), 3);
        assert_location_targets_dependency_name(
            &with_declaration[0],
            dep_qi,
...
#[test]
fn dependency_field_queries_work_on_if_question_value_receivers() {
    run_member_query_case(MemberKind::Field, StructuredKind::If, false);
}

#[test]
fn dependency_field_queries_work_on_if_question_value_receivers_without_semantic_analysis() {
    run_member_query_case(MemberKind::Field, StructuredKind::If, true);
}

#[test]
fn dependency_method_queries_work_on_if_question_value_receivers() {
    run_member_query_case(MemberKind::Method, StructuredKind::If, false);
}

#[test]
fn dependency_method_queries_work_on_if_question_value_receivers_without_semantic_analysis() {
    run_member_query_case(MemberKind::Method, StructuredKind::If, true);
}

#[test]
fn dependency_field_queries_work_on_match_question_value_receivers() {
    run_member_query_case(MemberKind::Field, StructuredKind::Match, false);
}

#[test]
fn dependency_field_queries_work_on_match_question_value_receivers_without_semantic_analysis() {
    run_member_query_case(MemberKind::Field, StructuredKind::Match, true);
}

#[test]
fn dependency_method_queries_work_on_match_question_value_receivers() {
    run_member_query_case(MemberKind::Method, StructuredKind::Match, false);
}

#[test]
fn dependency_method_queries_work_on_match_question_value_receivers_without_semantic_analysis() {
