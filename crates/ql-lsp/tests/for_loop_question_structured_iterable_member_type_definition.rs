...
fn run_type_definition_case(member: MemberKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-for-loop-question-structured-iterable-{}-member-type-definition{}",
        member.label(),
...
"#,
    ...
    if broken {
        ...
        .expect(
            "structured question iterable member type definition should exist without semantic analysis",
        );
...
#[test]
fn type_definition_bridge_follows_for_loop_if_question_structured_iterable_field_member_types() {
    run_type_definition_case(MemberKind::Field, false);
}

#[test]
fn type_definition_fallback_follows_for_loop_if_question_structured_iterable_field_member_types() {
