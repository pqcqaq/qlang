...
fn run_type_definition_case(broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-direct-indexed-iterable-field-member-type-definition{}",
...
"#
    ...
    if broken {
        ...
        let definition = type_definition_for_dependency_struct_field_types(source, &package, position)
            .expect("direct indexed iterable field member type definition should exist");
...
#[test]
fn type_definition_bridge_follows_direct_indexed_iterable_field_member_types() {
    run_type_definition_case(false);
}

#[test]
fn type_definition_fallback_follows_direct_indexed_iterable_field_member_types() {
