...
{broken_line}    return current{suffix}
}}
"#,
...
fn run_type_definition_case(root: RootKind, member: MemberKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-question-unwrapped-local-{}-{}-member-type-definition{}",
        root.label(),
        member.label(),
...
"#,
    ...
    if broken {
        ...
        .expect("question-unwrapped local member type definition should exist without semantic analysis");
        ...
        let definition =
            type_definition_for_package_analysis(&uri, &source, &analysis, &package, position)
                .expect("question-unwrapped local member type definition should exist");
        assert_targets_dependency_type(
            definition,
            &dep_qi,
            "pub struct Leaf {\n    value: Int,\n}",
...
#[test]
fn type_definition_bridge_follows_question_unwrapped_local_field_member_types() {
    run_type_definition_case(RootKind::Field, MemberKind::Field, false);
}

#[test]
fn type_definition_fallback_follows_question_unwrapped_local_field_member_types() {
    run_type_definition_case(RootKind::Field, MemberKind::Field, true);
}

#[test]
fn type_definition_bridge_follows_question_unwrapped_local_method_member_types() {
    run_type_definition_case(RootKind::Method, MemberKind::Method, false);
}

#[test]
fn type_definition_fallback_follows_question_unwrapped_local_method_member_types() {
    run_type_definition_case(RootKind::Method, MemberKind::Method, true);
}
