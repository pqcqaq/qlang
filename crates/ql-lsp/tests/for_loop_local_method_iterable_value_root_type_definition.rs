...
impl BindingKind {
    fn label(self) -> &'static str {
        match self {
            Self::ConfigChildren => "config-children",
            Self::SelfPair => "self-pair",
        }
    }
...
fn run_type_definition_case(binding: BindingKind, broken: bool) {
    let temp = TempDir::new(&format!(
        "ql-lsp-for-loop-local-method-iterable-{}-value-root-type-definition{}",
        binding.label(),
...
"#,
    ...
    if broken {
        ...
        .expect("local-method iterable value root type definition should exist");
...
#[test]
fn type_definition_bridge_follows_dependency_for_loop_config_children_local_method_value_roots() {
    run_type_definition_case(BindingKind::ConfigChildren, false);
}

#[test]
fn type_definition_bridge_follows_dependency_for_loop_self_pair_local_method_value_roots() {
    run_type_definition_case(BindingKind::SelfPair, false);
}

#[test]
fn type_definition_fallback_follows_dependency_for_loop_config_children_local_method_value_roots() {
    run_type_definition_case(BindingKind::ConfigChildren, true);
}

#[test]
fn type_definition_fallback_follows_dependency_for_loop_self_pair_local_method_value_roots() {
    run_type_definition_case(BindingKind::SelfPair, true);
}
