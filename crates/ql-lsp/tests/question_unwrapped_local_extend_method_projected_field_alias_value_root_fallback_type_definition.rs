...
#[test]
fn type_definition_fallback_follows_dependency_question_unwrapped_local_extend_method_projected_field_alias_roots(
) {
    let temp = TempDir::new(
        "ql-lsp-question-unwrapped-local-extend-method-projected-field-alias-value-root-fallback-type-definition",
    );
...
"#;
    ...
    let definition = type_definition_for_dependency_values(source, &package, position).expect(
        "dependency extend-method-result projected-field alias value root type definition should exist without semantic analysis",
    );
