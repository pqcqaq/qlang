...
fn type_definition_fallback_follows_dependency_question_unwrapped_local_method_roots() {
    let temp =
        TempDir::new("ql-lsp-question-unwrapped-local-method-value-root-fallback-type-definition");
...
"#;
    ...
    let definition = type_definition_for_dependency_values(source, &package, position)
        .expect("dependency value root type definition should exist without semantic analysis");
