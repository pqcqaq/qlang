...
#[test]
fn dependency_field_completion_works_on_grouped_question_indexed_iterable_receivers() {
    let temp = TempDir::new("ql-lsp-grouped-question-indexed-field-completion");
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!("grouped question indexed iterable field completion should exist");
    };
...
#[test]
fn dependency_method_completion_works_on_grouped_question_indexed_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-grouped-question-indexed-method-completion-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!(
            "grouped question indexed iterable method completion should exist without semantic analysis"
        );
    };
...
#[test]
fn dependency_method_completion_works_on_grouped_question_indexed_iterable_receivers() {
    let temp = TempDir::new("ql-lsp-grouped-question-indexed-method-completion");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!("grouped question indexed iterable method completion should exist");
    };
...
#[test]
fn dependency_method_member_completion_works_on_grouped_question_indexed_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-grouped-question-indexed-method-member-completion-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!(
            "grouped question indexed iterable method member completion should exist without semantic analysis"
        );
    };
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!("grouped question indexed iterable method-result member completion should exist");
    };
...
#[test]
fn dependency_method_result_member_completion_works_on_grouped_question_indexed_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new(
        "ql-lsp-grouped-question-indexed-method-result-member-completion-broken",
    );
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    let Some(CompletionResponse::Array(items)) =
        ...
        panic!(
            "grouped question indexed iterable method-result member completion should exist without semantic analysis"
        );
    };
...
#[test]
fn dependency_field_queries_work_on_grouped_question_indexed_iterable_receivers() {
    let temp = TempDir::new("ql-lsp-grouped-question-indexed-field-queries");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read() -> Int {
    let first = kids()?[0].value
    let second = kids()?[1].value
    return first + second
}
"#;
    ...
    .expect("grouped question indexed iterable field hover should exist");
    ...
    .expect("grouped question indexed iterable field definition should exist");
    ...
    .expect("grouped question indexed iterable field declaration should exist");
    ...
    .expect("grouped question indexed iterable field references should exist");
...
#[test]
fn dependency_field_member_queries_work_on_grouped_question_indexed_iterable_receivers() {
    let temp = TempDir::new("ql-lsp-grouped-question-indexed-field-member-queries");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("grouped question indexed iterable field member hover should exist");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    ...
    .expect("grouped question indexed iterable field member definition should exist");
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    ...
    .expect("grouped question indexed iterable field member declaration should exist");
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    ...
    .expect("grouped question indexed iterable field member references should exist");
    assert_eq!(with_declaration.len(), 3);
    ...
    .expect("grouped question indexed iterable field member references should exist without declaration");
    assert_eq!(without_declaration.len(), 2);
    assert!(without_declaration
        .iter()
        .all(|location| location.uri == uri));
...
#[test]
fn dependency_method_member_queries_work_on_grouped_question_indexed_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-grouped-question-indexed-method-member-queries-broken");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read() -> Int {
    let first = kids()?[0].leaf()
    let second = kids()?[1].leaf()
    return "oops"
}
"#;
    let app_file = temp.write("workspace/app/src/lib.ql", source);
    ...
    .expect("grouped question indexed iterable method member hover should exist without semantic analysis");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    assert!(markup.value.contains("**method** `leaf`"));
    ...
    .expect(
        "grouped question indexed iterable method member definition should exist without semantic analysis",
    );
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    ...
    .expect(
        "grouped question indexed iterable method member declaration should exist without semantic analysis",
    );
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    ...
    .expect(
        "grouped question indexed iterable method member references should exist without semantic analysis",
    );
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(
        &with_declaration[0],
    ...
    .expect(
        "grouped question indexed iterable method member references should exist without declaration in fallback",
    );
    assert_eq!(without_declaration.len(), 2);
    assert!(without_declaration
        .iter()
...
#[test]
fn dependency_method_result_member_queries_work_on_grouped_question_indexed_iterable_receivers() {
    let temp = TempDir::new("ql-lsp-grouped-question-indexed-method-result-member-queries");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("grouped question indexed iterable method-result member hover should exist");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    assert!(markup.value.contains("**field** `value`"));
    ...
    .expect("grouped question indexed iterable method-result member definition should exist");
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    assert_location_targets_dependency_name(&definition_location, &dep_qi, "value: Int", "value");
    ...
    .expect("grouped question indexed iterable method-result member declaration should exist");
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    assert_location_targets_dependency_name(&declaration_location, &dep_qi, "value: Int", "value");
    ...
    .expect("grouped question indexed iterable method-result member references should exist");
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(&with_declaration[0], &dep_qi, "value: Int", "value");
    assert!(with_declaration[1..]
        .iter()
    ...
    .expect(
        "grouped question indexed iterable method-result member references should exist without declaration",
    );
    assert_eq!(without_declaration.len(), 2);
    assert!(without_declaration
        .iter()
...
#[test]
fn dependency_method_queries_work_on_grouped_question_indexed_iterable_receivers() {
    let temp = TempDir::new("ql-lsp-grouped-question-indexed-method-queries");
    let app_root = temp.path().join("workspace").join("app");
...
pub fn read() -> Int {
    let first = kids()?[0].get()
    let second = kids()?[1].get()
    return first + second
}
"#;
    let app_file = temp.write("workspace/app/src/lib.ql", source);
    ...
    .expect("grouped question indexed iterable method hover should exist");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    ...
    .expect("grouped question indexed iterable method definition should exist");
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    assert_location_targets_dependency_name(
    ...
    .expect("grouped question indexed iterable method declaration should exist");
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    assert_location_targets_dependency_name(
    ...
    .expect("grouped question indexed iterable method references should exist");
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(
        &with_declaration[0],
        &dep_qi,
...
#[test]
fn dependency_method_result_member_queries_work_on_grouped_question_indexed_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-grouped-question-indexed-method-result-member-queries-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    .expect("grouped question indexed iterable method-result member hover should exist without semantic analysis");
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    assert!(markup.value.contains("**field** `value`"));
    ...
    .expect(
        "grouped question indexed iterable method-result member definition should exist without semantic analysis",
    );
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    ...
    .expect(
        "grouped question indexed iterable method-result member declaration should exist without semantic analysis",
    );
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    ...
    .expect(
        "grouped question indexed iterable method-result member references should exist without semantic analysis",
    );
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(
        &with_declaration[0],
    ...
    .expect(
        "grouped question indexed iterable method-result member references should exist without declaration in fallback",
    );
    assert_eq!(without_declaration.len(), 2);
    assert!(without_declaration
        .iter()
...
#[test]
fn dependency_method_queries_work_on_grouped_question_indexed_iterable_receivers_without_semantic_analysis(
) {
    let temp = TempDir::new("ql-lsp-grouped-question-indexed-method-queries-broken");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    let hover =
        hover_for_dependency_methods(source, &package, offset_to_position(source, first_offset))
            .expect(
            "grouped question indexed iterable method hover should exist without semantic analysis",
        );
    let HoverContents::Markup(markup) = hover.contents else {
        panic!("hover should use markdown")
    };
    ...
    .expect(
        "grouped question indexed iterable method definition should exist without semantic analysis"
    );
    let GotoDefinitionResponse::Scalar(definition_location) = definition else {
        panic!("definition should be one location")
    };
    ...
    .expect(
        "grouped question indexed iterable method declaration should exist without semantic analysis"
    );
    let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
        panic!("declaration should be one location")
    };
    ...
    .expect("grouped question indexed iterable method references should exist without semantic analysis");
    assert_eq!(with_declaration.len(), 3);
    assert_location_targets_dependency_name(
        &with_declaration[0],
        &dep_qi,
    ...
    .expect(
        "grouped question indexed iterable method references should exist without declaration in fallback",
    );
    assert_eq!(without_declaration.len(), 2);
    assert!(without_declaration
        .iter()
