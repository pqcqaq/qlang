...
#[test]
fn dependency_field_definition_works_on_struct_destructured_local_receiver() {
    let temp = TempDir::new("ql-lsp-destructured-field");
...
#[test]
fn dependency_method_definition_works_on_match_destructured_local_receiver_without_semantic_analysis()
 {
    let temp = TempDir::new("ql-lsp-destructured-method-broken");
    let app_root = temp.path().join("workspace").join("app");
