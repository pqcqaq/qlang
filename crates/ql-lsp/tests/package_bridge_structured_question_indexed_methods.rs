...
"#,
    ...
    if broken {
        ...
        .expect(
            "structured question indexed method hover should exist through package bridge without semantic analysis",
        );
        ...
        .expect(
            "structured question indexed method definition should exist through package bridge without semantic analysis",
        );
        ...
        .expect(
            "structured question indexed method declaration should exist through package bridge without semantic analysis",
        );
        ...
        .expect(
            "structured question indexed method references should exist through package bridge without semantic analysis",
        );
        ...
        .expect(
            "structured question indexed method references without declaration should exist through package bridge without semantic analysis",
        );
        ...
        .expect(
            "structured question indexed method definition should exist through package bridge",
        );
        let GotoDefinitionResponse::Scalar(definition_location) = definition else {
            panic!("definition should be one location")
        };
        ...
        .expect(
            "structured question indexed method references should exist through package bridge",
        );
        assert_eq!(with_declaration.len(), 3);
        assert_location_targets_dependency_name(
            &with_declaration[0],
...
"#,
    ...
    if broken {
        ...
        let Some(CompletionResponse::Array(items)) = completion_for_dependency_methods(
            ...
            panic!(
                "structured question indexed method completion should exist through package bridge without semantic analysis"
            );
        };
        ...
        let Some(CompletionResponse::Array(items)) = completion_for_package_analysis(
            ...
            panic!(
                "structured question indexed method completion should exist through package bridge"
            );
        };
...
fn run_bracket_target_method_completion_case(structured: StructuredKind, broken: bool) {
    let temp = TempDir::new(&format!(
...
"#,
    ...
    if broken {
        ...
        let Some(CompletionResponse::Array(items)) = completion_for_dependency_methods(
            ...
            panic!(
                "structured question indexed bracket target method completion should exist through package bridge without semantic analysis"
            );
        };
...
fn run_method_result_member_completion_case(structured: StructuredKind, broken: bool) {
    let temp = TempDir::new(&format!(
...
fn run_method_result_member_query_case(structured: StructuredKind, broken: bool) {
    let temp = TempDir::new(&format!(
...
"#,
    ...
    if broken {
        ...
        .expect(
            "structured question indexed method-result member hover should exist through package bridge without semantic analysis",
        );
        let HoverContents::Markup(markup) = hover.contents else {
            panic!("hover should use markdown")
        };
        ...
        .expect(
            "structured question indexed method-result member definition should exist through package bridge without semantic analysis",
        );
        let GotoDefinitionResponse::Scalar(definition_location) = definition else {
            panic!("definition should be one location")
        };
        ...
        .expect(
            "structured question indexed method-result member declaration should exist through package bridge without semantic analysis",
        );
        let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
            panic!("declaration should be one location")
        };
        ...
        .expect(
            "structured question indexed method-result member references should exist through package bridge without semantic analysis",
        );
        assert_eq!(with_declaration.len(), 3);
        assert_location_targets_dependency_name(
            &with_declaration[0],
        ...
        .expect(
            "structured question indexed method-result member references without declaration should exist through package bridge without semantic analysis",
        );
        assert_eq!(without_declaration.len(), 2);
        assert!(without_declaration
            .iter()
...
fn run_method_result_member_type_definition_case(structured: StructuredKind, broken: bool) {
    let temp = TempDir::new(&format!(
...
"#,
    ...
    if broken {
        ...
        let definition = type_definition_for_dependency_method_types(&source, &package, position)
            .expect(
                "structured question indexed method-result member type definition should exist through package bridge without semantic analysis",
            );
...
"#,
    ...
    if broken {
        ...
        let definition = type_definition_for_dependency_method_types(&source, &package, position)
            .expect(
                "structured question indexed method type definition should exist through package bridge without semantic analysis",
            );
        assert_targets_dependency_type(
            definition,
            &dep_qi,
...
"#,
    ...
    if broken {
        ...
        .expect(
            "structured question indexed bracket target method hover should exist through package bridge without semantic analysis",
        );
        let HoverContents::Markup(markup) = hover.contents else {
            panic!("hover should use markdown")
        };
        ...
        .expect(
            "structured question indexed bracket target method definition should exist through package bridge without semantic analysis",
        );
        let GotoDefinitionResponse::Scalar(definition_location) = definition else {
            panic!("definition should be one location")
        };
        ...
        .expect(
            "structured question indexed bracket target method declaration should exist through package bridge without semantic analysis",
        );
        let GotoDeclarationResponse::Scalar(declaration_location) = declaration else {
            panic!("declaration should be one location")
        };
        ...
        .expect(
            "structured question indexed bracket target method references should exist through package bridge without semantic analysis",
        );
        assert_eq!(with_declaration.len(), 3);
        assert_location_targets_dependency_name(
            &with_declaration[0],
        ...
        .expect(
            "structured question indexed bracket target method references without declaration should exist through package bridge without semantic analysis",
        );
        assert_eq!(without_declaration.len(), 2);
        assert!(without_declaration
            .iter()
        ...
        .expect(
            "structured question indexed bracket target method hover should exist through package bridge",
        );
        let HoverContents::Markup(markup) = hover.contents else {
            panic!("hover should use markdown")
        };
...
#[test]
fn package_bridge_surfaces_if_direct_structured_question_indexed_method_queries_without_semantic_analysis(
) {
    run_query_case(StructuredKind::If, true);
}

#[test]
fn package_bridge_surfaces_match_direct_structured_question_indexed_method_queries_without_semantic_analysis(
) {
...
#[test]
fn package_bridge_completes_if_direct_structured_question_indexed_methods_without_semantic_analysis(
) {
    run_completion_case(StructuredKind::If, true);
}

#[test]
fn package_bridge_completes_match_direct_structured_question_indexed_methods_without_semantic_analysis(
) {
...
fn package_bridge_completes_if_direct_structured_question_indexed_bracket_target_methods() {
    run_bracket_target_method_completion_case(StructuredKind::If, false);
}

#[test]
fn package_bridge_completes_match_direct_structured_question_indexed_bracket_target_methods() {
    run_bracket_target_method_completion_case(StructuredKind::Match, false);
}

#[test]
fn package_bridge_completes_if_direct_structured_question_indexed_bracket_target_methods_without_semantic_analysis(
) {
    run_bracket_target_method_completion_case(StructuredKind::If, true);
}
...
#[test]
fn package_bridge_follows_if_direct_structured_question_indexed_method_type_definitions_without_semantic_analysis(
) {
    run_type_definition_case(StructuredKind::If, true);
}

#[test]
fn package_bridge_follows_match_direct_structured_question_indexed_method_type_definitions_without_semantic_analysis(
) {
...
fn package_bridge_surfaces_if_direct_structured_question_indexed_bracket_target_method_queries() {
    run_bracket_target_method_query_case(StructuredKind::If, false);
}

#[test]
fn package_bridge_surfaces_match_direct_structured_question_indexed_bracket_target_method_queries()
{
    run_bracket_target_method_query_case(StructuredKind::Match, false);
}

#[test]
fn package_bridge_surfaces_if_direct_structured_question_indexed_bracket_target_method_queries_without_semantic_analysis(
) {
    run_bracket_target_method_query_case(StructuredKind::If, true);
}

#[test]
fn package_bridge_surfaces_match_direct_structured_question_indexed_bracket_target_method_queries_without_semantic_analysis(
) {
    run_bracket_target_method_query_case(StructuredKind::Match, true);
}
...
fn package_bridge_completes_if_direct_structured_question_indexed_method_result_members() {
    run_method_result_member_completion_case(StructuredKind::If, false);
}

#[test]
fn package_bridge_completes_match_direct_structured_question_indexed_method_result_members() {
    run_method_result_member_completion_case(StructuredKind::Match, false);
}

#[test]
fn package_bridge_surfaces_if_direct_structured_question_indexed_method_result_member_queries() {
    run_method_result_member_query_case(StructuredKind::If, false);
}

#[test]
fn package_bridge_surfaces_match_direct_structured_question_indexed_method_result_member_queries() {
    run_method_result_member_query_case(StructuredKind::Match, false);
}

#[test]
fn package_bridge_follows_if_direct_structured_question_indexed_method_result_member_type_definitions(
) {
    run_method_result_member_type_definition_case(StructuredKind::If, false);
}

#[test]
fn package_bridge_follows_match_direct_structured_question_indexed_method_result_member_type_definitions(
) {
    run_method_result_member_type_definition_case(StructuredKind::Match, false);
}

#[test]
fn package_bridge_completes_if_direct_structured_question_indexed_method_result_members_without_semantic_analysis(
) {
    run_method_result_member_completion_case(StructuredKind::If, true);
}

#[test]
fn package_bridge_completes_match_direct_structured_question_indexed_method_result_members_without_semantic_analysis(
) {
    run_method_result_member_completion_case(StructuredKind::Match, true);
}

#[test]
fn package_bridge_surfaces_if_direct_structured_question_indexed_method_result_member_queries_without_semantic_analysis(
) {
    run_method_result_member_query_case(StructuredKind::If, true);
}

#[test]
fn package_bridge_surfaces_match_direct_structured_question_indexed_method_result_member_queries_without_semantic_analysis(
) {
    run_method_result_member_query_case(StructuredKind::Match, true);
}

#[test]
fn package_bridge_follows_if_direct_structured_question_indexed_method_result_member_type_definitions_without_semantic_analysis(
) {
    run_method_result_member_type_definition_case(StructuredKind::If, true);
}

#[test]
fn package_bridge_follows_match_direct_structured_question_indexed_method_result_member_type_definitions_without_semantic_analysis(
) {
    run_method_result_member_type_definition_case(StructuredKind::Match, true);
}
