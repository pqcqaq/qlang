...
use ql_analysis::{
    ...
    RenameTarget, analyze_package, analyze_package_with_available_dependencies, analyze_source,
};
...
#[cfg(test)]
use workspace_symbols::workspace_symbols_for_documents;
use workspace_symbols::workspace_symbols_for_documents_and_roots;
...
impl Backend {
    ...
    async fn workspace_request_context(
        &self,
        package: ql_analysis::PackageAnalysis,
        analysis: Option<Analysis>,
    ) -> WorkspaceRequestContext {
        let open_docs = self.open_file_documents().await;
        self.workspace_request_context_from_parts(package, analysis, open_docs)
    }

    fn workspace_request_context_from_parts(
        &self,
        package: ql_analysis::PackageAnalysis,
        analysis: Option<Analysis>,
        open_docs: OpenDocuments,
    ...
    async fn workspace_request_context_for_source(
        &self,
        uri: &Url,
        source: &str,
...
fn document_links_for_package_imports(
    source: &str,
    package: &ql_analysis::PackageAnalysis,
) -> Option<Vec<DocumentLink>> {
...
fn document_link_dependency_for_import_segments<'a>(
    package: &'a ql_analysis::PackageAnalysis,
    segments: &[String],
