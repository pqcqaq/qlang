...
#[test]
fn package_analysis_surfaces_dependency_direct_question_receiver_member_completions_in_parse_error_source()
 {
    let temp = TempDir::new("ql-analysis-package-direct-question-member-completion-parse-error");
    let app_root = temp.path().join("workspace").join("app");
...
"#;
    ...
    let field = package
        .dependency_member_field_completions_at(source, nth_offset(source, ".va", 1) + ".va".len())
        .expect("dependency direct-question field completions should survive parse errors");
    assert_eq!(
        field
            .iter()
            .map(|item| item.label.as_str())
    ...
    let method = package
        .dependency_method_completions_at(source, nth_offset(source, ".ge", 1) + ".ge".len())
        .expect("dependency direct-question method completions should survive parse errors");
    assert_eq!(
        method
            .iter()
            .map(|item| item.label.as_str())
