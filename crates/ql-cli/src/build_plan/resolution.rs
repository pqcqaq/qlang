...
use ql_project::{
    WorkspaceBuildTargets, discover_package_build_targets, load_project_manifest, package_name,
};
...
pub(crate) fn resolve_project_build_plan_members(
    ...
    let workspace_members_by_manifest = members_by_manifest_path(workspace_members);
    let selected_members_by_manifest = members_by_manifest_path(selected_members);
...
pub(super) fn dependency_manifest_paths_for_selected_roots(
    workspace_members: &[WorkspaceBuildTargets],
    selected_members: &[WorkspaceBuildTargets],
    command_label: &str,
...
fn members_by_manifest_path(
    members: &[WorkspaceBuildTargets],
) -> BTreeMap<String, WorkspaceBuildTargets> {
    members
        .iter()
...
fn collect_project_build_plan_member(
    ...
    let manifest = load_project_manifest(start_path).map_err(|error| {
        super::report_project_build_dependency_error(command_label, owner_manifest_path, &error);
        1
    })?;
    ...
    push_project_build_plan_member(ordered, member, selected_manifest_paths, &manifest_key);
    Ok(())
}
...
fn push_project_build_plan_member(
    ordered: &mut Vec<ProjectBuildPlanMember>,
...
fn project_build_plan_member_targets(
    ...
    let package_name = package_name(manifest).map_err(|error| {
        super::report_project_build_dependency_error(command_label, None, &error);
        1
    })?;
