...
fn stdlib_templates_exercise_core_option_result_and_array_dependencies() {
    ...
    assert!(
        main.contains("let repeated_enabled: [Bool; 3] = [option_unwrap_or(enabled, false); 3]")
    );
    ...
    assert!(!smoke.contains("expect_status_ok"));
    ...
    assert!(smoke.contains("let option_value: Option[Int] = option_some(sum_int_array(numbers))"));
    assert!(smoke.contains("let result_value: Result[Int, Int] = result_ok_or(option_value, 9)"));
    ...
    assert!(smoke.contains("return expect_eq(total_check + length_check + contains_check + repeated_check + array_check + option_check + result_check, 0)"));
