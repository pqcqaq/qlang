...
fn missing_file_reports_invalid_input() {
    ...
    let error =
        prepare_single_source(&path, None).expect_err("missing file should report invalid input");
...
fn plain_file_source_is_preserved() {
    ...
    let prepared = prepare_single_source(&path, None).expect("plain source should load from disk");
