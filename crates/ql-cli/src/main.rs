...
fn resolve_project_init_stdlib_dependencies(
    ...
    Ok(vec![
        ...
        (
            "std.option".to_owned(),
            relative_path_from(&package_root, &option_root),
        ),
        (
            "std.result".to_owned(),
...
fn project_status_path(path: &Path, package_name: Option<&str>, json: bool) -> Result<(), u8> {
    let request_root = resolve_project_workspace_member_command_request_root(path);
    let manifest = load_project_manifest(request_root.as_deref().unwrap_or(path))
...
fn collect_project_status_member(
    ...
    let dependencies = find_workspace_member_dependencies(root_manifest, &member_manifest.manifest_path)?;
...
fn project_dependency_json(dependency: &ProjectDependencyMember) -> JsonValue {
    json!({
