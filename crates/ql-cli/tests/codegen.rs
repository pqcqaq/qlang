...
#[test]
fn question_wrapped_projected_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "question-wrapped projected aggregate match catch-all build regression:\n\n{message}"
        );
    }
}

#[test]
fn import_alias_question_wrapped_projected_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "import-alias question-wrapped projected aggregate match catch-all build regression:\n\n{message}"
        );
    }
}

#[test]
fn control_flow_question_wrapped_projected_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "control-flow question-wrapped projected aggregate match catch-all build regression:\n\n{message}"
        );
    }
}
...
fn import_alias_control_flow_question_wrapped_projected_aggregate_match_catch_all_codegen_case_matches(
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "import-alias control-flow question-wrapped projected aggregate match catch-all build regression:\n\n{message}"
        );
    }
}

#[test]
fn question_wrapped_nested_projected_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "question-wrapped nested projected aggregate match catch-all build regression:\n\n{message}"
        );
    }
}

#[test]
fn import_alias_question_wrapped_nested_projected_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "import-alias question-wrapped nested projected aggregate match catch-all build regression:\n\n{message}"
        );
    }
}

#[test]
fn control_flow_question_wrapped_nested_projected_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "control-flow question-wrapped nested projected aggregate match catch-all build regression:\n\n{message}"
        );
    }
}
...
fn import_alias_control_flow_question_wrapped_nested_projected_aggregate_match_catch_all_codegen_case_matches(
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "import-alias control-flow question-wrapped nested projected aggregate match catch-all build regression:\n\n{message}"
        );
    }
}
...
#[test]
fn control_flow_call_root_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!("control-flow call-root aggregate match catch-all build regression:\n\n{message}");
    }
}

#[test]
fn import_alias_control_flow_call_root_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "import-alias control-flow call-root aggregate match catch-all build regression:\n\n{message}"
        );
    }
}

#[test]
fn question_wrapped_call_root_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
...
#[test]
fn import_alias_question_wrapped_call_root_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "import-alias question-wrapped call-root aggregate match catch-all build regression:\n\n{message}"
        );
    }
}
...
fn import_alias_control_flow_question_wrapped_call_root_aggregate_match_catch_all_codegen_case_matches(
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "import-alias control-flow question-wrapped call-root aggregate match catch-all build regression:\n\n{message}"
        );
    }
}

#[test]
fn question_wrapped_nested_call_root_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "question-wrapped nested call-root aggregate match catch-all build regression:\n\n{message}"
        );
    }
}

#[test]
fn import_alias_question_wrapped_nested_call_root_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "import-alias question-wrapped nested call-root aggregate match catch-all build regression:\n\n{message}"
        );
    }
}

#[test]
fn control_flow_question_wrapped_nested_call_root_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "control-flow question-wrapped nested call-root aggregate match catch-all build regression:\n\n{message}"
        );
    }
}
...
fn import_alias_control_flow_question_wrapped_nested_call_root_aggregate_match_catch_all_codegen_case_matches(
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "import-alias control-flow question-wrapped nested call-root aggregate match catch-all build regression:\n\n{message}"
        );
    }
}
...
#[test]
fn awaited_call_root_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!("awaited call-root aggregate match catch-all build regression:\n\n{message}");
    }
}

#[test]
fn import_alias_awaited_call_root_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
...
#[test]
fn control_flow_awaited_call_root_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
...
#[test]
fn import_alias_control_flow_awaited_call_root_aggregate_match_catch_all_codegen_case_matches() {
    let workspace_root = workspace_root();
    let case = PassCase {
    ...
    if let Err(message) = run_pass_case(&workspace_root, &case) {
        panic!(
            "import-alias control-flow awaited call-root aggregate match catch-all build regression:\n\n{message}"
        );
    }
}
