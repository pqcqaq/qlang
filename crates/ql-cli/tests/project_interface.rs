...
#[test]
fn project_emit_interface_preserves_invalid_manifest_rerun_hint() {
    let workspace_root = workspace_root();
    let temp = TempDir::new("ql-project-interface-invalid-manifest");
    let project_root = temp.path().join("workspace").join("app");
    std::fs::create_dir_all(&project_root)
        .expect("create project root for invalid manifest emit-interface test");
    temp.write(
        "workspace/app/qlang.toml",
        r#"
[package
...
"#,
    ...
    let manifest_display = manifest_path.to_string_lossy().replace('\\', "/");
    let error_line =
        format!("error: `ql project emit-interface` invalid manifest `{manifest_display}`");
    ...
    command
        .args(["project", "emit-interface"])
        .arg(&project_root);
    let output = run_command_capture(&mut command, "`ql project emit-interface` invalid manifest");
    let (stdout, stderr) = expect_exit_code(
        "project-interface-invalid-manifest",
        "package interface emission with invalid manifest",
        &output,
        1,
    )
    .expect("package interface emission should fail when the manifest is syntactically invalid");
    expect_empty_stdout(
        "project-interface-invalid-manifest",
        "package interface emission with invalid manifest",
        &stdout,
    )
    .expect("invalid manifests should not report a written interface");
    let normalized_stderr = stderr.replace('\\', "/");
    expect_stderr_contains(
        "project-interface-invalid-manifest",
        "package interface emission with invalid manifest",
    ...
    .expect("invalid manifests should preserve the emit-interface command label");
    expect_stderr_not_contains(
        "project-interface-invalid-manifest",
        "package interface emission with invalid manifest",
        &normalized_stderr,
    ...
    .expect("invalid manifests should not fall back to the generic unlabeled project error");
    expect_stderr_contains(
        "project-interface-invalid-manifest",
        "package interface emission with invalid manifest",
        &normalized_stderr,
    ...
    .expect("invalid manifests should point to the failing package manifest");
    expect_stderr_contains(
        "project-interface-invalid-manifest",
        "package interface emission with invalid manifest",
        &normalized_stderr,
    ...
    .expect("invalid manifests should preserve the direct emit-interface rerun hint");
    assert!(
        !interface_path.exists(),
        "invalid manifest should not create `{}`",
        interface_path.display()
    );
}
...
#[test]
fn project_emit_interface_check_preserves_invalid_manifest_rerun_hint() {
    let workspace_root = workspace_root();
    let temp = TempDir::new("ql-project-interface-check-invalid-manifest");
    let project_root = temp.path().join("workspace").join("app");
    std::fs::create_dir_all(&project_root)
        .expect("create project root for invalid manifest check test");
    temp.write(
        "workspace/app/qlang.toml",
        r#"
[package
...
"#,
    ...
    let manifest_display = manifest_path.to_string_lossy().replace('\\', "/");
    let error_line =
        format!("error: `ql project emit-interface --check` invalid manifest `{manifest_display}`");
    let old_error_line = format!("error: invalid manifest `{manifest_display}`");
    let package_note = format!("note: failing package manifest: {manifest_display}");
    let rerun_hint = format!(
        "hint: rerun `ql project emit-interface {manifest_display} --check` after fixing the package manifest"
    );
    ...
    let output = run_command_capture(
        &mut command,
        "`ql project emit-interface --check` invalid manifest",
    );
    let (stdout, stderr) = expect_exit_code(
        "project-interface-check-invalid-manifest",
        "package interface check with invalid manifest",
        &output,
        1,
    )
    .expect("package interface check should fail when the manifest is syntactically invalid");
    expect_empty_stdout(
        "project-interface-check-invalid-manifest",
        "package interface check with invalid manifest",
        &stdout,
    )
    .expect("invalid manifests should not report a checked interface");
    let normalized_stderr = stderr.replace('\\', "/");
    expect_stderr_contains(
        "project-interface-check-invalid-manifest",
        "package interface check with invalid manifest",
    ...
    .expect("invalid manifests should preserve the emit-interface --check command label");
    expect_stderr_not_contains(
        "project-interface-check-invalid-manifest",
        "package interface check with invalid manifest",
        &normalized_stderr,
    ...
    .expect("invalid manifests should preserve the direct check rerun hint");
    assert!(
        !interface_path.exists(),
        "invalid manifest should not create `{}` during check",
        interface_path.display()
    );
}
...
#[test]
fn project_emit_interface_source_file_uses_workspace_root_context() {
    let workspace_root = workspace_root();
    let temp = TempDir::new("ql-project-interface-workspace-source-context");
    let project_root = temp.path().join("workspace-only");
    let app_root = project_root.join("packages").join("app");
    let tool_root = project_root.join("packages").join("tool");
    std::fs::create_dir_all(app_root.join("src"))
        .expect("create app package source directory for workspace source-context test");
    std::fs::create_dir_all(tool_root.join("src"))
        .expect("create tool package source directory for workspace source-context test");
    temp.write(
        "workspace-only/qlang.toml",
        r#"
[workspace]
...
"#,
    ...
    let output = run_command_capture(
        &mut command,
        "`ql project emit-interface` workspace member source path",
    );
    let (stdout, stderr) = expect_success(
        "project-interface-workspace-source-context",
        "workspace member source interface emission",
        &output,
    )
    .expect("workspace member source interface emission should succeed");
    let normalized_stdout = stdout.replace('\\', "/");
    let normalized_app_interface = app_interface.display().to_string().replace('\\', "/");
    let normalized_tool_interface = tool_interface.display().to_string().replace('\\', "/");
    expect_stdout_contains_all(
    ...
    .expect("workspace member source interface emission should keep the outer workspace context");
    expect_snapshot_matches(
        "project-interface-workspace-source-context",
        "workspace member source interface emission stderr",
        "",
        &stderr,
    )
    .expect("workspace member source interface emission should stay silent on stderr");
    expect_file_exists(
        "project-interface-workspace-source-context",
        &app_interface,
        "workspace app qi",
    ...
    .expect("workspace member source interface emission should create app qi");
    expect_file_exists(
        "project-interface-workspace-source-context",
        &tool_interface,
        "workspace tool qi",
    ...
    .expect("workspace member source interface emission should create tool qi");
}
...
#[test]
fn project_emit_interface_points_to_workspace_member_missing_package_manifest() {
    let workspace_root = workspace_root();
    let temp = TempDir::new("ql-project-interface-workspace-missing-package-manifest");
    let project_root = temp.path().join("workspace-only");
    let app_root = project_root.join("packages").join("app");
    let broken_root = project_root.join("packages").join("broken");
    std::fs::create_dir_all(app_root.join("src"))
...
"#,
    ...
    let output = run_command_capture(
        &mut command,
        "`ql project emit-interface` workspace member missing package manifest",
    );
    let (stdout, stderr) = expect_exit_code(
    ...
    .expect("workspace interface emission with member missing package manifest should fail");
    let normalized_stdout = stdout.replace('\\', "/");
    let normalized_stderr = stderr.replace('\\', "/");
    expect_stdout_contains_all(
        "project-interface-workspace-missing-package-manifest",
    ...
    .expect("workspace interface emission should still write healthy members before failing");
    let error_line = format!(
        "error: `ql project emit-interface` manifest `{normalized_broken_manifest}` does not declare `[package].name`"
    );
    let package_note = format!("note: failing package manifest: {normalized_broken_manifest}");
    let member_note =
        format!("note: failing workspace member manifest: {normalized_broken_manifest}");
    let rerun_hint = format!(
        "hint: rerun `ql project emit-interface {normalized_broken_manifest}` after fixing the package manifest"
    );
    expect_stderr_contains(
        "project-interface-workspace-missing-package-manifest",
        "workspace interface emission with member missing package manifest",
    ...
    .expect("workspace interface emission should preserve the emit command label for missing package metadata");
    expect_stderr_contains(
        "project-interface-workspace-missing-package-manifest",
        "workspace interface emission with member missing package manifest",
        &normalized_stderr,
    ...
    .expect("workspace interface emission should point missing package metadata at the package manifest");
    expect_stderr_contains(
        "project-interface-workspace-missing-package-manifest",
        "workspace interface emission with member missing package manifest",
        &normalized_stderr,
    ...
    .expect("workspace interface emission should keep the workspace member boundary visible");
    expect_stderr_contains(
        "project-interface-workspace-missing-package-manifest",
        "workspace interface emission with member missing package manifest",
        &normalized_stderr,
    ...
    .expect("workspace interface emission should suggest fixing the package manifest directly");
    expect_stderr_not_contains(
        "project-interface-workspace-missing-package-manifest",
        "workspace interface emission with member missing package manifest",
        &normalized_stderr,
    ...
    let package_note_index = normalized_stderr
        .find(&package_note)
        .expect("workspace interface emission should include the package manifest note");
    let member_note_index = normalized_stderr
        .find(&member_note)
        .expect("workspace interface emission should include the workspace member note");
    let rerun_hint_index = normalized_stderr
        .find(&rerun_hint)
        .expect("workspace interface emission should include the rerun hint");
    assert!(
        error_line_index < package_note_index
            && package_note_index < member_note_index
            && member_note_index < rerun_hint_index,
        "expected workspace interface emission missing-package-manifest context before rerun hint, got:\n{stderr}"
    );
    expect_stderr_contains(
        "project-interface-workspace-missing-package-manifest",
        "workspace interface emission with member missing package manifest",
    ...
    .expect("workspace interface emission should still create the healthy member interface");
    assert!(
        !broken_interface.is_file(),
        "expected failing workspace member not to create `{}`",
        broken_interface.display()
