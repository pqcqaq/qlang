...
"#,
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_awaited_call_root_aggregate_match_catch_all() {
        let dir = TestDir::new("ql-driver-awaited-call-root-aggregate-match-catch-all");
        let source = dir.write(
            "awaited_call_root_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/awaited_call_root_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("awaited call-root aggregate match catch-all should build");
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_import_alias_awaited_call_root_aggregate_match_catch_all() {
        let dir =
            TestDir::new("ql-driver-import-alias-awaited-call-root-aggregate-match-catch-all");
        let source = dir.write(
            "import_alias_awaited_call_root_aggregate_match_catch_all.ql",
            r#"
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/import_alias_awaited_call_root_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("import-alias awaited call-root aggregate match catch-all should build");
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_control_flow_awaited_call_root_aggregate_match_catch_all() {
        let dir =
            TestDir::new("ql-driver-control-flow-awaited-call-root-aggregate-match-catch-all");
        let source = dir.write(
            "control_flow_awaited_call_root_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/control_flow_awaited_call_root_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("control-flow awaited call-root aggregate match catch-all should build");
    ...
    fn build_file_writes_llvm_ir_with_import_alias_control_flow_awaited_call_root_aggregate_match_catch_all()
    {
        ...
        let source = dir.write(
            "import_alias_control_flow_awaited_call_root_aggregate_match_catch_all.ql",
            r#"
use pair_value as pair_alias
use state_value as state_alias
use values as values_alias
...
"#,
        ...
        let output = dir.path().join(
            "artifacts/import_alias_control_flow_awaited_call_root_aggregate_match_catch_all.ll",
        );
        let artifact = build_file(
            &source,
            &BuildOptions {
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_awaited_projected_aggregate_match_catch_all() {
        let dir = TestDir::new("ql-driver-awaited-projected-aggregate-match-catch-all");
        let source = dir.write(
            "awaited_projected_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/awaited_projected_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("awaited projected aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
...
async fn main() -> Int {
    match (await tuple_alias(1)).payload.values {
        (left, right) if left < right => sink(left + right),
        _ => sink(0),
    }

    match (await state_const_alias(3)).payload.current {
        State { value } if value == 3 => sink(value),
        _ => sink(0),
    }

    match (await array_alias(4)).payload.values {
        [first, middle, last] if middle == 5 => sink(first + middle + last),
        _ => sink(0),
    }

    defer match (await tuple_const_alias(1)).payload.values {
        (left, right) if left < right => sink(left + right),
        _ => sink(0),
    }

    defer match (await state_alias(3)).payload.current {
        State { value } if value == 3 => sink(value),
        _ => sink(0),
    }

    defer match (await array_const_alias(4)).payload.values {
        [first, middle, last] if middle == 5 => sink(first + middle + last),
        _ => sink(0),
    }
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/import_alias_awaited_projected_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("import-alias awaited projected aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/control_flow_awaited_projected_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("control-flow awaited projected aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    fn build_file_writes_llvm_ir_with_import_alias_control_flow_awaited_projected_aggregate_match_catch_all()
     {
        ...
        let source = dir.write(
            "import_alias_control_flow_awaited_projected_aggregate_match_catch_all.ql",
            r#"
use tuple_env as tuple_alias
use state_env as state_alias
use array_env as array_alias
...
"#,
        ...
        let output = dir.path().join(
            "artifacts/import_alias_control_flow_awaited_projected_aggregate_match_catch_all.ll",
        );
        let artifact = build_file(
            &source,
            &BuildOptions {
        ...
        .expect("import-alias control-flow awaited projected aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_awaited_nested_projected_aggregate_match_catch_all() {
        let dir = TestDir::new("ql-driver-awaited-nested-projected-aggregate-match-catch-all");
        let source = dir.write(
            "awaited_nested_projected_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/awaited_nested_projected_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("awaited nested projected aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    fn build_file_writes_llvm_ir_with_control_flow_awaited_nested_projected_aggregate_match_catch_all()
     {
        ...
        let source = dir.write(
            "control_flow_awaited_nested_projected_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/control_flow_awaited_nested_projected_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
...
"#,
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_fixed_array_match_catch_all() {
        let dir = TestDir::new("ql-driver-fixed-array-match-catch-all");
        let source = dir.write(
            "fixed_array_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir.path().join("artifacts/fixed_array_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("fixed-array match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");

        assert_eq!(artifact.path, output);
        assert!(rendered.contains("extractvalue [3 x i64]"));
        assert!(rendered.contains("cleanup_match_arm_"));
        assert!(rendered.matches("call void @sink").count() >= 4);
        assert!(!rendered.contains("does not support cleanup lowering yet"));
    }

    #[test]
    fn build_file_writes_llvm_ir_with_tuple_struct_match_catch_all() {
        let dir = TestDir::new("ql-driver-tuple-struct-match-catch-all");
        let source = dir.write(
            "tuple_struct_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir.path().join("artifacts/tuple_struct_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("tuple/struct match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
        ...
        assert!(rendered.contains("extractvalue { i64 }"));
        assert!(rendered.contains("cleanup_match_arm_"));
        assert!(rendered.matches("call void @sink").count() >= 6);
        assert!(!rendered.contains("does not support cleanup lowering yet"));
    }

    #[test]
    fn build_file_writes_llvm_ir_with_projected_aggregate_match_catch_all() {
        let dir = TestDir::new("ql-driver-projected-aggregate-match-catch-all");
        let source = dir.write(
            "projected_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/projected_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("projected aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_import_alias_projected_aggregate_match_catch_all() {
        let dir = TestDir::new("ql-driver-import-alias-projected-aggregate-match-catch-all");
        let source = dir.write(
            "import_alias_projected_aggregate_match_catch_all.ql",
            r#"
use BUNDLE as bundle_alias
use CLEANUP_BUNDLE as cleanup_bundle_alias
...
const CLEANUP_BUNDLE: Bundle = Bundle {
    pair: (4, 5),
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/import_alias_projected_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("import-alias projected aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_import_alias_control_flow_projected_aggregate_match_catch_all()
     {
        let dir =
            TestDir::new("ql-driver-import-alias-control-flow-projected-aggregate-match-catch-all");
        let source = dir.write(
            "import_alias_control_flow_projected_aggregate_match_catch_all.ql",
            r#"
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/import_alias_control_flow_projected_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect(
            "import-alias control-flow projected aggregate match catch-all should emit LLVM IR",
        );
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_control_flow_projected_aggregate_match_catch_all() {
        let dir = TestDir::new("ql-driver-control-flow-projected-aggregate-match-catch-all");
        let source = dir.write(
            "control_flow_projected_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
fn main() -> Int {
    ...
    match (if branch { left } else { right }).pair {
        (left, right) if left < right => sink(left + right),
        _ => sink(0),
    }

    match (match branch { true => left, false => right }).current {
        State { value } if value == 3 => sink(value),
        _ => sink(0),
    }

    match (if branch { left } else { right }).values {
        [first, middle, last] if middle == 5 => sink(first + middle + last),
        _ => sink(0),
    }

    defer match (match branch { true => left, false => right }).pair {
        (left, right) if left < right => sink(left + right),
        _ => sink(0),
    }

    defer match (if branch { left } else { right }).current {
        State { value } if value == 3 => sink(value),
        _ => sink(0),
    }

    defer match (match branch { true => left, false => right }).values {
        [first, middle, last] if middle == 5 => sink(first + middle + last),
        _ => sink(0),
    }
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/control_flow_projected_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("control-flow projected aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_nested_projected_aggregate_match_catch_all() {
        let dir = TestDir::new("ql-driver-nested-projected-aggregate-match-catch-all");
        let source = dir.write(
            "nested_projected_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/nested_projected_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("nested projected aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_import_alias_nested_projected_aggregate_match_catch_all() {
        let dir = TestDir::new("ql-driver-import-alias-nested-projected-aggregate-match-catch-all");
        let source = dir.write(
            "import_alias_nested_projected_aggregate_match_catch_all.ql",
            r#"
use TUPLE_ENV as tuple_alias
use STATE_ENV as state_alias
use ARRAY_ENV as array_alias
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/import_alias_nested_projected_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("import-alias nested projected aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_control_flow_nested_projected_aggregate_match_catch_all() {
        let dir = TestDir::new("ql-driver-control-flow-nested-projected-aggregate-match-catch-all");
        let source = dir.write(
            "control_flow_nested_projected_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/control_flow_nested_projected_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("control-flow nested projected aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    fn build_file_writes_llvm_ir_with_import_alias_control_flow_nested_projected_aggregate_match_catch_all()
    {
        ...
        let source = dir.write(
            "import_alias_control_flow_nested_projected_aggregate_match_catch_all.ql",
            r#"
...
"#,
        ...
        .expect(
            "import-alias control-flow nested projected aggregate match catch-all should emit LLVM IR",
        );
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_question_wrapped_projected_aggregate_match_catch_all() {
        let dir = TestDir::new("ql-driver-question-wrapped-projected-aggregate-match-catch-all");
        let source = dir.write(
            "question_wrapped_projected_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/question_wrapped_projected_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("question-wrapped projected aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/control_flow_question_wrapped_projected_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
    ...
    fn build_file_writes_llvm_ir_with_import_alias_control_flow_question_wrapped_projected_aggregate_match_catch_all(
        ...
        let source = dir.write(
            "import_alias_control_flow_question_wrapped_projected_aggregate_match_catch_all.ql",
            r#"
use bundle_value as bundle_alias
use alt_bundle_value as alt_bundle_alias
use cleanup_bundle_value as cleanup_bundle_alias
use cleanup_alt_bundle_value as cleanup_alt_bundle_alias
...
fn cleanup_alt_bundle_value() -> Bundle {
    return Bundle {
        pair: (13, 14),
        current: State { value: 15 },
        values: [16, 17, 18],
...
fn main() -> Int {
    ...
    match ((if branch { bundle_alias } else { alt_bundle_alias })()?).pair {
        (left, right) if left < right => sink(left + right),
        _ => sink(0),
    }

    match ((match branch { true => bundle_alias, false => alt_bundle_alias })()?).current {
        State { value } if value == 3 => sink(value),
        _ => sink(0),
    }

    match ((if branch { bundle_alias } else { alt_bundle_alias })()?).values {
        [first, middle, last] if middle == 5 => sink(first + middle + last),
        _ => sink(0),
    }

    defer match ((match branch { true => cleanup_bundle_alias, false => cleanup_alt_bundle_alias })()?).pair {
        (left, right) if left < right => sink(left + right),
        _ => sink(0),
    }

    defer match ((if branch { cleanup_bundle_alias } else { cleanup_alt_bundle_alias })()?).current {
        State { value } if value == 6 => sink(value),
        _ => sink(0),
    }

    defer match ((match branch { true => cleanup_bundle_alias, false => cleanup_alt_bundle_alias })()?).values {
        [first, middle, last] if middle == 8 => sink(first + middle + last),
        _ => sink(0),
    }
...
"#,
        ...
        let output = dir.path().join(
            "artifacts/import_alias_control_flow_question_wrapped_projected_aggregate_match_catch_all.ll",
        );
        let artifact = build_file(
            &source,
            &BuildOptions {
        ...
        .expect(
            "import-alias control-flow question-wrapped projected aggregate match catch-all should emit LLVM IR",
        );
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    fn build_file_writes_llvm_ir_with_question_wrapped_nested_projected_aggregate_match_catch_all(
        ...
        let source = dir.write(
            "question_wrapped_nested_projected_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/question_wrapped_nested_projected_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("question-wrapped nested projected aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    fn build_file_writes_llvm_ir_with_import_alias_question_wrapped_nested_projected_aggregate_match_catch_all(
        ...
        let source = dir.write(
            "import_alias_question_wrapped_nested_projected_aggregate_match_catch_all.ql",
            r#"
use TUPLE_ENV as tuple_alias
use STATE_ENV as state_alias
use ARRAY_ENV as array_alias
...
"#,
        ...
        let output = dir.path().join(
            "artifacts/import_alias_question_wrapped_nested_projected_aggregate_match_catch_all.ll",
        );
        let artifact = build_file(
            &source,
            &BuildOptions {
        ...
        .expect(
            "import-alias question-wrapped nested projected aggregate match catch-all should emit LLVM IR",
        );
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    fn build_file_writes_llvm_ir_with_control_flow_question_wrapped_nested_projected_aggregate_match_catch_all(
        ...
        let source = dir.write(
            "control_flow_question_wrapped_nested_projected_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir.path().join(
            "artifacts/control_flow_question_wrapped_nested_projected_aggregate_match_catch_all.ll",
        );
        let artifact = build_file(
            &source,
            &BuildOptions {
        ...
        .expect(
            "control-flow question-wrapped nested projected aggregate match catch-all should emit LLVM IR",
        );
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    fn build_file_writes_llvm_ir_with_import_alias_control_flow_question_wrapped_nested_projected_aggregate_match_catch_all(
        ...
        let source = dir.write(
            "import_alias_control_flow_question_wrapped_nested_projected_aggregate_match_catch_all.ql",
            r#"
use LEFT_TUPLE as left_tuple_alias
use RIGHT_TUPLE as right_tuple_alias
use LEFT_STATE as left_state_alias
...
"#,
        ...
        let output = dir.path().join(
            "artifacts/import_alias_control_flow_question_wrapped_nested_projected_aggregate_match_catch_all.ll",
        );
        let artifact = build_file(
            &source,
            &BuildOptions {
        ...
        .expect(
            "import-alias control-flow question-wrapped nested projected aggregate match catch-all should emit LLVM IR",
        );
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    fn build_file_writes_llvm_ir_with_call_root_aggregate_match_catch_all() {
        ...
        let source = dir.write(
            "call_root_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/call_root_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("call-root aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_import_alias_call_root_aggregate_match_catch_all() {
        let dir = TestDir::new("ql-driver-import-alias-call-root-aggregate-match-catch-all");
        let source = dir.write(
            "import_alias_call_root_aggregate_match_catch_all.ql",
            r#"
use pair_value as pair_alias
use state_value as state_alias
use values as values_alias
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/import_alias_call_root_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("import-alias call-root aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_control_flow_call_root_aggregate_match_catch_all() {
        let dir = TestDir::new("ql-driver-control-flow-call-root-aggregate-match-catch-all");
        let source = dir.write(
            "control_flow_call_root_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/control_flow_call_root_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("control-flow call-root aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    fn build_file_writes_llvm_ir_with_import_alias_control_flow_call_root_aggregate_match_catch_all(
        ...
        let source = dir.write(
            "import_alias_control_flow_call_root_aggregate_match_catch_all.ql",
            r#"
use pair_value as pair_alias
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/import_alias_control_flow_call_root_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_question_wrapped_call_root_aggregate_match_catch_all() {
        let dir = TestDir::new("ql-driver-question-wrapped-call-root-aggregate-match-catch-all");
        let source = dir.write(
            "question_wrapped_call_root_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/question_wrapped_call_root_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("question-wrapped call-root aggregate match catch-all should build");
    ...
    fn build_file_writes_llvm_ir_with_import_alias_question_wrapped_call_root_aggregate_match_catch_all(
        ...
        let source = dir.write(
            "import_alias_question_wrapped_call_root_aggregate_match_catch_all.ql",
            r#"
use pair_value as pair_alias
use state_value as state_alias
use values as values_alias
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/import_alias_question_wrapped_call_root_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("import-alias question-wrapped call-root aggregate match catch-all should build");
...
"#,
    ...
    fn build_file_writes_llvm_ir_with_import_alias_control_flow_question_wrapped_call_root_aggregate_match_catch_all(
        ...
        let source = dir.write(
            "import_alias_control_flow_question_wrapped_call_root_aggregate_match_catch_all.ql",
            r#"
use pair_value as pair_alias
use alt_pair_value as alt_pair_alias
use state_value as state_alias
...
"#,
        ...
        let output = dir.path().join(
            "artifacts/import_alias_control_flow_question_wrapped_call_root_aggregate_match_catch_all.ll",
        );
        let artifact = build_file(
            &source,
            &BuildOptions {
        ...
        .expect(
            "import-alias control-flow question-wrapped call-root aggregate match catch-all should build",
        );
...
fn main() -> Int {
    match (tuples(1)?).payload.values {
        (left, right) if left < right => sink(left + right),
        _ => sink(0),
    }

    match (states(3)?).payload.current {
        State { value } if value == 3 => sink(value),
        _ => sink(0),
    }

    match (arrays(4)?).payload.values {
        [first, middle, last] if middle == 5 => sink(first + middle + last),
        _ => sink(0),
    }

    match (deep(6)?).outer.payload.values {
        [first, middle, last] if middle == 7 => sink(first + middle + last),
        _ => sink(0),
    }

    defer match (tuples(1)?).payload.values {
        (left, right) if left < right => sink(left + right),
        _ => sink(0),
    }

    defer match (states(3)?).payload.current {
        State { value } if value == 3 => sink(value),
        _ => sink(0),
    }

    defer match (arrays(4)?).payload.values {
        [first, middle, last] if middle == 5 => sink(first + middle + last),
        _ => sink(0),
    }

    defer match (deep(6)?).outer.payload.values {
        [first, middle, last] if middle == 7 => sink(first + middle + last),
        _ => sink(0),
    }
...
"#,
    ...
    fn build_file_writes_llvm_ir_with_import_alias_control_flow_question_wrapped_nested_call_root_aggregate_match_catch_all(
        ...
        let source = dir.write(
            "import_alias_control_flow_question_wrapped_nested_call_root_aggregate_match_catch_all.ql",
            r#"
use tuple_env as tuples
...
"#,
        ...
        let output = dir.path().join(
            "artifacts/import_alias_control_flow_question_wrapped_nested_call_root_aggregate_match_catch_all.ll",
        );
        let artifact = build_file(
            &source,
            &BuildOptions {
        ...
        .expect(
            "import-alias control-flow question-wrapped nested call-root aggregate match catch-all should build",
        );
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    fn build_file_writes_llvm_ir_with_nested_call_root_aggregate_match_catch_all() {
        ...
        let source = dir.write(
            "nested_call_root_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/nested_call_root_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("nested call-root aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_import_alias_nested_call_root_aggregate_match_catch_all() {
        let dir = TestDir::new("ql-driver-import-alias-nested-call-root-aggregate-match-catch-all");
        let source = dir.write(
            "import_alias_nested_call_root_aggregate_match_catch_all.ql",
            r#"
use array_env as arrays
use tuple_env as tuples
use state_env as states
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/import_alias_nested_call_root_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("import-alias nested call-root aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
    ...
    #[test]
    fn build_file_writes_llvm_ir_with_control_flow_nested_call_root_aggregate_match_catch_all() {
        let dir = TestDir::new("ql-driver-control-flow-nested-call-root-aggregate-match-catch-all");
        let source = dir.write(
            "control_flow_nested_call_root_aggregate_match_catch_all.ql",
            r#"
extern "c" fn sink(value: Int)
...
"#,
        ...
        let output = dir
            .path()
            .join("artifacts/control_flow_nested_call_root_aggregate_match_catch_all.ll");
        let artifact = build_file(
            &source,
            &BuildOptions {
                emit: BuildEmit::LlvmIr,
        ...
        .expect("control-flow nested call-root aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
...
"#,
        ...
        let output = dir.path().join(
            "artifacts/import_alias_control_flow_nested_call_root_aggregate_match_catch_all.ll",
        );
        let artifact = build_file(
            &source,
            &BuildOptions {
        ...
        .expect("import-alias control-flow nested call-root aggregate match catch-all should emit LLVM IR");
        let rendered = fs::read_to_string(&artifact.path).expect("read generated LLVM IR");
