...
#[test]
fn emits_different_target_mutable_alias_capturing_closure_calls() {
    let rendered = emit(
        r#"
...
let direct_matched = match 42 {
    current if direct_check(current) => 1,
    current if chosen_check(current) => 2,
    _ => 0,
}
let match_direct_matched = match 42 {
    current if match_direct_check(current) => 3,
    _ => 0,
}
let match_bound_matched = match 42 {
    current if match_chosen_check(current) => 4,
    _ => 0,
}
let matched = direct_matched + match_direct_matched + match_bound_matched
...
"#,
    ...
    assert!(rendered.matches("call i64 @").count() >= 6);
    assert!(rendered.matches("call i1 @").count() >= 6);
...
#[test]
fn emits_cleanup_if_shared_local_control_flow_capturing_closure_alias_chain() {
    let rendered = emit(
        r#"
fn main() -> Int {
let target = 42
...
var alias = left
defer {
    let chosen = if true { alias = right } else { left }
    let rebound = chosen
    rebound(1)
}
return 0
...
"#,
    ...
    assert!(rendered.matches("__closure").count() >= 2);
    assert!(rendered.contains("cleanup_call_if_then"));
    assert!(rendered.contains("cleanup_call_if_else"));
    assert!(!rendered.contains("does not support cleanup lowering yet"));
    assert!(
        !rendered.contains("currently only supports a narrow non-`move` capturing-closure subset")
    );
...
#[test]
fn emits_cleanup_if_shared_local_control_flow_capturing_closure_guard_alias_chain() {
    let rendered = emit(
        r#"
extern "c" fn choose() -> Bool
extern "c" fn keep()
...
var alias = left
defer {
    let chosen = if choose() { alias = right } else { left }
    let rebound = chosen
    if rebound(42) {
        keep()
    }
...
#[test]
fn emits_cleanup_match_shared_local_control_flow_capturing_closure_alias_chain() {
    let rendered = emit(
        r#"
extern "c" fn choose() -> Bool
...
#[test]
fn emits_cleanup_match_shared_local_control_flow_capturing_closure_guard_alias_chain() {
    let rendered = emit(
        r#"
extern "c" fn choose() -> Bool
extern "c" fn keep()
