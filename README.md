...
- `ql-codegen-llvm` now also lowers fixed-shape `for` loops across the current program/library build surface without introducing async runtime hooks, covering fixed arrays plus homogeneous tuples; broader iterable shapes still stay on the structured `for`-lowering rejection path
