...
- LLVM 可执行主路径现在已支持本地与 direct local dependency 的 `impl` / `extend` / 唯一 trait `impl for` receiver method 直接调用，以及经不可变局部 alias 的 method value 直接调用（如 `let add = value.add; add(1)`）。当前边界仍然很窄：更广义的 escaping / higher-order method value 仍未打通。
...
- healthy package/workspace 下，LSP 的 source-preferred dependency tooling 已覆盖 workspace members 和 workspace 外本地路径依赖；definition、typeDefinition、references、current-document `documentHighlight`、dependency completion、`workspace/symbol`，以及 source-backed dependency `method / field / enum variant` workspace rename，都会按 manifest 身份区分同名本地依赖，且 definition / typeDefinition / references / `documentHighlight` / dependency completion / workspace rename 现在也会尊重已打开但未落盘的本地依赖源码，源码可用时优先返回源码符号；healthy source 的 workspace import references 现在同时覆盖 value import 和 type import 的 alias/use，并会读取已打开但未落盘的导出源码与其他 workspace consumer 源码；workspace root `function / const / static / struct / enum / trait / type alias` 的源码定义点和同文件使用点 references 也已能联动到当前文件引用、workspace 导入使用点，以及可见 broken consumers 的 import/use，这条 import/use 聚合同样会尊重 open docs。
- broken-source / parse-error 下，import references fallback、workspace import `hover/definition/typeDefinition`、direct imported-result member hover / completion / 查询 / `documentHighlight`、dependency struct field label completion、dependency enum variant 的 `completion/definition/typeDefinition/references/documentHighlight`、dependency value/member semantic tokens fallback，以及 source-backed dependency method / struct field / enum variant workspace rename 都会继续走保守可用路径；其中 broken-source workspace import references、workspace import query、dependency references / current-document `documentHighlight` / method completion / workspace rename 现在也会尊重已打开但未落盘的 workspace member / 本地依赖源码；当 broken consumer 聚合 healthy workspace import/use 时，也会读取这些 consumer 的 open docs；workspace root `function / const / static / struct / enum / trait / type alias` 现在也允许从当前 parse-error consumer 的 import/use 位置发起 rename（包含 alias import/use），并会保守联动当前 broken 文件、当前 package 其他源码文件、当前 package 可见的 workspace members / 本地路径依赖里的其他 consumer 源码，以及导出包源码；对应的 import/use references 聚合现在也会补回这些可见 broken consumers；同名本地依赖仍按 manifest 身份区分。
