...
- `ql-runtime` now also defines the first shared runtime hook ABI skeleton for the async surface, including stable symbol names and first-pass LLVM-facing contract strings, `ql runtime <file>` renders the deduped hook plan alongside capability requirements, and `ql-codegen-llvm` now reuses that shared ABI contract to emit runtime hook declarations without inventing backend-local symbol/signature copies
