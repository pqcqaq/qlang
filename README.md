...
- healthy package/workspace 下，LSP 的 source-preferred navigation 现在同时覆盖 workspace members 和 workspace 外本地路径依赖；import / dependency / method member 的 definition、typeDefinition、references 会优先落源码而不是 `.qi`，broken-source 下的 import references fallback 也覆盖这一条本地依赖路径；同名本地依赖现在按 manifest 身份区分，不会把 navigation / references / workspace symbol 串到另一个依赖实例；package-aware semantic tokens 已接通；`workspace/symbol` 在本地依赖源码可用时会优先返回源码里的 value / method / trait / extend symbols，并覆盖 `workspace_roots` / 无打开文档入口。
