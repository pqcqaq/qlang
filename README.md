...
- `qlsp` 现已支持 `textDocument/implementation`；当前会对 same-file trait/type surface 返回保守实现位置。trait/type 返回对应 `impl` / `extend` block，trait method 返回匹配的 impl method；暂不支持 workspace/dependency 级 implementation graph。
...
同一套 `qlsp` 现在也支持 `Go to Implementation`。当前先覆盖 same-file trait/type surface：trait/type 会跳到本文件里的 `impl` / `extend` block，trait method 会跳到匹配的 impl method。
