...
- `ql-runtime` now also defines the first shared runtime hook ABI skeleton for the async surface, including stable symbol names and first-pass LLVM-facing contract strings, and `ql runtime <file>` renders the deduped hook plan alongside capability requirements
