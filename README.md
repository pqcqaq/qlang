...
- `ql-codegen-llvm` now also lowers the minimal literal `match` build subset across the current program/library build surface: unguarded `true` / `false` plus `_` or single-name binding catch-all arms on a `Bool` scrutinee lower to LLVM branches, unguarded integer-literal plus `_` or single-name binding catch-all arms on an `Int` scrutinee lower to a stable compare-chain, literal `if true` / `if false` guards plus same-file `const`-backed `Bool` guards and their same-file `use ... as ...` aliases that resolve to those literals are folded during lowering, simple scalar guard comparisons now also lower in the current subset (`Bool` `==` / `!=`, plus `Int` `==` / `!=` / `>` / `>=` / `<` / `<=` built from integer literals, same-file const/alias `Int` operands, read-only same-scope scalar projection operands rooted in locals / parameters / `self`, and same-file `const` / alias-backed aggregate projections that fold through struct fields, tuple literal indices, or fixed-array indices to scalar values), the `Bool` scrutinee subset now also lowers direct same-scope `Bool` local / parameter name guards when later arms still provide guaranteed fallback coverage, and the `Int` scrutinee subset now also lowers direct same-scope `Bool` local / parameter name guards on integer-literal arms and guarded catch-all arms when a later unguarded catch-all arm provides fallback coverage; broader dynamic guards and broader match patterns stay on the stable rejection path
