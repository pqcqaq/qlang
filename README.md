...
- `ql-runtime` now also defines the shared runtime hook ABI skeleton for the async surface, including `qlrt_async_frame_alloc` plus `qlrt_async_task_create`; `ql runtime <file>` renders the deduped hook plan alongside capability requirements, `ql-codegen-llvm` reuses that shared ABI contract for runtime hook declarations, the backend now emits unified `async fn` body wrappers that always receive a frame pointer and can heap-materialize parameterized async frames conservatively, and `ql-typeck` now rejects direct `async fn` calls unless they are immediately consumed by `await` or `spawn`
