...
- rename 仍以 same-file 为主；LSP 现已开放 source-backed dependency `method / field / enum variant` 的 workspace rename，可从依赖使用点或导出包源码侧发起，并同时改写依赖源码定义点、源码内部引用和工作区使用点，其余符号仍未开放 cross-file rename / workspace edits。
