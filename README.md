...
- LLVM 可执行主路径现在已支持本地与 direct local dependency 的 `impl` / `extend` / 唯一 trait `impl for` receiver method 直接调用、经不可变局部 alias 的 method value 直接调用（如 `let add = value.add; add(1)`），以及 public 非泛型 `enum` 的按值返回与 unit / struct variant `match`。当前边界仍然很窄：更广义的 escaping / higher-order method value 与 tuple variant runtime 仍未打通。
