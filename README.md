...
- `ql-codegen-llvm` now also lowers the minimal literal `match` build subset across the current program/library build surface: unguarded `true` / `false` / `_` arms on a `Bool` scrutinee lower to LLVM branches, and unguarded integer-literal / `_` arms on an `Int` scrutinee lower to a stable compare-chain, while guarded matches and broader match patterns stay on the stable rejection path
