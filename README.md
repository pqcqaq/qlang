...
- `ql-driver` now also consumes that runtime requirement surface as the shared gate for still-unsupported async capabilities, so non-fixed-array `for await`, broader async program/bootstrap paths, and other closed surfaces keep stable build-time diagnostics while the supported `staticlib` subset, minimal library-style async `dylib` subset, and current `BuildEmit::LlvmIr` / `BuildEmit::Object` / `BuildEmit::Executable` `async fn main` subset expand conservatively
- `ql-codegen-llvm` now also lowers fixed-array `for` loops across the current program/library build surface without introducing async runtime hooks; non-fixed-array iterables still stay on the structured `for`-lowering rejection path
