...
- deeper struct-like shorthand tokens now also have explicit same-file parity coverage: when field semantics stay closed on paths like `Point.Scope.Config { x }` / `Point.Scope.Config { source }`, the shorthand token still follows its lexical local / binding / import identity, and same-file rename keeps those edits raw instead of fabricating unsupported field-label expansions
