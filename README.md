...
- started conservative Phase 7 async/runtime/library-build, program-build, and Rust interop foundation
...
- `if` / `match` expressions and short-circuit `&&` / `||` plus unary `!` on `Bool`
...
- `ql-analysis` now also exposes source-ordered runtime requirements for the current async surface, so later driver/codegen/runtime work can share one stable truth source for `async fn`, `spawn`, `await`, and `for await`; task-backed `for await` loops now also conservatively carry `task-await` capability so builds get the right runtime hook plan even when the loop body no longer spells an explicit inner `await`
- `ql-driver` now also consumes that runtime requirement surface as the shared gate for still-unsupported async capabilities, so broader dynamic/generalized `for await`, broader async program/bootstrap paths, and other closed surfaces keep stable build-time diagnostics while the supported `staticlib` subset, minimal library-style async `dylib` subset, and current `BuildEmit::LlvmIr` / `BuildEmit::Object` / `BuildEmit::Executable` `async fn main` subset expand conservatively
- `ql-codegen-llvm` now also lowers fixed-shape `for` loops across the current program/library build surface without introducing async runtime hooks, covering fixed arrays plus homogeneous tuples, including current local/projected roots and same-file foldable `const` / `static` roots plus their same-file `use ... as ...` aliases; broader iterable shapes still stay on the structured `for`-lowering rejection path
- ordinary expressions and bool conditions in the current build surface now also materialize same-file foldable `const` / `static` item values and their same-file `use ... as ...` aliases through the existing const-evaluation path, including aggregate values that can already be rendered by the current literal/aggregate subset
- direct resolved function calls in the current build surface now also lower named call arguments in declaration order, including named calls that rely on the existing expected-type back-propagation path for `[]` and other already-supported aggregate literals
- same-file `use ... as ...` aliases that resolve to local function items now also lower through that same direct-call backend path, including the current named-call-argument subset and the current `async fn main` executable subset, with alias-root direct `await` / `spawn` on local async callees plus aggregate-carried, fixed-array-carried, tuple-carried, helper-forwarded, helper-returned-task, helper-returned-task + helper-forwarded submit, direct fixed-array / task-tuple task-root `for await`, direct helper-returned fixed-array / task-tuple `for await`, projected fixed-array / task-tuple roots `for await`, awaited aggregate / awaited nested aggregate task-array `for await`, awaited aggregate / awaited nested aggregate task-tuple `for await`, import-aliased awaited aggregate task-array / task-tuple `for await`, inline-awaited fixed-array / task-tuple roots `for await`, awaited tuple-projected / array-projected fixed-array task roots `for await`, awaited tuple-projected / array-projected task-tuple roots `for await`, and `Task[Void]` fixed-array / task-tuple wildcard `for await` executable paths now covered by committed examples
- `ql-codegen-llvm` now also lowers short-circuit `&&` / `||` plus unary `!` on `Bool` expressions across the current program/library build surface, and the minimal literal `match` build subset across that same surface: unguarded `true` / `false` or same-file foldable `const`/`static`/local-alias bare path patterns that fold to `Bool`, plus `_` or single-name binding catch-all arms on a `Bool` scrutinee lower to LLVM branches; unguarded integer literals, unary-negated integer literals, or same-file foldable `const`/`static`/local-alias bare path patterns that fold to `Int`, plus `_` or single-name binding catch-all arms on an `Int` scrutinee lower to a stable compare-chain; for other current loadable scrutinee shapes, the backend now also accepts a conservative catch-all-only subset made of `_` / single-name binding arms plus the current bool-guard subset on those catch-all arms, so shapes like `match state { current => ... }` and `match state { current if current.slot.ready => ...; _ => ... }` no longer stay on the unconditional rejection path; literal `if true` / `if false` guards plus same-file foldable `const`/`static`-backed `Bool` guards and their same-file `use ... as ...` aliases that resolve to those literals are folded during lowering, including computed same-file foldable `const`/`static` `Bool` expressions built from the current supported `!`, `&&`, `||`, `==`, `!=`, and integer-comparison subset over foldable const/static operands; when a bool-arm guard is just the current scrutinee itself, a single leading `!` of that scrutinee, or that same scrutinee compared with a foldable `Bool` literal/const/static/alias via `==` / `!=`, the backend now also folds that guard to always/skip before ordered-arm coverage checks, so `match flag { true if flag => ...; false => ... }` and `match flag { true if flag == ON => ...; false => ... }` no longer trip fake unsupported fallback gaps; direct bool-valued guards on `Bool` scrutinee matches now also no longer require a later guaranteed fallback arm, so `match flag { true if enabled => ...; false => ... }` can lower directly instead of being conservatively rejected for missing a later `true` fallback; direct bool-valued guards on `Int` scrutinee matches likewise now no longer require a later unguarded catch-all fallback, so `match value { 1 if enabled => ...; 2 => ... }` can lower directly instead of being conservatively rejected for missing a later catch-all arm; unary `!` can now wrap the currently supported bool-valued guard subset; that same bool-valued guard subset can now also be combined with runtime `&&` / `||` inside `match` guards; current-arm single-name binding catch-all variables can now participate as direct scalar operands inside that same guard subset, can also serve as dynamic fixed-array index operands inside the current read-only guard projection subset, and can now also act as the read-only projection root for struct-field, tuple literal-index, and fixed-array index guard operands such as `current.slot.ready`, `current[1]`, and `current[0]`; fixed-array read-only guard projections now also accept the current runtime `Int` scalar subset as index expressions not only on local / parameter / `self` / current-arm-binding roots, but also on same-file `const` / `static` aggregate roots and their same-file `use ... as ...` aliases (for example `values[index + 1]`, `values[current + 1]`, `VALUES[index + 1]`, or `INPUT[state.offset]`), while tuple indices stay foldable-const-only; simple scalar guard comparisons now also lower in the current subset (`Bool` `==` / `!=`, plus `Int` `==` / `!=` / `>` / `>=` / `<` / `<=` built from integer literals, unary-negated supported `Int` operands, same-file foldable `const`/`static`-backed `Int` operands and their same-file `use ... as ...` aliases, minimal integer arithmetic expressions recursively composed from the current supported `Int` operand subset via `+` / `-` / `*` / `/` / `%`, read-only same-scope scalar projection operands rooted in locals / parameters / `self`, current-arm read-only projection operands rooted in single-name catch-all bindings, same-file `const` / `static` aggregate-root fixed-array projections that keep the current runtime `Int` index subset, and same-file foldable `const`/`static`-backed aggregate projections plus their same-file `use ... as ...` aliases that fold through struct fields, tuple literal indices, or fixed-array indices to scalar values, including tuple/fixed-array indices that themselves fold from the same same-file const/static-backed `Int` arithmetic subset); and direct bool-valued guards now also include same-scope `Bool` locals / parameters, read-only bool scalar projections rooted in locals / parameters / `self`, current-arm read-only bool projections rooted in single-name catch-all bindings, and same-file foldable `const`/`static`-backed aggregate projections plus their same-file `use ... as ...` aliases that fold to `Bool`; broader dynamic guards beyond the current bool-composition / scalar-comparison subset, broader current-arm projection usage beyond the current read-only struct-field / tuple-index / fixed-array-index subset, and broader match patterns beyond those literal/const-path and catch-all-only subsets stay on the stable rejection path
- that current `match` guard scalar subset now also includes direct resolved sync scalar calls: zero-arg or argument-mapped local/import-aliased calls returning `Bool` can serve as direct guards, and calls returning `Int` can participate in the current scalar-comparison subset (for example `true if enabled()` and `current if offset(delta: 2, value: current) == 22`); aggregate-parameter guard calls and broader call shapes remain on the rejection path
- `ql-runtime` now also defines the shared runtime hook ABI skeleton for the async surface, including `qlrt_async_frame_alloc`, `qlrt_async_task_create`, and the first task-result release hook `qlrt_task_result_release`; `ql runtime <file>` renders the deduped hook plan alongside capability requirements, `ql-codegen-llvm` reuses that shared ABI contract for runtime hook declarations in library-style builds, and `BuildEmit::Executable` program mode now goes further by synthesizing `__async_entry` thunks plus a minimal in-module runtime definition set so the currently supported async executable subset links and runs without a separate runtime library; the backend now emits unified `async fn` body wrappers that always receive a frame pointer, can heap-materialize parameterized async frames conservatively for recursively loadable aggregate params including zero-sized fixed arrays and recursive zero-sized aggregates, lowers recursively loadable tuple/array/plain-struct values by value in LLVM IR, precomputes loadable async task-result payload layouts for `Void`, scalar builtins, and recursively loadable aggregate results including zero-sized fixed arrays, recursive zero-sized aggregates, and recursively nested fixed-shape aggregate payloads that themselves continue carrying `Task[T]`, lowers backend-local loadable `await` through `qlrt_task_await` + `qlrt_task_result_release`, lowers nested read-only struct field / tuple index / array index projections through a shared place-projection path, and now also back-propagates concrete expected fixed-array types through direct temp locals and expected aggregate literals so `return []`, direct call arguments like `take([])`, and nested aggregate forms such as `( [], 1 )`, `Wrap { values: [] }`, and `[[]]` lower when a concrete `[T; N]` context is already known while bare `[]` still stays rejected; `ql-driver` now allows that supported async build subset through `staticlib` builds, the minimal library-style async `dylib` subset, and the current `BuildEmit::Executable` `async fn main` subset, with fixed-shape iterable `for await` enabled on both library emit paths and on the supported executable slice, covering fixed arrays plus homogeneous tuples, including current local/projected roots and same-file `const` / `static` roots plus their same-file `use ... as ...` aliases; fixed-array literal-index assignment stays enabled on mutable roots, conservative `Task[T]` dynamic array path tracking in borrowck, sibling-safe dynamic task-handle consume, explicit user-facing regression coverage for generic dynamic task-handle array assignment and sibling-safe consume/spawn across library/program builds, and a new stable-immutable-index-path precise consume/reinit slice so `await tasks[index]; tasks[index] = worker(); await tasks[index]` and `await tasks[slot.value]; tasks[slot.value] = worker(); await tasks[slot.value]` can succeed when the dynamic index expression is rooted in the same immutable stable path; same-file `const` / `static` items and their same-file `use ... as ...` aliases now also fold back into the corresponding literal/projection paths, including `const INDEX: Int = 0`, `static INDEX: Int = 0`, `const SLOT: Slot = Slot { value: 0 }`, `static SLOT: Slot = Slot { value: 0 }`, and `use SLOT as INDEX_ALIAS`, while arbitrary dynamic overlap/reinit, async executables outside the current subset, broader dynamic/generalized `for await`, non-`Task[T]` projections, and broader async payload paths stay conservative; the guard-refined projected-root alias-root variant backed by those same-file `static` / `use ... as ...` paths is now also locked in the driver `BuildEmit::Object` regression surface in addition to the existing CLI `llvm-ir` / `object` / `executable` matrix, and the same static/use-alias path now also has committed executable examples for direct alias reuse, nested aggregate submit, helper-forwarded nested fixed-array submit, alias-sourced composed dynamic submit, double-root alias-root submit, double-root double-source alias submit, double-root double-source row alias submit, double-root double-source row/slot alias submit, triple-root double-source row/slot alias submit, triple-root triple-source row/slot alias submit, triple-root triple-source tail-alias submit, triple-root triple-source tail-alias forwarded submit, triple-root triple-source queued-before-spawn submit, triple-root triple-source queued-root-before-spawn submit, triple-root triple-source queued-root-aliased-before-spawn submit, triple-root triple-source queued-root-chain-before-spawn submit, triple-root triple-source queued-local-aliased-before-spawn submit, triple-root triple-source queued-local-chain-before-spawn submit, triple-root triple-source queued-local-forwarded-before-spawn submit, triple-root triple-source queued-local-inline-forwarded-before-spawn submit, triple-root triple-source bundle-inline-forwarded-before-spawn submit, triple-root triple-source bundle-slot-inline-forwarded-before-spawn submit, triple-root triple-source tail-inline-forwarded-before-spawn submit, triple-root triple-source tail-inline-forwarded-before-await submit, triple-root triple-source bundle-slot-inline-forwarded-before-await submit, triple-root triple-source bundle-inline-forwarded-before-await submit, triple-root triple-source queued-local-inline-forwarded-before-await submit, triple-root triple-source queued-local-forwarded-before-await submit, triple-root triple-source queued-root-inline-forwarded-before-await submit, triple-root triple-source queued-root-forwarded-before-await submit, triple-root triple-source queued-root-aliased-forwarded-before-await submit, triple-root triple-source queued-root-chain-forwarded-before-await submit, triple-root triple-source queued-root-aliased-inline-forwarded-before-await submit, and triple-root triple-source queued-root-chain-inline-forwarded-before-await submit; and `ql-typeck` plus `ql-resolve` now expose a provisional `Task[T]` handle type surface so direct `async fn` calls, spawned tasks, and sync helpers can all flow through the same task-handle model before a later `await`, including helper-returned handles that are locally rebound before `await` or `spawn`
...
- bare path patterns that resolve to same-file `static` items, or to same-file `const` items / import aliases that do not fold to `Bool` or `Int`, are now rejected explicitly instead of silently passing through a deferred gap
...
- Phase 4 foundation currently includes:
  ...
  - current codegen support is intentionally narrow: top-level free functions, `extern "c"` declarations and `extern "c"` function definitions, scalar integer/bool/void types, direct function calls, arithmetic, short-circuit bool expressions, simple branching, and return
...
cargo run -p ql-cli -- runtime fixtures/codegen/pass/async_program_main.ql
```
