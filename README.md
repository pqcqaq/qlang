...
- `ql-codegen-llvm` now also lowers the minimal `Bool` `match` build subset across the current program/library build surface: unguarded `true` / `false` / `_` arms on a `Bool` scrutinee lower to LLVM branches, while guarded matches and broader match patterns stay on the stable rejection path
