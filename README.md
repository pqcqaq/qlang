...
- `ql-runtime` now also defines the first shared runtime hook ABI skeleton for the async surface, `ql runtime <file>` renders the deduped hook plan alongside capability requirements, `ql-codegen-llvm` reuses that shared ABI contract for runtime hook declarations, the backend now emits a parameterless `async fn` wrapper scaffold that calls `qlrt_async_task_create`, and `ql-typeck` now rejects direct `async fn` calls unless they are immediately consumed by `await` or `spawn`
