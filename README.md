...
- `ql-runtime` now also defines the first shared runtime hook ABI skeleton for the async surface, `ql runtime <file>` renders the deduped hook plan alongside capability requirements, `ql-codegen-llvm` reuses that shared ABI contract for runtime hook declarations, and the backend now emits a parameterless `async fn` wrapper scaffold that calls `qlrt_async_task_create` while keeping async builds conservatively closed at the driver boundary
