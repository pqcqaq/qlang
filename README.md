...
- `qlsp` 现已支持 `textDocument/implementation`；same-file trait/type surface 仍可直接跳转，workspace / 本地路径依赖里导入的 `struct / enum / trait` 现在也会优先跳到依赖源码中的 `impl` / `extend` / trait `impl` block。当前仍未做完整 workspace-wide implementation graph，也还没补 open-doc implementation remap。
...
同一套 `qlsp` 现在也支持 `Go to Implementation`。same-file trait/type surface 仍会跳到当前文件里的 `impl` / `extend` block；workspace / 本地路径依赖里导入的 `struct / enum / trait` 也会跳到依赖源码里的实现块。当前还不覆盖完整 workspace-wide implementation graph，也不处理未保存目标源码的 implementation range 重映射。
