...
- `ql-driver` now also consumes that runtime requirement surface as the shared gate for still-unsupported async capabilities, so `llvm-ir` / `obj` emits, non-fixed-array `for await`, broader async program/bootstrap paths, and other closed surfaces keep stable build-time diagnostics while the supported `staticlib` subset, minimal library-style async `dylib` subset, and minimal `BuildEmit::Executable` `async fn main` subset expand conservatively
