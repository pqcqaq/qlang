...
- known-invalid struct-literal roots now also report stable type-check diagnostics: builtin roots, enum roots without a struct-style variant constructor, tuple/unit variants, and same-file import aliases that canonicalize to those unsupported roots no longer silently degrade
