...
- 仓库内已开始随源码分发普通 Qlang package 形态的最小 `stdlib`，当前包含 `std.core`、`std.option`、`std.result` 与 `std.test`，覆盖基础整数/布尔 helper、concrete Option/Result carrier 与 smoke-test 断言；它仍需通过本地 `[dependencies]` 显式依赖，不是自动 prelude。
