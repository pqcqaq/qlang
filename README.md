...
- same-file enum variant lookup now also produces explicit unknown-variant diagnostics across struct literals plus tuple/struct/path patterns, including same-file import aliases canonicalized back to local enums; deeper module-path / cross-file variant semantics stay deliberately conservative
