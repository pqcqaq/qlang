...
- `ql-borrowck` now also treats direct-local task handles as consumed by `await`, `spawn`, and statically-known helper `Task[T]` argument passing, so reusing the same `Task[T]` local after submission, helper transfer, or awaiting surfaces stable moved / maybe-moved diagnostics while reassignment still reinitializes the slot
