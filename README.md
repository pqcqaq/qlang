...
- same-file `use ... as ...` aliases that resolve to local function items now also lower through that same direct-call backend path, including the current named-call-argument subset and the current `async fn main` executable subset, with alias-root direct `await` / `spawn` on local async callees plus aggregate-carried, fixed-array-carried, tuple-carried, helper-forwarded, helper-returned-task, helper-returned-task + helper-forwarded submit, direct helper-returned fixed-array task `for await`, projected fixed-array task-root `for await`, and `Task[Void]` wildcard `for await` executable paths now covered by committed examples
