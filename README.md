...
- `ql-codegen-llvm` now also lowers the minimal literal `match` build subset across the current program/library build surface: unguarded `true` / `false` plus `_` or single-name binding catch-all arms on a `Bool` scrutinee lower to LLVM branches, and unguarded integer-literal plus `_` or single-name binding catch-all arms on an `Int` scrutinee lower to a stable compare-chain, while guarded matches and broader match patterns stay on the stable rejection path
