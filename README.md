...
- healthy workspace/local dependency 下，source-backed dependency `method / field` 的 `hover`、`definition`、`references`、`documentHighlight` 现在也会在成员只存在于未保存源码、磁盘 `.qi` 仍旧过期时继续优先读取 open docs。
