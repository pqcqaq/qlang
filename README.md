...
- LLVM 可执行主路径现在已支持本地与 direct local dependency 的 `impl` / `extend` / 唯一 trait `impl for` receiver method 直接调用（如 `value.read()`）；当前只覆盖 direct call position，不包含 method value。
