...
- same-file `use ... as ...` aliases that resolve to local function items now also lower through that same direct-call backend path, including the current named-call-argument subset and the current `async fn main` executable subset, with alias-root direct `await` / `spawn` on local async callees plus aggregate-carried, fixed-array-carried, and tuple-carried task-handle submit now covered by committed executable examples
