...
- `ql-driver` now also consumes that runtime requirement surface conservatively for build rejection, so `async fn`, `await`, and `spawn` unsupported diagnostics stay stable at the driver boundary even before real async lowering exists
