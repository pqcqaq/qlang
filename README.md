...
- same-file `use ... as ...` aliases that resolve to local function items now also lower through that same direct-call backend path, including the current named-call-argument subset and the current `async fn main` executable subset, with alias-root direct `await` / `spawn` on local async callees plus aggregate-carried, fixed-array-carried, tuple-carried, helper-forwarded, helper-returned-task, helper-returned-task + helper-forwarded submit, direct helper-returned fixed-array task `for await`, direct helper-returned homogeneous task-tuple `for await`, projected fixed-array / task-tuple roots `for await`, awaited aggregate / awaited nested aggregate task-array `for await`, awaited aggregate / awaited nested aggregate task-tuple `for await`, import-aliased awaited aggregate task-array / task-tuple `for await`, inline-awaited fixed-array / task-tuple roots `for await`, awaited tuple-projected / array-projected fixed-array task roots `for await`, awaited tuple-projected / array-projected task-tuple roots `for await`, and `Task[Void]` wildcard `for await` executable paths now covered by committed examples
