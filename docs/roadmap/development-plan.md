...
- 继续抽 `ql-cli` project pipeline；`build/run/test/check` 的入口 request-context 与 `project emit-interface/graph/dependencies/dependents/add/status` 的 workspace member lookup 已共享，`emit-interface` 普通/check/output selector 已补齐 ambiguous/unresolved 回归；下一步收口真实 workspace smoke。
- 收紧 `ql test`；`--package` 已复用共享 workspace member lookup，并覆盖 missing/invalid/ambiguous/broken member 回归；测试 target 的 dependency/package-under-test/local-generic source override 拼接已收口；direct dependency generic consumer 已覆盖 named/expression args、carrier 和 result-context smoke。下一步继续收口 dependency-aware backend。
