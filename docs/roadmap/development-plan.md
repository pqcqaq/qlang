...
- `stdlib/packages/core` 能被 `ql check/build/test` 验证，并提供第一批稳定基础函数。（已落地整数/布尔 helper，含符号、比较、单边/双边 clamp、绝对差、零值/正负/非正/非负、奇偶、闭/开区间、整除、容差检查和基础布尔组合）
- `stdlib/packages/test` 能提供 smoke-test 友好的断言辅助。（已落地 true/false、bool equality/implies、int equality/order、zero/nonzero、正负/非正/非负、区间、奇偶、整除和容差断言）
