...
- `ql-codegen-llvm` 现已直接复用这份共享 hook ABI contract 渲染 LLVM declarations，`ql-driver` 也会把 dedupe 后的 `RuntimeHookSignature` 传入后端；backend 现在统一把 `async fn` body 降成 `ptr frame` 入口，并且能为带参数的 `async fn` 生成最小 heap-frame wrapper scaffold，同时为 async 返回值预计算 `AsyncTaskResultLayout`，先只接受 `Void` 或已支持的 scalar builtin 结果，冻结“await 拿到 opaque payload ptr 后再复制并释放”的内部前提，并已在 backend 内打通首个 scalar/void `await` lowering；`ql-driver` 也已开始允许这条受控子集通过 `staticlib` 构建，而 library-mode 下的 `spawn` / `for await` unsupported 合同仍保持稳定回归与噪声抑制，当前仍不开放更广义的任务结果协议、聚合 payload 布局或 `dylib`/program async build 承诺
