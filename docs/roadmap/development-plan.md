...
- `ql test` 的 package-under-test/direct-dependency/local-generic source override 已覆盖 package path 和直接 project test file，直接文件入口也覆盖 dependency generic public function 与 wrapper/helper bridge；下一步继续收紧更宽 dependency-aware backend 组合路径，并纳入共享 project pipeline。
