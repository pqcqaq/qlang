...
- 收紧 `ql test`；`--package` 已复用共享 workspace member lookup，并覆盖 missing/invalid/ambiguous/broken member 回归。下一步继续把测试专用 bridge/source override 抽进共享 project pipeline，并扩大 dependency consumer smoke 覆盖。
