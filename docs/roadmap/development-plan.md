...
- backend 已支持最小 async body wrapper、frame scaffold、loadable `await`（当前已覆盖 scalar / `Task[T]` / 递归可加载 aggregate payload，以及 tuple / fixed-array / non-generic struct payload 内继续携带 `Task[T]` 的受控子集）、task-handle-aware `spawn`
