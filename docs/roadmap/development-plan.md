...
- 当前查询覆盖 item / local / regular param / generic / receiver `self` / named type root / pattern path root / struct literal root
- import alias 和 builtin type 现在也能提供 hover 级语义信息，但不会伪造 definition span
...
- LSP bridge 已经把协议层与语义层解耦：
  ...
  - `ql-analysis` hover / definition 到 LSP 响应
...
3. 在当前 `qlsp` / 查询层之上继续补 references / rename / completion 所需的数据面
...
- 在 clang-style compiler 可用时，这层 harness 还会真实执行“Qlang dylib -> `ql ffi header` -> C loader harness -> 运行时装载并调用导出符号”的端到端回归
