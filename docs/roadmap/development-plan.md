...
- generic `Option[T]` / `Result[T, E]` carrier 已经进入可执行路径；`std.option` / `std.result` 首批 generic helper 已落地并进入 downstream smoke，`std.option.none_option` 与 `std.result.ok/err` 也已通过显式返回/初始化上下文推断解锁。下一步优先补完整 monomorphization、公用集合抽象和真实 downstream smoke，而不是继续堆 concrete/fixed-arity helper。可变参数语法仍需单独设计和 lowering，不直接拿来绕过当前 stdlib API 问题。
...
- `stdlib` 当前已发布的 concrete API 继续使用稳定语言面并保持可执行；generic `Option[T]` / `Result[T, E]` carriers 已经进入可执行路径，`std.option` / `std.result` 首批 generic helper 已进入真实 smoke，接下来用 failing tests 驱动 collection helper / prelude / monomorphization / codegen，而不是继续把 carrier 设计拖成远期后置项。
- 数组/集合 helper 是替代 `sum3/4/5`、`all3/4/5` 这类固定参数复制的近期方向；可变参数语法需要独立设计 ABI、`.qi` 表示、typeck、LSP 和 lowering，先进入设计 gate，不直接承诺为下一轮实现。

## 下一轮（已排定）

- stdlib/generics：按 [Stdlib Generics and Collections Roadmap](/plans/2026-05-02-stdlib-generics-and-collections-roadmap) 推进；`std.array` 的固定数组过渡面已先落地，generic type / carrier 执行面、uninstantiated generic function declaration 面、primitive literal / 基础表达式 / tuple 与 fixed-array 字面量 / typed value / generic carrier / 单字段 generic enum variant constructor / 显式结果上下文 inferred direct-call multi-instantiation generic public function bridge，以及 `std.option` / `std.result` 首批 generic helper smoke 已被回归锁住。`std.option.none_option` 已进入显式 `Option[T]` 上下文路径；下一刀继续推进完整 monomorphization 表、bridge 专用符号命名、LLVM specialization 和 collection helper，而不是再扩 concrete helper。
