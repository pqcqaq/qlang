...
- `ql-cli` 主入口已收口到命令分发；内部模块不再依赖 crate root re-export 中转。command、pipeline、project target、project target build prep、single-source input/reporting、build interface emission、source rewrite、dependency bridge、build/run/test reporting 和 project build execution 已拆成独立模块。`ql-driver` build pipeline 已拆出 codegen preparation、output resolution、emit dispatch、toolchain emission workspace、直接 IR/object-backed emission 模板，以及 workspace 级 failure/cleanup/promotion helper；build/run/test 复用的 build failure JSON 已统一 target identity 字段，下一步继续拆 build 文本 reporting 出口，所有移动必须保持行为不变并跑真实 workspace smoke。
