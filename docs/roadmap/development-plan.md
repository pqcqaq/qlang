...
- `project init --stdlib` starter 已覆盖 package/workspace 的 `check/build/run/test`、`emit-interface`、`emit-interface --check`、关键 JSON 输出和 workspace `graph/status --json --package`。下一步继续把它作为 downstream 可用性入口维护。
