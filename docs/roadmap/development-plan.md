...
- fixed-array literal index precision 现也会回收最小 immutable const alias / same-file const item：当 `let index = 0`、`let slot = Slot { value: 0 }`、`const INDEX: Int = 0` 或 `const SLOT: Slot = Slot { value: 0 }` 这类来源能在 borrowck / codegen 内保守折叠回 literal index 时，`await tasks[index]; await tasks[0]`、`await tasks[slot.value]; await tasks[0]`、`await tasks[INDEX]; await tasks[0]` 与 `await tasks[SLOT.value]; await tasks[0]` 都会走同一条 literal path overlap，而不是继续停留在 generic dynamic maybe-overlap
