...
- 继续收口 `ql-cli` project pipeline；共享路径归一化、相对路径、JSON 字符串转义、诊断渲染/打印、包名校验、project manifest error helper 和 build/run/test profile 参数逻辑已从 `main.rs` 抽出，build/run 的 target selector/listing 入口已直接依赖 `project_targets`，后续继续拆 request-context/reporting。
