...
- `stdlib/packages/core` 能被 `ql check/build/test` 验证，并提供第一批稳定基础函数。（已落地整数/布尔 helper，含符号、比较、三值 extrema / median、单边/双边/无序边界 clamp、边界归一化、绝对差、range span、range/bounds 距离、零值/正负/非正/非负、奇偶、闭/开区间、无序边界区间、区间外/无序边界外、升/降序判断、整除、容差内/外检查和基础布尔组合）
- `stdlib/packages/test` 能提供 smoke-test 友好的断言辅助，并通过 package-aware smoke test 直接导入自身 public helpers。（已落地 true/false、bool equality/ne/logic/implies、int equality/order、zero/nonzero、2-6 路 status 组合、正负/非正/非负、区间、无序边界区间、clamp / range-distance、升/降序、奇偶、整除和容差内/外断言）
