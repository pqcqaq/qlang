...
- 当前查询覆盖 item / local / regular param / generic / receiver `self` / struct field member / unique method member / named type root / pattern path root / struct literal root
...
- LSP bridge 已经把协议层与语义层解耦：
  ...
  - `ql-analysis` hover / definition / references 到 LSP 响应
...
3. 在当前 `qlsp` / 查询层之上继续补 rename / completion 所需的数据面，并把当前已落地的 field / unique-method member 查询继续扩到 variant / module-path / ambiguous method 语义
