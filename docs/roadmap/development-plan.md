...
- 继续补 `workspace/symbol` 的 package/workspace 搜索保真度；当前包可正常加载时，如果 sibling workspace member 因源码 diagnostics 无法整包分析，后端也已改成回退到 manifest-source 粒度，继续保留该 member 中仍可单独分析的模块符号。
