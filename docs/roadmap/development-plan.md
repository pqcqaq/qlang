...
- 已补 build 输出路径级跨进程锁，覆盖 artifact、链接输入、build-side header、`.qi` 输出写入、`fmt --write` 源文件写入，以及 `run/test` 执行期 executable；真实 CLI smoke 已覆盖并发 `build`、`run`、`test`、`fmt --write` 的共享依赖产物、运行中 executable 和同源文件写入。后续并发问题继续按“先保护真实产物，再补 CLI smoke”的顺序处理；project metadata manifest 写路径也按同一规则收口。
