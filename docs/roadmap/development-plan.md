...
- package-aware dependency tooling 的 fallback 也继续收紧：当某个包里同时存在健康和损坏的 dependency `.qi` 时，后端现在会保留其余健康依赖的 import hover / completion / definition 基础能力，而不是因为一个坏依赖把整包 dependency-backed 入口直接退成 `None`。
