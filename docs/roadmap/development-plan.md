...
- `ql-driver` 已开始保守消费 runtime requirement truth surface，当前会把 `async-function-bodies`、`task-spawn`、`task-await` 映射成稳定的 build diagnostics，并与 backend 同类 unsupported 诊断去重；`async-iteration` 仍继续封闭
