...
- `ql-cli` 主入口已收口到命令分发；内部模块不再依赖 crate root re-export 中转。command、pipeline、project target、project target build prep、single-source input/reporting、build interface emission、source rewrite、dependency bridge、build/run/test reporting 和 project build execution 已拆成独立模块。`ql-driver` build pipeline 已拆出 codegen preparation、output resolution、emit dispatch 和 toolchain emission workspace；下一步继续拆 compile/link/archive 阶段，所有移动必须保持行为不变并跑真实 workspace smoke。
