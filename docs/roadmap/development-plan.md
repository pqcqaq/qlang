...
- 已补 build 输出路径级跨进程锁，覆盖 artifact、链接输入、build-side header、`.qi` 输出写入、`fmt --write` 源文件写入，以及 `run/test` 执行期 executable；driver/CLI 的 artifact/header/source/manifest/lockfile/interface 关键写入已统一走同目录临时文件替换。真实 CLI smoke 已覆盖并发 `build`、`run`、`test`、`fmt --write`、manifest 编辑和 lockfile 读写。后续并发问题继续按“先保护真实产物，再补 CLI smoke”的顺序处理。
