...
- projected task-handle operand 已支持 tuple index / fixed-array literal index / struct field 只读投影路径；borrowck 现也会对 dynamic fixed-array index task-handle consume 记录保守的 dynamic-array path，因此 `await pending.tasks[index]` / `spawn pending.tasks[index]` 不再直接污染 sibling projection，但具体元素级后续读取仍保持 maybe-moved；mutable root 下的 projection write/reinit 仍只开放 tuple index / struct-field / fixed-array literal index
- 非 `Task[...]` 元素的 dynamic array assignment 已开放并在 driver/CLI 两层锁定；`Task[...]` 动态数组索引写入现已打开首条保守 write-only 子集，但仍不把 dynamic index 误当成 precise reinit
...
- 更广义的 place-sensitive task-handle lifecycle（当前 dynamic array 只开放“sibling-safe consume + write-only reinit”这条保守子集）
