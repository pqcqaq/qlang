...
- `ql-driver` / `ql-codegen-llvm` 也已把 `cleanup` 与 projected `await` 的混合失败合同锁住：同一文件里同时出现 `defer` 与 `await pair[0]` 时，cleanup lowering 失败与投影 task operand unsupported 都会稳定只出现一次，不再互相吞掉或级联污染其他 diagnostics
- `ql-driver` / `ql-codegen-llvm` 也已把 `cleanup` 与 projected `spawn` 的混合失败合同锁住：同一文件里同时出现 `defer` 与 `spawn pair[0]` 时，cleanup lowering 失败与投影 task operand unsupported 都会稳定只出现一次，不再互相吞掉或级联污染其他 diagnostics
...
- `ql-borrowck` 现已把 direct-local task handle 的 `await` / `spawn`、静态可判定的 helper `Task[T]` 参数传递和 direct-local `return task` 接进当前 ownership facts：这些路径会消费本地 `Task[T]`，后续复用会触发稳定的 moved / maybe-moved 诊断，而重赋值仍能重新初始化该 local；当前还额外锁住了零尺寸 `Task[Wrap]` 的 conditional cleanup reinit/consume、helper-consume/reinit 与 reverse-branch helper-consume/reinit 回归，但更广义的 helper 边界与 place-sensitive handle lifecycle 仍保持未开放
