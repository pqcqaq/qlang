...
- `ql-codegen-llvm` 现已直接复用这份共享 hook ABI contract 渲染 LLVM declarations，`ql-driver` 也会把 dedupe 后的 `RuntimeHookSignature` 传入后端；backend 现在统一把 `async fn` body 降成 `ptr frame` 入口，并且能为带参数的 `async fn` 生成最小 heap-frame wrapper scaffold，同时为 async 返回值预计算 `AsyncTaskResultLayout`，当前已支持 `Void`、scalar builtin 与 pure-scalar tuple 结果，冻结“await 拿到 opaque payload ptr 后再按 LLVM value `load` 并释放”的内部前提，并已在 backend 内打通 scalar/tuple/void `await` lowering；此外 backend 还支持把 `spawn` 降成 `qlrt_executor_spawn(ptr null, task)` 并返回可继续 `await` 的 task handle，`ql-resolve` / `ql-typeck` 现也已开放保留的 `Task[T]` 类型面，使 direct async call 可以在显式 task-handle 期望下流经 helper 的参数/返回值边界；`ql-driver` 现允许这条受控 async library 子集通过 `staticlib` 构建，当前仍不开放 struct/array payload、`for await` lowering、`dylib` async 或 program async build 承诺
- `ql-typeck` 现已显式拒绝 direct `async fn` calls，除非该 call 立即被 `await` 或 `spawn` 消费；这保证前端语义不会在 task/result ABI 仍未冻结时假定 async 调用拥有普通同步返回值
