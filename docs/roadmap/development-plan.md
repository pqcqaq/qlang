...
- `ql-typeck` / `ql-borrowck` 现也开始锁定无 cleanup 的 branch-join helper consume/reinit 边界：`if flag { forward(task) } else { task = fresh_worker() }` 这类 zero-sized `Task[Wrap]` flow 现在会在 typeck 层稳定放行、在 borrowck 层稳定报出 branch-join maybe-moved，继续把 task-handle 的控制流边界收紧在已有语义里
