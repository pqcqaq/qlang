...
- stdlib downstream gate 已覆盖复制 repo stdlib 后的接口同步、全量 `ql check --json`、`ql test`、`ql test --json`，以及仓库 stdlib `emit-interface --check` / `status --json` / `graph --json` / starter dependency JSON；新增 public API 必须进入 package-local smoke 或 starter/downstream smoke。
