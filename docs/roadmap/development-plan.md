...
- `ql-typeck` 现已把 direct `async fn` calls 统一建模成 `Task[T]` 句柄值：这些句柄可被局部绑定、helper 参数/返回值传递并在后续 `await`，而流入非 `Task[T]` 上下文时会退化成普通类型不匹配；这保证前端语义不会在 task/result ABI 仍未冻结时把 async 调用伪装成同步返回值
