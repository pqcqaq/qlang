...
- task-handle target 选择现也会沿 immutable alias 继续 canonicalize 到同一条 source root，即使最终 source local/source path 本身仍可重初始化：`let alias = task; await alias; await task` 现在会稳定报 `task` definite use-after-move，而 `var task = worker(); let alias = task; await alias; task = worker(); await alias` 也会在 source-local reinit 后恢复通过；同一条 root canonicalization 也已覆盖 dynamic/projected root 形态，例如 `let alias = tasks; await alias[index]; await tasks[index]` 与 `let alias = pending.tasks; await alias[index]; pending.tasks[index] = worker(); await alias[index]`
