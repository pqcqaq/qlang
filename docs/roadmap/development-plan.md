...
- 建立 LSP workspace index；真实 stdlib package 请求 smoke 已覆盖 current API 的 diagnostics、completion、hover、definition、typeDefinition、references、documentHighlight、signatureHelp、inlayHint、codeLens、semantic tokens、rename 和 `workspace/symbol`，下一步继续把 diagnostics 和其它 rich editor hints 收敛到同一份分析缓存。
