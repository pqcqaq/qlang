...
- `ql test` 的 package-under-test/direct-dependency/local-generic source override 已覆盖 package path 和直接 project test file，直接文件入口也覆盖 dependency generic public function、wrapper/helper bridge、`--list --json` listing 和 `--package` selector 合同；下一步继续收紧更宽 dependency-aware backend 组合路径，并纳入共享 project pipeline。
...
- JSON 输出要继续覆盖成功和 preflight/render/selection failure；`ql test --json` 已覆盖直接 project test file 的 dependency generic bridge 成功报告和 package selector mismatch，`ql test --list --json` 已覆盖 direct project test file listing。
