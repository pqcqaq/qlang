...
- LSP 已开始收口跨请求 workspace request context：`hover`、`definition`、`declaration`、`typeDefinition`、`codeLens`、`implementation`、`references`、`documentHighlight`、`completion`、`signatureHelp`、`inlayHint`、`semanticTokens/full`、`semanticTokens/range` 已共享 package/open-doc/analysis 准备；下一步继续把 rename/documentLink/diagnostics 等请求纳入共享 context，并推进真正 workspace index/cache 生命周期，减少重复扫描。diagnostics 已覆盖真实 stdlib app/source/open-buffer 场景，workspace/symbol 已覆盖 real stdlib open/change/close 生命周期，formatting、folding/selection、signatureHelp/inlayHint、documentLink、codeLens 已进入真实 stdlib request-family gate。
