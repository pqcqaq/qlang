...
- `ql-cli` 主入口已收口到命令分发；内部模块不再依赖 crate root re-export 中转。command、pipeline、project target、interface、source rewrite、dependency bridge、build/run/test reporting 已拆成独立模块。下一步只继续拆 build backend glue，所有移动必须保持行为不变并跑真实 workspace smoke。
