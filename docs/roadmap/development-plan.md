...
- generic `Option[T]` / `Result[T, E]` carrier 已经进入可执行路径；`std.option` / `std.result` 首批 generic helper 已落地并进入 downstream smoke，`std.result.ok/err` 也已通过显式返回/初始化上下文推断解锁。下一步优先补完整 monomorphization、多实例、公用集合抽象和真实 downstream smoke，而不是继续堆 concrete/fixed-arity helper。可变参数语法仍需单独设计和 lowering，不直接拿来绕过当前 stdlib API 问题。
- generic public function declaration 现在可以存在于 library package 中并进入 `.qi`，只要没有被实例化调用就不会毒化 library build；direct dependency / package-under-test bridge 已经能对单一具体实例的 public generic free function 做本地特化，前提是所有 generic 参数都能从 `Int` / `Bool` / `String` 字面量、top-level value、显式参数、显式 typed local、由这些值初始化的简单局部名字、generic carrier value、单字段 generic enum variant constructor，或显式返回类型 / typed initializer 结果上下文携带的具体类型表达式推断出来；`T = Box[Int]`、从 `Box[T]` 参数反推 `T = Int`、`Option.Some(42) -> Option[Int]` 和 `Result[Int, Int]` 上下文约束 `ok/err` 都已进入 build/test 回归。完整单态化仍是下一阶段 backend/bridge 重点，超出该切片继续明确返回 `dependency-function-unsupported-generic`。
...
- `stdlib/packages/option` 能被 `ql check/build/test` 验证，并提供当前 dependency bridge 可执行的 option surface。（已落地 `Option[T]` generic carrier、generic `some` / `is_some` / `is_none` / `unwrap_or` / `or_option`，以及 `IntOption` / `BoolOption` compatibility helper；完整 monomorphization / prelude 集成继续后置）
