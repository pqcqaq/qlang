...
- 建立 LSP workspace index；真实 stdlib LSP smoke 已按 request family 拆分，覆盖 completion/resolve、hover、definition/declaration/typeDefinition、references/documentHighlight、implementation、documentSymbol、signatureHelp/inlayHint、folding/selection、formatting、codeAction/resolve、call/type hierarchy、semanticTokens full/range、rename；`diagnostics`、`codeLens`、`workspace/symbol` 由独立 smoke 覆盖，其中 `diagnostics`、`codeLens`、`workspace/symbol` 和 rich app 请求已补 real stdlib open-doc 优先级回归。`workspace/symbol` 的 analyzed-package 和 manifest/source fallback 路径已共享，缺失接口等包级预检失败仍会索引源码符号；`references` 和 `codeLens` 已补真实 workspace consumer 回归，包括 open-doc consumer。下一步继续把 diagnostics、formatting、folding/selection、signatureHelp/inlayHint、codeLens 等请求收敛到统一 workspace analysis/cache 边界。
