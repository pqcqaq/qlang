...
- generic public function declaration 现在可以存在于 library package 中并进入 `.qi`，只要没有被实例化调用就不会毒化 library build；direct dependency / package-under-test bridge 已经能对同一 public generic free function 的多个直接调用生成多个本地特化，前提是每个调用的所有 generic 参数都能从 `Int` / `Bool` / `String` 字面量、top-level value、显式参数、显式 typed local、由这些值初始化的简单局部名字、generic carrier value、单字段 generic enum variant constructor，或显式返回类型 / typed initializer 结果上下文携带的具体类型表达式推断出来；位置实参与命名实参都会按形参声明顺序参与同一套 direct-call 推断；`identity[Int]` + `identity[Bool]` 同源多实例、`T = Box[Int]`、从 `Box[T]` 参数反推 `T = Int`、`Option.Some(42) -> Option[Int]`、`std.option.none_option() -> Option[Int]`、`Result[Int, Int]` 上下文约束 `ok/err` 和命名实参重排后的 direct call 都已进入 build/test 回归。完整单态化仍是下一阶段 backend/bridge 重点，方法、escaping function value 和更复杂表达式推断继续明确返回 `dependency-function-unsupported-generic`。
