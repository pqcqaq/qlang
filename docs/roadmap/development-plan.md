...
- 已创建的 workspace 能继续用 `ql project add/remove/target add/add-dependency/remove-dependency/dependencies/dependents` 维护成员、targets 和本地依赖；新增 scaffold、纳管现有 package、补新 bin target、从 workspace 根直接给指定 member 补/减依赖、按包名补 workspace member 依赖、按路径补 workspace 外本地依赖、按包名批量清理全部 dependents、查询正反向依赖，以及安全或级联移除 member 都不必手改 manifest。
