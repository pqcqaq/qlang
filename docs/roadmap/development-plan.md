...
- stdlib/generics：按 [Stdlib Generics and Collections Roadmap](/plans/2026-05-02-stdlib-generics-and-collections-roadmap) 推进；`std.array` 的固定数组过渡面已先落地，generic type / carrier 执行面、uninstantiated generic function declaration 面，以及 primitive literal/param/local inferred single-instantiation generic public function bridge 已被回归锁住。下一刀继续把实例推断扩到显式 carrier helper 和更完整调用点实例化表，补 bridge 专用符号命名和 LLVM specialization，而不是再扩 concrete helper。
