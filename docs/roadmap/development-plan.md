...
- 已创建的 workspace 能继续用 `ql project add/remove/add-dependency/remove-dependency/dependents` 维护成员和本地依赖；新增、补依赖、减依赖、按包名批量清理全部 dependents、查询反向依赖，以及安全或级联移除 member 都不必手改 manifest。
