...
- 继续补 `workspace/symbol` 的 package/workspace 搜索保真度；当前包可正常加载时，如果 sibling workspace member 因源码 diagnostics 无法整包分析，后端已改成回退到 manifest-source 粒度，继续保留该 member 中仍可单独分析的模块符号；当前包自身落入 broken-source fallback 时，这条 sibling member 搜索现在也已和 dependency `.qi` 加载解耦，不再因为依赖接口失败把 workspace member 一起丢掉。
