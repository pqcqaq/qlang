...
- `ql test` 已覆盖 package-under-test、direct dependency 和 local generic source override 的组合路径；test discovery 的 target/filter selection、target construction/output path、listing 与 no-tests/no-match reporting 已独立拆分并保留消息合同测试。后续重点是继续与 build/run 的 selector、profile、JSON failure 合同保持一致。
