...
- stdlib downstream gate 已覆盖复制 repo stdlib 后的接口同步、全量 `ql check --json`、`ql test` 和 `ql test --json`；新增 public API 必须进入 package-local smoke 或 starter/downstream smoke。
