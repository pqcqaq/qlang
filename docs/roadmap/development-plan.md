...
- 继续把 generic `Option[T]` / `Result[T, E]`、canonical `std.array` helpers（含数组长度泛型值）、single-file/project generic specialization、generic `std.test` assertions 和 downstream smoke 收紧到稳定可用；固定长度数组 helper 只保留薄兼容层。
