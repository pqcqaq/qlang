...
- 继续补 `workspace/symbol` 的 package/workspace 搜索保真度；当前包可正常加载时，这条路径现在不仅会保留 sibling workspace member 自身模块，也会继续纳入 sibling member 依赖 `.qi` 的 public symbols；如果 sibling workspace member 因源码 diagnostics 无法整包分析，后端已改成回退到 manifest-source 粒度，继续保留该 member 中仍可单独分析的模块符号；当前包自身落入 broken-source fallback 时，这条 sibling member 搜索现在也已和 dependency `.qi` 加载解耦，不再因为依赖接口失败把 workspace member 一起丢掉；而在当前包源码本身可分析但 dependency `.qi` 缺失、损坏或不可读时，这条 `workspace/symbol` 路径现在也会保留当前包模块与 sibling workspace members；现在 sibling member 自己命中同类 `.qi` 失败时，也会走同一条 manifest-source fallback，而不是整包跳过。
