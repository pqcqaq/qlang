...
- `ql-codegen-llvm` 也已把 `cleanup` 与 projected `await` 的混合失败合同锁住：同一文件里同时出现 `defer` 与 `await pair[0]` 时，cleanup lowering 失败与投影 task operand unsupported 都会稳定只出现一次，不再互相吞掉或级联污染其他 diagnostics
