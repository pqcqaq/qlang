...
- assignment target 目前仍故意保守：只对 bare mutable binding 做可写性约束，member/index place assignment 仍等更完整的 place-sensitive 语义后再收紧
...
- `ql-typeck` 现在还会把同文件单段 local import alias 规范化回本地 item，用于 struct literal 字段检查以及 struct / enum pattern root 检查
