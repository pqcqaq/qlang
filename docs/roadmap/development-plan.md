...
- LSP：继续把 `textDocument/implementation` 从 trait/type surface、workspace root/source-backed type definition surface、workspace root/source-backed concrete method call、trait method definition、已唯一解析的 concrete method call，扩到剩余的多文件 / broken-source member 导航面。
