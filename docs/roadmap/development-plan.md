...
- import alias 现在会作为 source-backed binding 进入统一 query surface，因此 hover / definition / 同文件 references / 同文件 rename 都共用同一份 semantic identity；builtin type 仍只提供 hover 级语义信息
- shorthand field token 当前仍故意保守，让 `Point { x }` 里的 `x` 继续落在 local/binding 语义上；但当从 source-backed field symbol 发起 rename 时，query 层现在会把这些 shorthand site 自动扩写成显式标签
- same-file rename 当前只开放 function / const / static / struct / enum / variant / trait / type alias / local / parameter / generic / import / field；method / receiver / builtin type / 从 shorthand field token 本身发起的 field rename 与 cross-file rename 仍然故意保守
...
- LSP bridge 已经把协议层与语义层解耦：
  ...
  - `ql-analysis` hover / definition / references / rename 到 LSP 响应
...
3. 在当前 `qlsp` / 查询层之上继续把 same-file rename 扩到更完整语义面，并先解决 field/method/module-path/shorthand 等剩余精度边界，再为 completion / cross-file rename 继续补 project indexing 等数据面
4. 在 import / module / prelude 规则稳定后补 unresolved global / type diagnostics
