...
- `ql test` 的 package-under-test/direct-dependency/local-generic source override 已覆盖 package path 和直接 project test file，直接文件入口也覆盖 dependency generic public function、wrapper/helper bridge、UI snapshot、`--list --json` listing、`--package` / `--target` / `--filter` selector 合同，以及组合 selector 下的 JSON 执行成功；下一步继续收紧更宽 dependency-aware backend 组合路径，并纳入共享 project pipeline。
...
- JSON 输出要继续覆盖成功和 preflight/render/selection failure；`ql test --json` 已覆盖直接 project test file 的 dependency generic bridge 成功报告、UI success/failure、组合 selector 成功、package selector mismatch、target selector miss 和 filter miss，`ql test --list --json` 已覆盖 direct project test file listing。
