...
- 继续抽 `ql-cli` project pipeline；`build/run` command-path resolution 已共享，下一步统一 `check/test/project build` 的 request context、target selection、dependency/interface prep 和 reporting。
