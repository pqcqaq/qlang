...
- 继续扩 `ql project graph`、`ql project emit-interface`、`ql build --emit-interface`、`ql check --sync-interfaces` 的一致性，下一步优先补 `ql check` / `ql check --sync-interfaces` 在 semantic-invalid manifest 与 dependency repair 提示上的剩余对齐，继续压平 direct package、workspace、build、sync 四条路径的失败面。
