...
- 继续收口 `ql-cli` project pipeline；共享路径归一化、相对路径、JSON 字符串转义、诊断渲染/打印、包名校验、project manifest error helper、文件扫描、版本输出和 build/run/test profile 参数逻辑已从 `main.rs` 抽出，`ql test` CLI 选项结构已移入 `test_command`，build/run 的 target selector/listing 与 workspace target loading 已直接归入 `project_targets`，project graph/status/dependencies 查询模块已改为直接依赖 `project_targets` / `project_workspace`，project member、lock、workspace lookup 入口已直接依赖目标模块，interface artifact emission 和 quiet dependency interface prep 已抽到 `project_interfaces`，`tests.rs` 不再通过 `super` 访问主模块。后续继续拆 command entrypoint、request-context/reporting，并把 build/check/run/test 根入口从 `main.rs` 下沉。
