...
- LSP：继续把 `textDocument/implementation` 从 trait/type surface、workspace root/source-backed type definition surface、workspace root/source-backed concrete method call、trait method definition、已唯一解析的 concrete method call，扩到当前 buffer broken implementation 导航面；更宽的 implementation index 继续后置。
