...
- 继续扩 `ql project graph`、`ql project emit-interface`、`ql build --emit-interface`、`ql check --sync-interfaces` 的一致性，下一步优先统一 package source failure 这条链路在“局部错误块 vs 最终汇总”之间的首个失败路径策略，特别是 `first failing source file` 何时只在多失败场景保留，避免剩余 stderr 面继续分叉。
