...
- `ql-codegen-llvm` 现已直接复用这份共享 hook ABI contract 渲染 LLVM declarations，`ql-driver` 也会把 dedupe 后的 `RuntimeHookSignature` 传入后端；backend 现在统一把 `async fn` body 降成 `ptr frame` 入口，并且能为带参数的 `async fn` 生成最小 heap-frame wrapper scaffold，同时为 async 返回值预计算 `AsyncTaskResultLayout`，当前已支持 `Void`、scalar builtin 与 pure-scalar tuple 结果，冻结“await 拿到 opaque payload ptr 后再按 LLVM value `load` 并释放”的内部前提，并已在 backend 内打通 scalar/tuple/void `await` lowering；此外 backend 还支持 statement-position fire-and-forget `spawn`，当前以 `qlrt_executor_spawn(ptr null, task)` 作为最小默认执行器合同并丢弃返回值；`ql-driver` 也已开始允许这条受控子集通过 `staticlib` 构建，而 library-mode 下的 `spawn` result value / `for await` unsupported 合同仍保持稳定回归与噪声抑制，当前仍不开放 struct/array payload、`dylib` async 或 program async build 承诺
