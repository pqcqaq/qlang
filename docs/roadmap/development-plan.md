...
- package/workspace import，以及当前文件里命中的 dependency value / enum variant / struct field / method member，其 `hover` 现在也已开始优先取 workspace 内唯一可定位的源码定义内容，而 `definition` / `declaration` 仍会优先跳到对应源码定义，找不到唯一源码目标时再回退 `.qi`；如果命中的是 struct / enum / trait / type alias 这类 workspace/package import root，`typeDefinition` 现在也会优先跳到同一份 workspace 源码定义，而 `references` 现在也会在当前文件 import occurrences 之外，额外带上 workspace 其他源码文件里的同路径 import occurrences，以及 workspace source 文件里的同文件 occurrences；对 dependency-backed current-document symbols，`references` 目前仍主要是当前文件 occurrence，并在 include declaration 时优先把声明位点替换成 workspace 源码定义，但当唯一 workspace 源码目标在源码侧本身还能恢复同文件 references 时，结果也会并入那部分 source-file occurrence；当前回归已锁定 source function / source method / source enum variant / source struct field 四类。而这轮同一类 best-effort fallback 也开始保留 parse-error 文件里的 workspace/package import-root 源码优先 `hover` / `definition` / `declaration` / `references` / `typeDefinition`，并继续保留 current-document `documentHighlight` / `prepareRename` / `rename`。Checkpoint C 后续继续沿这条“源码优先、接口兜底”的导航面扩。
