...
- `ql-cli` 主入口已收口到命令分发和少量 crate root re-export；command、pipeline、project target、interface、source rewrite、dependency bridge 和 test reporting 已拆成独立模块。下一步只继续拆 build backend glue、剩余 reporting 和 root re-export，所有移动必须保持行为不变并跑真实 workspace smoke。
