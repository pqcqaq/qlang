...
- `ql-codegen-llvm` 现已直接复用这份共享 hook ABI contract 渲染 LLVM declarations，`ql-driver` 也会把 dedupe 后的 `RuntimeHookSignature` 传入后端；当前仍仅限 declaration scaffolding，不开放 async lowering / call emission / 内存布局承诺
