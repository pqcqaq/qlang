...
- 继续扩 `ql project graph`、`ql project emit-interface`、`ql build --emit-interface`、`ql check --sync-interfaces` 的一致性，下一步优先把 dependency sync 失败时的 source-level summary 与最终 failing-reference summary 再做更细的去重和压缩，并减少重复 hint，同时避免 build/direct/workspace emit 在 package-manifest 提示上再次分叉。
