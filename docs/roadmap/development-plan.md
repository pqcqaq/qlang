...
- LSP 已开始收口跨请求 context：`hover`、`definition`、`declaration`、`typeDefinition`、`codeLens`、`implementation`、`references`、`documentHighlight`、`completion`、`signatureHelp`、`inlayHint`、`semanticTokens/full`、`semanticTokens/range`、`prepareRename`、`rename`、`documentLink` 已共享 package/open-doc/analysis 准备；diagnostics 的 request context 和 package preflight 映射已移入独立模块；workspace/symbol 的 request-scoped context 和扫描器 index 已移入独立模块。下一步推进真正 workspace index/cache 生命周期，减少重复扫描。diagnostics 已覆盖真实 stdlib app/source/open-buffer 场景，workspace/symbol 已覆盖 real stdlib open/change/close 生命周期，formatting、folding/selection、signatureHelp/inlayHint、documentLink、codeLens 已进入真实 stdlib request-family gate。
