...
- `ql-cli` 主入口、check/build/run/test pipeline、project selection/reporting、dependency bridge 和 build reporting 已持续模块化；dependency generic bridge 的 specialization/rendering/forwarder 合同测试已按子模块外置，instantiation 的模式/控制流推断覆盖已独立拆分，并锁住 alias/group import 行为。下一步继续拆 CLI 大边界并外置剩余内联测试，所有移动必须保持行为不变并跑真实 workspace smoke。
