...
- generic `Option[T]` / `Result[T, E]` carrier 已经进入可执行路径；`std.option` / `std.result` 首批 generic helper 已落地并进入 downstream smoke，`std.result` 现在也开放 generic `ok_or` / `to_option` / `error_to_option`，`std.array` 已有 canonical length-generic `[T; N]` `first_array` / `last_array` / `at_array_or` / `contains_array` / `count_array`，以及 `sum_int_array` / `product_int_array` / `max_int_array` / `min_int_array` / `all_bool_array` / `any_bool_array` / `none_bool_array`；对应旧 3/4/5 accessor/query/count/aggregate 降级为 compatibility surface。`reverse*_array` / `repeat*_array` 仍是过渡固定长度 API；`std.test` 现在提供数组断言并承接生成 smoke 的数组验证。`std.option.none_option` 与 `std.result.ok/err` 已通过显式返回/初始化上下文推断解锁。下一步优先补完整 monomorphization、公用集合构造和真实 downstream smoke，而不是继续堆 concrete/fixed-arity helper。可变参数语法仍需单独设计和 lowering，不直接拿来绕过当前 stdlib API 问题。
- generic public function declaration 现在可以存在于 library package 中并进入 `.qi`，只要没有被实例化调用就不会毒化 library build；direct dependency / package-under-test bridge 已经能对同一 public generic free function 的多个直接调用生成多个本地特化，前提是每个调用的所有 generic 参数都能从 `Int` / `Bool` / `String` 字面量、基础数值/布尔/比较表达式、tuple / fixed-array 字面量、一层 tuple/fixed-array projection、block / unsafe block tail、两侧结果一致的简单 `if` / `match` 表达式、top-level value、显式参数、显式 typed local、由这些值初始化的简单局部名字、generic carrier value、单字段 generic enum variant constructor，或显式返回类型 / typed initializer 结果上下文携带的具体类型表达式推断出来；位置实参与命名实参都会按形参声明顺序参与同一套 direct-call 推断；`identity[Int]` + `identity[Bool]` 同源多实例、`identity((1 + 2, !false))`、`identity([1, 2 + 3, 4])`、`identity(values[1])`、`identity(if flag { value } else { fallback })`、`identity(match value { ... })`、`T = Box[Int]`、从 `Box[T]` 参数反推 `T = Int`、从 `[T; 3]` 参数反推 `T = Int/Bool`、`Option.Some(42) -> Option[Int]`、`std.option.none_option() -> Option[Int]`、`Result[Int, Int]` 上下文约束 `ok/err` 和命名实参重排后的 direct call 都已进入 build/test 回归。完整单态化仍是下一阶段 backend/bridge 重点，方法、escaping function value 和更复杂 member/control-flow 表达式推断继续明确返回 `dependency-function-unsupported-generic`。
...
- `stdlib/packages/option` 能被 `ql check/build/test` 验证，并提供当前 dependency bridge 可执行的 option surface。（已落地 `Option[T]` generic carrier、generic `some` / `none_option` / `is_some` / `is_none` / `unwrap_or` / `or_option`，以及 `IntOption` / `BoolOption` compatibility helper；`none_option()` 当前依赖显式 `Option[T]` 结果上下文，完整 monomorphization / method-value generic import / prelude 集成继续后置）
- `stdlib/packages/result` 能被 `ql check/build/test` 验证，并提供当前 dependency bridge 可执行的 result surface。（已落地 `Result[T, E]` generic carrier、generic `ok` / `err` / `is_ok` / `is_err` / `unwrap_result_or` / `or_result` / `error_or`，以及 `IntResult` / `BoolResult` compatibility helper；完整 monomorphization / prelude 集成继续后置）
- `stdlib/packages/array` 能被 `ql check/build/test` 验证，并提供当前 dependency bridge 可执行的 fixed-array helper。（已落地 canonical length-generic first/last accessor、fallback-index、builtin equality-backed contains/count，以及 `Int` 数组 sum/product/min/max 与 `Bool` 数组 all/any/none 的 length-generic helper；3/4/5 聚合名只保留为 compatibility wrapper。reverse/repeat、动态集合和迭代器继续由 generic public API / backend 路线解锁）
- `stdlib/packages/test` 能提供 smoke-test 友好的断言辅助，并通过 package-aware smoke test 直接导入自身 public helpers。（已落地 true/false、bool equality/ne/logic/implies、Bool all/any/none、Bool-to-Int、array accessor/fallback/transform/query assertion、int equality/order、zero/nonzero、max/min/median、sum/product/average、sign/compare、abs/abs-diff/range-span/bounds、quotient/remainder/has-remainder/factor、Option/Result carrier、concrete/generic 转换与 error extraction 断言、2-6 路 status 组合、正负/非正/非负、区间、无序边界区间、单边/双边 clamp / range-distance、3/4/5 项升/降序、奇偶、整除和容差内/外断言）
- 用户项目模板能依赖 `std.core` / `std.option` / `std.result` / `std.array` / `std.test` 并通过 `ql test`。（已落地 `ql project init --stdlib <path>` 的 package 与 workspace member 生成路径；生成 lib/main 真实消费 `std.array` canonical accessor / fallback-index / contains / count / aggregate，以及过渡期 repeat / reverse，生成 smoke 通过 `std.test` array assertion 覆盖数组 helper）
- `stdlib` 当前已发布的 concrete API 继续使用稳定语言面并保持可执行；generic `Option[T]` / `Result[T, E]` carriers、`std.option` / `std.result` 首批 generic helper、generic Result/Option 转换 helper，以及 generic `[T; N]` array accessor / fallback-index / contains / count / aggregate helper 已进入真实 smoke，接下来用 failing tests 驱动 generic reverse / repeat / prelude / monomorphization / codegen，而不是继续把 carrier 和集合设计拖成远期后置项。
- 数组/集合 helper 是替代 `sum3/4/5`、`all3/4/5` 这类固定参数复制的近期方向；可变参数语法需要独立设计 ABI、`.qi` 表示、typeck、LSP 和 lowering，先进入设计 gate，不直接承诺为下一轮实现。

## 下一轮（已排定）

- stdlib/generics：按 [Stdlib Generics and Collections Roadmap](/plans/2026-05-02-stdlib-generics-and-collections-roadmap) 推进；`std.array` 的固定数组过渡面已先落地，`first_array[T, N]` / `last_array[T, N]` / `at_array_or[T, N]` / `contains_array[T, N]` / `count_array[T, N]` 与 `sum_int_array[N]` / `product_int_array[N]` / `max_int_array[N]` / `min_int_array[N]` / `all_bool_array[N]` / `any_bool_array[N]` / `none_bool_array[N]` 已成为 canonical length-generic API，旧 3/4/5 accessor/query/count/aggregate 保留兼容，`reverse*_array` / `repeat*_array` 仍是固定长度过渡 API；generic type / carrier 执行面、uninstantiated generic function declaration 面、primitive literal / 基础表达式 / tuple 与 fixed-array 字面量 / projection / 简单 `if` / `match` / typed value / generic carrier / 单字段 generic enum variant constructor / 显式结果上下文 inferred direct-call multi-instantiation generic public function bridge，以及 `std.option` / `std.result` 首批 generic helper、generic Result/Option 转换 helper、generic `[T; N]` array access/query/count/aggregate smoke 已被回归锁住。`std.option.none_option` 已进入显式 `Option[T]` 上下文路径；下一刀继续推进完整 monomorphization 表、bridge 专用符号命名、LLVM specialization 和 generic reverse/repeat，而不是再扩 concrete helper。
