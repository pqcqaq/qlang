...
- `project init --stdlib` starter template、interface sync/check、repo workspace interface/status/graph/dependencies/targets/dependents/listing current、repo starter metadata current、starter interface/package selectors、starter metadata selectors、dependency/dependent selectors、starter list targets、lockfile write/check/stale 和真实 workspace smoke 已迁出到独立 integration tests，并共享 stdlib fixture support；package/workspace 的 `check/build/run/test` 执行合同继续拆分覆盖。
