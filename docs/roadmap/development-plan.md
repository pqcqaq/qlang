...
- `project init --stdlib` starter template、interface sync/check、starter interface/package selectors、starter metadata selectors、dependency/dependent selectors、starter list targets、lockfile write/check/stale 和真实 workspace smoke 已迁出到独立 integration tests，并共享 stdlib fixture support；package/workspace 的 `check/build/run/test`、graph/status/targets/dependencies/dependents 和 JSON/listing 合同继续拆分覆盖。
