...
- LSP：`textDocument/implementation` 的已完成基线已明显超出这里最初记录，当前准确支持面以 `current-supported-surface.md` 为准；下一轮继续扩更宽的 implementation index，并优先收口 request/helper 尚未完全对齐的边界。`goto_implementation` 分发已先拆成 analyzed source 与 broken-source fallback 两条路径，后续继续在各自分支内收口 target resolution / response normalization。
