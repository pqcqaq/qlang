...
- fixed-array literal index precision 现也会回收最小 immutable const alias：当 `let index = 0` 或 `let slot = Slot { value: 0 }` 这类不可变绑定能在 borrowck 内保守折叠回 literal index 时，`await tasks[index]; await tasks[0]` 与 `await tasks[slot.value]; await tasks[0]` 也会走 literal path overlap，而不是继续停留在 generic dynamic maybe-overlap
