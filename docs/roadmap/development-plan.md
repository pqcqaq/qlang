...
- `ql-cli` 主入口已收口到命令分发；内部模块不再依赖 crate root re-export 中转。command、pipeline、project target、project target build prep、single-source input/reporting、build interface emission、source rewrite、dependency bridge、build/run/test reporting 和 project build execution 已拆成独立模块。下一步转向 `ql-driver` build pipeline 的 codegen/output/toolchain driver glue，所有移动必须保持行为不变并跑真实 workspace smoke。
