...
- package/workspace import，以及当前文件里命中的 dependency value / enum variant / struct field / method member，其 `hover` / `definition` / `declaration` 现在也已开始优先跳到 workspace 内唯一可定位的源码定义，找不到唯一源码目标时再回退 `.qi`；对 dependency-backed current-document symbols，`references` 目前仍主要是当前文件 occurrence，并在 include declaration 时优先把声明位点替换成 workspace 源码定义；而这轮同一类 best-effort fallback 也开始保留 parse-error 文件里的 workspace/package import-root 源码优先 `hover` / `definition` / `declaration` / `references`，并继续保留 current-document `documentHighlight` / `prepareRename` / `rename`。Checkpoint C 后续继续沿这条“源码优先、接口兜底”的导航面扩。
