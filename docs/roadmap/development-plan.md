...
- `stdlib/packages/array` 能被 `ql check/build/test` 验证，并提供当前 dependency bridge 可执行的固定长度数组 helper。（已落地 generic first/last accessor、fallback-index、reverse、repeat、builtin equality-backed contains/count，以及 `Int` 数组 sum/product/min/max 与 `Bool` 数组 all/any/none 的 3/4/5 长度 helper；这是 collection-shaped 过渡面，generic aggregate helper、动态集合和迭代器继续由 generic public API / backend 路线解锁）
...
- 用户项目模板能依赖 `std.core` / `std.option` / `std.result` / `std.array` / `std.test` 并通过 `ql test`。（已落地 `ql project init --stdlib <path>` 的 package 与 workspace member 生成路径；生成 lib/main 真实消费 `std.array` canonical accessor / fallback-index / contains / count，以及过渡期 repeat / reverse，生成 smoke 通过 `std.test` array assertion 覆盖数组 helper）
- `stdlib` 当前已发布的 concrete API 继续使用稳定语言面并保持可执行；generic `Option[T]` / `Result[T, E]` carriers、`std.option` / `std.result` 首批 generic helper、generic Result/Option 转换 helper，以及 generic `[T; N]` array accessor / fallback-index / contains / count helper 已进入真实 smoke，接下来用 failing tests 驱动 generic reverse / repeat / collection aggregate helper / prelude / monomorphization / codegen，而不是继续把 carrier 和集合设计拖成远期后置项。
- 数组/集合 helper 是替代 `sum3/4/5`、`all3/4/5` 这类固定参数复制的近期方向；可变参数语法需要独立设计 ABI、`.qi` 表示、typeck、LSP 和 lowering，先进入设计 gate，不直接承诺为下一轮实现。

## 下一轮（已排定）

- stdlib/generics：按 [Stdlib Generics and Collections Roadmap](/plans/2026-05-02-stdlib-generics-and-collections-roadmap) 推进；`std.array` 的固定数组过渡面已先落地，`first_array[T, N]` / `last_array[T, N]` / `at_array_or[T, N]` / `contains_array[T, N]` / `count_array[T, N]` 已成为 canonical length-generic API，旧 3/4/5 accessor/query/count 保留兼容，`reverse*_array` / `repeat*_array` / aggregate helper 仍是固定长度过渡 API；generic type / carrier 执行面、uninstantiated generic function declaration 面、primitive literal / 基础表达式 / tuple 与 fixed-array 字面量 / projection / 简单 `if` / `match` / typed value / generic carrier / 单字段 generic enum variant constructor / 显式结果上下文 inferred direct-call multi-instantiation generic public function bridge，以及 `std.option` / `std.result` 首批 generic helper、generic Result/Option 转换 helper、generic `[T; N]` array access/query/count smoke 已被回归锁住。`std.option.none_option` 已进入显式 `Option[T]` 上下文路径；下一刀继续推进完整 monomorphization 表、bridge 专用符号命名、LLVM specialization 和 collection aggregate helper，而不是再扩 concrete helper。
