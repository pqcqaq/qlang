...
- import alias 和 builtin type 现在也能提供 hover 级语义信息，并参与同文件 references 分组，但不会伪造 definition span
- shorthand field token 当前仍故意保守，让 `Point { x }` 里的 `x` 继续落在 local/binding 语义上，避免在字段标签与值/绑定双义位点上提前做错 rename
- same-file rename 当前只开放 function / const / static / struct / enum / variant / trait / type alias / local / parameter / generic；field / method / receiver / import / builtin type / shorthand field token 与 cross-file rename 仍然故意保守
