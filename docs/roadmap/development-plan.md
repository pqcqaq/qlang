...
- same-file rename 当前只开放 function / const / static / struct / enum / variant / trait / type alias / local / parameter / generic；field / method / receiver / import / builtin type 与 cross-file rename 仍然故意保守
...
3. 在当前 `qlsp` / 查询层之上继续把 same-file rename 扩到更完整语义面，并为 completion / cross-file rename 继续补 module-path / ambiguous method / project indexing 等数据面
