...
- invalid struct-literal root diagnostics 目前也覆盖 builtin / generic root 以及 root 已解析成功且明确不支持 struct-style 字段构造的 case；missing variant 与 deeper module-path 仍然保守
...
- invalid path-pattern root diagnostics 目前也只覆盖 path root 已解析成功且明确不支持 bare path 形状的 case；unit variant 仍允许
