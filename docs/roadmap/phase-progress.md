...
- stdlib：普通 Qlang package 形态的 `stdlib` 已开始落地，当前已有 `std.core` 的基础整数/布尔 helper 与 `std.test` 的 smoke-test 断言；本轮继续补单边 clamp、绝对差、容差判断和容差断言，并让 `ql project init --stdlib <path>` 生成的 package 与 workspace member smoke test 直接消费这些新增 helper 的成功和失败返回路径。下一步继续扩只依赖稳定语言面的基础 helper。
