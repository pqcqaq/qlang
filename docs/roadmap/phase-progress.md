...
- `ql project graph [file-or-dir]` 已可向上发现 manifest 并输出当前 package/workspace/reference graph；workspace-only 根 manifest 当前还会进一步展开 member package 的 manifest、包名、默认 `.qi` 路径与 references。
