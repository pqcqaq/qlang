...
- sync/async tuple constant indexing executable surface 现也补上 same-file `const` / `static`、projection、整数算术、same-file `use ... as ...` alias，以及 immutable direct local alias 复用驱动的 foldable tuple index 读写
- sync ordinary executable surface 现也补上 computed/projected `const` / `static` item value materialization：同文件 const/static item 的投影结果可继续作为后续 const/static item 的 source，进入普通表达式与 `if` 条件
- async ordinary executable surface 现也显式锁住同一条 computed/projected `const` / `static` item value materialization：同文件 const/static item 的投影结果可继续作为后续 const/static item 的 source，进入 `async fn main` 里的普通表达式与 `if` 条件
