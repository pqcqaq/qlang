...
- `ql-typeck` / `ql-borrowck` 现也已显式镜像 zero-sized helper `Task[Wrap]` 流动回归：`await schedule()`、`spawn schedule()`、local-return helper、bound `spawn` 后再 `await`、helper `Task[Wrap]` 参数传递、conditional helper return、conditional maybe-moved 分支、branch-join maybe-moved 分支、helper-consumed 后重赋值可恢复的路径、deferred cleanup 重初始化后继续读取的路径、deferred cleanup helper-consume use-after-move、conditional cleanup maybe-moved 分支、conditional cleanup helper-consume maybe-moved 分支、conditional cleanup 重初始化路径，以及对应的 borrowck debug render 事实，现在都有前端层专门测试，不再只依赖 backend/staticlib 黑盒覆盖
