...
- async aliased projected-root repackage/spawn family 现也把 same-file `use ... as ...` alias 包裹的 arithmetic-backed item value 拉进公开 executable 回归矩阵，包括 direct alias-root / projected-root 形态，以及 `alias[alias_slots[row]]` 这类 alias-sourced composed-dynamic arithmetic path 上的 tuple/fixed-array repackage 与 nested aggregate / helper-forwarded nested fixed-array repackage 提交路径
