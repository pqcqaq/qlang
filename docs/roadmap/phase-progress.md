...
- stdlib / test harness：普通 Qlang package 形态的 `stdlib` 已继续扩面，新增 `std.array` 固定数组 helper 并接入 `ql project init --stdlib` 下游 smoke；当前 concrete carrier 与 3/4/5 固定参数 helper 已明确降级为过渡兼容面。direct local dependency 的 generic public type carrier 和单实例、primitive 字面量/参数/local 可推断的 generic public free function bridge 已有回归。下一步不继续主要堆重复参数 helper，而是按 `docs/plans/2026-05-02-stdlib-generics-and-collections-roadmap.md` 继续打通更完整的调用点实例化、monomorphization/codegen 和 downstream stdlib generic helper smoke。
