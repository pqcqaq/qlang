...
- `crates/ql-cli/tests/executable_examples.rs` + `ramdon_tests/executable_examples/*.ql` / `ramdon_tests/async_program_surface_examples/*.ql`：新增真实 toolchain 的 build-and-run smoke harness；当前既锁住 3 个 sync executable 样例的实际退出码（`42 / 32 / 42`），也锁住 50 个 async executable 样例的实际退出码（`28 / 71 / 39 / 45 / 70 / 10 / 49 / 24 / 17 / 17 / 21 / 25 / 31 / 32 / 33 / 36 / 34 / 38 / 37 / 40 / 41 / 46 / 47 / 48 / 50 / 52 / 54 / 59 / 63 / 61 / 62 / 64 / 66 / 68 / 72 / 74 / 76 / 78 / 80 / 82 / 84 / 86 / 88 / 90 / 92 / 94 / 96 / 98 / 100 / 102`），避免 executable surface 只剩“可构建”而没有“可运行”回归
- 当前边界保持不变：仅 fixed-array iterable 被开放；library-mode 与当前 `llvm-ir` / `object` / `executable` `async fn main` 子集都可使用该 lowering，但 non-array iterable与更广 program bootstrap 仍关闭
