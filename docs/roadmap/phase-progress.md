...
- diagnostics bridge parity 现在也有显式回归覆盖，锁住了 compiler `Warning` / `Note` 到 LSP severity 的映射、无 label diagnostics 回退到 `0:0` range、无 primary label 时使用第一条 label span，以及 secondary label 的默认 related-information message 与 `source = ql` 协议行为
