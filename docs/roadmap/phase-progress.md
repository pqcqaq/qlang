...
- dependency-only completion 现在还带有最小编辑期容错：当当前文档自身暂时分析失败时，LSP 仍会走 dependency-only package load 回退，继续提供 `use ...` 导入路径上的 dependency path segment / public symbol completion、dependency enum import alias root 上的 variant completion，以及 dependency struct explicit field-label completion；该回退当前不扩大到 hover / definition / references 或其它同文件补全。
