...
- 这一轮把 healthy source 下 source-preferred dependency type `implementation` 的目标解析也补齐到了 value / enum variant / struct field 这些非 import 位点：从 `built.value`、`Cmd.Retry(...)`、`holder.child` 这类位置发起时，VSCode 现在也会回到真实 workspace dependency 源码里的 `impl` / `extend` block，而不是只在 import/type name 上生效。
