...
- `ql-typeck` / `ql-borrowck` 现也已显式镜像 zero-sized helper `Task[Wrap]` 流动回归：`await schedule()`、`spawn` 后再 `await`、helper `Task[Wrap]` 参数传递与对应的 use-after-move / consume-rendering 路径现在都有前端层专门测试，不再只依赖 backend/staticlib 黑盒覆盖
