...
- dependency generic bridge 现在可从部分可推断的 fixed-array literal 绑定 `[T; N]` 参数：即使数组中部分元素是当前签名索引还无法识别返回类型的调用，只要已知元素和字面量长度足够，就能推得 `T` 与 `N`。`ql project init --stdlib` 生成代码已恢复为内联 `reverse3_array([1, first_array(...), at_array_or(...)])` 与 `repeat3_array(unwrap_or_bool(...))` 形态，不再为桥接器补中间变量。
