...
- `ql-borrowck` 已把 direct-local task handle 的 `await` / `spawn` 接进当前 ownership facts：这些操作现在会显式 consume 任务句柄，本地句柄在消费后复用会得到稳定的 use-after-move / maybe-moved 诊断，而重赋值仍可恢复 local 可用性
...
- 当前 borrowck 只对 direct-local task handle 建立最小 consume 合同；helper 穿边界、place-sensitive handle lifecycle 与更广义 drop/submission 协议仍待后续切片
