...
- 这一轮继续把 `qlsp` 的 `textDocument/implementation` 往前推；VSCode 现在除了 same-file trait/type surface，还会为 workspace / 本地路径依赖 source-preferred 导航下的 `struct / enum / trait` 聚合当前包、可见 workspace members 与本地依赖源码里的实现块，trait method definition 也会聚合匹配的 `impl` method；两条路径都会优先读取 parseable open docs。same-file trait/type 继续返回本文件里的 `impl` / `extend` block，trait method 返回匹配的 impl method；更宽的全局 implementation index 继续后置。
