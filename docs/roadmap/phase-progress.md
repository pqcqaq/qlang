...
- sync/async tuple constant indexing executable surface 现也补上 same-file `const` / `static`、projection、整数算术、same-file `use ... as ...` alias、branch-selected const `if` / 最小 literal `match` item value，以及 immutable direct local alias 复用驱动的 foldable tuple index 读写
