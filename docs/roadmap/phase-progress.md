...
- `ql-typeck` / `ql-borrowck` 现也已显式镜像 zero-sized helper `Task[Wrap]` 流动回归：`await schedule()`、`spawn schedule()`、local-return helper、bound `spawn` 后再 `await`、helper `Task[Wrap]` 参数传递、conditional helper return 与 conditional maybe-moved 分支，以及对应的 use-after-move / deferred-cleanup / consume-rendering 路径现在都有前端层专门测试，不再只依赖 backend/staticlib 黑盒覆盖
