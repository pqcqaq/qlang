...
- healthy workspace / 本地路径依赖的 source-backed dependency `method / field` 这一轮也补上了 open-doc 成员解析；当成员只存在于未保存源码、磁盘 `.qi` 尚未更新时，`hover / definition / typeDefinition / references / documentHighlight / semantic tokens` 仍会继续命中真实源码。
