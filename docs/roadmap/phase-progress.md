...
- `ql-typeck` 新增保守 return-path 收口：函数与 closure body 现在会拒绝“部分路径 `return`、部分路径 fallthrough”的情形；`if` 与带 `_` catch-all 的 `match` 已进入最小 all-path return 推断
...
- 若后续需要支持 `loop { return ... }`、更深层表达式求值顺序或完整 `match` 穷尽性驱动的 must-return，应单独设计 CFG/exhaustiveness slice
