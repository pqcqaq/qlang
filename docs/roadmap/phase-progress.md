...
2. 主线先做 manifest、dependency-aware build/backend、真实 `stdlib`、真实 workspace LSP、安装与分发；P0 未完成前，不再把新语法和更宽 runtime 当主线，但 generic public API / generic `Option[T]` / `Result[T, E]` 已因 stdlib 可用性进入 unblock 路径，`std.option` / `std.result` 首批 generic helper 和 `std.array` 固定数组集合过渡面已服务真实项目。
3. 每一轮功能推进必须先落生产代码，再补测试和文档；只有测试或文档改动，不再计作一轮功能迭代。
4. 不再按固定日期承诺完成；每轮选择当前最能提升真实项目可用性的切片，做到实现、回归、文档一起收口。

## 下一轮

- stdlib / test harness：普通 Qlang package 形态的 `stdlib` 已继续扩面，新增 `std.array` 固定数组 helper 并接入 `ql project init --stdlib` 下游 smoke；当前 concrete carrier 与 3/4/5 固定参数 helper 已明确降级为过渡兼容面。direct local dependency 的 generic public type carrier，以及单实例、primitive 字面量 / 显式 typed value / generic carrier / 单字段 generic enum variant constructor 可推断的 generic public free function bridge 已有 build 与 package-under-test smoke 回归，覆盖 `T = Box[Int]`、从 `Box[T]` 参数反推 `T = Int`、`Option.Some(42) -> Option[Int]`，以及 `std.option` / `std.result` 首批 generic helper。`std.test` 已补齐 `Option[Int]`、`Option[Bool]`、`Result[Int, Int]`、`Result[Bool, Int]` 的 generic carrier 断言，`ql project init --stdlib` 生成 smoke 不再内联手写 carrier status 函数。下一步不继续主要堆重复参数 helper，而是按 `docs/plans/2026-05-02-stdlib-generics-and-collections-roadmap.md` 继续打通更完整的调用点实例化、monomorphization/codegen、generic constructors 和 collection helper。
