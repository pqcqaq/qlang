...
- diagnostics bridge parity 现在也有显式回归覆盖，锁住了 compiler `Warning` / `Note` 到 LSP severity 的映射、无 label diagnostics 回退到 `0:0` range、无 primary label 时使用第一条 label span、仅 primary label 时 `related_information = None`，以及 secondary label 的 related-information 顺序与 message 回退（默认 `related span`）和 `source = ql` 协议行为
