...
- 这一轮开始整理 `goto_implementation` 分发：`crates/ql-lsp/src/backend.rs` 已先把 analyzed source 与 broken-source fallback 拆成独立分支，后续新增 implementation case 需要进入对应分支，而不是继续扩大单个总链路。
