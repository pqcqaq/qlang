...
- stdlib / test harness：普通 Qlang package 形态的 `stdlib` 已继续扩面，新增 `std.array` 固定数组 helper 并接入 `ql project init --stdlib` 下游 smoke；当前 concrete carrier 与 3/4/5 固定参数 helper 已明确降级为过渡兼容面。下一步不继续主要堆重复参数 helper，而是按 `docs/plans/2026-05-02-stdlib-generics-and-collections-roadmap.md` 先补 direct local dependency 的 generic public function / generic struct / generic enum failing tests，再打通 `.qi`、dependency bridge、monomorphization/codegen 和 downstream stdlib smoke。
