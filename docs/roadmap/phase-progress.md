...
- broken-source 下，direct imported-result member query、dependency semantic tokens fallback、dependency enum variant 的 `completion/definition/typeDefinition/references/documentHighlight` fallback 已补齐到源码优先路径。
- 同名本地依赖在这条 broken-source 路径上继续按 manifest 身份区分；`build().ping()` / `build().value` 和 enum variant query / completion 都不会再串到兄弟依赖实例。
