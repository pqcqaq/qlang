...
- `ql project dependencies` 现在也已落地，并支持 `--json`；workspace package 当前直接依赖了哪些 workspace members 与 workspace 外本地路径依赖已可直接查询，JSON 会标出 `workspace` / `local` kind 与声明路径，正反向依赖审计不必再手读 manifest 或 `project graph`。从 package / workspace member 路径进入时也可自动推断目标包。
...
- dependency generic bridge 现在可从部分可推断的 fixed-array literal 绑定 `[T; N]` 参数：即使数组中部分元素是当前签名索引还无法识别返回类型的调用，只要已知元素和字面量长度足够，就能推得 `T` 与 `N`。同一套 source specialization 也已用于 package-local 非泛型调用方调用本地 generic free function，使 `std.array` 的 3/4/5 aggregate compatibility wrapper 可以委托 canonical length-generic aggregate。`ql project init --stdlib` 生成代码已恢复为内联 `reverse3_array([1, first_array(...), at_array_or(...)])` 与 `repeat3_array(unwrap_or_bool(...))` 形态，并改用 `sum_int_array` / `all_bool_array` 这类 canonical aggregate，不再为桥接器补中间变量或依赖固定长度聚合名。
...
- 这一轮把 full-sync 文档生命周期也补进真实 `LspService` notification 回归；当前已锁住 valid `didOpen` 发布空 diagnostics、parse-error `didChange` 发布 parser diagnostics、`didClose` 清空 diagnostics 三条编辑器入口合同。本轮继续把 diagnostics 推到 package preflight：当前 buffer 干净时，`didOpen` / `didChange` 会把缺失 dependency `.qi` 等 manifest / interface 错误发布到当前文档；parse-error buffer 仍优先发布当前 buffer 诊断，不会被 package preflight 覆盖。
...
2. 主线先做 manifest、dependency-aware build/backend、真实 `stdlib`、真实 workspace LSP、安装与分发；P0 未完成前，不再把新语法和更宽 runtime 当主线，但 generic public API / generic `Option[T]` / `Result[T, E]` 已因 stdlib 可用性进入 unblock 路径，`std.option` / `std.result` 首批 generic helper、generic Result/Option 转换 helper、generic `[T; N]` array access/query/count、`std.option.none_option` / `std.result.ok/err` 显式结果上下文推断和 `std.array` 固定数组集合过渡面已服务真实项目。
3. 每一轮功能推进必须先落生产代码，再补测试和文档；只有测试或文档改动，不再计作一轮功能迭代。
4. 不再按固定日期承诺完成；每轮选择当前最能提升真实项目可用性的切片，做到实现、回归、文档一起收口。

## 下一轮

- stdlib / test harness：普通 Qlang package 形态的 `stdlib` 已继续扩面，新增 `std.array` canonical length-generic `first_array[T, N]` / `last_array[T, N]` / `at_array_or[T, N]` / `contains_array[T, N]` / `count_array[T, N]`，以及 `sum_int_array[N]` / `product_int_array[N]` / `max_int_array[N]` / `min_int_array[N]` / `all_bool_array[N]` / `any_bool_array[N]` / `none_bool_array[N]`，并接入 `ql project init --stdlib` 下游 smoke；当前 concrete carrier 与 3/4/5 固定参数 helper 已明确降级为过渡兼容面。direct local dependency 的 generic public type carrier，以及 primitive 字面量 / 基础表达式 / tuple 与 fixed-array 字面量 / 显式 typed value / generic carrier / 单字段 generic enum variant constructor / 显式结果上下文可推断的 direct-call multi-instantiation generic public free function bridge 已有 build 与 package-under-test smoke 回归，覆盖同一 `identity[T]` 在 `Int` + `Bool` 下的多实例、`T = (Int, Bool)`、`T = [Int; 3]`、`T = Box[Int]`、从 `Box[T]` 参数反推 `T = Int`、从 `[T; N]` 参数同时反推元素类型和数组长度、`Option.Some(42) -> Option[Int]`、`std.option.none_option() -> Option[Int]`、`Result[Int, Int]` 上下文约束 `std.result.ok/err`，以及 `std.option` / `std.result` 首批 generic helper。`std.result` 已补上 generic `ok_or` / `to_option` / `error_to_option`，`std.array` 已补上 canonical length-generic access/query/count/aggregate；固定长度 `reverse*_array` / `repeat*_array` helper 仍是过渡面，`std.test` 已补齐 array assertion、`Option[Int]`、`Option[Bool]`、`Result[Int, Int]`、`Result[Bool, Int]` 的 generic carrier 与 generic conversion 断言，`ql project init --stdlib` 生成 smoke 不再内联手写 carrier status 函数或数组断言表达式，并在显式 `Option[Int]` / `Option[Bool]` 路径上消费 `std.option.none_option`。下一步不继续主要堆重复参数 helper，而是按 `docs/plans/2026-05-02-stdlib-generics-and-collections-roadmap.md` 继续打通更完整的 monomorphization/codegen 和 generic reverse/repeat。
