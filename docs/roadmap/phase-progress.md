...
- 这一轮开始整理 `goto_implementation` 分发：`crates/ql-lsp/src/backend.rs` 已先把 analyzed source 与 broken-source fallback 拆成独立分支，并把 source-backed dependency concrete method implementation 的 Method 过滤、源码定位和自引用抑制收敛到共享 helper；后续新增 implementation case 需要进入对应分支，而不是继续扩大单个总链路。
