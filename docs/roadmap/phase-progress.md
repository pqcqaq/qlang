...
- 这一轮已把 public 非泛型 `enum` 的最小 LLVM 执行闭环补到真实跨包路径：direct local dependency public function 现在可以按值返回 enum，consumer 侧 `match` 也已能稳定执行 unit / struct variant；tuple variant 目前仍只补到 dependency type bridge 与依赖排序，暂不算 runtime 已完成。
