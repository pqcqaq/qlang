...
- `ql project emit-interface [file-or-dir] [-o <output>] [--changed-only] [--check]` 已可对 package manifest 的 `src/**/*.ql` 做逐文件分析，并输出 text-based `.qi` public interface artifact；新增 `--changed-only` 后，若目标接口已是 `valid` 状态则会跳过并报告 `up-to-date interface: ...`；新增 `--check` 后，则会只验证默认 `.qi` 是否处于 `valid` 状态
