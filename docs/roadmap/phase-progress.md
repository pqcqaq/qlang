...
- `ql-borrowck` / `ql-driver` / `ql-cli` 现又补上最小 equality-guard dynamic-index refinement：当 `if index == 0` 或 `if slot.value == 0` 这类条件把 stable dynamic source path 收窄到具体元素时，borrowck 会在 dominated branch 内把 `tasks[index]` / `tasks[slot.value]` 回收到 `tasks[0]` 这条 literal/projection path，因此 guarded literal reuse 会稳定报 definite use-after-move，而 guarded `tasks[0] = worker()` / projected `tasks[0] = worker()` 也可恢复通过；对应的 staticlib build 与 CLI fixture 已同步锁定，避免这条 guard-sensitive 精度只停留在内部 ownership 单测
...
- 当前边界仍保持保守：`llvm-ir` / `object` emit 仍通过 driver capability gate 拒绝 async runtime surface；只有 `BuildEmit::Executable` 会在 program-mode host entry 上额外注入 `async fn main` 所需最小 runtime definitions / stubs，并开放当前可链接、可运行的最小程序入口生命周期。更广义的 program bootstrap、async dylib surface 与 task result transport 仍未开放
...
- `crates/ql-cli/tests/executable_examples.rs` + `ramdon_tests/executable_examples/*.ql` / `ramdon_tests/async_program_surface_examples/*.ql`：新增真实 toolchain 的 build-and-run smoke harness；当前既锁住 3 个 sync executable 样例的实际退出码（`42 / 32 / 42`），也锁住 6 个 async executable 样例的实际退出码（`28 / 71 / 39 / 45 / 70 / 10`），避免 executable surface 只剩“可构建”而没有“可运行”回归
