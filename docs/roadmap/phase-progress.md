...
- `crates/ql-cli/tests/executable_examples.rs` + `ramdon_tests/executable_examples/*.ql` / `ramdon_tests/async_program_surface_examples/*.ql`：新增真实 toolchain 的 build-and-run smoke harness；当前既锁住 4 个 sync executable 样例的实际退出码（`42 / 32 / 42 / 5`），也锁住 78 个 async executable 样例的实际退出码（完整列表以 `ramdon_tests/async_program_surface_examples/README.md` 为准），避免 executable surface 只剩“可构建”而没有“可运行”回归
