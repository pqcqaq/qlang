...
- dependency struct local-value member path 的首个 method-query contract 也已落地：当 `use demo.dep.Config as Cfg` 这类 alias 能唯一映射到 dependency public struct，并且 `.qi` 中该 nominal struct 的 public method 仍可唯一解析（当前继续遵守 impl 优先、再看 extend 的保守规则）时，`config.get()` / `built.get()` 这类 member method token 现在也已接通 dependency hover / definition / references；这条能力当前只覆盖 source analysis 成功时、语法局部可恢复出的 named local receiver，不扩展到 broken-source fallback、method completion 或任意表达式 receiver。
