...
- stdlib：普通 Qlang package 形态的 `stdlib` 已开始落地，当前已有 `std.core` 的基础整数/布尔 helper 与 `std.test` 的 smoke-test 断言；本轮继续补 bool ne / not / and / or / xor 断言，并把 `ql project init --stdlib <path>` 生成的 package 与 workspace member smoke test 改成用 `merge_status4` / `expect_status_ok` 按语义分组收口。下一步继续扩只依赖稳定语言面的基础 helper。
