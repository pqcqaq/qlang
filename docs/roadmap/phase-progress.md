...
- stdlib：普通 Qlang package 形态的 `stdlib` 已开始落地，当前已有 `std.core` 的基础整数/布尔 helper 与 `std.test` 的 smoke-test 断言；`ql project init --stdlib <path>` 已能生成 package 与 workspace member 的本地依赖消费模板，并通过真实 consumer smoke test 覆盖关系断言。下一步继续扩只依赖稳定语言面的基础 helper。
