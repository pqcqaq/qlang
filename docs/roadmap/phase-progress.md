...
- import alias / builtin type 支持 hover 级语义信息，并参与同文件 references
...
- explicit struct literal / struct pattern 字段标签现在也会进入 field query surface，但 shorthand `Point { x }` token 仍刻意保守，继续落在 local/binding 语义
- same-file rename 当前只开放 function / const / static / struct / enum / variant / trait / type alias / local / parameter / generic；field / method / receiver / import / builtin type / shorthand field token 与 cross-file rename 仍然刻意不开放
- 新增 `ql-lsp`
...
- field-or-method rename、shorthand field rename 与 cross-file rename
