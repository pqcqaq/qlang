...
- `crates/ql-cli/tests/executable_examples.rs` + `ramdon_tests/executable_examples/*.ql` / `ramdon_tests/async_program_surface_examples/*.ql`：新增真实 toolchain 的 build-and-run smoke harness；当前既锁住 3 个 sync executable 样例的实际退出码（`42 / 32 / 42`），也锁住 18 个 async executable 样例的实际退出码（`28 / 71 / 39 / 45 / 70 / 10 / 49 / 24 / 17 / 17 / 21 / 25 / 31 / 32 / 33 / 36 / 34 / 38`），避免 executable surface 只剩“可构建”而没有“可运行”回归
