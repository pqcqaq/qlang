...
- 本地与 direct local dependency 的 `impl` / `extend` / 唯一 trait `impl for` receiver method 直接调用现在都已打通到 LLVM 执行链路；`ql build` / `ql run` 已能真实执行 `value.read()` 这类 direct call。当前边界仍然很窄：只覆盖 direct call position，不包含 method value。
