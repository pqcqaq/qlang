...
- 这一轮把 `qlsp` 的 `textDocument/implementation` 第一版也补上了；VSCode 现在可直接从 same-file trait/type surface 跳到实现。当前 trait/type 返回本文件里的 `impl` / `extend` block，trait method 返回匹配的 impl method，workspace/dependency 级 implementation graph 继续后置。
