...
- stdlib / test harness：普通 Qlang package 形态的 `stdlib` 已继续扩面，新增 `std.array` 固定数组 helper 并接入 `ql project init --stdlib` 下游 smoke；当前 concrete carrier 与 3/4/5 固定参数 helper 已明确降级为过渡兼容面。direct local dependency 的 generic public type carrier，以及单实例、primitive 字面量 / 显式 typed value / generic carrier / 单字段 generic enum variant constructor / 显式结果上下文可推断的 generic public free function bridge 已有 build 与 package-under-test smoke 回归，覆盖 `T = Box[Int]`、从 `Box[T]` 参数反推 `T = Int`、`Option.Some(42) -> Option[Int]`、`std.option.none_option() -> Option[Int]`、`Result[Int, Int]` 上下文约束 `std.result.ok/err`，以及 `std.option` / `std.result` 首批 generic helper。`std.test` 已补齐 `Option[Int]`、`Option[Bool]`、`Result[Int, Int]`、`Result[Bool, Int]` 的 generic carrier 断言，`ql project init --stdlib` 生成 smoke 不再内联手写 carrier status 函数，并在一个显式 `Option[Int]` 路径上消费 `std.option.none_option`。下一步不继续主要堆重复参数 helper，而是按 `docs/plans/2026-05-02-stdlib-generics-and-collections-roadmap.md` 继续打通更完整的多实例 monomorphization/codegen 和 collection helper。
