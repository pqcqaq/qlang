...
- 这次实现层没有新增 ABI 或 lowering 分支；新回归证明的是 program-mode async body 已经复用既有 fixed-shape aggregate/task-handle lowering 与 projection write/reinit lowering，只是此前缺少显式 coverage
