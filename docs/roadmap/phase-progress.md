...
- 这一轮把 `qlsp` 的 `textDocument/implementation` 往前推了一步；VSCode 现在除了 same-file trait/type surface，还能从 workspace / 本地路径依赖里导入的 `struct / enum / trait` 跳到依赖源码实现。same-file trait/type 继续返回本文件里的 `impl` / `extend` block，trait method 返回匹配的 impl method；导入类型/trait 返回依赖源码里的 `impl` / `extend` / trait `impl` block。完整 workspace-wide implementation graph 与 open-doc implementation remap 继续后置。
