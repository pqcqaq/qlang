...
- `std.result` 当前提供可执行的 concrete result 面：`IntResult` / `BoolResult`，以及 `ok_int`、`err_int`、`is_ok_int`、`is_err_int`、`unwrap_result_or_int`、`or_result_int`、`error_or_zero_int`、`ok_bool`、`err_bool`、`is_ok_bool`、`is_err_bool`、`unwrap_result_or_bool`、`or_result_bool`、`error_or_zero_bool`。泛型 `Result[T, E]` 仍不是 prelude，也还没有进入跨包执行面。
