...
- 最小 literal `match` lowering 已开放在当前 program / library build surface 内：
  - `Bool` scrutinee：unguarded `true` / `false` / `_` arm 会直接 lower 成 LLVM branch。
  - `Int` scrutinee：unguarded integer literal / `_` arm 会 lower 成稳定 compare-chain。
...
- guarded `match`、非 `Bool` / `Int` scrutinee `match`、以及超出 `Bool true|false|_` / `Int literal|_` 的更广义 match pattern lowering。
