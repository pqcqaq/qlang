...
- `ql check/fmt/mir/ownership/runtime/build/run/test/project/ffi` 已实现；`check/build/run/test/fmt`、`mir/ownership/runtime`、`project status/targets/graph/dependencies/dependents/lock/target add/emit-interface/init/add/remove` 的参数校验有真实 CLI smoke。
...
- `ql project add/remove` 已覆盖 workspace root/member source 的成员编辑；`ql project add-dependency/remove-dependency` 已覆盖 workspace member source/directory 和 workspace root `--package` selector，依赖编辑逻辑已从 CLI 入口拆出；workspace/package `qlang.toml` 读改写已纳入输出路径级跨进程锁，并发 add/remove 同一 manifest 有真实 CLI smoke。
