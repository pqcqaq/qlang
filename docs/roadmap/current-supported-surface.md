...
- async executable examples：`189`

注意：

- async 目录文件编号从 `04` 编到 `192`，但真实 `.ql` 文件数是 `189`，不是 `192`
- `crates/ql-cli/tests/executable_examples.rs` 当前也只注册了 `189` 个 async executable case 和 `39` 个 sync executable case
