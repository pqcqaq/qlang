...
- statement-sequenced block wrapper：只接受当前已支持 cleanup expr statement、statement-level `while` / `loop`，外加可选 tail；当前已覆盖 direct cleanup body、cleanup guard / scrutinee block，以及 cleanup call-arg value block
