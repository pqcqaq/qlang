...
- repo stdlib workspace 的 source-only 复制 fixture 已覆盖 `ql check --sync-interfaces --json`、后续 `ql check --json`、starter `check/build/run/test --package --json`、starter `build/run/test --list --json --package`、starter `project emit-interface --check` / `--changed-only --check`、starter `project dependencies --package --json`、option/core `project dependents --package/--name --json`、同步接口后的 starter `project graph/status/targets --package --json`、`ql project lock --json`、`ql project lock --check --json`、manifest 漂移后的 lock stale failure JSON、全量 `ql test` 和 `ql test --json`；仓库内 stdlib 已覆盖 starter package `ql check --json` / `ql build --json` / `ql test --json` / `project graph --json` / `project status --json` / `project targets --json` / `project emit-interface --check` / `project emit-interface --changed-only --check`、workspace `ql build --list --json`、`ql build --json`、`ql run --list --json`、starter `ql run --json`、`ql test --list --json`、starter package test listing、`project emit-interface --check`、`project status --json`、`project graph --json`、`project targets --json`、`project lock --check --json`、starter `project dependencies --name/--package --json` 和 `std.option` / `std.core` 的 `project dependents --name --json`、`std.option` 的 `project dependents --package --json`；6 个 package/example smoke、接口同步产物、lockfile、构建产物与可执行产物存在性作为 downstream 可用性 gate。
...
- `ql-cli` 主链路仍需继续拆分；共享路径/JSON 字符串转义/诊断渲染和打印/diagnostic JSON 渲染/CLI analysis helper/project reporting helper/project interface reporting helper/reference-interface helper、包名校验/project manifest error helper/reference/workspace manifest path helper、文件扫描、版本输出、usage 输出、check JSON reporting、build JSON reporting、test JSON reporting 和 check 命令失败提示已从 `main.rs` 抽出，`ql check`、`ql build`、`ql run`、`ql test` 根入口已下沉到各自 command 模块，`ql build` 单文件和 project pipeline 已移入 `build_pipeline`，`ql run` project pipeline、JSON 报告和 executable 执行锁已移入 `run_pipeline`，`ql test` discovery/selection/listing/execution/UI snapshot pipeline 已移入 `test_pipeline`，build/run/test 共享 profile 参数逻辑、build output/profile/collision helper、build failure/reporting helper、build plan resolution、dependency build prep 和 project target build/test 编排已抽到独立模块，build/run target selector/listing 与 workspace target loading 已直接归入 `project_targets`，project graph/status/dependencies 查询模块和 project member/lock/workspace lookup 入口已改为直接依赖目标模块，interface artifact emission、quiet dependency interface prep、reference manifest path 解析、`ql project emit-interface` 执行路径、单文件 build/emit-interface 执行路径、本地泛型 source rewrite、dependency bridge extern 渲染、import/root-name 分析、模块加载、import path 解析、public global/type dependency bridge 排序、public type/value declaration 渲染、public function/method forwarder 渲染、export wrapper 渲染和 forwarder/name 渲染已抽到独立模块。剩余重点是拆 build backend helper、request-context 和剩余 reporting，用真实 workspace smoke 锁住行为。
