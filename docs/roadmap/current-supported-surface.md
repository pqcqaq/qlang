...
- `ql-cli` 主链路仍需继续拆分；共享路径/JSON 字符串转义/诊断渲染和打印/diagnostic JSON 渲染/CLI analysis helper/project reporting helper/project interface reporting helper/reference-interface helper/包名校验/project manifest error helper/reference/workspace manifest path helper、文件扫描、版本输出、usage 输出、check JSON reporting 和 check 命令失败提示已从 `main.rs` 抽出，`ql check`、`ql build`、`ql run`、`ql test` 根入口已下沉到各自 command 模块，`ql run` project pipeline、JSON 报告和 executable 执行锁已移入 `run_pipeline`，build/run/test 共享 profile 参数逻辑已抽到独立模块，build/run target selector/listing 与 workspace target loading 已直接归入 `project_targets`，project graph/status/dependencies 查询模块和 project member/lock/workspace lookup 入口已改为直接依赖目标模块，interface artifact emission、quiet dependency interface prep 和 reference manifest path 解析已抽到 `project_interfaces` / `project_manifest_paths`。剩余重点是继续拆 build/test pipeline 过渡 helper、request-context 和剩余 reporting，用真实 workspace smoke 锁住行为。
