...
- `textDocument/prepareRename` / `rename` 现在也有真实 `LspService` request 回归；当前已锁住 same-file rename 的 range/placeholder 与同文件 `WorkspaceEdit` 返回合同，workspace/source-backed rename 仍由专题 backend tests 覆盖。
