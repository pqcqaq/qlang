...
- 上述 dependency-only 回退里的 `for` loop receiver 现也不再只限 direct tuple/array root；对应的 parse-only value-root fallback 也会继续复用同一 binding identity，因此 structured iterable root 下的 `for current in { (config, config) } { current.value }` / `for current in match flag { ... } { current.get() }` 也会落在同一条 dependency-only 容错合同里，而 imported iterable root token（例如 `for current in children() { current.get() }` 上的 `children`、`for current in ITEMS { current.get() }` 上的 `ITEMS`，以及 grouped import alias 形态下 `kids()` / `items` 上的 alias token）现在也已在同一路径下接通 hover / definition / declaration / references 与 `typeDefinition`；其中 grouped alias iterable root token 的 `typeDefinition` 也已显式锁定在这条 fallback 上；同文件 helper 返回的 `[Child; 2]` 这类 iterable root 则继续复用同一条 dependency-only method query fallback。
