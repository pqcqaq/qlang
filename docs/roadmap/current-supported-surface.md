...
- `ql build/run/test/check` 已共享 project context / request-root resolution；`build/run` 复用 project source target selector，`test` 保留 project-aware test-file 语义并覆盖 workspace `--package` JSON 输出，`check` 保留 workspace-only `--package` 语义，且都有真实 CLI 回归覆盖。
