...
- `ql project dependencies` 已覆盖 standalone package local dependencies 的文本和 JSON 输出；`ql project dependencies/dependents --json` 已覆盖 workspace member source/directory 派生包名和 package selector failure，脚本不必重复传 `--name`。
