...
- `ql build` 已覆盖 workspace `--package` JSON dependency-closure 输出；`run/test` 已覆盖 workspace `--package` JSON 关键路径；`ql run --json` 已覆盖 manifest-load / target-discovery preflight JSON；`ql build/run --json --package --target` 已覆盖 selector miss JSON。
