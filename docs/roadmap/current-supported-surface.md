...
- cleanup bool guard path 现也接受最小 runtime `await` value 子集：当前已锁定 async body 内 `defer if await ready()`、cleanup `match` guard `true if await check(...)`，以及 `defer if await (if` / `match` 选出的 same-file async function item / alias 与 async callable `const` / `static` / alias `)(...)` 形态
