...
- `ql project emit-interface --changed-only` 在写出路径上只重发非 `valid` 接口；搭配 `--check` 时不会写文件，已 `valid` 的接口会报告 `up-to-date interface`；如果 check 失败，局部修复 hint 也会保留同一个 `--changed-only`，不再退回全量重发。
