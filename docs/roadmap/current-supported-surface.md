...
- sync backend 的首个 `String` 值传递子集现已进入真实 build surface：UTF-8 string literal 现可 lowering 为 `{ ptr, i64 }`，并经过 local binding、const/static materialization、普通参数传递、返回值与 fixed-shape aggregate transport 进入当前 LLVM/object build；string compare / string-pattern match lowering 与 C header `String` 导出仍保持保守关闭。
