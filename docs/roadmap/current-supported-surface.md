...
- awaited `match` scrutinee 子集
  ...
  - awaited aggregate catch-all binding family；当前也已锁定 ordinary `match await ... { current => ... }` 这类 non-scalar current-loadable scrutinee 的单名 binding catch-all 形态，现已覆盖 awaited struct / tuple aggregate scrutinee，其中 awaited root 同样可来自 runtime `if` / `match` 选出的 same-file async function item / alias 与 async callable `const` / `static` / alias
