...
- workspace/symbol 已拆出 request-scoped context/index，统一 open documents、non-file documents、workspace roots、query normalization 和 per-request 去重状态。
...
- LSP 还不是稳定 workspace service；主要编辑请求已开始共享 workspace request context，diagnostics 和 workspace/symbol 已拆出专用 request context；持久 workspace index/cache 生命周期仍需要继续统一。
