...
- `Option[T]`、`Result[T, E]`、`std.array` length-generic helpers（含 `reverse_array[T, N]`）、`std.test` generic assertions 已有真实 smoke。
- `std.array` 固定长度 helper 已删除；`repeat_array[T, N]` 等待语言支持泛型长度数组初始化。
...
- stdlib 仍有部分非数组固定 arity 和 concrete carrier 兼容层；数组 API 已向 length-generic 收敛，剩余重点是语言级数组初始化和更完整 generic backend。
