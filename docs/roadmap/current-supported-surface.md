...
- `crates/ql-cli/tests/executable_examples.rs` 会在真实本地 toolchain 上构建并运行这 142 个 async 样例，并锁定退出码；同一个 harness 也会继续运行 24 个 sync executable examples，其中现已包含 `04_sync_static_item_values.ql` 到 `24_sync_match_guard_call_backed_combos.ql`。
