...
- `ql project add-dependency/remove-dependency` 已覆盖 workspace member source/directory 和 workspace root `--package` selector，依赖编辑逻辑已从 CLI 入口拆出；`qlang.toml` 读改写已纳入输出路径级跨进程锁，并发 add/remove 同一 manifest 有真实 CLI smoke。
