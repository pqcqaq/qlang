...
- `ql build/run/test/check` 支持 project-aware 入口和常用 `--json`、`--list`、`--package`、`--target`；`ql build/run --list --json --package --target` 已覆盖包内相对 target 和 selection failure，`ql run --list --json` 已覆盖 target selector miss selection failure；`ql project targets --json` 已覆盖 target selector miss selection failure；`ql test --package` 已覆盖 workspace root/member directory/member file 的文本、JSON、`--list --json`、`--filter`、`--target` 和 selector 错误入口；`ql test` 文本路径已覆盖 package/workspace/member-file default profile 与显式 `--profile` / `--release` override；`ql test --list --json` 已覆盖 standalone package path、workspace package UI listing、workspace package UI selector miss、package/workspace/member-file/direct-file default profile 与显式 profile override、direct project smoke/UI file listing 和 direct file `--package` / `--target` / `--filter` selector listing；`ql test --json` 已覆盖 manifest-load、target-discovery（含 selected package）、direct-source/workspace/package-path/direct-file selector preflight、package/direct project smoke/ui file success、workspace/package/direct project UI snapshot failure、workspace package UI selector success、package/workspace/member-file/direct-file default profile 与显式 `--profile` / `--release` override、direct smoke/ui file combined selector success，以及 no-tests / target miss / filter miss selection failure；`ql check --package` 已覆盖 workspace root/member source/member directory 的文本、JSON、missing selector 和 `--sync-interfaces` 入口。
...
- `ql test` 已用真实 smoke 覆盖 dependency public functions、public values、generic public functions、generic wrapper/helper specialization、public struct/type alias/method/trait bridge；workspace package、package 和直接 project test 文件覆盖 UI snapshot JSON success/failure；workspace package UI listing 覆盖 `--list --json --package --target --filter` 成功和 selector miss 合同；package/workspace/member-file default profile 和显式 profile override 的文本合同已覆盖，package/workspace/member-file/direct-file 的 JSON 合同也已覆盖；直接 project test 文件也覆盖 local generic specialization 与 package-under-test public bridge、direct dependency bridge、dependency generic public function bridge 和 dependency generic wrapper/helper bridge 的组合路径，其中 dependency generic bridge 已覆盖 `--json` 成功报告，直接 smoke/UI 文件 listing、`--package`、`--target`、`--filter` selector、组合 selector listing 和组合 selector 执行已覆盖 JSON 合同。
