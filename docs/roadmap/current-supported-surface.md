...
- cleanup lowering 已不再是“全量关闭”：首个 `defer` + cleanup branch/match + 透明 `?` wrapper lowering 子集已进入真实 build 回归，并已接通 callable-value cleanup callee 与 cleanup guard-call 子路径的最小间接调用；direct local capturing closure cleanup callee 与 cleanup guard-call 的首个最小子集也已打通；broader cleanup control flow 仍保持保守拒绝。
...
- sync closure value surface 已从“仅 non-capturing”推进到首个 capturing 子集：当前开放 non-`move`、捕获 immutable same-function scalar binding，并允许原局部 direct ordinary call、immutable local alias ordinary call、direct local cleanup callee、direct local cleanup guard-call，以及 ordinary `match` guard-call；mutable alias、typed control-flow 选值、cleanup alias/control-flow 与其他逃逸路径仍关闭。
...
- 最小 first-class sync function value 子集
  ...
  - capturing sync closure value 的首个受控子集：当前只开放 non-`move` + immutable same-function scalar binding capture + direct local ordinary call、immutable local alias ordinary call、direct local cleanup callee、direct local cleanup guard-call，以及 ordinary `match` guard-call；mutable alias、typed control-flow 选值、cleanup alias / cleanup control-flow 与其他 broader callable-value flow 的路径仍保持关闭
...
- 其中 call-backed `defer` 当前已覆盖 direct resolved callee，以及 callable local / callable `const` / `static` / same-file alias 驱动的 positional indirect callee；对 capturing sync closure，还额外开放了 direct local cleanup callee 与 cleanup guard-call 的最小子集；runtime `if` / `match` typed-value path 当前也可选出 same-file function item / alias、function-item-backed callable `const` / `static` / alias，以及 closure-backed callable `const` / `static` / alias 作为 callable cleanup callee root，但 capturing closure 的 cleanup alias/control-flow 选值仍关闭
