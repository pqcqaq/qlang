...
- 同一条保守 `textDocument/typeDefinition` value slice 本轮也已补到 imported dependency root token 本身：`load().value` 上的 `load` 现在可跳到 dependency public return struct，`cfg?.value` 上的 imported dependency `const/static` root 现在可跳到解包后的 dependency public struct，而 parse-only 可恢复的 broken-source `const` root 也会继续复用同一条 dependency-only fallback；这轮仍只扩 `typeDefinition`，不改变这些 root token 的 hover / definition 语义。
