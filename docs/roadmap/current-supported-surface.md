...
- `textDocument/implementation` 已覆盖 same-file trait/type surface、workspace / 本地路径依赖 source-preferred 导航下的 `struct / enum / trait`，以及 trait method definition。same-file trait/type 返回本文件里的 `impl` / `extend` block，trait method 返回匹配的 impl method；source-preferred trait/type 会聚合当前包、可见 workspace members 与本地依赖源码里的 `impl` / `extend` / trait `impl` block，trait method definition 会聚合匹配的 `impl` method，并优先读取 parseable open docs。更宽的全局 implementation index 仍未做。
