...
- 如果 `ql build --emit-interface` 在 build 阶段就因为目标 package 源码 diagnostics、toolchain、主 build 输出路径、`dylib` 导出面不满足要求，或 build-side header 配置本身失败（header 输出路径不可写、与主 artifact 路径冲突，header 选项搭到了非 `dylib|staticlib` 上，或请求了 `imports|both` 但源码里没有 imported `extern "c"` 声明）而失败，stderr 现在也会补 `failing package manifest`，并按最终 build 选项重建直接重跑 hint（保留 `--emit` / `--release` / `--output` / C header 相关参数）；主输出路径失败时会额外补 `failing build output path`，`dylib` 缺少公开 `extern "c"` 导出时会改成先修 `dylib export surface`，header 输出失败或 collision 时会额外补 `failing build header output path`，header/emit 不兼容时会改成先修 `build header configuration`，header import 面为空时会改成先修 `build header import surface`，toolchain 失败时仍会保留 intermediate artifact 提示，但不会误报最终 build artifact 已保留。
