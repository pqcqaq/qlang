...
- `ql project dependencies/dependents` 已覆盖 standalone package root/source path 的文本和 JSON 输出；`dependencies` 支持 standalone package local dependencies 和 `--name` selector，`dependents` 支持 standalone package 空结果和 `--name` selector；workspace member source/directory 派生包名和 package selector failure 也有 JSON 回归。
- `ql project lock` 已覆盖 workspace root、workspace member source/directory 写入 workspace lockfile；standalone package source path 已覆盖 `--json` 写入和 `--check --json` up-to-date 检查；`--json` 下 manifest-load 和 lockfile-render 失败输出稳定 failure JSON。
