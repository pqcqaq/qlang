...
- `ql-cli` 主链路仍需继续拆分；主入口已收口到命令分发和少量 crate root re-export，command、pipeline、project target、interface、source rewrite、dependency bridge 和 test reporting 已模块化。剩余重点是拆 build backend glue、剩余 reporting 和 root re-export，用真实 workspace smoke 锁住行为。
