...
- `ql project graph` 会展示 package/member、references，以及默认 `.qi` 的 `valid` / `missing` / `invalid` / `stale` 状态；当状态为 `stale` 时，还会给出精简的 `stale_reasons`，指出是 manifest 还是某个源码文件比 artifact 更新。
