...
- `ql project lock` 已覆盖 workspace root、workspace member source/directory 写入 workspace lockfile；standalone package root/source path 已覆盖 `--json` 写入和 `--check --json` up-to-date 检查；repo stdlib source-only 复制 fixture 已覆盖 workspace lockfile 写入、up-to-date 和 stale failure JSON；`--json` 下 manifest-load 和 lockfile-render 失败输出稳定 failure JSON。
