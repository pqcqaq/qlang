...
- `Option[T]`、`Result[T, E]`、`std.core` / `std.array` length-generic aggregate helpers、`std.test` generic assertions 已有真实 smoke。
...
- stdlib 仍有部分固定 arity / concrete carrier 兼容层；聚合 API 已向数组泛型入口收敛，剩余重点是更完整 generic backend 和兼容层退场。
