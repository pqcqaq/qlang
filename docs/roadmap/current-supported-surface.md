...
- 同一条 imported iterable receiver truth surface 现在也已显式覆盖 grouped alias + question-wrapped root：当 `use demo.dep.{maybe_children as kids}` / `use demo.dep.{MAYBE_ITEMS as maybe_items}` 这类 grouped alias 唯一映射到 dependency `.qi` 里的 public function / `const` / `static`，且 `?` 解包后仍能唯一收敛到同一个 dependency iterable element binding 时，`for current in kids()? { current.va }` / `for current in kids()? { current.ge }` 现在都会继续进入成功 package analysis 路径下的 dependency field / method completion，而在 broken-source / semantic-error 场景下，`for current in kids()? { current.va }` / `for current in kids()? { current.ge }` 与 `for current in maybe_items? { current.va }` / `for current in maybe_items? { current.ge }` 现在也都会继续保留 dependency field / method completion fallback；前一轮已把 structured grouped-import completion matrix 全部补齐并在成功路径与 broken-source fallback 上单独回归锁住，而上一轮又把同一条 receiver/query 合同里的 member definition 补上：`for current in (if flag { kids()? } else { kids()? }) { current.value }` 这条 field definition 与 `for current in match flag { true => maybe_items?, false => maybe_items? } { current.get() }` 这条 method definition，现都已在成功路径与 broken-source fallback 上由独立 regression 锁住；本轮再把同一条 receiver/query/typeDefinition 合同里的 member typeDefinition 也补上了：`for current in (if flag { kids()? } else { kids()? }) { current.leaf.value }` 里的 `leaf` field token，以及 `for current in match flag { true => maybe_items?, false => maybe_items? } { current.leaf().value }` 里的 `leaf` method token，现也都已在成功路径与 broken-source fallback 上由独立 regression 锁住；这轮又把同一条 receiver/query 合同里的 grouped structured iterable member query 补齐：`for current in (if flag { kids()? } else { kids()? }) { current.value }` 上的 field token `hover` / `declaration` / `references`，以及 `for current in match flag { true => maybe_items?, false => maybe_items? } { current.get() }` 上的 method token `hover` / `declaration` / `references`，现也都已在成功路径与 broken-source fallback 上由独立 regression 锁住。
