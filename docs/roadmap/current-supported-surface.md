...
- 如果 `ql build --emit-interface` 在 build 阶段就因为目标 package 源码 diagnostics 失败，stderr 现在也会补 `failing package manifest`，并按最终 build 选项重建直接重跑 hint（保留 `--emit` / `--release` / `--output` / C header 相关参数）；这条路径不会再误报 build artifact 已保留。
