...
- `ql project init --stdlib` 从 `stdlib/examples/starter` 复制 starter，生成依赖 `std.core`、`std.option`、`std.result`、`std.array`、`std.test` 的 package/workspace 项目，并用 `check/run/test`、`project emit-interface`、`project emit-interface --check`、package `ql check/build/run/test --json`、package `ql project graph/status --json`、workspace `ql check --json`、workspace `ql project graph/status --json --package` 和 `ql build/run/test --json --package` 直接覆盖 generic option/result assertions、数组 equality/reverse assertions、length-generic array helpers、重复数组和 starter `.qi` 产物；check JSON 断言 stdlib interface 加载，graph/status JSON 断言生成接口和 stdlib local dependencies 可见，build JSON 断言 stdlib dependency closure、selected lib/bin 和 interface。
