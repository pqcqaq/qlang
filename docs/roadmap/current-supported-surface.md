...
- 上述 dependency-only 回退里的 `for` loop receiver 现也不再只限 direct tuple/array root；对应的 parse-only value-root fallback 也会继续复用同一 binding identity，因此 structured iterable root 下的 `for current in { (config, config) } { current.value }` / `for current in match flag { ... } { current.get() }` 也会落在同一条 dependency-only 容错合同里。
