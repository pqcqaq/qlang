...
- 第三方旧接口里的 stdlib 兼容 API 会在真实 `textDocument/completion`、`textDocument/hover` 和 `textDocument/semanticTokens/full/range` 请求中提示 deprecated 并带迁移 guidance；当前 stdlib 正式 API 使用 generic carrier 和 length-generic helpers，并有真实 stdlib package 请求 smoke 覆盖 completion、hover、definition、typeDefinition、signatureHelp 和 semantic tokens。semantic tokens 覆盖 parse-error fallback、注释 token 和 `self` keyword token。
