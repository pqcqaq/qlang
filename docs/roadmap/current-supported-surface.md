...
- 上述两条子集现在也额外支持当前简单 scalar comparison guard 子集：`Bool` `==` / `!=`，以及由 integer literal、same-file const/alias `Int`、direct same-scope `Int` local / parameter 组成的 `Int` `==` / `!=` / `>` / `>=` / `<` / `<=`。
