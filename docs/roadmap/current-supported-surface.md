...
- direct question-unwrapped dependency receiver 的最小 member token 合同现在也已由独立 regression 成对锁住：`config.child?.value` 上的 field definition / field `typeDefinition`，以及 `config.child()?.get()` 上的 method definition / method `typeDefinition`，当前都已在成功 package analysis 路径与 broken-source fallback 上保持一致，不再只覆盖单侧半矩阵
