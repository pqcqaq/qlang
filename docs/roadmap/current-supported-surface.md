...
- `textDocument/implementation` 已覆盖 same-file trait/type surface，以及 workspace / 本地路径依赖里导入的 `struct / enum / trait`。same-file trait/type 返回本文件里的 `impl` / `extend` block，trait method 返回匹配的 impl method；导入类型/trait 会聚合当前包、可见 workspace members 与本地依赖源码里的 `impl` / `extend` / trait `impl` block，并优先读取 parseable open docs。imported trait method 的跨包 implementation 与更宽的全局 implementation index 仍未做。
