...
- 赋值表达式的当前可运行子集
  - mutable local
  - tuple literal index projection，以及 same-file `const` / `static` / `use ... as ...` alias 驱动并支持 immutable direct local alias 复用的 foldable integer constant expression tuple index
  - struct field
