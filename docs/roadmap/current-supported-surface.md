...
- `ql build/run/test/check` 支持 project-aware 入口和常用 `--json`、`--list`、`--package`、`--target`；`ql build --list --json` 已覆盖 workspace member source/directory 入口，`ql test --list --json` 已覆盖 workspace member directory/test-file 入口，`ql check --package --json` 已覆盖 workspace root/member source/member directory 入口。
