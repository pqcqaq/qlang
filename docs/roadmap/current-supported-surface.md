...
- 如果 `ql build --emit-interface` 在 build 阶段就因为目标 package 源码 diagnostics 失败，stderr 现在也会补 `failing package manifest`，并给出针对原 build 源文件的直接重跑 hint；这条路径不会再误报 build artifact 已保留。
