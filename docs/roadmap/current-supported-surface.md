...
- `ramdon_tests/async_program_surface_examples/` 当前收录 97 个 async executable 样例，覆盖当前 `BuildEmit::Executable` program surface。
- `crates/ql-cli/tests/executable_examples.rs` 会在真实本地 toolchain 上构建并运行这 97 个 async 样例，并锁定退出码；同一个 harness 也会继续运行 sync executable examples，其中已包含上面的 `04_sync_static_item_values.ql`。
