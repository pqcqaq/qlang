...
- workspace 根 `ql check` / `ql check --sync-interfaces` 在 member package 源码本身报 diagnostics 时，现在也会在局部错误块后补 `failing workspace member manifest` 与针对该 member manifest 的直接 rerun hint。
