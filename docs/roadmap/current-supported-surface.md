...
- `ql-cli` 主链路仍需继续拆分；主入口已收口到命令分发，内部模块已改为直接依赖真实模块而不是 crate root re-export 中转。command、pipeline、project target、build interface emission、source rewrite、dependency bridge、build/run/test reporting 和 project build execution 已模块化。剩余重点是拆 single-source/backend prep glue，用真实 workspace smoke 锁住行为。
