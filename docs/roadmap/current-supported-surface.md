...
- broken-source 下的同名本地依赖 `workspace/symbol` 现在也补上了 `[dependencies]` 路径依赖入口；open document 和 `workspace_roots` 都已锁住 interface symbol，以及 method / trait method / extend method 的“源码优先 + 兄弟依赖 `.qi` 保留”合同。
