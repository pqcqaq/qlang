...
- `crates/ql-cli/tests/executable_examples.rs` 会在真实本地 toolchain 上构建并运行这 136 个 async 样例，并锁定退出码；同一个 harness 也会继续运行 10 个 sync executable examples，其中现已包含 `04_sync_static_item_values.ql` 到 `10_sync_for_const_static_fixed_shape.ql`。
