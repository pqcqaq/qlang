...
- `ql build --emit-interface` 会在成功 build 后写出当前 package 的默认 `.qi`；如果 build 已成功但 package 内其他源码导致接口发射失败，stderr 现在会先汇总所有 failing source file；只有多失败场景才额外补 `first failing source file`，随后再补 failing package manifest，并明确已经生成的 build artifact 仍保留在原输出路径；这条 build-side interface failure 的重跑 hint 现在也会保留原始 `ql build ... --emit-interface` 选项。
