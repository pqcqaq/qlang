...
- statement-level cleanup `for await`：当前在 async body 内开放 fixed array / homogeneous tuple iterable 的最小 lowering 子集；普通元素会直接逐项绑定，`Task[...]` 元素会复用既有 `await` + result-release 路径做逐项 auto-await，并支持 body-local `break` / `continue`
