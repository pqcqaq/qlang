...
- `[workspace].members`
- `[references].packages`
- `ql project graph [file-or-dir]` 文本输出当前 manifest graph；workspace-only 根 manifest 现在还会展开每个 member package 的 manifest、包名、默认 `.qi` 路径与 references
- `ql project emit-interface [file-or-dir] [-o <output>]`
