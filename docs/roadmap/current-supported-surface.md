...
- `ql check <package-dir> --sync-interfaces` 当前会在 package-aware check 前递归扫描本地 `[references].packages`，按默认输出位置同步写出 dependency `<package>.qi`；这个开关目前只支持 package directory / `qlang.toml` 路径，不支持普通 file-based check
