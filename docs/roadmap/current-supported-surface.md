...
- 上述两条 `match` pattern 子集现在也接受 same-file `const` bare path 及其 same-file local import alias，只要该 bare path 能折叠成 `Bool` / `Int` literal；`Int` 这条链现在也接受 unary-negated literal / const 值，以及 same-file `const` `Int` arithmetic expression 的折叠结果；`static` 与不能折叠成 `Bool` / `Int` 的 bare path pattern 仍维持显式拒绝。
