...
- build driver 对最终 artifact、链接输入和 build-side header 采用输出路径级跨进程锁，并重试 Windows 下锁目录并发创建/释放时的短暂 access denied；standalone `ql ffi header` 对 C header 输出写入加锁；`ql project emit-interface` 也对 `.qi` 输出写入加锁；`ql run/test` 在执行期继续持有可执行文件锁。并发 `ql build --package --json`、`ql project emit-interface` 默认 `.qi` 输出、`ql ffi header` 显式/default 输出、`ql run --package --json` 和 `ql test --json` 的真实 CLI smoke 已覆盖共享依赖产物、interface/header 输出与运行中 executable 的 Windows 访问冲突回归。
- `ql project graph/targets/status/emit-interface` 已覆盖 workspace root、workspace member path 和 workspace `--package` selector；`project graph/targets/status --json` 已覆盖 manifest-load failure，`project graph/targets/status --json` 已覆盖 standalone package root/source path，`project emit-interface` 已覆盖 standalone package source path 的普通、`--check`、`--changed-only` 和 `--changed-only --check` 组合，以及 workspace member source/directory；`project graph/targets/status --package --json` 已覆盖 member source/directory 入口，且 graph/status package selector failure 已输出 selection failure JSON；`ql project target add` 支持 workspace root `--package` 和 member directory 入口，且 package manifest 写入已加锁并有并发真实 CLI smoke。
- `ql project dependencies/dependents` 已覆盖 standalone package root/source path 的文本和 JSON 输出；`dependencies` 支持 standalone package local dependencies 和 `--name` / `--package` selector，`dependents` 支持 standalone package 空结果、source path JSON 和 `--name` / `--package` selector；workspace member source/directory 派生包名和 package selector failure 也有 JSON 回归。
- `ql project lock` 已覆盖 workspace root、workspace member source/directory 写入 workspace lockfile；standalone package root/source path 已覆盖 `--json` 写入和 `--check --json` up-to-date 检查；lockfile 读写采用输出路径级跨进程锁，并发 `--json` 写入 / `--check --json` 读取有真实 CLI smoke；repo stdlib source-only 复制 fixture 已覆盖 workspace lockfile 写入、up-to-date 和 stale failure JSON；`--json` 下 manifest-load 和 lockfile-render 失败输出稳定 failure JSON。
- `ql project add/remove` 已覆盖 workspace root/member source 的成员编辑；`ql project add-dependency/remove-dependency` 已覆盖 workspace member source/directory 和 workspace root `--package` selector，依赖编辑解析和编辑逻辑已从 CLI 主入口拆出；workspace/package `qlang.toml` 读改写已纳入输出路径级跨进程锁，并发 add/remove 同一 manifest 有真实 CLI smoke。
...
- `Option[T]`、`Result[T, E]`、`std.core` scalar/predicate/bool helpers、`std.core` / `std.array` length-generic aggregate/order/median helpers、`std.test` 泛型 equality/array/option/result assertions 和 `merge_statuses[N]` 已有真实 smoke；`std.array` Int/Bool aggregate helpers 委托 canonical `std.core` aggregate helpers；`std.array.count_mismatches_array[T, N]` 提供数组差异计数；`std.option` smoke 覆盖 `none_option[T]` 在 typed array、repeat array、tuple item 和 struct field 上下文中的推断；`std.test` 数组/option/result 断言内部复用 canonical `std.array` equality/access/query/reverse、`std.core` aggregate/order、`std.option`、`std.result` helper。
- stdlib package-local smoke 已改用 length-generic 状态数组聚合，不再保留测试内 `sum4` / `sum6` 固定 arity helper，也不保留为凑固定长度而写的 `0` padding。
...
- `hover`、`definition`、`declaration`、`typeDefinition`、`codeLens`、`implementation`、`references`、`documentHighlight`、`completion`、`signatureHelp`、`inlayHint`、`semanticTokens/full`、`semanticTokens/range`、`prepareRename`、`rename`、`documentLink` 已共享 workspace request context，统一 package analysis、open document overlay 和当前文档 analysis 准备；request context/open-doc snapshot 已从 LSP backend 主文件拆出。
...
- signatureHelp/inlayHint 的 workspace callable detail 已拆出独立模块，复用 same-file 和 dependency callable surface。
...
- initialize capabilities 已拆出独立模块；`qlsp` binary version 命令测试已从生产入口外置。
- formatting 已拆出独立模块；document/range/on-type formatting 复用 `ql fmt`。
...
- `ql-cli` 主入口已收口到命令分发，check/build/run/test 的主要 pipeline、reporting、target selection/discovery、execution 和 project build execution 已模块化；dependency bridge 顶层模块、dependency bridge names externs/identifiers、dependency generic bridge function-bindings/specialization-function-bindings/inferred-types/inferred-type-conversion/inferred-type-predicates/value-bindings/substitutions match helpers/expr-inference/call-args/call-inference/instantiation-scan-context/instantiation-block-scanner/instantiation-call-scanner/instantiation-scanner callable-body/instantiation-expr-scanner traversal helpers/specializations render helpers/specialized-forwarders target discovery/rendering/instantiations、通用 CLI utility、CLI scan/path helper、project manifest path/reporting、project workspace member lookup/selection、project graph selection/json/reporting、project dependencies/status/interface reporting、project interface reporting commands/checks、project dependencies query context/rendering/json、project status collection/rendering/json、project interfaces emission/reference prep、project reference interfaces reporting/sync、single-source input、check command project execution/reporting、run pipeline/target/reporting/execution/project path、test command/discovery selection/execution、project init scaffold/templates/member edits/lock/interface/reference interface emit reporting/targets/target build prep、project members target add/workspace member edits、project lock JSON/text reporting/check status、project emit-interface preflight/package/workspace emit/check handling、build command/failure/json/output/plan dependency resolution/project execution/single-source reporting/build reporting 测试已从生产模块外置，dependency generic bridge 核心单元测试已外置，build reporting build-result rendering、failure envelope/detail builder、workspace target loading/selection、project target selector、project targets selection/rendering/path scope/loader 已从主文件拆出，ql-cli root unit tests 已拆成 analysis/build output/dependency bridge/support 分组。剩余重点是继续拆 CLI 大边界并外置剩余内联测试，用真实 workspace smoke 锁住行为。
...
- LSP 还不是稳定 workspace service；主要编辑请求已开始共享 workspace request context，request context/open-doc snapshot、initialize capabilities、codeLens、diagnostics、documentLink、formatting、signatureHelp/inlayHint callable hints 和 workspace/symbol 已拆出独立模块；持久 workspace index/cache 生命周期仍需要继续统一。
- stdlib public API 已清掉 concrete carrier、主要固定 arity 包装和 `std.test` concrete carrier / 固定 arity facade；`std.core` package-local smoke 已覆盖公开 scalar/predicate/bool helpers，`std.result` package-local smoke 和 `project init --stdlib` starter 已直接覆盖 generic carrier 语义与 option/result assertions。剩余重点是更完整 generic backend、共享 project pipeline 和更宽 dependency-aware backend。
