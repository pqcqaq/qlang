...
- build driver 对最终 artifact、链接输入和 build-side header 采用输出路径级跨进程锁，并重试 Windows 下锁目录并发创建/释放时的短暂 access denied；standalone `ql ffi header` 对 C header 输出写入加锁；`ql project emit-interface` 也对 `.qi` 输出写入加锁；`ql run/test` 在执行期继续持有可执行文件锁。并发 `ql build --package --json`、`ql ffi header` 显式/default 输出、`ql run --package --json` 和 `ql test --json` 的真实 CLI smoke 已覆盖共享依赖产物、header 输出与运行中 executable 的 Windows 访问冲突回归。
...
- `ql-cli` 主链路仍需继续拆分；共享路径/JSON 字符串转义/诊断渲染和打印/diagnostic JSON 渲染/CLI analysis helper/project reporting helper/project interface reporting helper/reference-interface helper/包名校验/project manifest error helper/reference/workspace manifest path helper、文件扫描、版本输出、usage 输出、check JSON reporting、build JSON reporting、test JSON reporting 和 check 命令失败提示已从 `main.rs` 抽出，`ql check`、`ql build`、`ql run`、`ql test` 根入口已下沉到各自 command 模块，`ql build` 单文件和 project pipeline 已移入 `build_pipeline`，`ql run` project pipeline、JSON 报告和 executable 执行锁已移入 `run_pipeline`，`ql test` listing/execution/UI snapshot 执行层已移入 `test_pipeline`，build/run/test 共享 profile 参数逻辑已抽到独立模块，build/run target selector/listing 与 workspace target loading 已直接归入 `project_targets`，project graph/status/dependencies 查询模块和 project member/lock/workspace lookup 入口已改为直接依赖目标模块，interface artifact emission、quiet dependency interface prep 和 reference manifest path 解析已抽到 `project_interfaces` / `project_manifest_paths`。剩余重点是拆 test discovery/selection pipeline、build backend helper、request-context 和剩余 reporting，用真实 workspace smoke 锁住行为。
- `ql test` 的 package-under-test/direct-dependency bridge source override 已覆盖 package path 和直接 project test file 的 local generic 组合；`test_reporting` 已承载 target/report/failure JSON 合同，`test_pipeline` 已承载 listing/execution/UI snapshot 执行层。剩余重点是继续拆出 discovery/selection，并与 build/run 已拆出的 project pipeline 保持 selector、profile、JSON failure 合同一致。
