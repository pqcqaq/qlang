...
- direct question-unwrapped dependency receiver 的最小 member token 合同现在也已由独立 regression 全矩阵锁住：`config.child?.value` 上的 field hover / declaration / references / definition / field `typeDefinition`，以及 `config.child()?.get()` 上的 method hover / declaration / references / definition / method `typeDefinition`，当前都已在成功 package analysis 路径与 broken-source fallback 上保持一致，因此这条 direct question-unwrapped receiver 的 member query / definition / `typeDefinition` 合同现已闭合
