...
- 同构 inline tuple / array destructuring 产生的 dependency locals 现也支持 value-root hover / definition / declaration / references，并可继续进入 named-local member `typeDefinition`，例如 `let (first, second) = (config.child, config.child)` 与 `let [first, second] = [config.child, config.child]`
