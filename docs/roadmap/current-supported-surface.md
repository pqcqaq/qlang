...
- `ql-cli` 主链路仍需继续拆分；主入口已收口到命令分发，内部模块已改为直接依赖真实模块而不是 crate root re-export 中转。command、pipeline、project target、project target build prep、single-source input/reporting、build interface emission、source rewrite、dependency bridge、build/run/test reporting、run project pipeline、run target selection、run executable execution、test execution 和 project build execution 已模块化。`ql-driver` build pipeline 已拆出 codegen preparation、output resolution、emit dispatch、toolchain emission workspace、直接 IR/object-backed emission 模板和 workspace 级 failure/cleanup/promotion helper；build/run/test 复用的 build failure JSON 已统一 target identity 字段，build 文本 failure hint/note 已共享 manifest/rerun/artifact 文本构造，check JSON report、preflight/interface/build-plan/target-prep failure JSON 已共享 envelope 构造，target-prep details、interface/dependency-interface details、dependency bridge prep error 构造、direct dependency 和 package-under-test bridge 文本 failure/conflict/source 报告已命名化，public export wrapper prep 文本出口已收口到 bridge reporting，extern/public function/value/type 和 generic specialization bridge error mapping 已拆出共享模块，generic specialization rendering helpers、extern declaration collection、package-under-test collection、public function/method forwarder、value/type declaration collection 已拆出，extern interface traversal 和 function/method/value/type dependency source module 遍历已共享。剩余重点是继续拆 dependency bridge 模块边界，用真实 workspace smoke 锁住行为。
