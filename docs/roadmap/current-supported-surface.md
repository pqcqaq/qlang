...
- `ql fmt --write`、project manifest、lockfile 和 `.qi` interface 等关键 CLI 写入使用路径级锁配合同目录临时文件替换；并发格式化、manifest 编辑和 lockfile 读写有真实 CLI smoke。
...
- build driver 对最终 artifact、链接输入和 build-side header 采用输出路径级跨进程锁；`ql project emit-interface` 也对 `.qi` 输出写入加锁；`ql run/test` 在执行期继续持有可执行文件锁。并发 `ql build --package --json`、`ql run --package --json` 和 `ql test --json` 的真实 CLI smoke 已覆盖共享依赖产物与运行中 executable 的 Windows 访问冲突回归。
