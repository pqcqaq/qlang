...
- build driver 对最终 artifact、链接输入和 build-side header 采用输出路径级跨进程锁；`ql project emit-interface` 也对 `.qi` 输出写入加锁。并发 `ql build --package --json` 共享 dependency closure 的真实 CLI smoke 已覆盖 Windows 产物访问冲突回归。
