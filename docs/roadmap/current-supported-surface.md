...
- workspace 根 `ql project emit-interface` 在单个 member 发射失败时不会立刻中断；已成功的 members 会继续输出，最后再汇总失败成员数；只有多失败场景才额外补 `first failing member manifest`。如果某个 member 是因为 package 源码错误而发射失败，stderr 现在会当场同时补该 member 的 `failing package manifest`、`failing workspace member manifest` 和可直接重跑的 hint；如果 member manifest 自身无法加载，局部错误块也会立即补 `failing workspace member manifest`，不需要只靠最终汇总回看。
...
- workspace 根 `ql project emit-interface --check` 在单个 member manifest 无法加载时也不会立刻中断；已检查 members 会先输出，member manifest 加载失败的局部错误块现在也会立即补 `failing workspace member manifest`；如果 member 的默认 `.qi` 自身 `missing` / `invalid` / `unreadable` / `stale`，局部错误块现在也会同时补 `failing package manifest` 和 `failing workspace member manifest`。最后统一汇总 failing members；只有多失败场景才额外补 `first failing member manifest`。
