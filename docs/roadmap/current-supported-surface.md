...
- repo stdlib workspace 的 source-only 复制 fixture 已覆盖 `ql check --sync-interfaces --json`、后续 `ql check --json`、全量 `ql test` 和 `ql test --json`；仓库内 stdlib 已覆盖 `project emit-interface --check`、`project status --json`、`project graph --json` 和 starter `project dependencies --json`；6 个 package/example smoke、接口同步产物与可执行产物存在性作为 downstream 可用性 gate。
