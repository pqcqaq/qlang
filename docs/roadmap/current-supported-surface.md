...
- `Option[T]`、`Result[T, E]`、`std.core` scalar/predicate/bool helpers、`std.core` / `std.array` length-generic aggregate/order/median helpers、`std.test` 泛型 equality/array/option/result assertions 和 `merge_statuses[N]` 已有真实 smoke；`std.array` Int/Bool aggregate helpers 委托 canonical `std.core` aggregate helpers；`std.option` smoke 覆盖 `none_option[T]` 在 typed array、repeat array、tuple item 和 struct field 上下文中的推断；`std.test` 数组/option/result 断言内部复用 canonical `std.array` access/query/reverse、`std.core` aggregate/order、`std.option`、`std.result` helper。
