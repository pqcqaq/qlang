...
- same-file rename 现在也会把更深层 struct-like 简写绑定（如 `Point.Scope.Config { x }`）改写成显式绑定（如 `Point.Scope.Config { x: coord_x }`）；同一层上的显式字段标签（如 `Point.Scope.Config { x: value }`）也已进入 same-file field hover / definition / references / rename / semantic tokens；更深层 struct-like literal / pattern 路径里的字段标签补全（如 `Point.Scope.Config { fl: value }`）现在也会直接收敛到同一 field surface。
