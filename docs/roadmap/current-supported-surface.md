...
- workspace 根 `ql check` / `ql check --sync-interfaces` 在其他 member manifest 加载或解析失败时，局部 error line 现在也会保留真实命令标签，并继续补 `failing workspace member manifest` 与针对该 member manifest 的直接 rerun hint。
