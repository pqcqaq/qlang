...
- broken-source / parse-error 下，当前只保留保守子集，不等于完整恢复；workspace 外本地路径依赖的 import references fallback、dependency struct field label completion、dependency enum variant 的 `completion/definition/typeDefinition/references/documentHighlight`、dependency value/member semantic tokens fallback，以及 direct imported-result member query（如 `build().ping()` / `build().value`）都会继续走源码优先路径；同名本地依赖按 manifest 身份区分，不会串到兄弟依赖实例。
