...
- dependency struct local-value member path 的首个 method-query contract 现也已开放：当 `use demo.dep.Config as Cfg` 能唯一映射到 dependency public struct，并且 `.qi` 中存在对该 nominal struct 的唯一 public method（遵守 impl 优先、再看 extend 的当前同文件规则）时，`config.get()` / `built.get()` 这类 member method token 现在也已支持 cross-file hover / definition / references；当前这条能力只覆盖 source analysis 成功时、能从语法局部恢复出 dependency struct receiver 的 named local value，不扩展到 broken-source fallback、method completion、任意表达式 receiver 或 broader dependency member semantics
