...
- `[workspace].members`
- `[references].packages`
- `ql project graph [file-or-dir]` 文本输出当前 manifest graph；package manifest 现会额外显示当前包默认 `.qi` 路径/状态与引用 interface 状态，workspace-only 根 manifest 则会继续展开每个 member package 的 manifest、包名、默认 `.qi` 路径/状态、references 与引用 interface 状态
- `ql project emit-interface [file-or-dir] [-o <output>]`
