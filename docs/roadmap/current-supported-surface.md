...
- 如果 `ql build --emit-interface` 在 build 阶段就因为目标 package 源码 diagnostics、toolchain、主 build 输出路径，或 build-side header 输出路径本身不可写/与主 artifact 路径冲突而失败，stderr 现在也会补 `failing package manifest`，并按最终 build 选项重建直接重跑 hint（保留 `--emit` / `--release` / `--output` / C header 相关参数）；主输出路径失败时会额外补 `failing build output path`，header 输出失败或 collision 时会额外补 `failing build header output path`，toolchain 失败时仍会保留 intermediate artifact 提示，但不会误报最终 build artifact 已保留。
