...
- `ql-cli` 主链路仍需继续拆分；主入口已收口到命令分发，内部模块已改为直接依赖真实模块而不是 crate root re-export 中转。command、pipeline、project target、project target build prep、single-source input/reporting、build interface emission、source rewrite、dependency bridge、build/run/test reporting 和 project build execution 已模块化。`ql-driver` build pipeline 已拆出 codegen preparation、output resolution、emit dispatch、toolchain emission workspace、直接 IR/object-backed emission 模板和 workspace 级 failure/cleanup/promotion helper；build/run/test 复用的 build failure JSON 已统一 target identity 字段，build 文本 failure hint/note 已共享 manifest/rerun/artifact 文本构造，preflight/interface/build-plan/target-prep failure JSON 已共享 envelope 构造，target-prep detail tuple 已拆成命名 details，剩余重点是 interface/dependency-interface prep 错误归一化重复，用真实 workspace smoke 锁住行为。
