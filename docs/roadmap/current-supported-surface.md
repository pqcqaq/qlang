...
- direct indexed iterable receiver 现也开放同一最小 slice；question-unwrapped direct indexed receiver 也落在同一合同内，例如 `config.children[0].value`、`config.children()[0].get()`、`config.maybe_children()?[0].value`、`config.maybe_children()?[0].get()`，以及 grouped alias 形态 `kids()?[0].value`、`kids()?[0].get()`；同轴的 member `typeDefinition` 目标如 `config.maybe_children()?[0].leaf`、`config.maybe_children()?[0].leaf()`、`kids()?[0].leaf`、`kids()?[0].leaf()` 也已打通；direct structured question-indexed receiver 例如 `(if flag { maybe_children()? } else { maybe_children()? })[0].value` / `.get()` 与 `(match flag { ... })[0].value` / `.get()` 也已落进同一最小 member 合同；grouped alias direct indexed bracket target 的 value-root 现也支持 hover / definition / declaration / references / `typeDefinition`
