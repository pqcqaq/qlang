...
- task-producing same-file `const` / `static` / item-alias root 进入 `for await` / cleanup `for await`；当前 const item lowering 仍只覆盖 foldable scalar / loadable aggregate 子集，不覆盖 `worker(...)` 这类 runtime task-producing initializer
