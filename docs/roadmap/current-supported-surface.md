...
- 更广义动态 guard 的 `match`（包括任意表达式 guard、当前 arm 新绑定名直接参与 guard、以及不具备 guaranteed fallback coverage 的 direct name guard）、非 `Bool` / `Int` scrutinee `match`、以及超出 `Bool true|false|_|single-name binding` / `Int literal|_|single-name binding` 的更广义 match pattern lowering。
