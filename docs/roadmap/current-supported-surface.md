...
- `ramdon_tests/async_program_surface_examples/` 当前收录 140 个 async executable 样例，覆盖当前 `BuildEmit::Executable` program surface。
...
- `137_async_main_const_tuple_for_await.ql`、`138_async_main_static_array_for_await.ql`、`139_async_main_import_alias_projected_const_tuple_for_await.ql` 与 `140_async_main_import_alias_projected_static_array_for_await.ql` 则把 same-file `const` / `static` fixed-shape root 及其 same-file import-alias projected root 的 scalar `for await` lowering 收口到真实 `async fn main` build-and-run 样例。
- `crates/ql-cli/tests/executable_examples.rs` 会在真实本地 toolchain 上构建并运行这 140 个 async 样例，并锁定退出码；同一个 harness 也会继续运行 10 个 sync executable examples，其中现已包含 `04_sync_static_item_values.ql` 到 `10_sync_for_const_static_fixed_shape.ql`。
