...
- `Option[T]`、`Result[T, E]`、`std.array` length-generic helpers（含 `reverse_array[T, N]`、`repeat_array[T, N]`）、`std.test` generic assertions 已有真实 smoke。
...
- stdlib 仍有部分非数组固定 arity 和 concrete carrier 兼容层；数组 API 已向 length-generic 收敛，剩余重点是更完整 generic backend。
