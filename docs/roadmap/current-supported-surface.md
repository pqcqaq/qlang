...
- aliased projected-root aggregate repackage / submit
  ...
  - source-root reinit 后的 same-file arithmetic item / same-file `use ... as ...` alias 驱动 direct alias-root、projected-root、alias-sourced composed-dynamic 与 guard-refined alias-sourced composed-dynamic 形态，包括 bundle-alias-forwarded、queued-root-forwarded、queued-local-forwarded 与 bundle-chain-forwarded await/spawn
