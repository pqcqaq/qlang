...
- build driver、standalone/build-side C header、`ql fmt --write`、project manifest、lockfile 和 `.qi` interface 等关键文件写入使用同目录临时文件替换；并发格式化、standalone `ql ffi header`、manifest 编辑和 lockfile 读写有真实 CLI smoke。
