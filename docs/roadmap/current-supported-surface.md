...
- `ql-cli` 主入口已收口到命令分发，check/build/run/test 的主要 pipeline、reporting、target selection/discovery、execution 和 project build execution 已模块化；dependency bridge 的 error mapping/reporting/collection/rendering、imports、extern declarations、interface traversal、direct dependency source traversal、direct/package-under-test bridge、public export wrappers、public function/method forwarder、public value/type declaration collection 已分层，dependency bridge 顶层模块测试已从生产模块外置。剩余重点是继续拆更宽 CLI 模块边界，用真实 workspace smoke 锁住行为。
