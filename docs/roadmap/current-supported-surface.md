...
- `ql-cli` 主链路仍需继续拆分；共享路径/JSON 字符串转义/包名校验/project manifest error helper 已从 `main.rs` 抽到 `cli_utils`，build/run/test 共享 profile 参数逻辑已抽到独立模块，build/run target selector/listing 入口已直接依赖 `project_targets`，`build/run/test/check` 入口 request-context 和 project workspace member lookup 已部分共享。剩余重点是继续拆 request-context/reporting，并用真实 workspace smoke 锁住行为。
