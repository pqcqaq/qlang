...
- `crates/ql-cli/tests/executable_examples.rs` 会在真实本地 toolchain 上构建并运行这 106 个 async 样例，并锁定退出码；同一个 harness 也会继续运行 sync executable examples，其中已包含上面的 `04_sync_static_item_values.ql` 与 `05_sync_named_call_arguments.ql`。
