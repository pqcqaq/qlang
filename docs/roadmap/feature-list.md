...
| 功能 | 级别 | 说明 |
| --- | --- | --- |
| `Option[T]` / `Result[T, E]` | P0 | 错误和缺失值的统一载体；当前 `std.option` / `std.result` 已开放 generic carrier 和首批 generic helper，concrete `IntOption` / `BoolOption` / `IntResult` / `BoolResult` 保留为兼容面 |
