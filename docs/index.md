...
- 当前 build surface 已开放 fixed-array `for` 与最小 literal `match` lowering，其中 `Bool true|false|_|single-name binding` 与 `Int literal|_|single-name binding` 两条 `match` 子集都已可真实编译；两者都额外接受 literal / same-file const-backed `Bool` guard 及其 same-file `use ... as ...` 别名，以及由当前已支持布尔 guard 操作数组成的简单 `==` / `!=` 比较；`Bool` scrutinee 子集支持带 guaranteed fallback coverage 的 direct same-scope `Bool` local / parameter name guard，而 `Int` scrutinee 子集支持带 later unguarded catch-all fallback 的 integer-literal-arm / guarded-catch-all-arm direct same-scope `Bool` local / parameter name guard
