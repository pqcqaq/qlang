...
- 当前 build surface 已开放 fixed-array `for` 与最小 literal `match` lowering，其中 `Bool true|false|_|single-name binding` 与 `Int literal|_|single-name binding` 两条 `match` 子集都已可真实编译，并额外接受 literal / same-file const-backed `Bool` guard 及其 same-file `use ... as ...` 别名
