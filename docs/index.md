...
- 当前 `match` guard 的标量子集现在也覆盖 direct resolved sync scalar calls：返回 `Bool` 的 local / same-file import-alias 调用可直接作为 guard，返回 `Int` 的同类调用可进入当前标量比较子集；聚合参数 guard call 与更广义调用形态仍保持拒绝。
