...
这一版不是“展示几段漂亮示例”，而是要给 Qlang 一套足够完整的表层语法骨架，后续 parser、formatter、LSP、语义分析和文档生成都应以这份草案为基础。
...
Qlang 的表层语法会保留静态语言的清晰结构，但尽量避开最 Rust 化的视觉元素。当前定稿方向如下：
...
- `none` 只代表 `Option[T]` 的空分支，不是任意类型都可接收的空引用
- `f"..."` 作为唯一官方插值字符串写法，避免多套模板字符串并存
