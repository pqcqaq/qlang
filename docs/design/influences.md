...
Qlang 吸收 Kotlin、Go、TypeScript 中已经证明有效的工程经验，但不照搬它们的历史包袱。主要借鉴方向：
...
Qlang 直接吸收 Kotlin smart cast 的核心收益：
...
这样可以增强可读性，同时控制隐式行为范围。

### 4. 命名参数和默认参数

这项设计适合配置型、服务端和工具链 API，继续保留并作为公共 API 的推荐风格。

## 从 Go 借什么

### 1. 工具链单一真相源

Qlang 继续吸收 Go 的单一工具链路径：
...
Qlang 已经引入 `defer` 方向，后续继续保证它与析构模型、`unsafe` 资源包装和异步取消语义兼容。
...
Qlang 虽然走结构化并发路线，但仍会借鉴取消原因、超时和上下文传播，并将其标准化到 async 运行时。

## 从 TypeScript 借什么

### 1. 控制流驱动的类型收窄

Qlang 会吸收 TypeScript 在 discriminated unions 和 narrowing 上的经验：
...
`satisfies` 可以在保留具体推断结果的同时检查形状约束，适合引入：
...
Qlang 的方向是吸收这些语言中已经验证有效的部分，增强系统级语言的工程体验。
