...
- 已补充 `dylib` 路径上的 async backend 回归：当前在存在合法同步 `extern "c"` 导出时，最小 library-style async body 已可稳定通过；`dylib` 上的 `for await` 与更广义 async surface 仍会给出稳定诊断
...
- 已在 `ql-codegen-llvm` 打开最小 place projection lowering：当前嵌套 struct field read、constant tuple index read、array index read 已走统一投影链并进入 LLVM lowering；struct-field / constant tuple-index write 也已接入同一条 lowering，而 array element assignment 仍保持关闭
...
- 已在 `ql-typeck` / `ql-borrowck` / `ql-codegen-llvm` / `ql-driver` / `ql-cli` 锁住 aggregate-carried task-handle payload 合同：`async fn outer() -> (Task[Int], Task[Int])`、`async fn outer() -> [Task[Int]; 2]` 与带 `Task[Wrap]` 字段的 struct 结果现在都允许先 `await outer()`，再通过 `await pair[0]` / `await pair[1]`、`await tasks[0]` / `await tasks[1]` 与 `await pending.first` / `await pending.second` 继续消费内部 task handle；这条路径复用现有 loadable aggregate result、projection-sensitive consume 与 projection await lowering，而不额外引入第二套 await-join 协议
...
- 已在 `ql-driver` 放开 public async build 子集：`staticlib` 现在允许已被 backend 支持的 async library body + scalar/task-handle/tuple/array/struct/void `await` + `spawn` task-handle 绑定/await 路径通过构建，并对 fixed-array iterable 打开首个 `for await` lowering；`dylib` 也已开放最小 library-style async body 子集，只要公开导出面仍收敛在同步 `extern "c"` C ABI。async program entry、更广义 `dylib` surface 与更广义 async iteration 仍保持保守拒绝
...
- 当前 backend/driver 对这些 hook 已进入“declaration + async body wrapper + frame hydration + scalar/task-handle/tuple/array/struct/void await lowering（含 aggregate-carried task-handle payload）+ task-handle-aware spawn lowering + fixed-array literal index projected consume + fixed-array `for await` lowering + `staticlib` / 最小 `dylib` library 子集开放”阶段，但这仍不代表更广义的 async iteration 协议、任务结果协议、frame 生命周期管理或调度语义已经进入可执行阶段
...
- 当前 `async-iteration` 已不再只是纯失败合同：`staticlib` async library body 内的 fixed-array `for await` 已走通首个 lowering 竖切片；但共享 runtime hook / capability 仍主要承担 ABI 预留语义，不代表通用 async iterator 调度协议已经冻结
