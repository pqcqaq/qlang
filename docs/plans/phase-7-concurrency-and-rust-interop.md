...
- 已在 `crates/ql-runtime` 起草第一版共享 runtime hook ABI skeleton：当前固定 `async-task-create`、`executor-spawn`、`task-await`、`async-iter-next` 及其稳定符号名，用于后续 backend/runtime 接线
...
- 当前 hook ABI skeleton 只固定名称、符号名和责任边界，还没有冻结真实 LLVM 签名、调用约定或内存布局
