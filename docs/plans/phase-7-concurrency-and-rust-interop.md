...
- 已在 `ql-codegen-llvm` / `ql-driver` / `ql-cli` 补上 library-mode async 边界回归：当前 statement-position fire-and-forget `spawn` 已有独立 backend/driver/CLI 正向覆盖，`spawn` result value / `for await` 仍保持 unsupported 覆盖，`await` 则保留缺失 hook / 不支持结果布局等边界回归，且 `for await` 不再额外泄露泛化的 ``for`` lowering 或 iterable 预物化噪声
...
- 已在 `ql-codegen-llvm` 打开首个真实 `spawn` lowering 子集：当前在 backend 内支持 statement-position fire-and-forget `spawn <async-call>`，把它降成“读取 task handle -> `qlrt_executor_spawn(ptr null, task)` 并丢弃返回值”；`spawn` result value 仍保持显式 unsupported，避免过早冻结 join handle 语义
- 已在 `ql-driver` 放开第一条 public async build 子集：`staticlib` 现在允许已被 backend 支持的 async library body + scalar/tuple/void `await` + statement-position fire-and-forget `spawn` 通过构建；`spawn` result value、`for await`、struct/array async result、`dylib` async 与 program async 入口仍保持保守拒绝
...
- 当前仍保持 conservative 类型策略：`spawn` 结果类型仍保留 `Unknown`，虽然 backend 已能 lower statement-position fire-and-forget `spawn`，但 result value / join handle 语义仍未冻结；`await` 暂不引入 Future/effect 全类型建模
