...
- 当前 borrowck 只对 direct-local task handle 建立最小 consume 合同；`await` / `spawn`、静态可判定的 helper `Task[T]` 参数传递和 direct-local `return task` 已经接入，tuple/struct-field 与 fixed-array literal index projected task-handle 也已有 precise consume/write-reinit 合同；动态 task-handle 数组写入现只打开首条保守 write-only 子集，`tasks[index] = worker()` 在未污染 move-path 时可直接通过，而已有 consume 后的后续读取只会被降级成 maybe-moved，不代表 dynamic index 的精细 place-sensitive reinit 已完成；更广义的 helper 返回/drop 边界、place-sensitive handle move/drop 与更广义提交协议仍待后续切片
