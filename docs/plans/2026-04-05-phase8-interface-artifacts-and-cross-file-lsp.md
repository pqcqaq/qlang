...
- 同一 receiver slice 现已再向前推进一格：当 dependency iterable 来自 direct field projection 或 direct method result 时，indexed receiver 也已开放同一最小 member contract，例如 `config.children[0].value`、`config.children()[0].get()`
