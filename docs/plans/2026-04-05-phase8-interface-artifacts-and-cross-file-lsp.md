...
3. 最后再逐步开放受限 workspace rename / code actions。
...
5. 在 identity 与 workspace edit graph 稳定后，再做受限 workspace rename
...
- 更广义的 cross-file rename（受限 source-backed dependency / workspace root rename 已在后续迭代开放）
- 更广义的 code actions（auto-import / missing dependency import 已在后续迭代开放）
- call hierarchy
...
- 同一 receiver slice 现已再向前推进一格：当 dependency iterable 来自 direct field projection、direct method result、question-unwrapped direct method result，或 grouped import alias 的 question-unwrapped root 时，indexed receiver 也已开放同一最小 member contract，例如 `config.children[0].value`、`config.children()[0].get()`、`config.maybe_children()?[0].value`、`config.maybe_children()?[0].get()`、`kids()?[0].value`、`kids()?[0].get()`
...
- imported dependency struct alias root 现已在原始显式字段标签 completion + hover / definition / declaration / references 之上，小步扩到 shorthand field token、named local value 的 `value.field` member token，以及同一 syntax-local receiver slice 上的唯一 `value.method` member token；其中 `value.field` / `value.method` 现都已覆盖成功分析路径与 broken-source fallback 下的 hover / definition / declaration / references，并额外开放同一 slice 上的最小 field / method completion；这条 slice 现也覆盖 direct indexed iterable receiver，包括 `config.maybe_children()?[0].value`、`config.maybe_children()?[0].get()` 这类 question-unwrapped direct indexed receiver、grouped alias 形态 `kids()?[0].value` / `kids()?[0].get()`、direct structured question-indexed 形态 `(if flag { maybe_children()? } else { maybe_children()? })[0].value` / `.get()` 与 `(match flag { ... })[0].value` / `.get()`，以及同轴的 member `typeDefinition` 跳转如 `config.maybe_children()?[0].leaf`、`config.maybe_children()?[0].leaf()`、`kids()?[0].leaf`、`kids()?[0].leaf()`、`(if flag { maybe_children()? } else { maybe_children()? })[0].leaf`、`(match flag { ... })[0].leaf()`；grouped alias 与 direct structured question-indexed bracket target 的 value-root hover / definition / declaration / references / `typeDefinition` 现也已落进同一保守边界，但不会自动扩展到任意表达式 receiver 或更广义 dependency member completion
- 仍不做更广义 cross-file rename；后续只在 identity 与 workspace edit graph 已稳定的切片里开放受限 workspace rename
