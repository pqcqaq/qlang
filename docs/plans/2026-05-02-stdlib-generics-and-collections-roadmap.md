...
Status: sixth executable slice landed. `std.array` now provides generic fixed-array accessors `first3_array` / `first4_array` / `first5_array`, `last3_array` / `last4_array` / `last5_array`, fallback-index helpers `at3_array_or` / `at4_array_or` / `at5_array_or`, reverse helpers `reverse3_array` / `reverse4_array` / `reverse5_array`, repeat helpers `repeat3_array` / `repeat4_array` / `repeat5_array`, query helpers `contains3_array` / `contains4_array` / `contains5_array`, and count helpers `count3_array` / `count4_array` / `count5_array`, alongside concrete fixed-array helpers for `Int` and `Bool`. The query/count helpers are generic at the API boundary but rely on current builtin equality lowering; smoke coverage is intentionally `Int`, `Bool`, and `String`, not user-defined equality. `std.test` now depends on `std.array` and exposes `expect_int_array_*` / `expect_bool_array_*` helpers for concrete array aggregates and generic accessor / fallback-index / reverse / repeat / contains / count calls. Package-local smoke tests cover `Int`, `Bool`, and `String` query instantiations plus pass/failure assertion paths, and `ql project init --stdlib` package/workspace templates consume `std.array.first3_array` / `std.array.at3_array_or` / `std.array.reverse3_array` / `std.array.repeat3_array` / `std.array.contains3_array` / `std.array.count3_array` in generated lib/main code while generated smoke uses `std.test` array assertions instead of hand-written array status expressions. Next work in this task should move aggregate helpers toward generic array APIs once broader generic collection execution is available.
