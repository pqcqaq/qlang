...
Status: third executable slice landed. `std.array` now provides generic fixed-array accessors `first3_array` / `first4_array` / `first5_array` and `last3_array` / `last4_array` / `last5_array`, alongside concrete fixed-array helpers for `Int` and `Bool`. `std.test` now depends on `std.array` and exposes `expect_int_array_*` / `expect_bool_array_*` helpers for concrete array aggregates and generic accessor calls. Package-local smoke tests cover `Int` and `Bool` accessor instantiations plus pass/failure assertion paths, and `ql project init --stdlib` package/workspace templates consume `std.array.first3_array` in generated lib code while generated smoke uses `std.test` array assertions instead of hand-written array status expressions. Next work in this task should move aggregate helpers toward generic array APIs once broader generic collection execution is available.
