...
- The current API shape is still partly concrete because backend and dependency bridge support only cover a narrow generic function import/call slice. Generic public type execution is partially open for explicit `struct` / `enum` instantiations in concrete signatures and contextual aggregate usage, and `std.array` now has canonical executable length-generic `[T; N]` `first_array` / `last_array` accessors while the remaining fallback-index / reverse / repeat / contains / count helpers are still fixed-length transition APIs.
...
Status: eighth executable slice landed. `std.array` now provides canonical length-generic fixed-array `first_array[T, N]` / `last_array[T, N]` / `at_array_or[T, N]` / `contains_array[T, N]` / `count_array[T, N]`, keeps `first3_array` / `first4_array` / `first5_array`, `last3_array` / `last4_array` / `last5_array`, `at3_array_or` / `at4_array_or` / `at5_array_or`, `contains3_array` / `contains4_array` / `contains5_array`, and `count3_array` / `count4_array` / `count5_array` as compatibility APIs, and still provides fixed-length reverse helpers `reverse3_array` / `reverse4_array` / `reverse5_array`, repeat helpers `repeat3_array` / `repeat4_array` / `repeat5_array`, alongside concrete fixed-array helpers for `Int` and `Bool`. The query/count helpers are generic at the API boundary but rely on current builtin equality lowering; smoke coverage is intentionally `Int`, `Bool`, and `String`, not user-defined equality. `std.test` now depends on `std.array` and exposes `expect_int_array_*` / `expect_bool_array_*` helpers for concrete array aggregates and generic accessor / fallback-index / reverse / repeat / contains / count calls. Package-local smoke tests cover canonical `first_array` / `last_array` / `at_array_or` / `contains_array` / `count_array`, compatibility accessors, `Int` / `Bool` / `String` query instantiations, and pass/failure assertion paths; `ql project init --stdlib` package/workspace templates consume canonical `std.array` access/query/count helpers in generated lib/main code while generated smoke uses `std.test` array assertions instead of hand-written array status expressions. Next work in this task should move reverse/repeat/aggregate helpers toward generic array APIs once broader generic collection construction/execution is available.
