...
- Generic `Option[T]` / `Result[T, E]` are in the P0 stdlib unblock path; `std.option` and `std.result` both have first executable generic helper slices, while generic constructors and wider monomorphization remain open.
