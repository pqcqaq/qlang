...
Status: fourth executable slice landed. `std.array` now provides generic fixed-array accessors `first3_array` / `first4_array` / `first5_array`, `last3_array` / `last4_array` / `last5_array`, and fallback-index helpers `at3_array_or` / `at4_array_or` / `at5_array_or`, alongside concrete fixed-array helpers for `Int` and `Bool`. `std.test` now depends on `std.array` and exposes `expect_int_array_*` / `expect_bool_array_*` helpers for concrete array aggregates and generic accessor / fallback-index calls. Package-local smoke tests cover `Int` and `Bool` accessor instantiations plus pass/failure assertion paths, and `ql project init --stdlib` package/workspace templates consume `std.array.first3_array` / `std.array.at3_array_or` in generated lib code while generated smoke uses `std.test` array assertions instead of hand-written array status expressions. Next work in this task should move aggregate helpers toward generic array APIs once broader generic collection execution is available.
