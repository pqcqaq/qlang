...
Status: second executable slice landed. `std.array` now provides generic fixed-array accessors `first3_array` / `first4_array` / `first5_array` and `last3_array` / `last4_array` / `last5_array`, alongside concrete fixed-array helpers for `Int` and `Bool`. Package-local smoke tests cover `Int` and `Bool` accessor instantiations, and `ql project init --stdlib` package/workspace templates consume `std.array.first3_array` in generated lib code plus `first3_array` / `last5_array` in generated smoke. Next work in this task should move aggregate helpers toward generic array APIs once broader generic collection execution is available.
