...
- The current API shape is still partly concrete because backend and dependency bridge support only cover a narrow generic function import/call slice. Generic public type execution is partially open for explicit `struct` / `enum` instantiations in concrete signatures and contextual aggregate usage, and `std.array` now has canonical executable length-generic `[T; N]` access/query/count helpers plus canonical `Int` / `Bool` aggregate helpers. Reverse/repeat remain fixed-length transition APIs because arbitrary-`N` array construction and return is still unavailable.
...
- Generic `Option[T]` / `Result[T, E]` are in the P0 stdlib unblock path; `std.option` and `std.result` both have executable generic helper slices, and `std.result` now covers generic Result/Option conversion through `ok_or`, `to_option`, and `error_to_option`. `std.option.none_option` is executable when an explicit `Option[T]` return or initializer context fixes `T`, and `std.result.ok/err` are executable when an explicit `Result[T, E]` return or initializer context fixes the unconstrained side. Wider constructors and full monomorphization remain open.
...
Status: twentieth execution slice landed. Direct local dependencies can now expose public generic `struct` / `enum` declarations, use explicit instantiations such as `Box[Int]` / `Maybe[Int]` in non-generic public function signatures consumed by a root project, and build contextual generic struct literals plus field projection when the expected type carries concrete args. Typeck also substitutes those args through struct/enum patterns and enum unit/tuple/struct variant construction. `.qi` emission has regression coverage for generic enum and generic function declarations, and library-mode codegen no longer fails just because a generic function/method declaration exists but is never instantiated. Direct local dependency, package-under-test, and package-local non-generic callers now support multiple direct-call concrete instantiations of the same public generic free function when every generic parameter can be inferred from direct `Int` / `Bool` / `String` literals, simple numeric / boolean / comparison expressions, tuple / fixed-array literals, fixed-array literals whose known items and literal length are enough to bind `[T; N]` parameters, one-level tuple or fixed-array projections, simple `if` / `match` expressions whose result arms agree, matching top-level `const/static` values, explicit function parameters, explicit typed `let/var` bindings, simple local names initialized from those values, generic carrier values, single-field generic enum variant constructor expressions such as `Option.Some(42)`, or explicit return / typed initializer result context; named call arguments are reordered to declaration order before running the same inference. Those explicit type bindings now carry structured type expressions, so one source can call `identity[T](value: T) -> T` as both `T = Int` and `T = Bool`, specialize `T = (Int, Bool)` and `T = [Int; 3]` from direct literals, specialize `T = Box[Int]`, infer `T = Int` for carrier-shaped parameters such as `keep_box[T](value: Box[T]) -> Box[T]`, infer `T = Int/Bool` for array-shaped parameters such as `first3[T](values: [T; 3]) -> T`, infer `T = Int` and `N = 3` for array-shaped parameters even when some literal items are calls whose return type is not locally inferable, infer `T = Int` from `values[1]`, infer `T = Bool` from `pair[1]`, infer simple `if` / `match` result expressions, infer `T = Int` for `is_some[T](value: Option[T]) -> Bool` from `Option.Some(42)`, infer `T = Int` for `none_option[T]() -> Option[T]` from an explicit `Option[Int]` result context, infer the side not fixed by the argument for `ok[T, E](value: T) -> Result[T, E]` / `err[T, E](error: E) -> Result[T, E]` from an explicit `Result[Int, Int]` context, and infer reordered calls such as `choose(value: 7, fallback: 0)`. The stdlib now uses this path for generic `std.result.ok_or`, `std.result.to_option`, `std.result.error_to_option`, canonical `std.array.first_array` / `last_array` / `at_array_or` / `contains_array` / `count_array`, and canonical `std.array.sum_int_array` / `product_int_array` / `max_int_array` / `min_int_array` / `all_bool_array` / `any_bool_array` / `none_bool_array`; generated `--stdlib` templates now keep nested array-transform calls inline and consume canonical array aggregates instead of fixed-arity aggregate wrappers. The backend also now analyzes task-handle containment through generic enum payloads, so local `Option[Int]` aliases do not trip struct-only alias analysis. Uninferred calls with no argument or result context, generic-to-generic nested specialization, method calls, escaping function values, and broader member/control-flow expression inference cases still report `dependency-function-unsupported-generic` or remain intentionally unsupported. Full generic function monomorphization, generic aliases, and broader generic stdlib helper execution remain open.
...
Status: option/result helper slices landed. `std.option` now exposes generic `some`, `none_option`, `is_some`, `is_none`, `unwrap_or`, and `or_option`; `std.result` now exposes generic `ok`, `err`, `is_ok`, `is_err`, `unwrap_result_or`, `or_result`, `error_or`, `ok_or`, `to_option`, and `error_to_option`. Package-local smoke covers explicit `Option[Int]` / `Result[Int, Int]` locals, enum variants, helper calls, `std.option.none_option`, `std.result.ok/err`, generic Result/Option conversion, and match-based status checks. `std.test` now exposes generic carrier and conversion assertions for `Option[Int]`, `Option[Bool]`, `Result[Int, Int]`, and `Result[Bool, Int]`; direct carrier assertions match carriers directly where useful, while generic conversion assertions call the public `std.result` helpers. `ql project init --stdlib` generated package code now consumes generic `std.option.some` / `std.option.unwrap_or`, generic `std.result.ok`, generic `std.result.Result` / `std.result.unwrap_result_or`, canonical length-generic `std.array.first_array`, `std.array.at_array_or`, `std.array.contains_array`, and `std.array.count_array` directly, while generated smoke uses the public `std.test` generic carrier/conversion and array assertions instead of hand-written local status functions and consumes `std.option.none_option` on explicit `Option[Int]` / `Option[Bool]` paths. Direct-call multi-instantiation support is now in the bridge, but method/value generic imports and full monomorphization remain open.
...
5. Update `std.test` to cover generic and concrete carrier paths. Done for `Option[Int]`, `Option[Bool]`, `Result[Int, Int]`, `Result[Bool, Int]`, and their generic Result/Option conversion paths.
...
Status: ninth executable slice landed. `std.array` now provides canonical length-generic fixed-array `first_array[T, N]` / `last_array[T, N]` / `at_array_or[T, N]` / `contains_array[T, N]` / `count_array[T, N]`, plus canonical `Int` / `Bool` aggregate helpers `sum_int_array[N]` / `product_int_array[N]` / `max_int_array[N]` / `min_int_array[N]` / `all_bool_array[N]` / `any_bool_array[N]` / `none_bool_array[N]`. The previous 3/4/5 access/query/count/aggregate names remain compatibility APIs and now delegate where the backend can support it. Reverse helpers `reverse3_array` / `reverse4_array` / `reverse5_array` and repeat helpers `repeat3_array` / `repeat4_array` / `repeat5_array` remain fixed-length because arbitrary-`N` array construction/return is still not available. The query/count helpers are generic at the API boundary but rely on current builtin equality lowering; smoke coverage is intentionally `Int`, `Bool`, and `String`, not user-defined equality. `std.test` now depends on `std.array` and exposes `expect_int_array_*` / `expect_bool_array_*` helpers that reuse canonical aggregate APIs for concrete aggregate assertions. Package-local smoke tests cover canonical access/query/count/aggregate helpers, compatibility wrappers, `Int` / `Bool` / `String` query instantiations, and pass/failure assertion paths; `ql project init --stdlib` package/workspace templates consume canonical `std.array` access/query/count/aggregate helpers in generated lib/main code while generated smoke uses `std.test` array assertions instead of hand-written array status expressions. Next work in this task should move reverse/repeat toward generic array APIs once broader generic collection construction/execution is available.
