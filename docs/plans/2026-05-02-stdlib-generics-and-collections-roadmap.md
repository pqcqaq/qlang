...
- Generic `Option[T]` / `Result[T, E]` are in the P0 stdlib unblock path; `std.option` has the first executable generic helper slice, while `std.result` generic helpers and wider monomorphization remain open.
...
Status: first option-helper slice landed. `std.option` now exposes generic `some`, `is_some`, `is_none`, `unwrap_or`, and `or_option` beside the concrete compatibility APIs; package-local smoke covers explicit `Option[Int]` locals, `Option.None`, helper calls, and match-based status checks. `ql project init --stdlib` generated package code now consumes generic `std.option.some` / `std.option.unwrap_or` directly. `std.result` generic helpers, `std.test` generic carrier assertions, and broader multi-instantiation support remain open.
