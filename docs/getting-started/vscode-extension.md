...
- package-aware `semantic tokens` 现在也已经开始覆盖 dependency value roots、imported dependency enum variant、显式 struct field label 与唯一 method member；如果当前文件本身有 source diagnostics、但 package/dependency 上下文仍可恢复，这条 dependency-backed 高亮也会继续保留，因此真实项目里不再是一有错误就整份掉回 TextMate fallback。
