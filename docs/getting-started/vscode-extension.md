...
- 这轮开始已经补上两条更接近真实项目的路径：workspace roots 驱动的保守 `workspace symbol` 搜索，以及 healthy package/workspace 下的 package/workspace import `definition` / `declaration`，和当前文件里命中的 dependency value / enum variant / struct field / method member 的 `definition` / `declaration` / `references` / `typeDefinition` 会优先跳到 workspace 内唯一可定位的源码定义，找不到唯一源码目标时再回退 `.qi`；其中 `references` 目前仍主要是当前文件 occurrence 加 declaration 位置的源码优先替换，不等于完整 workspace-wide reference index，而这轮也把同一类 source-preferred `references` 接进了 parse-error 文件里的 workspace/package import root。
