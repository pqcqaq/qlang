...
- package-aware `semantic tokens` 现在也已经开始覆盖 resolved dependency import roots、dependency value roots、imported dependency enum variant、显式 struct field label 与唯一 method member；其中 healthy package path 下的 dependency import roots 现在也会优先覆盖 generic import token 并提升成解析后的真实 symbol kind。如果当前文件本身有 source diagnostics、但 package/dependency 上下文仍可恢复，这条 dependency-backed 高亮也会继续保留，因此真实项目里不再是一有错误就整份掉回 TextMate fallback；另外，broken-source fallback 下的 dependency import roots 现在也会按 resolved dependency symbol kind 继续保留，所以 `Cfg` / `Cmd` 这类 alias root 不会只剩 TextMate/namespace fallback。
