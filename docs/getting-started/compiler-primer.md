...
| CLI | `crates/ql-cli` | `ql check`、`ql build`、`ql project`（含 `init/graph/emit-interface`）、`ql ffi`、`ql fmt`、`ql mir`、`ql ownership`、`ql runtime` |
...
- P8：package/workspace manifest、`.qi` interface artifact、dependency-backed cross-file tooling 已进入真实实现
