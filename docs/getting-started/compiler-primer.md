...
这会在新 package 的 `[dependencies]` 中写入 quoted-key 形式的 `std.core` / `std.test` 本地依赖，并生成直接消费 `std.core` / `std.test` 的 `src/lib.ql`、`src/main.ql` 与 `tests/smoke.ql`；workspace 初始化也支持同一个 `--stdlib <path>` 选项。
