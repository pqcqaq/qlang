...
- 新增 same-file const/static path-pattern literal folding：同文件 `const` / `static` item 及其 local import alias 作为 bare path pattern 时，如果能折叠成 `Bool` / `Int` literal，就会复用现有 literal pattern typing / exhaustiveness surface；不能折叠成 `Bool` / `Int` 的 path pattern 仍显式报 unsupported，而不是静默漏诊
...
- 未解析成员调用、通用索引协议、import prelude 细节时，表达式类型会主动退化为 `unknown`；当前只对源码层 fixed array、inferred array，以及同文件 foldable integer constant expression / immutable direct local alias-backed tuple index 开放一层 typing
...
- invalid path-pattern root diagnostics 当前也只在“path pattern root 已解析成功且 bare path 形状已知必错”时触发；unit variant 仍允许，same-file 已解析二段 enum variant path 的 unknown variant 现在也会显式报错；同文件 bare const/static path 如果能折叠成 `Bool` / `Int` literal 则会继续参与 literal compatibility checking，否则仍给出显式诊断；deeper module-path case 仍保持保守，不提前下结论
- 非标量 const/static bare path pattern 当前也已经改成显式 unsupported diagnostics；但这仍然只覆盖 same-file root / same-file local import alias，cross-file / deeper module-path case 继续保守
...
- 在 project-aware `build/run/test` 路径上，driver 现在也会把 direct dependency `.qi` 里的 public `extern "c"` 签名注入 root target，并在 executable/dylib link 时附上预构建的 dependency staticlibs；dependency bridge 已开放 bridgeable `const/static`、受限 free function、direct-call 多实例 public generic free function specialization、type declaration bridge 和受限 receiver method forwarder，完整跨包 Qlang lowering 仍未开放
