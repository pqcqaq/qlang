...
生成的 consumer smoke test 会用 `merge_status4` 按语义分组组合断言状态，并用 `expect_status_ok(...)` 返回，避免模板退化成长串手写加法。
