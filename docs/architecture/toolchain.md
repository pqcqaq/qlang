...
- `ql build --json` 现已把构建结果导出为稳定的 `ql.build.v1` schema，覆盖 source build 与 project-aware build 两条成功路径，以及两类 project 失败结果：project-aware preflight failure（当前已覆盖 `project-context`、`manifest-load`、`target-discovery`、`target-selection`、`output-planning`，以及 selected package 无 discovered targets 的 `build-plan` 校验）和 actual target build failure。当前 JSON 会稳定导出请求路径、`scope`（`file` / `project`）、resolved `project_manifest_path`、请求的 `emit/profile`、`profile_overridden`、`emit_interface`，以及按实际构建顺序展开的 `built_targets` 与 `interfaces`；成功时 `status = "ok"` 且 `failure = null`，失败时 `status = "failed"` 且 `failure` 会定位到 preflight failure point 或失败 target。actual target build failure 继续使用 `diagnostics` / `invalid-input` / `io` / `toolchain` 这四类 `error_kind` 及对应细节；preflight failure 则额外稳定导出 `stage`，并按场景补 `selector`、`conflict_path`、`target_count` 等 detail。`built_targets` 当前收敛成 `manifest_path`、`package_name`、`selected`、`dependency_only`、`kind`、`path`、`emit`、`profile`、`artifact_path`、`c_header_path`；`interfaces` 当前收敛成 `manifest_path`、`package_name`、`selected`、`status`（`wrote` / `up-to-date`）与 `path`。project-aware build 在前置同步 dependency `.qi` 时也会保持 stdout 静默，因此 JSON stdout 现在可以直接给脚本/CI 消费，不会被 `wrote interface: ...` 这类预备日志污染。dependency/interface 预备失败、更深的 build-plan recursion failure，以及 build-side interface emit / artifact failure 当前仍保留既有 stderr contract，不在这条 JSON slice 里假装已经完全结构化。
