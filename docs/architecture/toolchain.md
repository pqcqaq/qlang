...
- sync executable examples 现也额外收录 `ramdon_tests/executable_examples/04_sync_static_item_values.ql` 与 `ramdon_tests/executable_examples/05_sync_named_call_arguments.ql`，分别把 same-file foldable `const` / `static` item value 及其 same-file `use ... as ...` alias 的普通表达式 / bool 条件 lowering，以及 direct named call argument lowering（含 `[]` 的 expected-type back-propagation）锁进真实 `--emit exe` 合同
