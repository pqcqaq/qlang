...
- 当前最小 async executable program subset 还额外由 193 个 committed 样例锁定，见 `ramdon_tests/async_program_surface_examples/` 与 `crates/ql-cli/tests/executable_examples.rs`
- `174_async_main_spawned_aggregate_results.ql` 到 `196_async_main_aggregate_param_families.ql` 现进一步把 spawned aggregate result、zero-sized / recursive aggregate result、regular-size / zero-sized helper-returned / forwarded task-handle flow、regular-size tuple / array / nested aggregate task-handle payload family、regular-size / zero-sized / recursive aggregate param family、zero-sized projected task-handle `await` / `spawn` consume、zero-sized projected reinit、zero-sized conditional `spawn/await/reinit` family、regular-size conditional task-handle flow、bound local task-handle `spawn` flow、regular-size returned / nested / struct-carried task-handle shape family、regular-size projected reinit family、guard-refined dynamic path family，以及 static-alias-backed projected-root dynamic reinit family 推进到 `async fn main` 的真实 build-and-run surface
