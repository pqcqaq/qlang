...
- `stdlib/packages/option`：package `std.option`，提供 concrete `IntOption` / `BoolOption` 以及 `some_int`、`none_int`、`is_some_int`、`is_none_int`、`unwrap_or_int`、`or_int`、`value_or_zero_int`、`some_bool`、`none_bool`、`is_some_bool`、`is_none_bool`、`unwrap_or_bool`、`or_bool`、`value_or_false_bool`、`value_or_true_bool`。这不是泛型 prelude `Option[T]`；它只使用当前 dependency bridge 已能执行的非泛型 enum / function surface。
