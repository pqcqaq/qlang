...
- 当前最小 async executable program subset 还额外由 174 个 committed 样例锁定，见 `ramdon_tests/async_program_surface_examples/` 与 `crates/ql-cli/tests/executable_examples.rs`
- `174_async_main_spawned_aggregate_results.ql`、`175_async_main_zero_sized_and_recursive_aggregate_results.ql`、`176_async_main_zero_sized_helper_task_handle_flows.ql` 与 `177_async_main_zero_sized_projected_task_handle_consumes.ql` 现进一步把 spawned aggregate result、zero-sized aggregate result、zero-sized helper-returned / forwarded task-handle flow，以及 zero-sized projected task-handle `await` / `spawn` consume family 推进到 `async fn main` 的真实 build-and-run surface
