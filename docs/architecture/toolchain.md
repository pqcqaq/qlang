...
- sync executable examples 现也额外收录 `ramdon_tests/executable_examples/04_sync_static_item_values.ql` 到 `12_sync_match_scrutinee_bool_comparison_guard.ql`：除了 same-file foldable `const` / `static` item value 普通表达式 / bool 条件 lowering、direct named call argument lowering、same-file function import alias call lowering、direct fixed-array `for`、direct homogeneous tuple `for`、projected fixed-shape `for`，以及 same-file `const` / `static` fixed-shape root 与 same-file `use ... as ...` const alias 的 sync `for` lowering 之外，现在也已经把 bool `match` scrutinee self-guard folding 与 scrutinee-bool-comparison guard folding 一并锁进真实 `--emit exe` 合同
