...
- `stdlib/packages/result`：package `std.result`，提供可执行的 generic carrier `Result[T, E]`，以及 generic helper `is_ok`、`is_err`、`unwrap_result_or`、`or_result`、`error_or`。concrete `IntResult` / `BoolResult` 和 `ok_int`、`err_int`、`is_ok_int`、`is_err_int`、`unwrap_result_or_int`、`or_result_int`、`error_or_zero_int`、`error_to_option_int`、`ok_or_int`、`to_option_int`、`ok_bool`、`err_bool`、`is_ok_bool`、`is_err_bool`、`unwrap_result_or_bool`、`or_result_bool`、`error_or_zero_bool`、`error_to_option_bool`、`ok_or_bool`、`to_option_bool` 保留为 compatibility surface。generic carrier 现在已经能在 downstream concrete function signatures 和 smoke tests 里执行；generic helper function bridge 当前开放单实例、primitive 字面量、显式 typed value、generic carrier value 与单字段 generic enum variant constructor 可推断的 free function 特化，automatic prelude integration 与完整单态化仍未开放。generic `ok` / `err` 构造 helper 仍未开放，因为当前桥接不从返回上下文推断未出现在参数中的类型参数；当前 concrete Option/Result 互转和无损 error extraction 通过 `std.result` 对 `std.option` 的本地依赖实现，并保持 basename 不与 `std.option` 冲突。
...
这套 stdlib 当前只使用已稳定的 direct local dependency free function bridge；自动 prelude、泛型集合、IO、字符串完整库面和 registry/version solving 都继续后置。
