...
- 最小 literal `match` lowering：`Bool` scrutinee 仅支持 unguarded `true` / `false` + `_` 或单名 binding catch-all arm，并额外接受 literal `if true` / `if false` guard、same-file `const`-backed `Bool` guard 与其 same-file `use ... as ...` 别名、由当前已支持布尔 guard 操作数组成的简单 `==` / `!=` 比较，以及 direct same-scope `Bool` local / parameter name guard（要求后续 arm 仍提供 guaranteed fallback coverage）；`Int` scrutinee 仅支持 unguarded integer literal + `_` 或单名 binding catch-all arm，并额外接受 literal `if true` / `if false` guard、same-file `const`-backed `Bool` guard 与其 same-file `use ... as ...` 别名、由当前已支持布尔 guard 操作数组成的简单 `==` / `!=` 比较，以及 integer-literal arm 或 guarded catch-all arm 上的 direct same-scope `Bool` local / parameter name guard（要求后续存在 unguarded catch-all fallback）
