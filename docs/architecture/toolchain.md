...
- `stdlib/packages/array`：package `std.array`，提供 canonical length-generic `first_array[T, N]` / `last_array[T, N]` / `at_array_or[T, N]` / `contains_array[T, N]` / `count_array[T, N]`，保留 compatibility `first3_array` / `first4_array` / `first5_array` / `last3_array` / `last4_array` / `last5_array` / `at3_array_or` / `at4_array_or` / `at5_array_or` / `contains3_array` / `contains4_array` / `contains5_array` / `count3_array` / `count4_array` / `count5_array`，并提供固定长度 transform `reverse3_array` / `reverse4_array` / `reverse5_array` / `repeat3_array` / `repeat4_array` / `repeat5_array`，以及 concrete 固定长度数组 helper `sum3_int_array` / `sum4_int_array` / `sum5_int_array`、`product3_int_array` / `product4_int_array` / `product5_int_array`、`max3_int_array` / `max4_int_array` / `max5_int_array`、`min3_int_array` / `min4_int_array` / `min5_int_array`、`all3_bool_array` / `all4_bool_array` / `all5_bool_array`、`any3_bool_array` / `any4_bool_array` / `any5_bool_array`、`none3_bool_array` / `none4_bool_array` / `none5_bool_array`。`contains_array` / `count_array` 及其兼容 API 依赖 builtin equality lowering，当前通过 `Int` / `Bool` / `String` smoke 覆盖，不表示用户自定义类型已经有通用 equality 约束。
