...
- 最小 literal `match` lowering：`Bool` scrutinee 仅支持 unguarded `true` / `false` + `_` 或单名 binding catch-all arm；`Int` scrutinee 仅支持 unguarded integer literal + `_` 或单名 binding catch-all arm；这两条子集都额外接受 literal `if true` / `if false` guard，以及 same-file `const`-backed `Bool` guard
