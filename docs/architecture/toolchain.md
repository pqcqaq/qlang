...
- 普通表达式与 `if` / `while` 条件里可直接 materialize same-file foldable `const` / `static` item value 及其 same-file `use ... as ...` alias；当前已支持的 tuple / fixed-array / plain-struct literal 子集，以及能经由 struct field / tuple index / fixed-array index 回收到同一条 foldable source 的 computed/projected item value，都会复用同一条 const-evaluation lowering
