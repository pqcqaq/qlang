...
- 当前最小 async executable program subset 还额外由 184 个 committed 样例锁定，见 `ramdon_tests/async_program_surface_examples/` 与 `crates/ql-cli/tests/executable_examples.rs`
- `174_async_main_spawned_aggregate_results.ql` 到 `187_async_main_spawned_recursive_aggregate_params.ql` 现进一步把 spawned aggregate result、zero-sized / recursive aggregate result、zero-sized helper-returned / forwarded task-handle flow、zero-sized projected task-handle `await` / `spawn` consume、zero-sized projected reinit、zero-sized conditional `spawn/await/reinit` family、zero-sized returned / nested / struct-carried task-handle shapes、aggregate params，以及 zero-sized call-root / awaited-aggregate / import-alias / inline / nested-call-root projected consume family 推进到 `async fn main` 的真实 build-and-run surface
