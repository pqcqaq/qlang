...
- `crates/ql-cli/tests/codegen.rs` 当前也已把 direct resolved sync guard-call `match` 显式锁进 CLI 的 `llvm-ir` / `obj` / `exe` pass matrix，`fixtures/codegen/pass/match_guard_direct_calls.ql` 覆盖 `enabled()` 这类 bool guard call 与 `offset(delta: 2, value: current) == 22` 这类命名实参重排后的 integer guard-call 组合，不再继续保留过时的 `unsupported_match_build` fail 合同
