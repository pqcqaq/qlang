...
- 当前最小 async executable program subset 还额外由 150 个 committed 样例锁定，见 `ramdon_tests/async_program_surface_examples/` 与 `crates/ql-cli/tests/executable_examples.rs`
...
- `143_async_main_call_root_projected_task_handle_consumes.ql` 则进一步把 direct call-root / nested call-root projected task-handle consume 推进到 `async fn main` 的真实 build-and-run surface，显式覆盖 direct call-root tuple projection await、direct call-root struct-field projection spawn，以及 direct nested call-root fixed-array projection await/spawn
