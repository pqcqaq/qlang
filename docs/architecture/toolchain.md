...
- deeper struct-like shorthand token 现在也有显式 parity 回归：当 `Point.Scope.Config { x }` / `Point.Scope.Config { source }` 这类路径仍处于 field semantics 关闭状态时，analysis 会继续把 shorthand token 保留在 local / binding / import lexical surface，LSP bridge 也会继续按该 lexical symbol 提供 hover / definition / rename，并保持 raw binding edit 而不是伪造 `label: new_name` 扩写
