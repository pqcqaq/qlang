...
- `stdlib/packages/option`：package `std.option`，提供 concrete `IntOption` / `BoolOption` 以及 `some_int`、`none_int`、`is_some_int`、`is_none_int`、`unwrap_or_int`、`or_int`、`or_option_int`、`value_or_zero_int`、`some_bool`、`none_bool`、`is_some_bool`、`is_none_bool`、`unwrap_or_bool`、`or_option_bool`、`value_or_false_bool`、`value_or_true_bool`。这不是泛型 prelude `Option[T]`；它只使用当前 dependency bridge 已能执行的非泛型 enum / function surface。
- `stdlib/packages/result`：package `std.result`，提供 concrete `IntResult` / `BoolResult` 以及 `ok_int`、`err_int`、`is_ok_int`、`is_err_int`、`unwrap_result_or_int`、`or_result_int`、`error_or_zero_int`、`error_to_option_int`、`ok_or_int`、`to_option_int`、`ok_bool`、`err_bool`、`is_ok_bool`、`is_err_bool`、`unwrap_result_or_bool`、`or_result_bool`、`error_or_zero_bool`、`error_to_option_bool`、`ok_or_bool`、`to_option_bool`。这不是泛型 prelude `Result[T, E]`；它先用 `Int` error code 匹配当前可执行的非泛型 enum / function surface，依赖 `std.option` 提供 concrete Option/Result 互转和无损 error extraction，并保持 basename 不与 `std.option` 冲突。
