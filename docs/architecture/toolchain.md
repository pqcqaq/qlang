...
- `crates/ql-cli/tests/codegen.rs` 当前也已把 direct resolved sync guard-call `match` 与 call-projection-root guard `match` 显式锁进 CLI 的 `llvm-ir` / `obj` / `exe` pass matrix：`fixtures/codegen/pass/match_guard_direct_calls.ql` 覆盖 `enabled()` 这类 bool guard call 与 `offset(delta: 2, value: current) == 22` 这类命名实参重排后的 integer guard-call 组合，而 `fixtures/codegen/pass/match_guard_call_projection_roots.ql` 继续覆盖 `pair(current)[1]`、`state(current).value` 与 `values(current)[1]` 这类 call-root projection guard，不再只依赖 driver 单测和 sync executable 样例间接证明
