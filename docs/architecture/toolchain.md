...
- 最小 literal `match` lowering：`Bool` scrutinee 仅支持 unguarded `true` / `false` + `_` 或单名 binding catch-all arm；`Int` scrutinee 仅支持 unguarded integer literal + `_` 或单名 binding catch-all arm
