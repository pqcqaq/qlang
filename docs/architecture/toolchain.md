...
- sync/async executable surface 现也显式锁住 ordinary assignment-expression 子集：mutable local、struct-field 与 fixed-array literal-index assignment 继续走既有 statement lowering，但现在也通过 executable examples 把“赋值表达式结果值可继续参与后续普通标量计算”这条用户面固定下来
