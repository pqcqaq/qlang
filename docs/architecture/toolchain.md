...
- 上述 `match` guard 标量子集现在也包含 direct resolved sync scalar calls：返回 `Bool` 的 direct/local-import-alias 调用可直接作为 guard，返回 `Int` 的同类调用可参与当前 `Int` 标量比较与命名实参重排子集；聚合参数 guard call、async guard call 与更广的调用表达式仍未开放。
