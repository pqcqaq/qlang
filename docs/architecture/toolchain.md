...
- `crates/ql-cli/tests/codegen.rs` 现也已把 async library `match` family 锁进两条 library surface：新增 `fixtures/codegen/pass/async_library_match_families.ql` 与 `fixtures/codegen/pass/ffi_export_async_match_families.ql`，在同一对 fixture 里显式覆盖 awaited scalar scrutinee + direct-call guard，以及 awaited aggregate scrutinee + projection guard；因此当前 library-mode async 子集已不再只公开 `await` / `spawn` / `for await`，也已公开既有最小 `match` lowering 在 async helper body 内的稳定构建合同
