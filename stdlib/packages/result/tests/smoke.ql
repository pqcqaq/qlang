...
fn main() -> Int {
    ...
    let constructor_status = sum_statuses([check_int(result_unwrap_result_or(direct_ok, 0), 21), check_int(result_error_or(direct_err, 0), 6), check_bool(result_is_ok(bool_ok), true), check_bool(result_is_err(bool_err), true), check_bool(result_is_ok(direct_ok), true), check_bool(result_is_err(direct_err), true)])
