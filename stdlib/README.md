...
This is real standard-library code for downstream packages, not only a test fixture. The current `IntOption` / `BoolOption`, `IntResult` / `BoolResult`, and fixed-arity helpers such as `sum3_int` are transitional compatibility APIs. Generic carrier types `Option[T]` and `Result[T, E]` are now executable in downstream package signatures and smoke tests. Uninstantiated generic function declarations can live in library packages and `.qi`, but importing or calling direct dependency generic helper functions still reports an explicit `dependency-function-unsupported-generic` diagnostic until monomorphization lands.
