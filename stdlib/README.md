...
Generated smoke tests group assertion statuses with `merge_status4` / `merge_status5` / `merge_status6`, use carrier-specific `std.test` assertions for `std.option` / `std.result`, cover the `std.result` <-> `std.option` conversion helpers, and return `expect_status_ok(...)` so larger tests can keep the same `0` pass / non-zero failure contract without one long status expression.
