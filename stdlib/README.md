...
- `std.test` 的泛型 `expect_eq` / `expect_ne`、泛型数组 equality/access/query/reverse 断言、泛型 `expect_option_*` / `expect_result_*` 断言、具体 Int/Bool 包装和 `merge_statuses` 状态合并 helper
...
`std.test` 的新断言优先使用泛型 API，例如 `expect_eq[T]`、`expect_array_eq[T, N]`、`expect_array_contains[T, N]`、`expect_array_reverse[T, N]`；`expect_array_reverse` 会比较完整反转结果。具体 Int/Bool 断言保留为兼容包装。聚合断言、顺序断言和状态合并使用数组 API；固定 arity 的 `expect_*3/4/5` 聚合/顺序断言和 `merge_status3/4/5/6` 已删除。
