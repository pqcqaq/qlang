...
This is real standard-library code for downstream packages, not only a test fixture. The current `IntOption` / `BoolOption`, `IntResult` / `BoolResult`, and fixed-arity helpers such as `sum3_int` are transitional compatibility APIs. Generic carrier types `Option[T]` and `Result[T, E]` are now executable in downstream package signatures and smoke tests, and `std.option` has a first executable generic helper slice. Uninstantiated generic function declarations can live in library packages and `.qi`; direct dependency and package-under-test builds also support one local specialization of a generic public free function when every generic parameter is inferred from primitive `Int` / `Bool` / `String` literals or explicit typed values, including `T = Option[Int]` and carrier-shaped parameters like `Option[T]`. Wider generic helper calls still report `dependency-function-unsupported-generic` until full monomorphization lands.
...
- `std.option`: executable generic carrier `Option[T]` plus generic helpers `some`, `is_some`, `is_none`, `unwrap_or`, and `or_option`; concrete `IntOption` / `BoolOption` helpers `some_int`, `none_int`, `is_some_int`, `is_none_int`, `unwrap_or_int`, `or_int`, `or_option_int`, `value_or_zero_int`, `some_bool`, `none_bool`, `is_some_bool`, `is_none_bool`, `unwrap_or_bool`, `or_option_bool`, `value_or_false_bool`, and `value_or_true_bool` remain as compatibility APIs. Generic none is currently expressed as `Option.None` with an explicit or contextual `Option[T]` type.
- `std.result`: executable generic carrier `Result[T, E]` plus concrete `IntResult` / `BoolResult` helpers `ok_int`, `err_int`, `is_ok_int`, `is_err_int`, `unwrap_result_or_int`, `or_result_int`, `error_or_zero_int`, `error_to_option_int`, `ok_or_int`, `to_option_int`, `ok_bool`, `err_bool`, `is_ok_bool`, `is_err_bool`, `unwrap_result_or_bool`, `or_result_bool`, `error_or_zero_bool`, `error_to_option_bool`, `ok_or_bool`, and `to_option_bool`.
...
Generated `--stdlib` projects depend on `std.core`, `std.option`, `std.result`, `std.array`, and `std.test`. `std.option` and `std.result` now expose executable generic carriers alongside their concrete helper APIs: direct dependency builds support explicit generic `struct` / `enum` instantiations in non-generic function signatures, and the generated smoke tests now exercise `Option[T]` / `Result[T, E]` through function parameters, return values, and `match` branches. `std.option` now also exposes executable generic `some`, `is_some`, `is_none`, `unwrap_or`, and `or_option` helpers for the current single-instantiation bridge; generated `--stdlib` package code uses generic `std.option.some` / `std.option.unwrap_or` directly. Full generic helper function monomorphization, automatic prelude integration, and broader multi-instantiation imports are still open, so the concrete Option/Result helpers remain compatibility APIs. `std.array` is also intentionally concrete today, but it moves repeated-value APIs toward collection-shaped calls instead of adding more `foo3/foo4/foo5` argument lists. `std.test` now also ships carrier-specific assertions for `std.option` and `std.result`.
