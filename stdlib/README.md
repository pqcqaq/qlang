...
This is real standard-library code for downstream packages, not only a test fixture. The current `IntOption` / `BoolOption`, `IntResult` / `BoolResult`, and fixed-arity helpers such as `sum3_int` are transitional compatibility APIs. Generic carrier types `Option[T]` and `Result[T, E]` are now executable in downstream package signatures and smoke tests, while generic helper APIs, collection helpers, and eventually variadic syntax are still being opened in the compiler/backend.
