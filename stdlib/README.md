...
Generated `--stdlib` projects depend on `std.core`, `std.option`, and `std.test`. `std.option` is intentionally concrete today: generic `Option[T]` and automatic prelude integration remain language/runtime work, while `IntOption` / `BoolOption` are executable through the current dependency bridge.
