...
async fn main() -> Int {
    var total = 0
    for await value in (await array_env(8)).payload.values {
        total = total + value
    }
    for await value in (await tuple_env(4)).payload.values {
        total = total + value
    }
    for await value in (await task_tuple_env(2)).payload.values {
        total = total + value
    }
    for await value in (await deep_task_env(5)).outer.payload.tasks {
        total = total + value
    }
    return total
}
