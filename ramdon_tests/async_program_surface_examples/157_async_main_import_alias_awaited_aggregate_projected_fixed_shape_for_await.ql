...
async fn main() -> Int {
    var total = 0
    for await value in (await arrays(8)).payload.values {
        total = total + value
    }
    for await value in (await tuples(4)).payload.values {
        total = total + value
    }
    for await value in (await task_tuples(2)).payload.values {
        total = total + value
    }
    for await value in (await deep(5)).outer.payload.tasks {
        total = total + value
    }
    return total
}
