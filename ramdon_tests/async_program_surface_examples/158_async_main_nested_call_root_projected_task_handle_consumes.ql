...
async fn main() -> Int {
    let first = await tuple_env(10).payload.values[0]
    let running = spawn pair_env(11).payload.left
    let third = await deep_env(20).outer.payload.tasks[0]
    let last_running = spawn deep_env(0).outer.payload.tasks[1]
