...
async fn branch_case(flag: Bool) -> Wrap {
    var task = worker()
    if flag {
...
async fn reverse_branch_case(flag: Bool) -> Wrap {
    var task = worker()
    if flag {
        task = fresh_worker()
    } else {
...
async fn choose_task(flag: Bool, task: Task[Wrap]) -> Wrap {
    if flag {
        let running = spawn task
        return await running
    }
...
async fn choose_task_reverse(flag: Bool, task: Task[Wrap]) -> Wrap {
    if flag {
        return await task
    }
    let running = spawn task
