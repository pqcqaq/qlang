...
async fn main() -> Int {
    let first = await tuples(10).payload.values[0]
    let running = spawn pairs(11).payload.left
    let third = await deep(20).outer.payload.tasks[0]
    let last_running = spawn deep(0).outer.payload.tasks[1]
