...
async fn main() -> Int {
    var total = 0
    for await value in array_env(8).payload.values {
        total = total + value
    }
    for await value in tuple_env(4).payload.values {
        total = total + value
    }
    for await value in task_tuple_env(2).payload.values {
        total = total + value
    }
    for await value in deep_task_env(5).outer.payload.tasks {
        total = total + value
    }
    return total
}
