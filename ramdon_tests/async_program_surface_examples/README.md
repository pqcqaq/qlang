...
- `crates/ql-cli/tests/executable_examples.rs` now builds and runs these two-hundred-twenty examples with the real local toolchain and locks their exit codes.
- The filenames run from `04` through `223`, but the real async executable example count is `220`.
...
- `192_async_main_guard_refined_dynamic_path_families.ql` now locks the guard-refined dynamic path family for `async fn main`, where direct dynamic reinit, projected dynamic reinit, aliased projected-root guard refinement, const-backed alias-root guard refinement, arithmetic-backed direct / projected guard refinement, and same-file import-alias wrapped arithmetic direct / alias-root projected guard refinement all survive the executable surface and still exit with `144`.
...
- `198_async_main_dynamic_task_handle_path_families.ql` now locks the stable-dynamic task-handle success-path family for `async fn main`, where aliased projected-root dynamic reinit, const-backed alias-root dynamic reinit, composed dynamic reinit, alias-sourced composed dynamic reinit, direct inline foldable `if` / minimal literal `match` index reinit, branch-selected `const` / `static` item-backed dynamic reinit, arithmetic-backed literal / projection reinit, same-file import-alias wrapped branch-selected item reinit, and same-file import-alias wrapped arithmetic item reinit all survive the executable surface and still exit with `207`.
- `199_async_main_aliased_projected_root_repackage_families.ql` now locks the aliased projected-root repackage family for `async fn main`, where source-root reinit is followed by tuple, struct, and nested aggregate repackaging of the same alias-root task handle before `await`, and that same family now also covers same-file import-alias wrapped arithmetic item values on direct alias-root, projected-root, alias-sourced composed-dynamic, and guard-refined alias-sourced composed-dynamic forms, including the guarded bundle-alias-forwarded, bundle-alias-inline-forwarded, queued-root-forwarded, queued-root-inline-forwarded, queued-root-alias-forwarded, queued-root-alias-inline-forwarded, queued-root-chain-forwarded, queued-root-chain-inline-forwarded, queued-local-alias, queued-local-chain, queued-local-forwarded, queued-local-inline-forwarded, bundle-chain-forwarded, and bundle-chain-inline-forwarded await variants, while the executable surface exits with `690`.
- `200_async_main_aliased_projected_root_spawn_families.ql` now locks the aliased projected-root spawn family for `async fn main`, where source-root reinit is followed by nested aggregate, fixed-array aggregate, nested fixed-array aggregate, and helper-forwarded nested fixed-array aggregate repackaging before `spawn -> await`, and that same family now also covers same-file import-alias wrapped arithmetic item values on direct alias-root, projected-root, alias-sourced composed-dynamic, and guard-refined alias-sourced composed-dynamic forms, including the guarded bundle-alias-forwarded, bundle-alias-inline-forwarded, queued-root-forwarded, queued-root-inline-forwarded, queued-root-alias-forwarded, queued-root-alias-inline-forwarded, queued-root-chain-forwarded, queued-root-chain-inline-forwarded, queued-local-alias, queued-local-chain, queued-local-forwarded, queued-local-inline-forwarded, bundle-chain-forwarded, and bundle-chain-inline-forwarded spawn variants, while the executable surface exits with `772`.
...
- `192_async_main_guard_refined_dynamic_path_families.ql` -> `144`
...
- `198_async_main_dynamic_task_handle_path_families.ql` -> `207`
- `199_async_main_aliased_projected_root_repackage_families.ql` -> `690`
- `200_async_main_aliased_projected_root_spawn_families.ql` -> `772`
