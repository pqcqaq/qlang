...
- `223_async_const_backed_tuple_assignment_expressions.ql` now locks the async tuple constant-index read/write surface where same-file `const` / `static` items, branch-selected const `if` / literal `match` item values, struct-field projections, integer arithmetic, same-file `use ... as ...` aliases, and immutable direct local aliases can drive tuple indices inside `async fn main`, and the executable surface still exits with `22`.
