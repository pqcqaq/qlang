...
- `crates/ql-cli/tests/executable_examples.rs` now builds and runs these one-hundred-ninety-one examples with the real local toolchain and locks their exit codes.
- The filenames run from `04` through `194`, but the real async executable example count is `191`.
